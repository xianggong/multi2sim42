...
struct str_map_t si2bin_token_map = {
    si2bin_token_count, {{"<invalid>", si2bin_token_invalid},
                         ...
                         {"\%vopc_64_src0", si2bin_token_vopc_64_src0},
                         {"\%64_vsrc1", si2bin_token_64_vsrc1},
                         ...
                         {"\%vop2_lit", si2bin_token_vop2_lit},
...
void si2bin_token_free(struct si2bin_token_t *token) { free(token); }

int si2bin_token_is_arg_allowed(struct si2bin_token_t *token,
  ...
  switch (token->type) {
    ...
    default:
      si2bin_yyerror_fmt("%s: unsupported token (code = %d)", __FUNCTION__,
                         token->type);
      return 0;
  }
}
