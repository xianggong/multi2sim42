...
struct si2bin_inst_t *si2bin_inst_create(int opcode, struct list_t *arg_list) {
  ...
  info = inst->info;
  if (!info) fatal("%s: opcode %d not supported", __FUNCTION__, opcode);
...
struct si2bin_inst_t *si2bin_inst_create_with_name(char *name,
  ...
  /* Initialize */
  if (!arg_list) arg_list = list_create();
  inst->arg_list = arg_list;
  ...
  for (info = hash_table_get(si2bin_inst_info_table, name); info;
    ...
    LIST_FOR_EACH(arg_list, index) {
      ...
      /* Check that actual argument type is acceptable for token */
      if (!si2bin_token_is_arg_allowed(token, arg)) {
        snprintf(err_str, sizeof err_str, "invalid type for argument %d",
                 index + 1);
        break;
      }
    }
  ...
  if (!info) si2bin_yyerror(err_str);
...
void si2bin_inst_dump_assembly(struct si2bin_inst_t *inst, FILE *f) {
  ...
  LIST_FOR_EACH(inst->arg_list, i) {
    ...
    if (i < inst->arg_list->count - 1) fprintf(f, ", ");
  }
    ...
    switch (token->type) {
      case si2bin_token_simm16: {
        ...
        if (arg->type == si2bin_arg_literal ||
          ...
          if (arg->value.literal.val > 0xff00)
            si2bin_yyerror_fmt(
                "%s: Literal in simm16 needs to fit in 16 bit field",
                __FUNCTION__);
          ...
          if (!IN_RANGE(value, 0, 255)) si2bin_yyerror("invalid argument type");
      ...
      case si2bin_token_64_ssrc0: {
        ...
        if (arg->type == si2bin_arg_literal) {
          ...
          if (inst->size == 8) si2bin_yyerror("only one literal allowed");
          inst->size = 8;
      ...
      case si2bin_token_mt_maddr: {
        ...
        if (!IN_RANGE(soffset, 0, 253)) si2bin_yyerror("invalid offset");
      ...
      case si2bin_token_label: {
        ...
        if (symbol->defined) {
          inst_bytes->sopp.simm16 =
              (symbol->value - si2bin_entry->text_section_buffer->offset) / 4 -
              1;
        } else {
          task = si2bin_task_create(si2bin_entry->text_section_buffer->offset,
                                    symbol);
      ...
      case si2bin_token_offset:
        ...
        switch (arg->type) {
          ...
          default:
            panic("%s: invalid argument type for token 'offset'", __FUNCTION__);
        }
        break;
      ...
      case si2bin_token_series_sbase:
        ...
        switch (inst_info->inst) {
          ...
          default:
            si2bin_yyerror_fmt(
                "%s: unsupported opcode for 'series_sbase' token: %s",
                __FUNCTION__, info->name);
        }
      ...
      case si2bin_token_series_sdst:
        ...
        switch (inst_info->inst) {
          ...
          default:
            si2bin_yyerror_fmt(
                "%s: unsupported opcode for 'series_sdst' token: %s",
                __FUNCTION__, info->name);
        }
      ...
      case si2bin_token_series_srsrc: {
        ...
        if (high != low + 3)
          si2bin_yyerror_fmt(
              "register series must span 4 registers in s[%d:%d]", low, high);
      ...
      case si2bin_token_vaddr:
        ...
        switch (arg->type) {
          ...
          case si2bin_arg_vector_register_series:
            ...
            inst_bytes->mtbuf.vaddr = arg->value.vector_register_series.low;
      ...
      case si2bin_token_vop3_src0:
        ...
        if (arg->abs) inst_bytes->vop3a.abs |= 0x1;
        if (arg->neg) inst_bytes->vop3a.neg |= 0x1;
        break;

      case si2bin_token_vop3_src1:

        inst_bytes->vop3a.src1 = si2bin_arg_encode_operand(arg);
        if (arg->abs) inst_bytes->vop3a.abs |= 0x2;
        if (arg->neg) inst_bytes->vop3a.neg |= 0x2;
        break;

      case si2bin_token_vop3_src2:

        inst_bytes->vop3a.src2 = si2bin_arg_encode_operand(arg);
        if (arg->abs) inst_bytes->vop3a.abs |= 0x4;
        if (arg->neg) inst_bytes->vop3a.neg |= 0x4;
        break;
      ...
      case si2bin_token_wait_cnt:
        ...
        if (arg->value.wait_cnt.vmcnt_active) {
          ...
          inst_bytes->sopp.simm16 = SET_BITS_32(
              inst_bytes->sopp.simm16, 3, 0, arg->value.wait_cnt.vmcnt_value);
        } else {
          inst_bytes->sopp.simm16 =
              SET_BITS_32(inst_bytes->sopp.simm16, 3, 0, 0xf);
        }
        ...
        if (arg->value.wait_cnt.lgkmcnt_active) {
          ...
          inst_bytes->sopp.simm16 =
              SET_BITS_32(inst_bytes->sopp.simm16, 12, 8,
                          arg->value.wait_cnt.lgkmcnt_value);
        } else {
          inst_bytes->sopp.simm16 =
              SET_BITS_32(inst_bytes->sopp.simm16, 12, 8, 0x1f);
        }
        ...
        if (arg->value.wait_cnt.expcnt_active) {
          ...
          inst_bytes->sopp.simm16 = SET_BITS_32(
              inst_bytes->sopp.simm16, 6, 4, arg->value.wait_cnt.expcnt_value);
        } else {
          inst_bytes->sopp.simm16 =
              SET_BITS_32(inst_bytes->sopp.simm16, 6, 4, 0x7);
        }
        break;
      ...
      default:
        si2bin_yyerror_fmt("unsupported token for argument %d", index + 1);
    }
  }
}
