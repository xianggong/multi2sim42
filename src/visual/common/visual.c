...
static void visual_cycle_bar_refresh(void *user_data, long long cycle) {
  ...
  /* Evergreen */
  if (vi_evg_gpu->active) vi_evg_panel_refresh(vi_evg_panel);

  /* Southern Islands */
  if (vi_si_gpu->active) vi_si_panel_refresh(vi_si_panel);

  /* x86 */
  if (vi_x86_cpu->active) vi_x86_panel_refresh(vi_x86_panel);
}
...
void visual_run(char *file_name) {
  ...
  m2s_dist_file("close-sel.png", m2s_images_path, m2s_images_path,
                ...
                sizeof vi_list_image_close_sel_path);
  m2s_dist_file("m2s-icon.png", m2s_images_path, m2s_images_path, vi_icon_path,
                sizeof vi_icon_path);
  ...
  g_signal_connect(G_OBJECT(window), "destroy",
                   G_CALLBACK(visual_destroy_event), NULL);
  ...
  gtk_box_pack_start(GTK_BOX(vbox), vi_mem_panel_get_widget(vi_mem_panel), TRUE,
                     TRUE, 0);
