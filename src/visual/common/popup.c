...
static gboolean vi_popup_button_press_event(GtkWidget *widget,
  ...
  if (event->x < -20 || event->y < -20 || event->x > width + 20 ||
      event->y > height + 20) {
...
static gboolean vi_popup_motion_event(GtkWidget *widget, GdkEventMotion *event,
                                      gpointer data) {
  int width;
  int height;
...
static void vi_popup_destroy_event(GtkWidget *widget,
...
void vi_popup_free(struct vi_popup_t *popup) { free(popup); }
...
void vi_popup_show(char *text) {
  ...
  gdk_device_grab(gtk_get_current_event_device(),
                  gtk_widget_get_window(popup->window), GDK_OWNERSHIP_WINDOW,
