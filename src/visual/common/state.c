...
struct vi_state_checkpoint_t *vi_state_checkpoint_create(
    long long cycle, long int unzipped_trace_file_offset,
    long int checkpoint_file_offset) {
...
struct vi_state_category_t *vi_state_category_create(
    char *name, vi_state_read_checkpoint_func_t read_checkpoint_func,
    vi_state_write_checkpoint_func_t write_checkpoint_func, void *user_data) {
...
static void vi_state_read_checkpoint(int index) {
  ...
  if (!checkpoint) panic("%s: invalid checkpoint index", __FUNCTION__);

  /* Set file positions */
  fseek(vi_state->checkpoint_file, checkpoint->checkpoint_file_offset,
        SEEK_SET);
  fseek(vi_state->unzipped_trace_file, checkpoint->unzipped_trace_file_offset,
        SEEK_SET);
...
void vi_state_init(char *trace_file_name) {
  ...
  while ((trace_line = vi_trace_line_create_from_trace(trace_file))) {
    ...
    if (!strcmp(vi_trace_line_get_command(trace_line), "c"))
      vi_state->num_cycles =
          vi_trace_line_get_symbol_long_long(trace_line, "clk");
    ...
    if (num_trace_lines % VI_STATE_PROGRESS_INTERVAL == 1) {
      printf("Uncompressing trace (%.1fMB, %lld cycles)   \r",
             ftell(vi_state->unzipped_trace_file) / 1.048e6,
             vi_state->num_cycles);
      fflush(stdout);
    }
  }
...
void vi_state_done(void) {
  ...
  if (vi_state->body_trace_line) vi_trace_line_free(vi_state->body_trace_line);
...
long long vi_state_get_num_cycles(void) { return vi_state->num_cycles; }

long long vi_state_get_current_cycle(void) { return vi_state->cycle; }
...
void vi_state_create_checkpoints(void) {
  ...
  vi_state->cycle = 0;
  while ((trace_line =
              vi_trace_line_create_from_file(vi_state->unzipped_trace_file))) {
    struct vi_state_checkpoint_t *checkpoint;
    ...
    if (!strcasecmp(command, "c")) {
      ...
      while (vi_state->cycle >=
             last_checkpoint_cycle + VI_STATE_CHECKPOINT_INTERVAL) {
        last_checkpoint_cycle += VI_STATE_CHECKPOINT_INTERVAL;
        checkpoint = vi_state_checkpoint_create(
            last_checkpoint_cycle, vi_trace_line_get_offset(trace_line),
    ...
    /* Process trace line if it is not a header  */
      ...
      state_command->process_trace_line_func(state_command->user_data,
                                             trace_line);
    }
  ...
  if (!list_count(vi_state->checkpoint_list)) fatal("empty trace");

  /* Load first checkpoint */
  vi_state_read_checkpoint(0);
}

void vi_state_new_category(
    char *name, vi_state_read_checkpoint_func_t read_checkpoint_func,
    vi_state_write_checkpoint_func_t write_checkpoint_func, void *user_data) {
  struct vi_state_category_t *category;
...
struct vi_trace_line_t *vi_state_header_next(void) {
  ...
  if (vi_state->header_trace_line_offset < 0) return NULL;

  /* Read trace line */
  fseek(vi_state->unzipped_trace_file, vi_state->header_trace_line_offset,
        SEEK_SET);
...
struct vi_trace_line_t *vi_state_trace_line_first(long long cycle) {
  ...
  /* Invalid cycle */
  if (cycle > vi_state->num_cycles) return NULL;
  ...
  for (;;) {
    /* Read trace line */
    vi_state->body_trace_line =
        vi_trace_line_create_from_file(vi_state->unzipped_trace_file);
    vi_state->body_trace_line_offset = checkpoint->unzipped_trace_file_offset;
    if (!vi_state->body_trace_line || checkpoint_cycle == cycle) break;

    /* Check if target cycle is exceeded */
    if (!strcmp(vi_trace_line_get_command(vi_state->body_trace_line), "c") &&
        vi_trace_line_get_symbol_long_long(vi_state->body_trace_line, "clk") >=
            cycle)
      break;
...
struct vi_trace_line_t *vi_state_trace_line_next(void) {
  ...
  /* Get next trace line */
  fseek(vi_state->unzipped_trace_file, vi_state->body_trace_line_offset,
        SEEK_SET);
  vi_state->body_trace_line =
      vi_trace_line_create_from_file(vi_state->unzipped_trace_file);
...
void vi_state_go_to_cycle(long long cycle) {
  ...
  if (vi_state->cycle == cycle) return;
