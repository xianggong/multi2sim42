...
static void vi_list_item_free(struct vi_list_item_t *item) { free(item); }
...
static gboolean vi_list_popup_image_close_enter_notify_event(
    GtkWidget *widget, GdkEvent *event, struct vi_list_popup_t *popup) {
  gtk_image_set_from_file(GTK_IMAGE(popup->image_close),
                          vi_list_image_close_sel_path);
  return FALSE;
}

static gboolean vi_list_popup_image_close_leave_notify_event(
    GtkWidget *widget, GdkEvent *event, struct vi_list_popup_t *popup) {
  gtk_image_set_from_file(GTK_IMAGE(popup->image_close),
                          vi_list_image_close_path);
  return FALSE;
}

static void vi_list_popup_image_close_clicked_event(
    GtkWidget *widget, GdkEventButton *event, struct vi_list_popup_t *popup) {
  gtk_widget_destroy(popup->window);
}

static void vi_list_popup_destroy_event(GtkWidget *widget,
...
struct vi_list_popup_t *vi_list_popup_create(struct vi_list_t *list) {
  ...
  g_signal_connect(G_OBJECT(window), "destroy",
                   G_CALLBACK(vi_list_popup_destroy_event), popup);
  ...
  gtk_widget_add_events(event_box_close,
                        GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK);
  g_signal_connect(G_OBJECT(event_box_close), "enter-notify-event",
                   G_CALLBACK(vi_list_popup_image_close_enter_notify_event),
                   popup);
  g_signal_connect(G_OBJECT(event_box_close), "leave-notify-event",
                   G_CALLBACK(vi_list_popup_image_close_leave_notify_event),
                   popup);
  ...
  gtk_table_attach(GTK_TABLE(main_table), title_event_box, 0, 1, 0, 1,
                   GTK_FILL | GTK_EXPAND, GTK_FILL, 0, 0);
  gtk_table_attach(GTK_TABLE(main_table), event_box_close, 1, 2, 0, 1, GTK_FILL,
                   GTK_FILL, 0, 0);
  gtk_table_attach(GTK_TABLE(main_table), hsep, 0, 2, 1, 2, GTK_FILL, GTK_FILL,
                   0, 0);
  ...
  for (i = 0; i < count; i++) {
    ...
    gtk_table_attach(GTK_TABLE(table), event_box, 0, 1, i, i + 1, GTK_FILL,
                     GTK_FILL, 0, 0);
  }
...
void vi_list_popup_free(struct vi_list_popup_t *popup) { free(popup); }
...
static void vi_list_size_allocate_event(GtkWidget *widget,
...
struct vi_list_t *vi_list_create(char *title, int width, int height,
  ...
  g_signal_connect(G_OBJECT(list->widget), "size_allocate",
                   G_CALLBACK(vi_list_size_allocate_event), list);
...
GtkWidget *vi_list_get_widget(struct vi_list_t *list) { return list->widget; }
