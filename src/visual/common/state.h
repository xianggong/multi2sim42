...
typedef void (*vi_state_process_trace_line_func_t)(
    void *user_data, struct vi_trace_line_t *trace_line);

#define VI_STATE_FOR_EACH_HEADER(trace_line)                 \
  for ((trace_line) = vi_state_header_first(); (trace_line); \
       (trace_line) = vi_state_header_next())
