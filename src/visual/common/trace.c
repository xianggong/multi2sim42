...
struct vi_trace_line_t *vi_trace_line_create_from_file(FILE *f) {
  ...
  if (count != 4) panic("%s: invalid format", __FUNCTION__);
...
struct vi_trace_line_t *vi_trace_line_create_from_trace(
  ...
  while (isidchar(*buf_ptr)) buf_ptr++;
...
void vi_trace_line_dump(struct vi_trace_line_t *line, FILE *f) {
  ...
  if (count != 4) fatal("%s: error writing to file", __FUNCTION__);
...
char *vi_trace_line_get_symbol(struct vi_trace_line_t *line,
                               char *symbol_name) {
  char *value;
...
int vi_trace_line_get_symbol_int(struct vi_trace_line_t *line,
                                 char *symbol_name) {
  char *value;
...
long long vi_trace_line_get_symbol_long_long(struct vi_trace_line_t *line,
                                             char *symbol_name) {
  char *value;
...
unsigned int vi_trace_line_get_symbol_hex(struct vi_trace_line_t *line,
                                          char *symbol_name) {
  char *value;
  unsigned int hex;
  ...
  value = hash_table_get(line->symbol_table, symbol_name);
  if (value) {
    if (strlen(value) >= 2 && !strncasecmp(value, "0x", 2)) value += 2;
    sscanf(value, "%x", &hex);
    return hex;
  } else
    return 0;
