...
static void vi_si_time_dia_widget_destroy(GtkWidget *widget,
                                          struct vi_si_time_dia_t *time_dia) {
  vi_si_time_dia_free(time_dia);
}

static void vi_si_time_dia_size_allocate(GtkWidget *widget,
...
static void vi_si_time_dia_scrollbar_value_changed(
    GtkRange *range, struct vi_si_time_dia_t *time_dia) {
  vi_si_time_dia_refresh(time_dia);
}

static void vi_si_time_dia_refresh_scrollbars(
    struct vi_si_time_dia_t *time_dia) {
  ...
  if (table_width > time_dia->content_layout_width) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(time_dia->hscrollbar), VI_SI_TIME_DIA_CELL_WIDTH / 3,
        time_dia->content_layout_width - VI_SI_TIME_DIA_CELL_WIDTH / 3);
  ...
  if (table_height > time_dia->content_layout_height) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(time_dia->vscrollbar), VI_SI_TIME_DIA_CELL_HEIGHT,
        time_dia->content_layout_height - VI_SI_TIME_DIA_CELL_HEIGHT);
...
static void vi_si_time_dia_refresh_content_layout(
    struct vi_si_time_dia_t *time_dia) {
  GtkWidget *content_layout;
  GtkWidget *content_table;
  ...
  num_rows =
      MIN(num_insts,
          time_dia->content_layout_height / VI_SI_TIME_DIA_CELL_HEIGHT + 2);
  num_cols =
      MIN(num_cycles,
          time_dia->content_layout_width / VI_SI_TIME_DIA_CELL_WIDTH + 2);
  ...
  MATRIX_FOR_EACH(time_dia->content_matrix, row, col) {
    ...
    PangoAttribute *size_attr =
        pango_attr_size_new_absolute(VI_SI_TIME_DIA_FONT_SIZE << 10);
...
static void vi_si_time_dia_refresh_inst_layout(
    struct vi_si_time_dia_t *time_dia) {
  GtkWidget *inst_layout;
  GtkWidget *inst_table;
  ...
  /* Get new dimensions */
  num_insts = time_dia->compute_unit->num_insts;
  num_rows = MIN(num_insts,
                 time_dia->inst_layout_height / VI_SI_TIME_DIA_CELL_HEIGHT + 2);
...
static void vi_si_time_dia_refresh_cycle_layout(
    struct vi_si_time_dia_t *time_dia) {
  GtkWidget *cycle_layout;
  GtkWidget *cycle_table;
...
static void vi_si_time_dia_refresh_content(struct vi_si_time_dia_t *time_dia) {
  ...
  MATRIX_FOR_EACH_COLUMN(time_dia->content_matrix, col) {
    ...
    MATRIX_FOR_EACH_ROW(time_dia->content_matrix, row) {
      ...
      GdkColor color;
      char *color_str;
      color_str = inst ? str_map_value(&vi_si_inst_stage_color_map, inst->stage)
                       : "white";
...
struct vi_si_time_dia_t *vi_si_time_dia_create(
  ...
  gtk_widget_set_size_request(content_layout,
                              VI_SI_TIME_DIA_CONTENT_LAYOUT_WIDTH,
  ...
  g_signal_connect(G_OBJECT(hscrollbar), "value-changed",
                   G_CALLBACK(vi_si_time_dia_scrollbar_value_changed),
                   time_dia);
...
void vi_si_time_dia_go_to_cycle(struct vi_si_time_dia_t *time_dia,
  ...
  gtk_range_set_value(GTK_RANGE(time_dia->hscrollbar),
                      cycle * VI_SI_TIME_DIA_CELL_WIDTH);
  gtk_range_set_value(GTK_RANGE(time_dia->vscrollbar),
                      top_inst * VI_SI_TIME_DIA_CELL_HEIGHT);
}
