...
static void vi_si_gpu_map_work_group(struct vi_si_gpu_t *gpu,
  ...
  work_group = vi_si_work_group_create(work_group_name, work_group_id,
                                       work_item_id_first, work_item_count,
                                       wavefront_id_first, wavefront_count);

  /* Compute unit */
  compute_unit = list_get(vi_si_gpu->compute_unit_list, compute_unit_id);
  if (!compute_unit) panic("%s: invalid compute unit id", __FUNCTION__);

  /* Insert work-group */
  if (!hash_table_insert(compute_unit->work_group_table, work_group_name,
                         work_group))
...
static void vi_si_gpu_unmap_work_group(struct vi_si_gpu_t *gpu,
  ...
  snprintf(work_group_name, sizeof work_group_name, "wg-%d", work_group_id);
  work_group =
      hash_table_remove(compute_unit->work_group_table, work_group_name);
  if (!work_group) panic("%s: invalid work-group", __FUNCTION__);
 ...
 */
static void vi_si_gpu_new_inst(struct vi_si_gpu_t *gpu,
                               struct vi_trace_line_t *trace_line) {
  ...
  stage = str_map_string(&vi_si_inst_stage_map,
                         vi_trace_line_get_symbol(trace_line, "stg"));
  asm_code = vi_trace_line_get_symbol(trace_line, "asm");
  uop_id_in_wavefront =
      vi_trace_line_get_symbol_long_long(trace_line, "uop_id");

  /* Create */
  snprintf(inst_name, sizeof inst_name, "i-%lld", inst_id);
  inst = vi_si_inst_create(inst_name, inst_id, compute_unit_id, inst_buffer_id,
                           work_group_id, wavefront_id, stage,
                           uop_id_in_wavefront, asm_code);

  /* Get compute unit */
  compute_unit = list_get(gpu->compute_unit_list, compute_unit_id);
  if (!compute_unit) panic("%s: invalid compute unit", __FUNCTION__);
 ...
 * 	stg=<stage>	(e.g., "cf-fe")
 */
static void vi_si_gpu_inst(struct vi_si_gpu_t *gpu,
                           struct vi_trace_line_t *trace_line) {
  char inst_name[MAX_STRING_SIZE];
 ...
 * 	cu=<cu-id>	(e.g., 4)
 */
static void vi_si_gpu_end_inst(struct vi_si_gpu_t *gpu,
                               struct vi_trace_line_t *trace_line) {
  char inst_name[MAX_STRING_SIZE];
  ...
  if (!inst) panic("%s: invalid instruction", __FUNCTION__);
...
/* Version of the Southern Islands trace consumer. The major version must match
 * with the
...
static char *err_vi_si_trace_version =
    "\tThe Southern Islands trace file has been created with an incompatible "
    "version\n"
...
void vi_si_gpu_init(void) {
  ...
  /* State file */
  vi_state_new_category(
      "Southern Islands GPU",
      (vi_state_read_checkpoint_func_t)vi_si_gpu_read_checkpoint,
      (vi_state_write_checkpoint_func_t)vi_si_gpu_write_checkpoint, vi_si_gpu);

  /* Commands */
  vi_state_new_command(
      "si.new_ndrange",
      (vi_state_process_trace_line_func_t)vi_si_gpu_new_ndrange, vi_si_gpu);
  vi_state_new_command(
      "si.map_wg", (vi_state_process_trace_line_func_t)vi_si_gpu_map_work_group,
      vi_si_gpu);
  vi_state_new_command(
      "si.unmap_wg",
  ...
  vi_state_new_command("si.new_inst",
                       ...
                       vi_si_gpu);
  vi_state_new_command(
      "si.inst", (vi_state_process_trace_line_func_t)vi_si_gpu_inst, vi_si_gpu);
  ...
  VI_STATE_FOR_EACH_HEADER(trace_line) {
    ...
    if (!strcmp(command, "si.init")) {
      ...
      if (version) sscanf(version, "%d.%d", &version_major, &version_minor);
      if (version_major != VI_SI_TRACE_VERSION_MAJOR ||
          version_minor > VI_SI_TRACE_VERSION_MINOR)
        fatal(
            "incompatible Southern Islands trace version.\n"
      ...
      num_compute_units =
          vi_trace_line_get_symbol_int(trace_line, "num_compute_units");
