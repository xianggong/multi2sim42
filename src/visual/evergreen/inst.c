...
struct str_map_t vi_evg_inst_cat_map = {3,
...
struct vi_evg_inst_t *vi_evg_inst_create(
    char *name, long long id, int compute_unit_id, int work_group_id,
    int wavefront_id, enum vi_evg_inst_cat_t cat,
    enum vi_evg_inst_stage_t stage, char *asm_code, char *asm_code_x,
    char *asm_code_y, char *asm_code_z, char *asm_code_w, char *asm_code_t) {
...
void vi_evg_inst_get_markup(struct vi_evg_inst_t *inst, char *buf, int size) {
  ...
  /* Instruction ID */
  str_printf(&buf, &size, "%s<b>I-%lld</b>%s", begin_color, inst->id,
             end_color);
