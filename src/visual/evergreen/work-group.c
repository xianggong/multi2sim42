...
struct vi_evg_work_group_t *vi_evg_work_group_create(char *name, int id,
  ...
  /* Initialize */
  work_group = xcalloc(1, sizeof(struct vi_evg_work_group_t));
...
void vi_evg_work_group_get_name_short(char *work_group_name, char *buf,
                                      int size) {
...
void vi_evg_work_group_get_desc(char *work_group_name, char *buf, int size) {
  ...
  LIST_FOR_EACH(vi_evg_gpu->compute_unit_list, compute_unit_id) {
    ...
    work_group =
        hash_table_get(compute_unit->work_group_table, work_group_name);
    if (work_group) break;
  }
  if (!work_group)
...
void vi_evg_work_group_read_checkpoint(struct vi_evg_work_group_t *work_group,
                                       FILE *f) {
  int count;
  ...
  if (count != 20) panic("%s: cannot read checkpoint", __FUNCTION__);
...
void vi_evg_work_group_write_checkpoint(struct vi_evg_work_group_t *work_group,
                                        FILE *f) {
  int count;
  ...
  if (count != 20) panic("%s: cannot write checkpoint", __FUNCTION__);
}
