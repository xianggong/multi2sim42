 ...
 * 	t=<code>
 */
static void vi_evg_gpu_new_inst(struct vi_evg_gpu_t *gpu,
                                struct vi_trace_line_t *trace_line) {
  ...
  cat = str_map_string(&vi_evg_inst_cat_map,
                       vi_trace_line_get_symbol(trace_line, "cat"));
  stage = str_map_string(&vi_evg_inst_stage_map,
  ...
  inst = vi_evg_inst_create(inst_name, inst_id, compute_unit_id, work_group_id,
                            wavefront_id, cat, stage, asm_code, asm_code_x,
                            asm_code_y, asm_code_z, asm_code_w, asm_code_t);
 ...
 * 	stg=<stage>	(e.g., "cf-fe")
 */
static void vi_evg_gpu_inst(struct vi_evg_gpu_t *gpu,
                            struct vi_trace_line_t *trace_line) {
  char inst_name[MAX_STRING_SIZE];
 ...
 * 	cu=<cu-id>	(e.g., 4)
 */
static void vi_evg_gpu_end_inst(struct vi_evg_gpu_t *gpu,
                                struct vi_trace_line_t *trace_line) {
  char inst_name[MAX_STRING_SIZE];
...
/* Version of the Evergreen trace consumer. The major version must match with
 * the
...
void vi_evg_gpu_init(void) {
  ...
  vi_state_new_command(
      "evg.new_ndrange",
      (vi_state_process_trace_line_func_t)vi_evg_gpu_new_ndrange, vi_evg_gpu);
  vi_state_new_command(
      "evg.map_wg",
  ...
  VI_STATE_FOR_EACH_HEADER(trace_line) {
    ...
    if (!strcmp(command, "evg.init")) {
      ...
      if (version_major != VI_EVG_TRACE_VERSION_MAJOR ||
        ...
        fatal(
            "incompatible Evergreen trace version.\n"
