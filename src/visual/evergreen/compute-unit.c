...
struct vi_evg_compute_unit_t *vi_evg_compute_unit_create(char *name) {
  ...
  /* Initialize */
  compute_unit = xcalloc(1, sizeof(struct vi_evg_compute_unit_t));
...
void vi_evg_compute_unit_free(struct vi_evg_compute_unit_t *compute_unit) {
  ...
  /* Free work-groups */
  HASH_TABLE_FOR_EACH(compute_unit->work_group_table, work_group_name,
                      work_group)
...
void vi_evg_compute_unit_read_checkpoint(
  ...
  for (i = 0; i < num_work_groups; i++) {
    ...
    if (!hash_table_insert(compute_unit->work_group_table, work_group->name,
                           work_group))
  ...
  for (i = 0; i < num_insts; i++) {
    inst = vi_evg_inst_create(NULL, 0, 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL,
                              NULL, NULL);
...
void vi_evg_compute_unit_write_checkpoint(
  ...
  HASH_TABLE_FOR_EACH(compute_unit->work_group_table, work_group_name,
                      work_group)
  vi_evg_work_group_write_checkpoint(work_group, f);
  ...
  /* Write instructions */
  HASH_TABLE_FOR_EACH(compute_unit->inst_table, inst_name, inst)
  vi_evg_inst_write_checkpoint(inst, f);
}
