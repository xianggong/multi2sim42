...
#define VI_EVG_TIME_DIA_WINDOW_SIZE_X 600
#define VI_EVG_TIME_DIA_WINDOW_SIZE_Y 250
...
static struct vi_evg_time_dia_window_t *vi_evg_time_dia_window_create(
    struct vi_evg_compute_unit_t *compute_unit,
    GtkWidget *parent_toggle_button);
static void vi_evg_time_dia_window_free(
    struct vi_evg_time_dia_window_t *time_dia_window);

static void vi_evg_time_dia_window_destroy(
    GtkWidget *widget, struct vi_evg_time_dia_window_t *time_dia_window) {
  vi_evg_time_dia_window_free(time_dia_window);
}

static gboolean vi_evg_time_dia_window_delete(
    GtkWidget *widget, GdkEvent *event,
    struct vi_evg_time_dia_window_t *time_dia_window) {
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(time_dia_window->parent_toggle_button), FALSE);
  return TRUE;
}
...
static struct vi_evg_time_dia_window_t *vi_evg_time_dia_window_create(
  ...
  /* Initialize */
  time_dia_window = xcalloc(1, sizeof(struct vi_evg_time_dia_window_t));
  ...
  gtk_window_set_default_size(GTK_WINDOW(window), VI_EVG_TIME_DIA_WINDOW_SIZE_X,
                              VI_EVG_TIME_DIA_WINDOW_SIZE_Y);

  /* Time diagram */
  struct vi_evg_time_dia_t *time_dia;
  time_dia = vi_evg_time_dia_create(compute_unit);
  gtk_container_add(GTK_CONTAINER(window),
                    vi_evg_time_dia_get_widget(time_dia));
...
static void vi_evg_time_dia_window_go_to_cycle(
    struct vi_evg_time_dia_window_t *time_dia_window, long long cycle) {
  vi_evg_time_dia_go_to_cycle(time_dia_window->time_dia, cycle);
}

static void vi_evg_time_dia_window_refresh(
    struct vi_evg_time_dia_window_t *time_dia_window) {
  vi_evg_time_dia_refresh(time_dia_window->time_dia);
}

static GtkWidget *vi_evg_time_dia_window_get_widget(
    struct vi_evg_time_dia_window_t *time_dia_window) {
...
static struct vi_cu_board_t *vi_cu_board_create(
    struct vi_evg_compute_unit_t *compute_unit);
...
static void vi_cu_board_destroy(GtkWidget *widget,
                                struct vi_cu_board_t *board) {
  vi_cu_board_free(board);
}

static gboolean vi_cu_board_time_dia_toggled(GtkWidget *widget,
                                             struct vi_cu_board_t *board) {
  struct vi_evg_time_dia_window_t *time_dia_window;
  ...
  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(board->time_dia_toggle_button));

  /* Show */
  if (active && !board->time_dia_window) {
    time_dia_window = vi_evg_time_dia_window_create(
        board->compute_unit, board->time_dia_toggle_button);
  ...
  if (!active && board->time_dia_window) {
    gtk_widget_destroy(
        vi_evg_time_dia_window_get_widget(board->time_dia_window));
...
static struct vi_cu_board_t *vi_cu_board_create(
  ...
  /* Work-group list */
  struct vi_list_t *work_group_list = vi_list_create(
      "Work-group list", 10, 10,
      (vi_list_get_elem_name_func_t)vi_evg_work_group_get_name_short,
      (vi_list_get_elem_desc_func_t)vi_evg_work_group_get_desc);
  gtk_box_pack_start(GTK_BOX(vbox), vi_list_get_widget(work_group_list), TRUE,
                     TRUE, 0);
  ...
  g_signal_connect(G_OBJECT(board->widget), "destroy",
                   G_CALLBACK(vi_cu_board_destroy), board);
...
struct vi_evg_panel_t *vi_evg_panel_create(void) {
  ...
  /* Initialize */
  panel = xcalloc(1, sizeof(struct vi_evg_panel_t));
  ...
  gtk_widget_set_size_request(scrolled_window, VI_CU_BOARD_WIDTH * 3 / 2,
                              layout_height);
...
void vi_evg_panel_free(struct vi_evg_panel_t *panel) { free(panel); }
