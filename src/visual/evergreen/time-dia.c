...
static void vi_evg_time_dia_widget_destroy(GtkWidget *widget,
                                           struct vi_evg_time_dia_t *time_dia) {
  vi_evg_time_dia_free(time_dia);
}

static void vi_evg_time_dia_size_allocate(GtkWidget *widget,
...
static void vi_evg_time_dia_scrollbar_value_changed(
    GtkRange *range, struct vi_evg_time_dia_t *time_dia) {
  vi_evg_time_dia_refresh(time_dia);
}

static void vi_evg_time_dia_refresh_scrollbars(
    struct vi_evg_time_dia_t *time_dia) {
  ...
  if (table_width > time_dia->content_layout_width) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(time_dia->hscrollbar), VI_EVG_TIME_DIA_CELL_WIDTH / 3,
        time_dia->content_layout_width - VI_EVG_TIME_DIA_CELL_WIDTH / 3);
  ...
  if (table_height > time_dia->content_layout_height) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(time_dia->vscrollbar), VI_EVG_TIME_DIA_CELL_HEIGHT,
        time_dia->content_layout_height - VI_EVG_TIME_DIA_CELL_HEIGHT);
...
static void vi_evg_time_dia_refresh_content_layout(
    struct vi_evg_time_dia_t *time_dia) {
  GtkWidget *content_layout;
  GtkWidget *content_table;
  ...
  num_rows =
      MIN(num_insts,
          time_dia->content_layout_height / VI_EVG_TIME_DIA_CELL_HEIGHT + 2);
  num_cols =
      MIN(num_cycles,
          time_dia->content_layout_width / VI_EVG_TIME_DIA_CELL_WIDTH + 2);
  ...
  MATRIX_FOR_EACH(time_dia->content_matrix, row, col) {
    ...
    PangoAttribute *size_attr =
        pango_attr_size_new_absolute(VI_EVG_TIME_DIA_FONT_SIZE << 10);
...
static void vi_evg_time_dia_refresh_inst_layout(
    struct vi_evg_time_dia_t *time_dia) {
  GtkWidget *inst_layout;
  GtkWidget *inst_table;
  ...
  num_rows =
      MIN(num_insts,
          time_dia->inst_layout_height / VI_EVG_TIME_DIA_CELL_HEIGHT + 2);
...
static void vi_evg_time_dia_refresh_cycle_layout(
    struct vi_evg_time_dia_t *time_dia) {
  GtkWidget *cycle_layout;
  GtkWidget *cycle_table;
...
static void vi_evg_time_dia_refresh_content(
    struct vi_evg_time_dia_t *time_dia) {
  struct vi_evg_compute_unit_t *compute_unit;
    ...
    MATRIX_FOR_EACH_ROW(time_dia->content_matrix, row) {
      ...
      if (inst)
        gtk_label_set_markup(
            GTK_LABEL(label),
            str_map_value(&vi_evg_inst_stage_name_map, inst->stage));
      else
        gtk_label_set_text(GTK_LABEL(label), NULL);
      ...
      char *color_str;
      color_str = inst
                      ? str_map_value(&vi_evg_inst_stage_color_map, inst->stage)
                      : "white";
...
struct vi_evg_time_dia_t *vi_evg_time_dia_create(
  ...
  /* Initialize */
  time_dia = xcalloc(1, sizeof(struct vi_evg_time_dia_t));
  ...
  gtk_widget_set_size_request(content_layout,
                              VI_EVG_TIME_DIA_CONTENT_LAYOUT_WIDTH,
  ...
  gtk_widget_set_size_request(legend_layout,
                              VI_EVG_TIME_DIA_LEGEND_LAYOUT_WIDTH,
  ...
  g_signal_connect(G_OBJECT(hscrollbar), "value-changed",
                   G_CALLBACK(vi_evg_time_dia_scrollbar_value_changed),
                   time_dia);
...
void vi_evg_time_dia_go_to_cycle(struct vi_evg_time_dia_t *time_dia,
  ...
  if (!hash_table_count(compute_unit->inst_table)) top_inst = 0;

  /* Set scroll bar values */
  gtk_range_set_value(GTK_RANGE(time_dia->hscrollbar),
                      cycle * VI_EVG_TIME_DIA_CELL_WIDTH);
  gtk_range_set_value(GTK_RANGE(time_dia->vscrollbar),
                      top_inst * VI_EVG_TIME_DIA_CELL_HEIGHT);
}
