...
static void vi_mem_system_set_block(struct vi_mem_system_t *system,
  ...
  if (!mod) panic("%s: invalid module name '%s'", __FUNCTION__, mod_name);
...
static void vi_mem_system_clear_all_sharers(
    struct vi_mem_system_t *system, struct vi_trace_line_t *trace_line) {
  char *mod_name;
...
static void vi_mem_system_end_access(struct vi_mem_system_t *system,
  ...
  if (!access) panic("%s: access not found", __FUNCTION__);
...
static void vi_mem_system_end_access_block(struct vi_mem_system_t *system,
  ...
  access = hash_table_get(mod->access_table, access_name);
  if (!access)
    panic("%s: module %s: access %s: invalid access", __FUNCTION__, mod_name,
          access_name);
  ...
  if (!access) panic("%s: %s: invalid access", __FUNCTION__, access_name);
...
static void vi_mem_system_access(struct vi_mem_system_t *system,
  ...
  if (!access) panic("%s: %s: access not found", __FUNCTION__, name);

  /* Update access */
  vi_mod_access_set_state(access, state);
}

static void vi_mem_system_read_checkpoint(struct vi_mem_system_t *system,
                                          FILE *f) {
  ...
  for (i = 0; i < hash_table_count(vi_mem_system->mod_table); i++) {
    ...
    mod = hash_table_get(vi_mem_system->mod_table, mod_name);
    if (!mod) panic("%s: %s: invalid module name", __FUNCTION__, mod_name);
...
static void vi_mem_system_write_checkpoint(struct vi_mem_system_t *system,
                                           FILE *f) {
  struct vi_mod_t *mod;
  struct vi_mod_access_t *access;
...
/* Version of the memory system trace consumer. The major version must match
 * with the
...
static char *err_vi_mem_system_trace_version =
    "\tThe memory system trace file has been created with an incompatible "
    "version\n"
  ...
  VI_STATE_FOR_EACH_HEADER(trace_line) {
    ...
    if (!strcmp(command, "mem.init")) {
      ...
      if (version_major != VI_MEM_SYSTEM_TRACE_VERSION_MAJOR ||
        ...
        fatal(
            "incompatible memory system trace version.\n"
      ...
      if (mod->level < 1)
        panic("%s: %s: invalid level (%d)", __FUNCTION__, mod->name,
              mod->level);
