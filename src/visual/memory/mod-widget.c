...
/* Forwards */
static struct sharers_label_t *sharers_label_create(char *mod_name, int set,
                                                    int way);
static void sharers_label_free(struct sharers_label_t *sharers_label);
static GtkWidget *sharers_label_get_widget(
...
static gboolean sharers_label_enter_notify(
...
static gboolean sharers_label_leave_notify(
    GtkWidget *widget, GdkEventCrossing *event,
    struct sharers_label_t *sharers_label) {
  PangoAttrList *attrs;
  PangoAttribute *underline_attr;
  ...
  pango_attr_list_change(attrs, underline_attr);
  gtk_widget_modify_fg(sharers_label->label, GTK_STATE_NORMAL,
                       &sharers_label->label_color);

  return FALSE;
}

static void sharers_label_clicked(GtkWidget *widget, GdkEventButton *event,
                                  struct sharers_label_t *sharers_label) {
  struct vi_mod_t *mod;
  ...
  str_printf(&buf_ptr, &size, "%sModule %s - Block at set %d / way %d%s\n\n",
             title_format_begin, mod->name, sharers_label->set,
             sharers_label->way, title_format_end);
  ...
  for (sub_block = 0; sub_block < mod->num_sub_blocks; sub_block++) {
    ...
    if (dir_entry->owner >= 0) {
      ...
      if (!mod_sharer) panic("%s: invalid owner", __FUNCTION__);
    }
    ...
    for (sharer = 0; sharer < mod->num_sharers; sharer++) {
      ...
      if (!mod_sharer) panic("%s: invalid sharer", __FUNCTION__);
...
/* Forwards */
static struct accesses_label_t *accesses_label_create(char *mod_name, int set,
                                                      int way);
static void accesses_label_free(struct accesses_label_t *accesses_label);
static GtkWidget *accesses_label_get_widget(
...
static gboolean accesses_label_enter_notify(
    GtkWidget *widget, GdkEventCrossing *event,
    struct sharers_label_t *accesses_label) {
  GdkColor color;
...
static gboolean accesses_label_leave_notify(
    GtkWidget *widget, GdkEventCrossing *event,
    struct accesses_label_t *accesses_label) {
  PangoAttrList *attrs;
  PangoAttribute *underline_attr;
  ...
  underline_attr = pango_attr_underline_new(PANGO_UNDERLINE_NONE);
  pango_attr_list_change(attrs, underline_attr);
  gtk_widget_modify_fg(accesses_label->label, GTK_STATE_NORMAL,
                       &accesses_label->label_color);

  return FALSE;
}

static void accesses_label_clicked(GtkWidget *widget, GdkEventButton *event,
                                   struct accesses_label_t *accesses_label) {
  long long cycle;
  ...
  str_printf(&buf_ptr, &size, "%sModule %s - Accesses at set %d / way %d%s\n\n",
             title_format_begin, mod->name, accesses_label->set,
             accesses_label->way, title_format_end);

  /* Number of accesses */
  access_list =
      vi_mod_get_access_list(mod, accesses_label->set, accesses_label->way);
  ...
  LINKED_LIST_FOR_EACH(access_list) {
    ...
    str_printf(&buf_ptr, &size, "%sAccess[%d]%s = %s\n", title_format_begin,
               index, title_format_end, access->name);
...
static struct accesses_label_t *accesses_label_create(char *mod_name, int set,
  ...
  if (!num_accesses) strcpy(str, "-");

  /* Accesses label */
  GtkWidget *label;
  label = gtk_label_new(str);
  gtk_widget_set_size_request(label, VI_MOD_ACCESSES_LABEL_WIDTH,
                              VI_MOD_CELL_HEIGHT - 1);
  gtk_widget_show(label);
...
static void vi_mod_widget_size_allocate(GtkWidget *widget,
...
static void vi_mod_widget_scroll_bar_value_changed(
    GtkRange *range, struct vi_mod_widget_t *mod_widget) {
  vi_mod_widget_refresh(mod_widget);
}
...
struct vi_mod_widget_t *vi_mod_widget_create(char *name) {
  ...
  g_signal_connect(G_OBJECT(hscrollbar), "value-changed",
                   G_CALLBACK(vi_mod_widget_scroll_bar_value_changed),
                   mod_widget);
  ...
  pango_attr_list_insert(attrs,
                         pango_attr_size_new_absolute(VI_MOD_FONT_SIZE << 10));
  ...
  gtk_table_attach(GTK_TABLE(table), top_left_event_box, 0, 1, 0, 1, 0, 0, 0,
                   0);
  gtk_table_attach(GTK_TABLE(table), first_row_layout, 1, 2, 0, 1,
                   GTK_EXPAND | GTK_FILL | GTK_SHRINK, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(table), first_col_layout, 0, 1, 1, 2, 0,
                   GTK_EXPAND | GTK_FILL | GTK_SHRINK, 0, 0);
  gtk_table_attach(GTK_TABLE(table), hscrollbar, 1, 2, 2, 3,
                   GTK_EXPAND | GTK_FILL | GTK_SHRINK, GTK_FILL, 0, 0);
  gtk_table_attach(GTK_TABLE(table), vscrollbar, 2, 3, 1, 2, GTK_FILL,
                   GTK_EXPAND | GTK_FILL | GTK_SHRINK, 0, 0);
...
void vi_mod_widget_refresh(struct vi_mod_widget_t *mod_widget) {
  ...
  if (!mod) panic("%s: invalid module", __FUNCTION__);
  ...
  while ((child = gtk_container_get_children(
              GTK_CONTAINER(mod_widget->first_row_layout))))
    gtk_container_remove(GTK_CONTAINER(mod_widget->first_row_layout),
                         child->data);
  while ((child = gtk_container_get_children(
              GTK_CONTAINER(mod_widget->first_col_layout))))
    gtk_container_remove(GTK_CONTAINER(mod_widget->first_col_layout),
                         child->data);
  ...
  if (table_width > width) {
    gtk_range_set_range(GTK_RANGE(mod_widget->hscrollbar), 0,
                        table_width - width);
    gtk_range_set_increments(GTK_RANGE(mod_widget->hscrollbar),
                             VI_MOD_CELL_WIDTH / 3,
                             width - VI_MOD_CELL_WIDTH / 3);
  ...
  if (table_height > height) {
    gtk_range_set_range(GTK_RANGE(mod_widget->vscrollbar), 0,
                        table_height - height);
  ...
  while (y < height && set < mod->num_sets) {
    ...
    while (x < width && way < mod->assoc) {
      ...
      event_box = gtk_event_box_new();
      gtk_container_add(GTK_CONTAINER(event_box), label);
      gtk_layout_put(GTK_LAYOUT(layout), event_box, x + VI_MOD_LABEL_BLOCK_LEFT,
                     y);
      gtk_widget_show(event_box);
      ...
      gtk_layout_put(GTK_LAYOUT(layout),
                     sharers_label_get_widget(sharers_label),
      ...
      gtk_layout_put(GTK_LAYOUT(layout),
                     accesses_label_get_widget(accesses_label),
