...
static struct vi_mod_window_t *vi_mod_window_create(
    struct vi_mod_t *mod, GtkWidget *parent_toggle_button);
static void vi_mod_window_free(struct vi_mod_window_t *mod_window);

static void vi_mod_window_destroy(GtkWidget *widget,
                                  struct vi_mod_window_t *mod_window) {
  vi_mod_window_free(mod_window);
}

static gboolean vi_mod_window_delete(GtkWidget *widget, GdkEvent *event,
                                     struct vi_mod_window_t *mod_window) {
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(mod_window->parent_toggle_button), FALSE);
  return TRUE;
}
...
static struct vi_mod_window_t *vi_mod_window_create(
  ...
  gtk_container_add(GTK_CONTAINER(window),
                    vi_mod_widget_get_widget(mod_widget));
  mod_window->mod_widget = mod_widget;

  /* Associate widget */
  mod_window->widget = window;
  gtk_widget_show_all(mod_window->widget);
  g_signal_connect(G_OBJECT(mod_window->widget), "destroy",
                   G_CALLBACK(vi_mod_window_destroy), mod_window);
  g_signal_connect(G_OBJECT(mod_window->widget), "delete_event",
                   G_CALLBACK(vi_mod_window_delete), mod_window);
...
static void vi_mod_board_destroy(GtkWidget *widget,
                                 struct vi_mod_board_t *board) {
  vi_mod_board_free(board);
}

static gboolean vi_mod_board_toggle_button_toggled(
    GtkWidget *widget, struct vi_mod_board_t *board) {
  struct vi_mod_window_t *mod_window;
  ...
  /* Get button state */
  active =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(board->toggle_button));
...
static struct vi_mod_board_t *vi_mod_board_create(struct vi_mod_t *mod) {
  ...
  g_signal_connect(G_OBJECT(board->widget), "destroy",
                   G_CALLBACK(vi_mod_board_destroy), board);
...
static void vi_mod_board_refresh(struct vi_mod_board_t *board) {
  ...
  if (board->mod_window) vi_mod_window_refresh(board->mod_window);
}
...
struct vi_mem_panel_t *vi_mem_panel_create(void) {
  ...
  gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(scrolled_window),
                                        layout);
  gtk_widget_set_size_request(scrolled_window, VI_MOD_BOARD_WIDTH * 3 / 2,
                              VI_MOD_BOARD_HEIGHT * 3 / 2);
  ...
  LIST_FOR_EACH(vi_mem_system->mod_level_list, level_id) {
    ...
    LIST_FOR_EACH(mod_list, mod_id) {
      ...
      /* Get board position */
      x = VI_MOD_BOARD_PADDING +
          mod_id * (VI_MOD_BOARD_PADDING + VI_MOD_BOARD_WIDTH);
      y = VI_MOD_BOARD_PADDING +
          level_id * (VI_MOD_BOARD_PADDING + VI_MOD_BOARD_HEIGHT);
      ...
      /* Size of layout */
      layout_width =
          MAX(layout_width, x + VI_MOD_BOARD_WIDTH + VI_MOD_BOARD_PADDING);
      layout_height =
          MAX(layout_height, y + VI_MOD_BOARD_HEIGHT + VI_MOD_BOARD_PADDING);
    }
  }
...
void vi_mem_panel_free(struct vi_mem_panel_t *panel) { free(panel); }
