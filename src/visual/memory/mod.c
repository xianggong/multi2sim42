...
struct vi_mod_t *vi_mod_create(struct vi_trace_line_t *trace_line) {
  ...
  mod->sub_block_size =
      vi_trace_line_get_symbol_int(trace_line, "sub_block_size");
  ...
  mod->high_net_node_index =
      vi_trace_line_get_symbol_int(trace_line, "high_net_node");
  ...
  mod->low_net_node_index =
      vi_trace_line_get_symbol_int(trace_line, "low_net_node");
  ...
  /* Blocks */
  mod->blocks =
      xcalloc(mod->num_sets * mod->assoc, sizeof(struct vi_mod_block_t));
  for (set = 0; set < mod->num_sets; set++) {
    for (way = 0; way < mod->assoc; way++) {
      ...
      block->dir_entries =
          xcalloc(mod->num_sub_blocks, VI_MOD_DIR_ENTRY_SIZE(mod));
...
struct vi_mod_access_t *vi_mod_find_access(struct vi_mod_t *mod, int set,
                                           int way, char *access_name) {
  struct vi_mod_block_t *block;
  struct vi_mod_access_t *access;
  struct linked_list_t *access_list;
  ...
  LINKED_LIST_FOR_EACH(access_list) {
    ...
    if (!strcmp(access->name, access_name)) break;
  }

  /* Not found */
  if (linked_list_is_end(access_list)) return NULL;

  /* Return access */
  return access;
}

struct vi_mod_access_t *vi_mod_remove_access(struct vi_mod_t *mod, int set,
                                             int way, char *access_name) {
  struct vi_mod_block_t *block;
  struct vi_mod_access_t *access;
  struct linked_list_t *access_list;
...
struct linked_list_t *vi_mod_get_access_list(struct vi_mod_t *mod, int set,
                                             int way) {
  struct vi_mod_block_t *block;
...
void vi_mod_block_set(struct vi_mod_t *mod, int set, int way, unsigned int tag,
                      char *state) {
  struct vi_mod_block_t *block;

  if (!IN_RANGE(set, 0, mod->num_sets - 1))
    fatal("%s: invalid set", __FUNCTION__);
  if (!IN_RANGE(way, 0, mod->assoc - 1)) fatal("%s: invalid way", __FUNCTION__);
...
void vi_mod_read_checkpoint(struct vi_mod_t *mod, FILE *f) {
  ...
  if (count != 4) fatal("%s: error reading from checkpoint", __FUNCTION__);
  ...
  for (set = 0; set < mod->num_sets; set++) {
    for (way = 0; way < mod->assoc; way++) {
      ...
      if (count != 1) fatal("%s: error reading from checkpoint", __FUNCTION__);
      block->state = state;
...
void vi_mod_write_checkpoint(struct vi_mod_t *mod, FILE *f) {
  ...
  if (count != 4) fatal("%s: cannot write to checkpoint file", __FUNCTION__);
...
struct vi_mod_dir_entry_t *vi_mod_dir_entry_get(struct vi_mod_t *mod, int set,
                                                int way, int sub_block) {
  struct vi_mod_block_t *block;
  struct vi_mod_dir_entry_t *dir_entry;
  ...
  block = &mod->blocks[set * mod->assoc + way];
  dir_entry =
      (struct vi_mod_dir_entry_t *)(((void *)block->dir_entries) +
                                    VI_MOD_DIR_ENTRY_SIZE(mod) * sub_block);

  return dir_entry;
}

void vi_mod_dir_entry_set_sharer(struct vi_mod_t *mod, int x, int y, int z,
                                 int sharer) {
  struct vi_mod_dir_entry_t *dir_entry;
...
void vi_mod_dir_entry_clear_sharer(struct vi_mod_t *mod, int x, int y, int z,
                                   int sharer) {
  struct vi_mod_dir_entry_t *dir_entry;
...
void vi_mod_dir_entry_clear_all_sharers(struct vi_mod_t *mod, int x, int y,
                                        int z) {
  struct vi_mod_dir_entry_t *dir_entry;
...
int vi_mod_dir_entry_is_sharer(struct vi_mod_t *mod, int x, int y, int z,
                               int sharer) {
  struct vi_mod_dir_entry_t *dir_entry;
...
void vi_mod_dir_entry_set_owner(struct vi_mod_t *mod, int x, int y, int z,
                                int owner) {
  struct vi_mod_dir_entry_t *dir_entry;
...
void vi_mod_dir_entry_read_checkpoint(struct vi_mod_t *mod, int x, int y, int z,
                                      FILE *f) {
  struct vi_mod_dir_entry_t *dir_entry;
...
void vi_mod_dir_entry_write_checkpoint(struct vi_mod_t *mod, int x, int y,
                                       int z, FILE *f) {
  struct vi_mod_dir_entry_t *dir_entry;
