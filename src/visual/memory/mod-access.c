...
void vi_mod_access_read_checkpoint(struct vi_mod_access_t *access, FILE *f) {
  ...
  count = fread(&access->state_update_cycle, 1,
                sizeof access->state_update_cycle, f);
...
void vi_mod_access_write_checkpoint(struct vi_mod_access_t *access, FILE *f) {
  ...
  count = fwrite(&access->state_update_cycle, 1,
                 sizeof access->state_update_cycle, f);
  if (count != sizeof access->state_update_cycle)
    panic("%s: cannot write checkpoint", __FUNCTION__);
}
...
void vi_mod_access_get_desc(char *access_name, char *buf, int size) {
  ...
  for (i = 0; i < 50; i++) str_printf(&buf, &size, "-");
  str_printf(&buf, &size, "\n");
  cycle = access->creation_cycle;

  /* Log */
  for (trace_line = vi_state_trace_line_first(cycle); trace_line;
       trace_line = vi_state_trace_line_next()) {
    char *command;
    char *access_name;
    char *state;
    ...
    /* Access starts */
    if ((!strcmp(command, "mem.new_access") &&
         !strcmp(access_name, access->name)) ||
        (!strcmp(command, "mem.access") &&
         !strcmp(access_name, access->name))) {
    ...
    /* Access ends */
    if (!strcmp(command, "mem.end_access") &&
        !strcmp(access_name, access->name))
      break;
