...
void vi_x86_context_get_desc(char *context_name, char *buf, int size) {
  ...
  if (!context) panic("%s: %s: invalid context", __FUNCTION__, context_name);

  /* Title */
  str_printf(&buf, &size, "%sDescription for context %s%s\n\n", title_begin,
             context->name, title_end);
...
void vi_x86_context_write_checkpoint(struct vi_x86_context_t *context,
                                     FILE *f) {
  int count;
  ...
  /* Creation cycle */
  count =
      fwrite(&context->creation_cycle, 1, sizeof context->creation_cycle, f);
  if (count != sizeof context->creation_cycle)
    panic("%s: cannot write checkpoint", __FUNCTION__);
}
