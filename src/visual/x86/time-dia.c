...
#define VI_X86_TIME_DIA_MACRO_INST_LAYOUT_HEIGHT \
...
static void vi_x86_time_dia_widget_destroy(GtkWidget *widget,
                                           struct vi_x86_time_dia_t *time_dia) {
  vi_x86_time_dia_free(time_dia);
}

static void vi_x86_time_dia_size_allocate(GtkWidget *widget,
...
static void vi_x86_time_dia_scrollbar_value_changed(
    GtkRange *range, struct vi_x86_time_dia_t *time_dia) {
  vi_x86_time_dia_refresh(time_dia);
}

static void vi_x86_time_dia_refresh_scrollbars(
    struct vi_x86_time_dia_t *time_dia) {
  ...
  if (table_width > time_dia->content_layout_width) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(time_dia->hscrollbar), VI_X86_TIME_DIA_CELL_WIDTH / 3,
        time_dia->content_layout_width - VI_X86_TIME_DIA_CELL_WIDTH / 3);
  ...
  if (table_height > time_dia->content_layout_height) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(time_dia->vscrollbar), VI_X86_TIME_DIA_CELL_HEIGHT,
        time_dia->content_layout_height - VI_X86_TIME_DIA_CELL_HEIGHT);
...
static void vi_x86_time_dia_refresh_content_layout(
    struct vi_x86_time_dia_t *time_dia) {
  GtkWidget *content_layout;
  GtkWidget *content_table;
  ...
  num_rows =
      MIN(num_insts,
          time_dia->content_layout_height / VI_X86_TIME_DIA_CELL_HEIGHT + 2);
  num_cols =
      MIN(num_cycles,
          time_dia->content_layout_width / VI_X86_TIME_DIA_CELL_WIDTH + 2);

  /* Create new matrix */
  if (time_dia->content_matrix) matrix_free(time_dia->content_matrix);
  time_dia->content_matrix = matrix_create(num_rows, num_cols);

  /* Create new table */
  content_layout = time_dia->content_layout;
  if (time_dia->content_table) gtk_widget_destroy(time_dia->content_table);
  ...
  MATRIX_FOR_EACH(time_dia->content_matrix, row, col) {
    ...
    PangoAttribute *size_attr =
        pango_attr_size_new_absolute(VI_X86_TIME_DIA_FONT_SIZE << 10);
    ...
    gtk_table_attach_defaults(GTK_TABLE(content_table), frame, col, col + 1,
                              row, row + 1);
...
static void vi_x86_time_dia_refresh_macro_inst_layout(
    struct vi_x86_time_dia_t *time_dia) {
  GtkWidget *macro_inst_layout;
  GtkWidget *macro_inst_table;
  ...
  num_rows =
      MIN(num_insts,
          time_dia->macro_inst_layout_height / VI_X86_TIME_DIA_CELL_HEIGHT + 2);

  /* Clear instruction label list */
  if (!time_dia->macro_inst_list) time_dia->macro_inst_list = list_create();
  ...
  gtk_layout_put(GTK_LAYOUT(macro_inst_layout), macro_inst_table, 0,
                 time_dia->top_offset);
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    gtk_widget_set_size_request(event_box, time_dia->macro_inst_layout_width,
                                VI_X86_TIME_DIA_CELL_HEIGHT);
    gtk_container_add(GTK_CONTAINER(event_box), label);
    gtk_table_attach_defaults(GTK_TABLE(macro_inst_table), event_box, 0, 1, row,
                              row + 1);
...
static void vi_x86_time_dia_refresh_inst_layout(
    struct vi_x86_time_dia_t *time_dia) {
  GtkWidget *inst_layout;
  GtkWidget *inst_table;
  ...
  num_rows =
      MIN(num_insts,
          time_dia->inst_layout_height / VI_X86_TIME_DIA_CELL_HEIGHT + 2);

  /* Clear instruction label list */
  if (!time_dia->inst_list) time_dia->inst_list = list_create();
  list_clear(time_dia->inst_list);

  /* Create new table */
  inst_layout = time_dia->inst_layout;
  if (time_dia->inst_table) gtk_widget_destroy(time_dia->inst_table);
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    gtk_widget_set_size_request(event_box, time_dia->inst_layout_width,
                                VI_X86_TIME_DIA_CELL_HEIGHT);
    gtk_container_add(GTK_CONTAINER(event_box), label);
    gtk_table_attach_defaults(GTK_TABLE(inst_table), event_box, 0, 1, row,
                              row + 1);
...
static void vi_x86_time_dia_refresh_cycle_layout(
    struct vi_x86_time_dia_t *time_dia) {
  GtkWidget *cycle_layout;
  GtkWidget *cycle_table;
  ...
  if (!time_dia->cycle_list) time_dia->cycle_list = list_create();
  list_clear(time_dia->cycle_list);

  /* Create new table */
  cycle_layout = time_dia->cycle_layout;
  if (time_dia->cycle_table) gtk_widget_destroy(time_dia->cycle_table);
  cycle_table = gtk_table_new(1, num_cols, TRUE);
  gtk_layout_put(GTK_LAYOUT(cycle_layout), cycle_table, time_dia->left_offset,
                 0);
  ...
  for (col = 0; col < num_cols; col++) {
    ...
    GtkWidget *event_box = gtk_event_box_new();
    gtk_container_add(GTK_CONTAINER(event_box), label);
    gtk_table_attach_defaults(GTK_TABLE(cycle_table), event_box, col, col + 1,
                              0, 1);
...
static void vi_x86_time_dia_refresh_content(
    struct vi_x86_time_dia_t *time_dia) {
  struct vi_x86_core_t *core;
  ...
  MATRIX_FOR_EACH_COLUMN(time_dia->content_matrix, col) {
    ...
    HASH_TABLE_FOR_EACH(core->inst_table, inst_name, inst) {
      ...
      if (!IN_RANGE(inst_row, 0, list_count(time_dia->inst_list) - 1)) continue;
      ...
      /* Instruction */
      snprintf(str, sizeof str, "%s%s%s", begin_color, inst->asm_micro_code,
               end_color);
      ...
      if (inst->asm_code && *inst->asm_code && event_box) {
        label = gtk_bin_get_child(GTK_BIN(event_box));
        snprintf(str, sizeof str, "%s<b>%s</b>%s", begin_color, inst->asm_code,
                 end_color);
        gtk_label_set_markup(GTK_LABEL(label), str);
      }
    }
    ...
    MATRIX_FOR_EACH_ROW(time_dia->content_matrix, row) {
      ...
      char *color_str;
      color_str = inst
                      ? str_map_value(&vi_x86_inst_stage_color_map, inst->stage)
                      : "white";
...
struct vi_x86_time_dia_t *vi_x86_time_dia_create(struct vi_x86_core_t *core) {
  ...
  gtk_widget_set_size_request(content_layout,
                              VI_X86_TIME_DIA_CONTENT_LAYOUT_WIDTH,
                              VI_X86_TIME_DIA_CONTENT_LAYOUT_HEIGHT);
  ...
  gtk_widget_set_size_request(macro_inst_layout,
                              VI_X86_TIME_DIA_MACRO_INST_LAYOUT_WIDTH,
  ...
  gtk_widget_set_size_request(inst_title_layout,
                              VI_X86_TIME_DIA_INST_LAYOUT_WIDTH,
                              VI_X86_TIME_DIA_CELL_HEIGHT);
  ...
  gtk_widget_set_size_request(macro_inst_title_layout,
                              VI_X86_TIME_DIA_MACRO_INST_LAYOUT_WIDTH,
                              VI_X86_TIME_DIA_CELL_HEIGHT);
  ...
  gtk_box_pack_start(GTK_BOX(left_vbox), macro_inst_title_layout, FALSE, FALSE,
                     0);
  ...
  g_signal_connect(G_OBJECT(hscrollbar), "value-changed",
                   G_CALLBACK(vi_x86_time_dia_scrollbar_value_changed),
                   time_dia);
...
void vi_x86_time_dia_free(struct vi_x86_time_dia_t *time_dia) {
  ...
  if (time_dia->macro_inst_list) list_free(time_dia->macro_inst_list);

  /* Instruction label list */
  if (time_dia->inst_list) list_free(time_dia->inst_list);

  /* Cycle label list */
  if (time_dia->cycle_list) list_free(time_dia->cycle_list);
...
void vi_x86_time_dia_refresh(struct vi_x86_time_dia_t *time_dia) {
  ...
  content_layout_width =
      gtk_widget_get_allocated_width(time_dia->content_layout);
  content_layout_height =
      gtk_widget_get_allocated_height(time_dia->content_layout);
  macro_inst_layout_width =
      gtk_widget_get_allocated_width(time_dia->macro_inst_layout);
  macro_inst_layout_height =
      gtk_widget_get_allocated_height(time_dia->macro_inst_layout);
  ...
  macro_inst_layout_width_changed =
      macro_inst_layout_width != time_dia->macro_inst_layout_width;
  macro_inst_layout_height_changed =
      macro_inst_layout_height != time_dia->macro_inst_layout_height;
  ...
  if (left_offset_changed || top_offset_changed)
    gtk_layout_move(GTK_LAYOUT(time_dia->content_layout),
                    time_dia->content_table, time_dia->left_offset,
                    time_dia->top_offset);
  ...
  if (top_offset_changed)
    gtk_layout_move(GTK_LAYOUT(time_dia->macro_inst_layout),
                    time_dia->macro_inst_table, 0, time_dia->top_offset);
  ...
  if (top_offset_changed)
    gtk_layout_move(GTK_LAYOUT(time_dia->inst_layout), time_dia->inst_table, 0,
                    time_dia->top_offset);
...
void vi_x86_time_dia_go_to_cycle(struct vi_x86_time_dia_t *time_dia,
  ...
  if (!hash_table_count(core->inst_table)) top_inst = 0;

  /* Set scroll bar values */
  gtk_range_set_value(GTK_RANGE(time_dia->hscrollbar),
                      cycle * VI_X86_TIME_DIA_CELL_WIDTH);
  gtk_range_set_value(GTK_RANGE(time_dia->vscrollbar),
                      top_inst * VI_X86_TIME_DIA_CELL_HEIGHT);
}
