...
static void vi_x86_cpu_map_context(struct vi_x86_cpu_t *cpu,
  ...
  if (!core) panic("%s: invalid core", __FUNCTION__);
...
static void vi_x86_cpu_unmap_context(struct vi_x86_cpu_t *cpu,
  ...
  snprintf(context_name, sizeof context_name, "ctx-%d", context_id);
  context = hash_table_get(vi_x86_cpu->context_table, context_name);
  if (!context || context->core_id != core_id ||
      context->thread_id != thread_id)
    panic("%s: invalid context", __FUNCTION__);
...
static void vi_x86_cpu_end_context(struct vi_x86_cpu_t *cpu,
  ...
  if (!context) panic("%s: invalid context", __FUNCTION__);
...
static void vi_x86_cpu_new_inst(struct vi_x86_cpu_t *cpu,
  ...
  stage = str_map_string(&vi_x86_inst_stage_map,
  ...
  inst =
      vi_x86_inst_create(id, name, asm_code, asm_micro_code, spec_mode, stage);
...
static void vi_x86_cpu_inst(struct vi_x86_cpu_t *cpu,
  ...
  if (!inst) panic("%s: invalid instruction - %s", __FUNCTION__, name);
...
void vi_x86_cpu_init(void) {
  ...
  vi_state_new_category(
      "x86 CPU", (vi_state_read_checkpoint_func_t)vi_x86_cpu_read_checkpoint,
      (vi_state_write_checkpoint_func_t)vi_x86_cpu_write_checkpoint,
      vi_x86_cpu);

  /* Commands */
  vi_state_new_command(
      "x86.map_ctx", (vi_state_process_trace_line_func_t)vi_x86_cpu_map_context,
      vi_x86_cpu);
  vi_state_new_command(
      "x86.unmap_ctx",
      (vi_state_process_trace_line_func_t)vi_x86_cpu_unmap_context, vi_x86_cpu);
  vi_state_new_command(
      "x86.end_ctx", (vi_state_process_trace_line_func_t)vi_x86_cpu_end_context,
      vi_x86_cpu);
  ...
  VI_STATE_FOR_EACH_HEADER(trace_line) {
    ...
    if (!strcmp(command, "x86.init")) {
      ...
      if (version_major != VI_X86_TRACE_VERSION_MAJOR ||
        ...
        fatal(
            "incompatible x86 trace version.\n"
