...
char *net_config_help =
    ...
    "      bytes per cycle. If a link's bandwidth is not specified, this "
    "value\n"
    "      will be used.\n"
    "\n"
    ...
    "  OutputBufferSize = <size> (Default = "
    "<network>.DefaultOutputBufferSize)\n"
    ...
    "network <network>. A link connects an output buffer of a source node "
    "with\n"
...
static void net_traffic_uniform(struct net_t *net, double *inject_time) {
  while (1) {
    ...
    for (i = 0; i < net->node_count; i++) {
      ...
      if (node->kind != net_node_end) continue;

      /* Turn for next injection? */
      if (inject_time[i] > cycle) continue;

      /* Get a random destination node */
      do {
        dst_node =
            list_get(net->node_list, random() % list_count(net->node_list));
