...
static long long net_command_get_llint(struct list_t *token_list,
  ...
  if (err || command_cycle < 1)
    fatal("%s: %s: invalid %s , integer >= 1 expected.\n\t> %s", __FUNCTION__,
          str_token_list_first(token_list), command_line, expected);
...
static long long net_command_get_def(struct list_t *token_list,
                                     char *command_line, char *expected,
                                     long long def) {
  if (!list_count(token_list)) {
    return def;
  } else
    return net_command_get_llint(token_list, command_line, expected);
}

static struct net_node_t *net_command_get_node(
    struct net_t *net, struct list_t *token_list, char *command_line,
...
static struct net_buffer_t *net_command_get_buffer(struct net_node_t *node,
  ...
  if (!buffer) {
    fatal("%s: %s: invalid buffer name in node %s.\n\t> %s", __FUNCTION__,
          buffer_name, node->name, command_line);
  }
  return buffer;
}
...
void net_command_handler(int event, void *data) {
  ...
  *msg_detail_str = '\0';
  ...
  if (event == EV_NET_COMMAND) {
    ...
    long long command_cycle =
        net_command_get_llint(token_list, command_line, "cycle value");
    ...
    net_command_get_string(token_list, command_line, command, sizeof command);
    ...
    if (!strcasecmp(command, "Send")) {
      ...
      src_node =
          net_command_get_node(net, token_list, command_line, net_node_end);
      ...
      msg_size = (int)net_command_get_def(token_list, command_line,
                                          "Message size value",
                                          (long long)net_msg_size);

      /* Getting the Message ID; or default value */
      msg_id = net_command_get_def(token_list, command_line, "Message id value",
                                   net->msg_id_counter + 1);

      if (msg_id != net->msg_id_counter + 1) {
        fatal(
            "%s: The message id is out of order. "
            "\n\t\"You are out o' order. "
      ...
      msg_id =
          net_command_get_llint(token_list, command_line, "Message id value");

      /* Output */
      str_printf(&msg_str, &msg_size,
                 "CHECK: Cycle %lld: Receive in node %s for message"
                 " %lld ",
                 cycle, dst_node->name, msg_id);
       ...
       * is the destination node for the message*/
        ...
        if (dst_node != msg->dst_node)
          warning(
              "%s: The node %s in the receive "
              "command is \n\tnot intended node "
              "%s in the send command\n > %s",
              __FUNCTION__, dst_node->name, msg->dst_node->name, command_line);
      ...
      node = net_command_get_node(net, token_list, command_line, 0);

      msg_id =
          net_command_get_llint(token_list, command_line, "invalid message id");
      /* Output */
      str_printf(&msg_str, &msg_size,
                 "CHECK: Cycle %lld: message %lld is in input buffer of "
                 "node %s",
                 cycle, msg_id, node->name);
      ...
      if (!msg) {
        ...
        } else if (buffer != list_get(node->input_buffer_list, buffer->index)) {
          test_failed = 1;
          str_printf(&msg_detail_str, &msg_detail_size,
      ...
      if (!msg) {
        ...
        if (msg->node != node) {
          ...
          str_printf(&msg_detail_str, &msg_detail_size,
                   ...
                   list_get(node->output_buffer_list, buffer->index)) {
          test_failed = 1;
          str_printf(&msg_detail_str, &msg_detail_size,
      ...
      /* Output */
      str_printf(&msg_str, &msg_size,
                 "CHECK: Cycle %lld: message %lld is in node %s", cycle, msg_id,
                 node->name);
      ...
      buffer = net_command_get_buffer(node, token_list, command_line);
      ...
      str_printf(&msg_str, &msg_size,
                 ...
                 " of node %s",
                 cycle, msg_id, buffer->name, node->name);
      ...
      if (!msg) {
        ...
        if (msg->buffer != buffer || msg->node != node) {
          ...
          str_printf(&msg_detail_str, &msg_detail_size,
                     ...
                     "(node %s)\n",
                     buffer->name, node->name, msg->buffer->name,
                     msg->node->name);
        }
      }
      /* Output */
