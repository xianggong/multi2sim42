...
struct net_node_t *net_node_create(struct net_t *net, enum net_node_kind_t kind,
                                   int index, int input_buffer_size,
...
struct net_buffer_t *net_get_buffer_by_name(struct net_node_t *node,
  ...
  for (i = 0; i < list_count(node->input_buffer_list); i++) {
    ...
    if (!strcasecmp(buffer->name, buffer_name)) return buffer;
  }
...
void net_node_free(struct net_node_t *node) {
  ...
  if (node->kind == net_node_bus) {
    ...
    if (node->src_buffer_list) list_free(node->src_buffer_list);
    if (node->dst_buffer_list) list_free(node->dst_buffer_list);
  }
...
struct net_buffer_t *net_node_schedule(struct net_node_t *node,
  ...
  assert(list_get(node->output_buffer_list, output_buffer->index) ==
         output_buffer);
   ...
   * same value. */
  if (output_buffer->sched_when == cycle) return output_buffer->sched_buffer;

  /* Make a new decision */
  output_buffer->sched_when = cycle;
  input_buffer_count = list_count(node->input_buffer_list);
  last_input_buffer_index =
      output_buffer->sched_buffer ? output_buffer->sched_buffer->index : 0;
  ...
  for (i = 0; i < input_buffer_count; i++) {
    input_buffer_index = (last_input_buffer_index + i + 1) % input_buffer_count;
    input_buffer = list_get(node->input_buffer_list, input_buffer_index);
    ...
    if (msg->busy >= cycle) continue;

    /* Input buffer must be ready to be read */
    if (input_buffer->read_busy >= cycle) continue;

    /* Message must target this output buffer */
    entry = net_routing_table_lookup(routing_table, node, msg->dst_node);
    assert(entry->output_buffer);
    if (entry->output_buffer != output_buffer) continue;

    /* Message must fit in this output buffer */
    if (output_buffer->count + msg->size > output_buffer->size) continue;
