...
struct net_link_t *net_link_create(struct net_t *net,
  ...
  if (bandwidth < 1) panic("%s: invalid bandwidth", __FUNCTION__);
...
void net_link_dump_report(struct net_link_t *link, FILE *f) {
  ...
  fprintf(f, "BytesPerCycle = %.4f\n",
          cycle ? (double)link->transferred_bytes / cycle : 0.0);
  fprintf(f, "Utilization = %.4f\n",
          cycle ? (double)link->transferred_bytes / (cycle * link->bandwidth)
                : 0.0);
  fprintf(f, "\n");
}
...
struct net_buffer_t *net_link_arbitrator_vc(struct net_link_t *link,
  ...
  for (i = 0; i < output_buffer_count; i++) {
    ...
    if (temp_out_buffer->link == link) out_vc_count++;
    if (out_vc_count == 1) first_src_buffer_act_index = temp_out_buffer->index;
  }
  ...
  for (int j = 0; j < input_buffer_count; j++) {
    ...
    if (temp_in_buffer->link == link) in_vc_count++;
    if (in_vc_count == 1) first_dst_buffer_act_index = temp_in_buffer->index;
  }
  ...
  if (link->sched_when == cycle) return link->sched_buffer;

  /* make a new decision */
  link->sched_when = cycle;
  last_output_buffer_vc_index =
      link->sched_buffer
          ? (link->sched_buffer->index - first_src_buffer_act_index)
          : 0;
  ...
  for (i = 0; i < link->virtual_channel; i++) {
    output_buffer_vc_index =
        (last_output_buffer_vc_index + i + 1) % link->virtual_channel;
    output_buffer =
    ...
    if (output_buffer->read_busy >= cycle) continue;
    ...
    input_buffer =
        list_get(dst_node->input_buffer_list,
                 (output_buffer_vc_index + first_dst_buffer_act_index));
