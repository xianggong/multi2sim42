...
struct net_buffer_t *net_buffer_create(struct net_t *net,
  ...
  buffer->size = size;
  if (size < 1) panic("%s: invalid size", __FUNCTION__);
...
void net_buffer_dump_report(struct net_buffer_t *buffer, FILE *f) {
  ...
  fprintf(f, "%s.MessageOccupancy = %.2f\n", buffer->name,
          cycle ? (double)buffer->occupancy_msgs_acc / cycle : 0.0);
  fprintf(f, "%s.ByteOccupancy = %.2f\n", buffer->name,
          cycle ? (double)buffer->occupancy_bytes_acc / cycle : 0.0);
  fprintf(
      f, "%s.Utilization = %.4f\n", buffer->name,
      cycle ? (double)buffer->occupancy_bytes_acc / cycle / buffer->size : 0.0);
}
