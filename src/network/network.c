...
static void net_config_route_create(struct net_t *net, struct config_t *config,
                                    char *section) {
  char *token;
  ...
  for (int i = 0; i < net->node_count; i++) {
    for (int j = 0; j < net->node_count; j++) {
      ...
      if (dst_node_r->kind == net_node_end) {
        ...
        nxt_node_name =
            config_read_string(config, section, spr_result_size, "---");
        ...
        if (name_check == 1) {
          if (nxt_node_r == NULL)
            ...
            net_routing_table_route_create(net->routing_table, src_node_r,
                                           dst_node_r, nxt_node_r, vc_used);
        }
      }
    }
  }
...
static void net_config_command_create(struct net_t *net,
  ...
  if (net_injection_rate > 0.001)
    fatal(
        "Network %s:%s: Using Command section; \n"
  ...
  if (strcmp(net_traffic_pattern, "") &&
    ...
    fatal(
        "Network %s: Command option doesn't comply with other "
        "traffic pattern\n (%s)",
        net->name, net_traffic_pattern);
  net_traffic_pattern = "command";
  command_var_id = 0;

  /* Register events for command handler*/
  EV_NET_COMMAND = esim_register_event_with_name(
      net_command_handler, net_domain_index, "net_command");
  EV_NET_COMMAND_RCV = esim_register_event_with_name(
      net_command_handler, net_domain_index, "net_command_receive");
...
struct net_msg_t *net_msg_table_get(struct net_t *net, long long id) {
  ...
  while (msg && msg->id != id) msg = msg->bucket_next;
  return msg;
}
...
struct net_msg_t *net_msg_table_extract(struct net_t *net, long long id) {
  ...
  if (!msg) panic("%s: message %lld not in hash table", __FUNCTION__, id);
  if (prev)
...
struct net_t *net_create_from_config(struct config_t *config, char *name) {
  ...
  for (section = config_section_first(config); section;
    ...
    if (strcasecmp(section, section_str)) continue;

    net->def_input_buffer_size =
        config_read_int(config, section, "DefaultInputBufferSize", 0);
    net->def_output_buffer_size =
        config_read_int(config, section, "DefaultOutputBufferSize", 0);
  ...
  for (section = config_section_first(config); section;
    ...
    if (!token || strcasecmp(token, "Network")) continue;

    /* Second token must be the name of the network */
    token = strtok(NULL, delim);
    if (!token || strcasecmp(token, name)) continue;

    /* Third token must be 'Node' */
    token = strtok(NULL, delim);
    if (!token || strcasecmp(token, "Node")) continue;
    ...
    input_buffer_size = config_read_int(config, section, "InputBufferSize",
                                        def_input_buffer_size);
    output_buffer_size = config_read_int(config, section, "OutputBufferSize",
    ...
    if (!strcasecmp(node_type, "EndNode"))
      net_add_end_node(net, input_buffer_size, output_buffer_size, node_name,
                       NULL);
    else if (!strcasecmp(node_type, "Switch"))
      net_add_switch(net, input_buffer_size, output_buffer_size, bandwidth,
                     node_name);
      ...
      if (input_buffer_size != def_input_buffer_size ||
        ...
        fatal(
            "%s:%s: BUS does not contain input/output buffers. "
  ...
  for (section = config_section_first(config); section;
    ...
    src_buffer_size = config_read_int(config, section, "SourceBufferSize", 0);
    dst_buffer_size = config_read_int(config, section, "DestBufferSize", 0);
    ...
    if (!src_node)
      fatal("%s: %s: %s: source node does not exist.\n%s", name, section,
            src_node_name, net_err_config);
    if (!dst_node)
      fatal("%s: %s: %s: destination node does not exist.\n%s", name, section,
            dst_node_name, net_err_config);
    ...
    if (src_node->kind != net_node_bus && dst_node->kind != net_node_bus) {
      ...
      if (v_channel_count >= 1) {
        if (!strcasecmp(link_type, "Unidirectional")) {
          ...
          link_dst_bsize =
              (dst_buffer_size) ? dst_buffer_size : dst_node->input_buffer_size;

          net_add_link(net, src_node, dst_node, bandwidth, link_src_bsize,
                       link_dst_bsize, v_channel_count);
    ...
    /* If is is a Bus Connection */
      ...
      if (!strcasecmp(link_type, "Unidirectional")) {
        if ((src_node->kind == net_node_bus && src_buffer_size) ||
            (dst_node->kind == net_node_bus && dst_buffer_size)) {
          fatal("%s: %s: Source/Destination BUS cannot have buffer. \n %s ",
                name, section, net_err_config);
        }

        net_add_bus_port(net, src_node, dst_node, src_buffer_size,
                         dst_buffer_size);
      } else if (!strcasecmp(link_type, "Bidirectional")) {
        net_add_bidirectional_bus_port(net, src_node, dst_node, src_buffer_size,
                                       dst_buffer_size);
      }
    }
  }
  ...
  for (section = config_section_first(config); section;
    ...
    token = strtok(NULL, delim);
    if (!token || strcasecmp(token, "Routes")) continue;
  ...
  for (section = config_section_first(config); section;
    ...
    token = strtok(NULL, delim);
    if (!token || strcasecmp(token, "Commands")) continue;
  ...
  if (routing_type == 0) net_routing_table_floyd_warshall(net->routing_table);
...
void net_dump_report(struct net_t *net, FILE *f) {
  ...
  fprintf(f, "AverageMessageSize = %.2f\n",
          net->transfers ? (double)net->msg_size_acc / net->transfers : 0.0);
  fprintf(f, "AverageLatency = %.4f\n",
          net->transfers ? (double)net->lat_acc / net->transfers : 0.0);
  fprintf(f, "\n");
  ...
  for (i = 0; i < list_count(graph->edge_list); i++) {
    ...
    if (edge->kind == net_edge_link)
      fprintf(f, "link = %f %d %f %d %d %d \n",
              ...
              edge->src_vertex->y_coor,
              (double)edge->dst_vertex->x_coor / graph->xscale,
              edge->dst_vertex->y_coor,
              (int)((cycle
                         ? (double)edge->downstream->transferred_bytes /
                               (cycle * edge->downstream->bandwidth)
                         : 0.0) *
                    10),
              1);
      ...
      fprintf(f, "link = %f %d %f %d %d %d \n",
              ...
              edge->src_vertex->y_coor,
              (int)((cycle
                         ? (double)edge->upstream->transferred_bytes /
                               (cycle * edge->upstream->bandwidth)
                         : 0.0) *
                    10),
              2);
...
struct net_node_t *net_add_end_node(struct net_t *net, int input_buffer_size,
                                    int output_buffer_size, char *name,
                                    void *user_data) {
  struct net_node_t *node;

  /* Create node */
  node =
      net_node_create(net, net_node_end,                        /* kind */
                      net->node_count,                          /* index */
                      input_buffer_size, output_buffer_size, 0, /* bandwidth */
                      name, user_data);
...
struct net_node_t *net_add_bus(struct net_t *net, int bandwidth, char *name,
  ...
  struct net_node_t *node;
  /* Create node */
  node = net_node_create(net, net_node_bus,      /* kind */
                         net->node_count,        /* index */
                         0,                      /* input_buffer_size */
                         0,                      /* output_buffer_size */
                         bandwidth, name, NULL); /* user_data */
...
struct net_node_t *net_add_switch(struct net_t *net, int input_buffer_size,
                                  int output_buffer_size, int bandwidth,
                                  char *name) {
  struct net_node_t *node;

  /* Create node */
  node = net_node_create(net, net_node_switch, /* kind */
                         net->node_count,      /* index */
                         input_buffer_size, output_buffer_size, bandwidth, name,
                         NULL); /* user_data */
...
struct net_node_t *net_get_node_by_user_data(struct net_t *net,
  ...
  for (i = 0; i < list_count(net->node_list); i++) {
    node = list_get(net->node_list, i);
    if (node->user_data == user_data) return node;
  }
  return NULL;
}

/* Create link with virtual channel */
struct net_link_t *net_add_link(struct net_t *net, struct net_node_t *src_node,
  ...
  link = net_link_create(net, src_node, dst_node, bandwidth, link_src_bsize,
                         link_dst_bsize, vc_count);
...
void net_add_bidirectional_link(struct net_t *net, struct net_node_t *src_node,
                                struct net_node_t *dst_node, int bandwidth,
                                int link_src_bsize, int link_dst_bsize,
                                int vc_count) {
  int src_buffer_size;
  ...
  src_buffer_size =
      (link_src_bsize) ? link_src_bsize : src_node->output_buffer_size;
  dst_buffer_size =
      (link_dst_bsize) ? link_dst_bsize : dst_node->input_buffer_size;
  net_add_link(net, src_node, dst_node, bandwidth, src_buffer_size,
               dst_buffer_size, vc_count);

  src_buffer_size =
      (link_src_bsize) ? link_src_bsize : dst_node->output_buffer_size;
  dst_buffer_size =
      (link_dst_bsize) ? link_dst_bsize : src_node->input_buffer_size;
  net_add_link(net, dst_node, src_node, bandwidth, src_buffer_size,
               dst_buffer_size, vc_count);
}

void net_add_bus_port(struct net_t *net, struct net_node_t *src_node,
                      struct net_node_t *dst_node, int bus_src_buffer,
                      int bus_dst_buffer) {
  /* Checks */
  assert(src_node->net == net);
  assert(dst_node->net == net);
  ...
  if (src_node->kind == net_node_bus && dst_node->kind == net_node_bus)
    fatal("network \"%s\" : BUS to BUS connection is not supported.",
          net->name);
  ...
  if (src_node->kind == net_node_bus && dst_node->kind != net_node_bus) {
    int dst_buffer_size =
        (bus_dst_buffer) ? bus_dst_buffer : dst_node->input_buffer_size;
   ...
   * BUS */
    ...
    int src_buffer_size =
        (bus_src_buffer) ? bus_src_buffer : src_node->output_buffer_size;
...
int net_can_send(struct net_t *net, struct net_node_t *src_node,
  ...
  if (output_buffer->write_busy >= cycle) return 0;
...
int net_can_send_ev(struct net_t *net, struct net_node_t *src_node,
                    struct net_node_t *dst_node, int size, int retry_event,
                    void *retry_stack) {
  struct net_routing_table_t *routing_table = net->routing_table;
  struct net_routing_table_entry_t *entry;
  struct net_buffer_t *output_buffer;
...
void net_receive(struct net_t *net, struct net_node_t *node,
  ...
  assert(node->net == net);
  assert(msg->net == net);
  if (msg->node != node) panic("%s: message not at end node", __FUNCTION__);

  /* Get buffer */
  buffer = msg->buffer;
  assert(buffer->node == node);
  if (!list_count(buffer->msg_list)) panic("%s: empty buffer", __FUNCTION__);
