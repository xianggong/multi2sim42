 ...
 * be required. */
static void routing_table_cycle_detection_dfs_visit(
    struct net_routing_table_t *routing_table, struct list_t *buffer_list,
  ...
  for (j = 0; j < routing_table->dim && !routing_table->has_cycle; j++) {
    ...
    if (entry->output_buffer == buffer_elem) {
      ...
      entry_adj =
          net_routing_table_lookup(routing_table, entry->next_node, node_adj);
      ...
      for (int i = 0; i < buffer_count; i++) {
        ...
        if (buffer_adj == entry_adj->output_buffer) {
          ...
          if (buffer_color == NET_NODE_COLOR_GRAY &&
              parent_index != buffer_elem) {
 ...
 * O(|E|+|V|) */
static void net_routing_table_cycle_detection(
  ...
  for (i = 0; i < buffer_count && !routing_table->has_cycle; i++) {
    ...
    if (buffer_color == NET_NODE_COLOR_WHITE) {
      routing_table_cycle_detection_dfs_visit(
          routing_table, buffer_list, color_list, parent_list, i, buffer_count);
    }
  }
...
void net_routing_table_free(struct net_routing_table_t *routing_table) {
  if (routing_table->entries) free(routing_table->entries);
  free(routing_table);
}
...
void net_routing_table_initiate(struct net_routing_table_t *routing_table) {
  ...
  if (routing_table->entries)
    panic("%s: network \"%s\": routing table already allocated", __FUNCTION__,
          net->name);
  ...
  for (i = 0; i < net->node_count; i++) {
    ...
    for (j = 0; j < list_count(src_node->output_buffer_list); j++) {
      ...
      if (buffer->kind == net_buffer_link) {
        ...
        assert(link);
        entry =
            net_routing_table_lookup(routing_table, src_node, link->dst_node);
        entry->cost = 1;
       ...
       * connected to the BUS */
        ...
        for (k = 0; k < list_count(bus_node->dst_buffer_list); k++) {
          ...
          /*For BUS. Even though we never get to BUS in the routing table (no
           * routes path
           * through BUS node. They all pass through BUS node's buffer) we still
           * provide a
           * Path from BUS nodes to all the nodes. There is no path from nodes
           * that ends in
           * BUS. Next buffer is still NULL*/
          entry = net_routing_table_lookup(routing_table, bus_node,
                                           dst_buffer->node);
          entry->cost = 1;
          entry->next_node = dst_buffer->node;
        }
...
void net_routing_table_floyd_warshall(
    struct net_routing_table_t *routing_table) {
  int i, j, k;
  struct net_t *net = routing_table->net;
      ...
      if (node_i->kind != net_node_bus) {
        ...
        for (k = 0; k < list_count(node_i->output_buffer_list); k++) {
          ...
          if (buffer->link) {
            link = buffer->link;
            assert(link);
            if (link->dst_node == next_node) break;
          } else if (buffer->bus) {
            int l;
            int check = 0;
            struct net_node_t *bus_node;
  ...
  for (i = 0; i < net->node_count; i++)
  {
          for (j = 0; j < net->node_count; j++)
          {
                  ...
                  while (k != j)
                  {
                          ...
                          node_k = list_get(net->node_list, k);
                          entry_k_j = net_routing_table_lookup(routing_table,
  node_k, node_j);
...
struct net_routing_table_entry_t *net_routing_table_lookup(
  ...
  entry =
      &routing_table
           ->entries[src_node->index * routing_table->dim + dst_node->index];
  return entry;
}
...
void net_routing_table_route_update(struct net_routing_table_t *routing_table,
  ...
  for (k = 0;
       (k < list_count(src_node->output_buffer_list) && route_check != 1);
       k++) {
    ...
    if (buffer->kind == net_buffer_link) {
      ...
      if ((link->dst_node == next_node)) {
        if (vc_num == 0) {
          ...
          if (link->virtual_channel <= vc_num)
            fatal(
                "Network %s: %s.to.%s: wrong virtual channel "
          ...
          vc_buffer =
              list_get(src_node->output_buffer_list, (buffer->index) + vc_num);
      ...
      if (vc_num != 0)
        fatal(
            "Network %s: %s.to.%s: BUS does not contain virtual channel \n %s",
            routing_table->net->name, src_node->name, dst_node->name,
            net_err_config);
    }
  }
  ...
  if (route_check == 0)
    fatal("Network %s : route %s.to.%s = %s : Missing connection \n%s ",
          routing_table->net->name, src_node->name, dst_node->name,
...
void net_routing_table_route_create(struct net_routing_table_t *routing_table,
  ...
  if (nxt_node_r) {
    if (src_node_r == dst_node_r)
      ...
      if (vc_used > 0)
        net_routing_table_route_update(routing_table, src_node_r, dst_node_r,
                                       nxt_node_r, vc_used);
      else {
        vc_used = 0;
        net_routing_table_route_update(routing_table, src_node_r, dst_node_r,
