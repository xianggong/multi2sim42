...
struct net_graph_t *net_visual_calc(struct net_t *net) {
  ...
  int width;
  width = list_count(graph->vertex_list) / 2 + 1;  // choosing a width for
                                                   // graph.
  ...
  for (int i = 0; i < layer_count + 1; i++) {
    ...
    if (max_node_in_layer < count) max_node_in_layer = count;
    for (int j = 0; j < list_count(graph->vertex_list); j++) {
      vertex = list_get(graph->vertex_list, j);
      if (vertex->y_coor == i) vertex->neighbours = count;
    }
  }
  ...
  for (int i = 0; i < list_count(graph->vertex_list); i++) {
    ...
    vertex->x_coor =
        (2 * (vertex->x_coor) + (max_node_in_layer - vertex->neighbours)) *
        opt_distance;
    graph->xscale = scale;
  }
...
struct net_graph_vertex_t *net_graph_vertex_create(struct net_graph_t *graph,
                                                   char *name) {
  struct net_graph_vertex_t *vertex;
...
void net_graph_edge_free(struct net_graph_edge_t *edge) { free(edge); }
  ...
  while (label <= graph->vertex_count) {
    ...
    for (int i = 0; i < list_count(graph->vertex_list); i++) {
      ...
      if (vertex->key_val == 0) {
        ...
        for (int j = 0; j < list_count(vertex->incoming_vertex_list); j++) {
          ...
          if (inc_vertex->key_val == 0) checker = 1;
        }
    ...
    if (list_count(candidate_list) != 0) {
      ...
      for (int i = 1; i < list_count(candidate_list); i++) {
        ...
        assert(vertex);
        int min_num = ((candidate->indeg > vertex->indeg) ? vertex->indeg
                                                          : candidate->indeg);
        int resolved = 0;
        int i_index = 1;
        // comparison
        while ((resolved == 0) && (i_index <= min_num)) {
          ...
          vertex1 = list_get(candidate->incoming_vertex_list,
                             candidate->indeg - i_index);
          assert(vertex1);
          vertex2 =
              list_get(vertex->incoming_vertex_list, vertex->indeg - i_index);
          assert(vertex2);
          if (vertex1 == vertex2)
            i_index++;
        ...
        if (resolved == 0) {
          if (candidate->indeg > vertex->indeg) candidate = vertex;
        }
      }
    }
  ...
  while (list_count(V) != 0) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (vertex->outdeg == 0) {
        ...
        for (int j = 0; j < list_count(vertex->outgoint_vertex_list); j++) {
          ...
          for (int num = 0; num < list_count(U); num++) {
            ...
            if (out_vertex == U_elem) check_count++;
          }
        }
      ...
      for (int j = 0; j < list_count(candidate->outgoint_vertex_list); j++) {
        ...
        if (temp_vertex->y_coor >= k) k = temp_vertex->y_coor + 1;
      }
    ...
    for (int i = 0; i < list_count(graph->vertex_list); i++) {
      ...
      if (vertex->y_coor == x) {
        ...
        if (list_count(vertex->incoming_vertex_list) != 0)
          vertex->cross_num =
              vertex->cross_num / list_count(vertex->incoming_vertex_list);
        else
    ...
    for (int i = 0; i < list_count(graph->vertex_list); i++) {
      ...
      if (vertex->y_coor == x) {
        ...
        if (list_count(vertex->outgoint_vertex_list))
          vertex->cross_num =
              vertex->cross_num / list_count(vertex->outgoint_vertex_list);
        else
          vertex->cross_num = (float)vertex->x_coor;
      }
    }
...
struct net_graph_vertex_t *net_get_vertex_by_node(struct net_graph_t *graph,
  ...
  for (int i = 0; i < list_count(graph->vertex_list); i++) {
    vertex = list_get(graph->vertex_list, i);
    if (vertex->node == node) return vertex;
  }
  return NULL;
}
