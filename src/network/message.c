...
struct net_msg_t *net_msg_create(struct net_t *net, struct net_node_t *src_node,
  ...
  if (size < 1) panic("%s: bad size", __FUNCTION__);

  /* Return */
  return msg;
}

void net_msg_free(struct net_msg_t *msg) { free(msg); }
...
void net_event_handler(int event, void *data) {
  ...
  if (event == EV_NET_SEND) {
    ...
    net_debug(
        ...
        "src=\"%s\" "
        "dst=\"%s\"\n",
        net->name, msg->id, msg->size, src_node->name, dst_node->name);
    ...
    if (buffer->kind == net_buffer_link) {
      ...
      entry = net_routing_table_lookup(routing_table, msg->node, msg->dst_node);
      ...
      if (input_buffer_detection == 0)
        fatal(
            "%s: Something went wrong so there is no appropriate input"
            "buffer for the route between %s and %s \n",
            net->name, msg->node->name, entry->next_node->name);
    ...
    if (stack->ret_event == ESIM_EV_NONE) net_receive(net, node, msg);
