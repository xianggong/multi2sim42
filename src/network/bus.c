...
static struct net_buffer_t *net_bus_arbiteration_per_lane(
    struct net_node_t *bus_node, struct net_bus_t *bus) {
   ...
   * same value. */
  if (bus->sched_when == cycle) return bus->sched_buffer;
  ...
  for (i = 0; i < input_buffer_count; i++) {
    ...
    if (src_buffer->read_busy >= cycle) continue;
...
struct net_bus_t *net_bus_arbitration(struct net_node_t *bus_node,
  ...
  for (i = 0; i < list_count(bus_node->bus_lane_list); i++) {
    ...
    if (net_bus_arbiteration_per_lane(bus_node, check_bus) == buffer)
      return check_bus;
  }
  return NULL;
}
...
void net_bus_dump_report(struct net_bus_t *bus, FILE *f) {
  ...
  fprintf(f, "%s.BytesPerCycle = %.4f\n", bus->name,
          cycle ? (double)bus->transferred_bytes / cycle : 0.0);
  fprintf(
      f, "%s.Utilization = %.4f\n", bus->name,
      cycle ? (double)bus->transferred_bytes / (cycle * bus->bandwidth) : 0.0);
  fprintf(f, "\n");
}
