...
struct mod_stack_t *mod_stack_create(long long id, struct mod_t *mod,
                                     unsigned int addr, int ret_event,
  ...
  if (ret_stack != NULL) stack->client_info = ret_stack->client_info;
  stack->way = -1;
  stack->set = -1;
  stack->tag = -1;
...
void mod_stack_wait_in_mod(struct mod_stack_t *stack, struct mod_t *mod,
                           int event) {
...
void mod_stack_wait_in_port(struct mod_stack_t *stack, struct mod_port_t *port,
                            int event) {
...
void mod_stack_wakeup_stack(struct mod_stack_t *master_stack) {
  ...
  if (!master_stack->waiting_list_count) return;

  /* Debug */
  mem_debug("  %lld %lld 0x%x wake up accesses:", esim_time, master_stack->id,
            master_stack->addr);
...
struct mod_t *mod_stack_set_peer(struct mod_t *peer, int state) {
  ...
  if (state == cache_block_owned || mem_peer_transfers) ret = peer;
