...
struct prefetcher_t *prefetcher_create(int prefetcher_ghb_size,
  ...
  pref->index_table =
      xcalloc(prefetcher_it_size, sizeof(struct prefetcher_it_t));
  pref->ghb_head = -1;
...
static void get_it_index_tag(struct prefetcher_t *pref,
                             struct mod_stack_t *stack, int *it_index,
                             unsigned *tag) {
...
static int prefetcher_update_tables(struct mod_stack_t *stack,
  ...
  if (pref->index_table[it_index].tag > 0) {
    ...
    if (pref->index_table[it_index].tag != it_tag) {
      mem_debug(
          "  %lld it_index = %d, old_tag = 0x%x, new_tag = 0x%x"
          "prefetcher: replace index_table entry\n",
          stack->id, it_index, pref->index_table[it_index].tag, it_tag);
 ...
 * of the instruction causing the miss. The GHB entries are looked at for
 * finding
 * constant stride accesses. Based on this, prefetching is done. */
static void prefetcher_ghb_pc_cs(struct mod_t *mod, struct mod_stack_t *stack,
                                 int it_index) {
  struct prefetcher_t *pref;
  int chain, stride, i;
  ...
  if (pref->ghb[chain].next == -1) return;
...
void prefetcher_access_miss(struct mod_stack_t *stack,
                            struct mod_t *target_mod) {
  int it_index;
