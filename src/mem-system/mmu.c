...
static struct mmu_page_t *mmu_get_page(int address_space_index,
  ...
  /* Look for page */
  index = ((vtladdr >> mmu_log_page_size) + address_space_index * 23) %
          MMU_PAGE_HASH_SIZE;
...
void mmu_dump_report(void) {
  ...
  for (i = 0; i < list_count(mmu->page_list); i++) {
    ...
    fprintf(f, "%5d %5d %9x %9x %10lld %10lld %10lld %10lld\n", i + 1,
            page->address_space_index, page->vtl_addr, page->phy_addr,
            num_accesses, page->num_read_accesses, page->num_write_accesses,
            page->num_execute_accesses);
  }
  fclose(f);
