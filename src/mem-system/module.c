...
struct str_map_t mod_access_kind_map = {3,
...
struct mod_t *mod_create(char *name, enum mod_kind_t kind, int num_ports,
  ...
  mod->client_info_repos =
      repos_create(sizeof(struct mod_client_info_t), mod->name);
...
void mod_free(struct mod_t *mod) {
  ...
  if (mod->cache) cache_free(mod->cache);
  if (mod->dir) dir_free(mod->dir);
  free(mod->ports);
...
long long mod_access(struct mod_t *mod, enum mod_access_kind_t access_kind,
                     unsigned int addr, int *witness_ptr,
  ...
  mod_stack_id++;
  stack = mod_stack_create(mod_stack_id, mod, addr, ESIM_EV_NONE, NULL);
...
int mod_can_access(struct mod_t *mod, unsigned int addr) {
  ...
  if (mod->num_locked_ports == mod->num_ports) return 0;

  /* If no MSHR is given, module can be accessed */
  if (!mod->mshr_size) return 1;
...
void mod_block_set_prefetched(struct mod_t *mod, unsigned int addr, int val) {
  ...
  if (mod->cache->prefetcher &&
      mod_find_block(mod, addr, &set, &way, NULL, NULL)) {
 ...
 * The address of the access is passed as well because this lookup is done on
 * the
...
int mod_in_flight_access(struct mod_t *mod, long long id, unsigned int addr) {
  ...
  for (stack = mod->access_hash_table[index].bucket_list_head; stack;
    ...
    if (stack->id == id) return 1;

  /* Not found */
  return 0;
}

/* Return the youngest in-flight access older than 'older_than_stack' to block
 * containing 'addr'.
 * If 'older_than_stack' is NULL, return the youngest in-flight access
 * containing 'addr'.
 * The function returns NULL if there is no in-flight access to block containing
 * 'addr'.
 */
struct mod_stack_t *mod_in_flight_address(
    struct mod_t *mod, unsigned int addr,
  ...
  for (stack = mod->access_hash_table[index].bucket_list_head; stack;
    ...
    if (older_than_stack && stack->id >= older_than_stack->id) continue;
...
/* Return the youngest in-flight write older than 'older_than_stack'. If
 * 'older_than_stack'
 * is NULL, return the youngest in-flight write. Return NULL if there is no
 * in-flight write.
 */
struct mod_stack_t *mod_in_flight_write(struct mod_t *mod,
  ...
  if (!older_than_stack) return mod->write_access_list_tail;

  /* Search */
  for (stack = older_than_stack->access_list_prev; stack;
       stack = stack->access_list_prev)
    if (stack->access_kind == mod_access_store) return stack;
...
int mod_serves_address(struct mod_t *mod, unsigned int addr) {
  ...
  if (mod->range_kind == mod_range_bounds)
    return addr >= mod->range.bounds.low && addr <= mod->range.bounds.high;

  /* Interleaved addresses */
  if (mod->range_kind == mod_range_interleaved)
    return (addr / mod->range.interleaved.div) % mod->range.interleaved.mod ==
...
struct mod_t *mod_get_low_mod(struct mod_t *mod, unsigned int addr) {
  ...
  LINKED_LIST_FOR_EACH(mod->low_mod_list) {
    ...
    if (!mod_serves_address(low_mod, addr)) continue;

    /* Address served by more than one module */
    if (server_mod)
      fatal("%s: low modules %s and %s both serve address 0x%x", mod->name,
            server_mod->name, low_mod->name, addr);
  ...
  if (!server_mod)
    fatal("module %s: no lower module serves address 0x%x", mod->name, addr);
...
struct mod_stack_t *mod_can_coalesce(struct mod_t *mod,
  ...
  assert(access_kind);
  if (!mod_in_flight_address(mod, addr, older_than_stack)) return NULL;
  ...
  switch (access_kind) {
    ...
    case mod_access_store: {
      ...
      if (stack->access_kind != mod_access_store) return NULL;
      ...
      if (stack->port_locked) return NULL;
    ...
    case mod_access_nc_store: {
      ...
      if (stack->access_kind != mod_access_nc_store) return NULL;
...
void mod_coalesce(struct mod_t *mod, struct mod_stack_t *master_stack,
  ...
  mem_debug("  %lld %lld 0x%x %s coalesce with %lld\n", esim_time, stack->id,
            stack->addr, mod->name, master_stack->id);
