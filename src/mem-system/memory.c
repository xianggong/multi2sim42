...
/* Return the memory page following addr in the current memory map. This
 * function
...
struct mem_page_t *mem_page_get_next(struct mem_t *mem, unsigned int addr) {
  ...
  while (page && page->tag != tag) page = page->next;
  if (page) return page;
...
/* Create new mem page */
static struct mem_page_t *mem_page_create(struct mem_t *mem, unsigned int addr,
                                          int perm) {
  unsigned int index, tag;
...
void mem_copy(struct mem_t *mem, unsigned int dest, unsigned int src,
              int size) {
  ...
  while (size > 0) {
    ...
    if (page_src->data) {
      if (!page_dest->data) page_dest->data = xmalloc(MEM_PAGE_SIZE);
      memcpy(page_dest->data, page_src->data, MEM_PAGE_SIZE);
    } else {
      if (page_dest->data) memset(page_dest->data, 0, MEM_PAGE_SIZE);
    }
...
void *mem_get_buffer(struct mem_t *mem, unsigned int addr, int size,
  ...
  if (offset + size > MEM_PAGE_SIZE) return NULL;
  ...
  if (!page->data) page->data = xcalloc(1, MEM_PAGE_SIZE);
...
static void mem_access_page_boundary(struct mem_t *mem, unsigned int addr,
  ...
  if (!page) {
    if (mem->safe) fatal("illegal access at 0x%x: page not allocated", addr);
    if (access == mem_access_read || access == mem_access_exec) {
      memset(buf, 0, size);
      return;
    }
    if (access == mem_access_write || access == mem_access_init) {
      page = mem_page_create(mem, addr, mem_access_read | mem_access_write |
                                            mem_access_exec | mem_access_init);
    }
  }
  assert(page);
  ...
  if (access == mem_access_write) page->perm |= mem_access_modif;
...
void mem_clear(struct mem_t *mem) {
  ...
  for (i = 0; i < MEM_PAGE_COUNT; i++)
    while (mem->pages[i]) mem_page_free(mem, mem->pages[i]->tag);
}
...
unsigned int mem_map_space(struct mem_t *mem, unsigned int addr, int size) {
  ...
  for (;;) {
    ...
    if (!tag_end) return (unsigned int)-1;
    ...
    /* Enough free pages */
    if (tag_end - tag_start + MEM_PAGE_SIZE == size) break;
...
unsigned int mem_map_space_down(struct mem_t *mem, unsigned int addr,
                                int size) {
  unsigned int tag_start;
  unsigned int tag_end;
  ...
  for (;;) {
    /* Address space overflow */
    if (!tag_start) return (unsigned int)-1;
...
void mem_map(struct mem_t *mem, unsigned int addr, int size,
  ...
  for (tag = tag1; tag <= tag2; tag += MEM_PAGE_SIZE) {
    ...
    if (!page) page = mem_page_create(mem, tag, perm);
    page->perm |= perm;
  }
}
...
void mem_unmap(struct mem_t *mem, unsigned int addr, int size) {
  ...
  /* Deallocate pages */
  for (tag = tag1; tag <= tag2; tag += MEM_PAGE_SIZE) mem_page_free(mem, tag);
}

/* Assign protection attributes to pages */
void mem_protect(struct mem_t *mem, unsigned int addr, int size,
                 enum mem_access_t perm) {
  unsigned int tag1, tag2, tag;
  struct mem_page_t *page;
...
void mem_zero(struct mem_t *mem, unsigned int addr, int size) {
  ...
  while (size--) mem_access(mem, addr++, 0, &zero, mem_access_write);
}

void mem_dump(struct mem_t *mem, char *filename, unsigned int start,
              unsigned int end) {
  FILE *f;
  unsigned int size;
  ...
  f = fopen(filename, "wb");
  if (!f) fatal("mem_dump: cannot open file '%s'", filename);
...
void mem_load(struct mem_t *mem, char *file_name, unsigned int start) {
  ...
  if (!f) fatal("mem_load: cannot open file '%s'", file_name);
...
/* Copy the entire content of a memory image into another. Any previously
 * existing
...
void mem_clone(struct mem_t *dst_mem, struct mem_t *src_mem) {
  ...
  for (i = 0; i < MEM_PAGE_COUNT; i++) {
    for (page = src_mem->pages[i]; page; page = page->next) {
      ...
      if (page->data)
        mem_access(dst_mem, page->tag, MEM_PAGE_SIZE, page->data,
                   mem_access_init);
    }
  }
