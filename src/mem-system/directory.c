...
#define DIR_ENTRY(X, Y, Z)                                                \
  ((struct dir_entry_t *)(((void *)&dir->data) +                          \
                          DIR_ENTRY_SIZE * ((X)*dir->ysize * dir->zsize + \
                                            (Y)*dir->zsize + (Z))))
...
void dir_entry_dump_sharers(struct dir_t *dir, int x, int y, int z) {
  ...
  for (i = 0; i < dir->num_nodes; i++)
    if (dir_entry_is_sharer(dir, x, y, z, i)) mem_debug("%d ", i);
  mem_debug("}\n");
}
...
void dir_entry_set_sharer(struct dir_t *dir, int x, int y, int z, int node) {
  ...
  dir_entry = dir_entry_get(dir, x, y, z);
  if (dir_entry->sharer[node / 8] & (1 << (node % 8))) return;
...
void dir_entry_clear_all_sharers(struct dir_t *dir, int x, int y, int z) {
  ...
  for (i = 0; i < DIR_ENTRY_SHARERS_SIZE; i++) dir_entry->sharer[i] = 0;
...
int dir_entry_group_shared_or_owned(struct dir_t *dir, int x, int y) {
  ...
  for (z = 0; z < dir->zsize; z++) {
    ...
    if (dir_entry->num_sharers || DIR_ENTRY_VALID_OWNER(dir_entry)) return 1;
  }
  return 0;
}
...
int dir_entry_lock(struct dir_t *dir, int x, int y, int event,
 ...
 * This causes a deadlock when, for example, A-10 keeps retrying an up-down
 * access and
 ...
 */
      ...
      /* ------------------------------------------------------------------------
       */
      /* FIXME - Replaced with code below, just inserting at the end of the
       * queue.
  ...
  /* Trace */
  mem_trace(
      "mem.new_access_block cache=\"%s\" access=\"A-%lld\" set=%d way=%d\n",
...
void dir_entry_unlock(struct dir_t *dir, int x, int y) {
  ...
  if (dir_lock->lock_queue) {
    ...
    /* Wake up access */
    esim_schedule_event(dir_lock->lock_queue->dir_lock_event,
  ...
  /* Trace */
  mem_trace(
      "mem.end_access_block cache=\"%s\" access=\"A-%lld\" set=%d way=%d\n",
