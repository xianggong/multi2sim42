...
char *mem_config_help =
    ...
    "configuration file is a plain-text file in the IniFile format. The "
    "memory\n"
    "system is formed of a set of cache modules, main memory modules, and\n"
    "interconnects.\n"
    "\n"
    "Interconnects can be defined in two different configuration files. The "
    "first\n"
    "way is using option '--net-config <file>' (use option "
    "'--help-net-config'\n"
    "for more information). Any network defined in the network configuration "
    "file\n"
    "can be referenced from the memory configuration file. These networks will "
    "be\n"
    "referred hereafter as external networks.\n"
    "\n"
    "The second option to define a network straight in the memory system\n"
    "configuration. This alternative is provided for convenience and brevity. "
    "By\n"
    "using sections [Network <name>], networks with a default topology are\n"
    "created which include a single switch, and one bidirectional link from "
    "the\n"
    ...
    "      Memory page size. Virtual addresses are translated into new "
    "physical\n"
    ...
    "      difference between a cache and a main memory module is that the "
    "former\n"
    "      contains only a subset of the data located at the memory locations "
    "it\n"
    "      serves.\n"
    "  Geometry = <geo>\n"
    ...
    "i.e.,\n"
    "      modules closer to main memory. This variable is mandatory for "
    "caches,\n"
    "      and should not appear for main memory modules. Value <net> can "
    "refer\n"
    "      to an internal network defined in a [Network <net>] section, or to "
    "an\n"
    ...
    "      should be omitted.\n"
    "  HighNetwork = <net>\n"
    "      Network connecting the module with other higher-level modules, "
    "i.e.,\n"
    "      modules closer to CPU cores or GPU compute units. For highest "
    "level\n"
    "      modules accessible by CPU/GPU, this variable should be omitted.\n"
    "  HighNetworkNode = <node>\n"
    "      If 'HighNetwork' points to an external network, node that the "
    "module\n"
    "      is mapped to.\n"
    ...
    "      block size is specified in the corresponding cache geometry "
    "section).\n"
    "  Latency = <cycles>\n"
    "      Memory access latency. This variable is required for a main memory\n"
    "      module, and should be omitted for a cache module (the access "
    "latency\n"
    "      is specified in the corresponding cache geometry section).\n"
    "  Ports = <num>\n"
    "      Number of read/write ports. This variable is only allowed for a "
    "main\n"
    ...
    "      limits the number of different blocks that can reside in "
    "upper-level\n"
    ...
    "      directory, and all its occurrences in the memory hierarchy need to "
    "be\n"
    ...
    "      entire address space is served by the module. There are two "
    "possible\n"
    ...
    "      multiple of the module block size, and the value in <high> must be "
    "a\n"
    ...
    "      this module. The value of <div> must be a multiple of the block "
    "size.\n"
    "      When a module serves only a subset of the address space, the user "
    "must\n"
    "      make sure that the rest of the modules at the same level serve the\n"
    "      remaining address space.\n"
    "\n"
    "Section [CacheGeometry <geo>] defines a geometry for a cache. Caches "
    "using\n"
    ...
    "      Cache associativity. The total number of blocks contained in the "
    "cache\n"
    "      is given by the product Sets * Assoc.\n"
    "  BlockSize = <size> (Required)\n"
    "      Size of a cache block in bytes. The total size of the cache is "
    "given\n"
    ...
    "      Miss status holding register (MSHR) size in number of entries. "
    "This\n"
    "      value determines the maximum number of accesses that can be in "
    "flight\n"
    "      for the cache, including the time since the access request is\n"
    "      received, until a potential miss is resolved.\n"
    "  Ports = <num> (Default = 2)\n"
    "      Number of ports. The number of ports in a cache limits the number "
    "of\n"
    "      concurrent hits. If an access is a miss, it remains in the MSHR "
    "while\n"
    ...
    "      The hardware prefetcher does global history buffer based "
    "prefetching.\n"
    ...
    "\n"
    "Section [Network <net>] defines an internal default interconnect, formed "
    "of\n"
    "a single switch connecting all modules pointing to the network. For "
    "every\n"
    "module in the network, a bidirectional link is created automatically "
    "between\n"
    "the module and the switch, together with the suitable input/output "
    "buffers\n"
    ...
    "      Bandwidth for links and switch crossbar in number of bytes per "
    "cycle.\n"
    "\n"
    "Section [Entry <name>] creates an entry into the memory system. An entry "
    "is\n"
    "a connection between a CPU core/thread or a GPU compute unit with a "
    "module\n"
    ...
    "  Core = <core>\n"
    "      CPU core identifier. This is a value between 0 and the number of "
    "cores\n"
    "      minus 1, as defined in the CPU configuration file. This variable\n"
    "      should be omitted for GPU entries.\n"
    "  Thread = <thread>\n"
    "      CPU thread identifier. Value between 0 and the number of threads "
    "per\n"
    "      core minus 1. Omitted for GPU entries.\n"
    "  ComputeUnit = <id>\n"
    "      GPU compute unit identifier. Value between 0 and the number of "
    "compute\n"
    "      units minus 1, as defined in the GPU configuration file. This "
    "variable\n"
    "      should be omitted for CPU entries.\n"
    "  DataModule = <mod>\n"
    "  ConstantDataModule = <mod>\n"
    "  InstModule = <mod>\n"
    "      In architectures supporting separate data/instruction caches, "
    "modules\n"
    "      used to access memory for each particular purpose.\n"
    "  Module = <mod>\n"
    "      Module used to access the memory hierarchy. For architectures\n"
    "      supporting separate data/instruction caches, this variable can be "
    "used\n"
...
static void mem_config_default(struct arch_t *arch, void *user_data) {
  ...
  if (arch->sim_kind != arch_sim_kind_detailed) return;
...
static void mem_config_read_general(struct config_t *config) {
  ...
  /* Frequency */
  mem_frequency = config_read_int(config, section, "Frequency", mem_frequency);
  ...
  /* Peer transfers */
  mem_peer_transfers = config_read_bool(config, section, "PeerTransfers", 1);
}
...
static void mem_config_read_networks(struct config_t *config) {
  ...
  for (section = config_section_first(config); section;
    ...
    /* Network section */
    if (strncasecmp(section, "Network ", 8)) continue;
    net_name = section + 8;
...
static void mem_config_insert_module_in_network(
    struct config_t *config, struct mod_t *mod, char *net_name,
    char *net_node_name, struct net_t **net_ptr,
    struct net_node_t **net_node_ptr) {
  struct net_t *net;
  ...
  *net_node_ptr = NULL;
  ...
  if (!net) goto try_external_network;
  ...
  if (net_get_node_by_user_data(net, mod))
    fatal("%s: network '%s' already contains module '%s'.\n%s",
          mem_config_file_name, net->name, mod->name, mem_err_config_note);

  /* Read buffer sizes from network */
  def_input_buffer_size =
      config_read_int(config, buf, "DefaultInputBufferSize", 0);
  def_output_buffer_size =
      config_read_int(config, buf, "DefaultOutputBufferSize", 0);
  if (!def_input_buffer_size) {
    fatal(
        "%s: network %s: variable 'DefaultInputBufferSize' "
        "missing.\n%s",
        mem_config_file_name, net->name, mem_err_config_note);
  }
  if (!def_output_buffer_size) {
    fatal(
        "%s: network %s: variable 'DefaultOutputBufferSize' "
        "missing.\n%s",
        mem_config_file_name, net->name, mem_err_config_note);
  }
  if (def_input_buffer_size < mod->block_size + 8) {
    fatal(
        "%s: network %s: minimum input buffer size is %d for "
        "cache '%s'.\n%s",
        mem_config_file_name, net->name, mod->block_size + 8, mod->name,
        mem_err_config_note);
  }
  if (def_output_buffer_size < mod->block_size + 8)
    fatal(
        "%s: network %s: minimum output buffer size is %d for "
        "cache '%s'.\n%s",
        mem_config_file_name, net->name, mod->block_size + 8, mod->name,
        mem_err_config_note);
...
try_external_network:
  ...
  if (!node)
    fatal("%s: network %s: node %s: invalid node name.\n%s%s",
          mem_config_file_name, net_name, net_node_name, mem_err_config_note,
          err_mem_config_net);

  /* No module must have been assigned previously to this node */
  if (node->user_data)
    fatal("%s: network %s: node '%s' already assigned.\n%s",
          mem_config_file_name, net->name, net_node_name, mem_err_config_note);
...
static struct mod_t *mem_config_read_cache(struct config_t *config,
  ...
  prefetcher_type_str =
      config_read_string(config, buf, "PrefetcherType", "GHB_PC_CS");
  ...
  prefetcher_lookup_depth =
      config_read_int(config, buf, "PrefetcherLookupDepth", 2);
  ...
  if (policy == cache_policy_invalid)
    fatal("%s: cache %s: %s: invalid block replacement policy.\n%s",
          mem_config_file_name, mod_name, policy_str, mem_err_config_note);
  if (num_sets < 1 || (num_sets & (num_sets - 1)))
    fatal(
        "%s: cache %s: number of sets must be a power of two "
        "greater than 1.\n%s",
        mem_config_file_name, mod_name, mem_err_config_note);
  if (assoc < 1 || (assoc & (assoc - 1)))
    fatal(
        "%s: cache %s: associativity must be power of two "
        "and > 1.\n%s",
        mem_config_file_name, mod_name, mem_err_config_note);
  if (block_size < 4 || (block_size & (block_size - 1)))
    fatal(
        "%s: cache %s: block size must be power of two and "
        "at least 4.\n%s",
        mem_config_file_name, mod_name, mem_err_config_note);
  if (dir_latency < 1)
    fatal(
        "%s: cache %s: invalid value for variable "
        "'DirectoryLatency'.\n%s",
        mem_config_file_name, mod_name, mem_err_config_note);
  if (latency < 1)
  ...
  if (enable_prefetcher) {
    prefetcher_type =
        str_map_string_case(&prefetcher_type_map, prefetcher_type_str);
  ...
  /* Create module */
  mod = mod_create(mod_name, mod_kind_cache, num_ports, block_size, latency);
  ...
  if (enable_prefetcher) {
    mod->cache->prefetcher =
        prefetcher_create(prefetcher_ghb_size, prefetcher_it_size,
...
static struct mod_t *mem_config_read_main_memory(struct config_t *config,
  ...
  if (dir_assoc < 1 || (dir_assoc & (dir_assoc - 1)))
    fatal(
        "%s: %s: directory associativity must be a power of "
        "two.\n%s",
        mem_config_file_name, mod_name, mem_err_config_note);
  ...
  /* Create module */
  mod = mod_create(mod_name, mod_kind_main_memory, num_ports, block_size,
                   latency);
...
static void mem_config_read_module_address_range(struct config_t *config,
  ...
  if (!strcasecmp(token, "BOUNDS")) {
    ...
    /* Low bound */
    if (!(token = strtok(NULL, delim))) goto invalid_format;
    ...
    if (mod->range.bounds.low % mod->block_size)
      fatal(
          "%s: %s: low address bound must be a multiple "
          "of block size.\n%s",
          mem_config_file_name, mod->name, mem_err_config_note);
    ...
    if ((mod->range.bounds.high + 1) % mod->block_size)
      fatal(
          "%s: %s: high address bound must be a multiple "
    ...
    if (err)
      fatal("%s: %s: invalid value '%s' in 'AddressRange'",
            mem_config_file_name, mod->name, token);
    if (mod->range.interleaved.div < 1) goto invalid_format;
    ...
    if (err)
      fatal("%s: %s: invalid value '%s' in 'AddressRange'",
            mem_config_file_name, mod->name, token);
    if (mod->range.interleaved.mod < 1) goto invalid_format;
...
static void mem_config_read_modules(struct config_t *config) {
  ...
  for (section = config_section_first(config); section;
    ...
    if (strncasecmp(section, "Module ", 7)) continue;
...
static void mem_config_check_route_to_main_memory(struct mod_t *mod,
  ...
  if (level > MEM_SYSTEM_MAX_LEVELS)
    fatal("%s: %s: too many cache levels.\n%s%s", mem_config_file_name,
          mod->name, err_mem_levels, mem_err_config_note);

  /* Check block size */
  if (mod->block_size < block_size)
    fatal("%s: %s: decreasing block size.\n%s%s", mem_config_file_name,
          mod->name, err_mem_block_size, mem_err_config_note);
  block_size = mod->block_size;

  /* Dump current module */
  mem_debug("\t");
  for (i = 0; i < level * 2; i++) mem_debug(" ");
  mem_debug("%s\n", mod->name);
  ...
  /* Dump children */
  for (linked_list_head(mod->low_mod_list);
       !linked_list_is_end(mod->low_mod_list);
...
static void mem_config_read_low_modules(struct config_t *config) {
  ...
  for (i = 0; i < list_count(mem_system->mod_list); i++) {
    /* Get cache module */
    mod = list_get(mem_system->mod_list, i);
    if (mod->kind != mod_kind_cache) continue;
    ...
    delim = ", ";
    for (low_mod_name = strtok(low_mod_name_list, delim); low_mod_name;
         low_mod_name = strtok(NULL, delim)) {
...
static void mem_config_read_entries(struct config_t *config) {
  ...
  for (section = config_section_first(config); section;
    ...
    if (strncasecmp(section, "Entry ", 6)) continue;
    ...
    if (!entry_name_trimmed[0])
      fatal("%s: section [%s]: invalid entry name.\n%s", mem_config_file_name,
            section, mem_err_config_note);

    /* Check if variable 'Type' is used in the section. This variable was used
     * in
     * previous versions, now it is replaced with 'Arch'. */
    if (config_var_exists(config, section, "Type"))
      fatal(
          "%s: section [%s]: Variable 'Type' is obsolete, use 'Arch' "
          "instead.\n%s",
    ...
    if (!arch) {
      ...
      fatal(
          "%s: section [%s]: '%s' is an invalid value for 'Arch'.\n"
          "\tPossible values are %s.\n%s",
          mem_config_file_name, section, arch_name_trimmed, arch_list_names,
          mem_err_config_note);
    }
    ...
    if (arch->sim_kind == arch_sim_kind_functional)
      fatal(
          "%s: section [%s]: %s architecture not under detailed simulation.\n"
          "\tA CPU/GPU architecture uses functional simulation by default. "
          "Please\n"
          "\tactivate detailed simulation for the %s architecture using "
          "command-line\n"
...
static void mem_config_create_switches(struct config_t *config) {
  ...
  for (i = 0; i < list_count(mem_system->net_list); i++) {
    ...
    net_switch =
        net_add_switch(net, def_input_buffer_size, def_output_buffer_size,
    ...
    for (j = 0; j < list_count(net->node_list); j++) {
      ...
      if (net_node != net_switch) {
        net_add_bidirectional_link(net, net_node, net_switch, def_bandwidth,
                                   def_output_buffer_size,
                                   def_input_buffer_size, 1);
...
static void mem_config_check_routes(void) {
  ...
  for (i = 0; i < list_count(mem_system->mod_list); i++) {
    ...
    for (linked_list_head(mod->low_mod_list);
      ...
      entry =
          net_routing_table_lookup(mod->low_net->routing_table,
                                   mod->low_net_node, low_mod->high_net_node);

      if (!entry->output_buffer)
        fatal("%s: %s: network does not connect '%s' with '%s'.\n%s",
              mem_config_file_name, mod->low_net->name, mod->name,
              low_mod->name, err_mem_connect);
    }

    /* List of high modules */
    mem_debug("\n\t\tHigh modules:");
    for (linked_list_head(mod->high_mod_list);
         !linked_list_is_end(mod->high_mod_list);
      ...
      /* Check that there is a route */
      entry =
          net_routing_table_lookup(mod->high_net->routing_table,
                                   mod->high_net_node, high_mod->low_net_node);
      if (!entry->output_buffer)
        fatal("%s: %s: network does not connect '%s' with '%s'.\n%s",
              mem_config_file_name, mod->high_net->name, mod->name,
              high_mod->name, err_mem_connect);
    }
...
/* Recursive test-and-set of module architecture. If module 'mod' or any of its
 * lower-level
 * modules is set to an architecture other than 'arch', return this other
 * architecture.
 * Otherwise, set the architecture of 'mod' and all its lower-level modules to
 * 'arch', and
 * return 'arch'. */
static struct arch_t *mem_config_set_mod_arch(struct mod_t *mod,
                                              struct arch_t *arch) {
  struct mod_t *low_mod;
  struct arch_t *low_mod_arch;

  /* This module has the color */
  if (mod->arch) return mod->arch;

  /* Check lower-level modules */
  LINKED_LIST_FOR_EACH(mod->low_mod_list) {
    low_mod = linked_list_get(mod->low_mod_list);
    low_mod_arch = mem_config_set_mod_arch(low_mod, arch);
    if (low_mod_arch != arch) return low_mod_arch;
  }
...
static void mem_config_calculate_sub_block_sizes(void) {
  ...
  for (i = 0; i < list_count(mem_system->mod_list); i++) {
    ...
    mod->dir = dir_create(mod->name, mod->dir_num_sets, mod->dir_assoc,
                          mod->num_sub_blocks, num_nodes);
    mem_debug(
        "\t%s - %dx%dx%d (%dx%dx%d effective) - %d entries, %d sub-blocks\n",
        mod->name, mod->dir_num_sets, mod->dir_assoc, num_nodes,
        mod->dir_num_sets, mod->dir_assoc,
        linked_list_count(mod->high_mod_list), mod->dir_size,
        mod->num_sub_blocks);
  }
...
static void mem_config_trace(void) {
  ...
  if (!mem_tracing()) return;

  /* Initialization */
  mem_trace_header("mem.init version=\"%d.%d\"\n",
                   MEM_SYSTEM_TRACE_VERSION_MAJOR,
                   MEM_SYSTEM_TRACE_VERSION_MINOR);
  ...
  LIST_FOR_EACH(mem_system->mod_list, i) {
    ...
    /* Trace header */
    mem_trace_header(
        "mem.new_mod name=\"%s\" num_sets=%d assoc=%d "
        ...
        mod->level, high_net_name, high_net_node_index, low_net_name,
        low_net_node_index);
  }
}
...
static void mem_config_read_commands(struct config_t *config) {
  ...
  if (!config_section_exists(config, section)) return;

  /* Register events related with commands */
  EV_MEM_SYSTEM_COMMAND = esim_register_event_with_name(
      mem_system_command_handler, mem_domain_index, "mem_system_command");
  EV_MEM_SYSTEM_END_COMMAND =
      esim_register_event_with_name(mem_system_end_command_handler,
...
void mem_config_read(void) {
  ...
  if (!si_gpu_fused_device) arch_for_each(mem_config_check_disjoint, NULL);
