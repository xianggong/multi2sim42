...
void mod_handler_nmoesi_load(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_LOAD) {
    ...
    mem_debug("%lld %lld 0x%x %s load\n", esim_time, stack->id, stack->addr,
              mod->name);
    mem_trace(
        "mem.new_access name=\"A-%lld\" type=\"load\" "
  ...
  if (event == EV_MOD_NMOESI_LOAD_LOCK) {
    ...
    /* If there is any older access to the same address that this access could
     * not
  ...
  if (event == EV_MOD_NMOESI_LOAD_FINISH) {
    ...
    if (stack->witness_ptr) (*stack->witness_ptr)++;
    ...
    if (stack->client_info) mod_client_info_free(mod, stack->client_info);
...
void mod_handler_nmoesi_store(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_STORE) {
    ...
    mem_debug("%lld %lld 0x%x %s store\n", esim_time, stack->id, stack->addr,
              mod->name);
    mem_trace(
        "mem.new_access name=\"A-%lld\" type=\"store\" "
  ...
  if (event == EV_MOD_NMOESI_STORE_UNLOCK) {
    ...
    /* Impose the access latency before continuing */
    esim_schedule_event(EV_MOD_NMOESI_STORE_FINISH, stack, mod->latency);
    return;
  }
...
void mod_handler_nmoesi_nc_store(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_NC_STORE) {
    ...
    mem_debug("%lld %lld 0x%x %s nc store\n", esim_time, stack->id, stack->addr,
              mod->name);
    mem_trace(
        "mem.new_access name=\"A-%lld\" type=\"nc_store\" "
    ...
    /* Coalesce access */
    master_stack =
        mod_can_coalesce(mod, mod_access_nc_store, stack->addr, stack);
    if (master_stack) {
      mod->nc_writes++;
      mod_coalesce(mod, master_stack, stack);
  ...
  if (event == EV_MOD_NMOESI_NC_STORE_WRITEBACK) {
    ...
    /* If the block has modified data, evict it so that the lower-level cache
     * will
  ...
  if (event == EV_MOD_NMOESI_NC_STORE_ACTION) {
    ...
    /* E state must tell the lower-level module to remove this module as an
       owner */
     ...
     * evicted the block so that the lower-level cache will have the latest
     * value
...
void mod_handler_nmoesi_prefetch(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_PREFETCH) {
    ...
    /* Coalesce access */
    master_stack =
        mod_can_coalesce(mod, mod_access_prefetch, stack->addr, stack);
    if (master_stack) {
...
void mod_handler_nmoesi_find_and_lock(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_FIND_AND_LOCK) {
    mem_debug("  %lld %lld 0x%x %s find and lock (blocking=%d)\n", esim_time,
              stack->id, stack->addr, mod->name, stack->blocking);
  ...
  if (event == EV_MOD_NMOESI_FIND_AND_LOCK_PORT) {
    ...
    } else if (stack->nc_write) /* Must go after read */
    {
      ...
      stack->blocking ? mod->blocking_nc_writes++
                      : mod->non_blocking_nc_writes++;
      if (stack->hit) mod->nc_write_hits++;
    } else if (stack->write) {
      mod->writes++;
    ...
    if (!stack->retry) {
      ...
      if (stack->hit) mod->no_retry_hits++;

      if (stack->read) {
        mod->no_retry_reads++;
        if (stack->hit) mod->no_retry_read_hits++;
      } else if (stack->nc_write) /* Must go after read */
      {
        mod->no_retry_nc_writes++;
        if (stack->hit) mod->no_retry_nc_write_hits++;
      } else if (stack->write) {
        mod->no_retry_writes++;
        if (stack->hit) mod->no_retry_write_hits++;
      } else if (stack->prefetch) {
    ...
    if (dir_lock->lock && !stack->blocking) {
      mem_debug(
          "    %lld 0x%x %s block locked at set=%d, way=%d by A-%lld - "
          "aborting\n",
  ...
  if (event == EV_MOD_NMOESI_FIND_AND_LOCK_FINISH) {
    mem_debug("  %lld %lld 0x%x %s find and lock finish (err=%d)\n", esim_time,
              stack->id, stack->tag, mod->name, stack->err);
    ...
    /* If this is a main memory, the block is here. A previous miss was just a
     * miss
     * in the directory. */
...
void mod_handler_nmoesi_evict(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_EVICT) {
    ...
    mem_debug("  %lld %lld 0x%x %s evict (set=%d, way=%d, state=%s)\n",
              ...
              stack->way, str_map_value(&cache_block_state_map, stack->state));
    ...
    new_stack =
        mod_stack_create(stack->id, mod, 0, EV_MOD_NMOESI_EVICT_INVALID, stack);
  ...
  if (event == EV_MOD_NMOESI_EVICT_ACTION) {
    ...
    /* Send message */
    stack->msg =
        net_try_send_ev(mod->low_net, mod->low_net_node, low_node, msg_size,
  ...
  if (event == EV_MOD_NMOESI_EVICT_RECEIVE) {
    ...
    if (stack->state == cache_block_noncoherent) {
      new_stack =
          mod_stack_create(stack->id, target_mod, stack->src_tag,
  ...
  if (event == EV_MOD_NMOESI_EVICT_REPLY) {
    ...
    /* Send message */
    stack->msg = net_try_send_ev(
        target_mod->high_net, target_mod->high_net_node, mod->low_net_node, 8,
  ...
  if (event == EV_MOD_NMOESI_EVICT_REPLY_RECEIVE) {
    ...
    assert(!dir_entry_group_shared_or_owned(mod->dir, stack->src_set,
                                            stack->src_way));
    esim_schedule_event(EV_MOD_NMOESI_EVICT_FINISH, stack, 0);
    return;
  }
...
void mod_handler_nmoesi_read_request(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST) {
    ...
    stack->msg = net_try_send_ev(net, src_node, dst_node, 8,
                                 EV_MOD_NMOESI_READ_REQUEST_RECEIVE, stack,
                                 event, stack);
    return;
  }
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST_ACTION) {
    ...
    esim_schedule_event(stack->request_dir == mod_request_up_down
                            ...
                            : EV_MOD_NMOESI_READ_REQUEST_DOWNUP,
                        stack, 0);
    return;
  }
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST_UPDOWN) {
    ...
    if (stack->state) {
      ...
      for (z = 0; z < dir->zsize; z++) {
        ...
        /* No owner */
        if (!DIR_ENTRY_VALID_OWNER(dir_entry)) continue;

        /* Owner is mod */
        if (dir_entry->owner == mod->low_net_node->index) continue;
        ...
        if (dir_entry_tag % owner->block_size) continue;

        /* Send read request */
        stack->pending++;
        new_stack =
            mod_stack_create(stack->id, target_mod, dir_entry_tag,
      ...
      /* State = I */
      assert(!dir_entry_group_shared_or_owned(target_mod->dir, stack->set,
                                              stack->way));
      new_stack =
          mod_stack_create(stack->id, target_mod, stack->tag,
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST_UPDOWN_FINISH) {
    ...
    int latency =
        stack->reply == reply_ack_data_sent_to_peer ? 0 : target_mod->latency;
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST_DOWNUP) {
    ...
    for (z = 0; z < dir->zsize; z++) {
      ...
      stack->pending++;
      new_stack = mod_stack_create(
          stack->id, target_mod, dir_entry_tag,
    ...
    esim_schedule_event(EV_MOD_NMOESI_READ_REQUEST_DOWNUP_WAIT_FOR_REQS, stack,
                        0);
    return;
  }
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST_DOWNUP_WAIT_FOR_REQS) {
    ...
    mem_trace(
        "mem.access name=\"A-%lld\" "
        "state=\"%s:read_request_downup_wait_for_reqs\"\n",
        stack->id, target_mod->name);
  ...
  if (event == EV_MOD_NMOESI_READ_REQUEST_REPLY) {
    ...
    /* Send message */
    stack->msg =
        net_try_send_ev(net, src_node, dst_node, stack->reply_size,
...
void mod_handler_nmoesi_write_request(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_WRITE_REQUEST) {
    ...
    /* Send message */
    stack->msg = net_try_send_ev(net, src_node, dst_node, 8,
                                 EV_MOD_NMOESI_WRITE_REQUEST_RECEIVE, stack,
                                 event, stack);
    return;
  }
  ...
  if (event == EV_MOD_NMOESI_WRITE_REQUEST_DOWNUP_FINISH) {
    ...
    int latency =
        ret->reply == reply_ack_data_sent_to_peer ? 0 : target_mod->latency;
    esim_schedule_event(EV_MOD_NMOESI_WRITE_REQUEST_REPLY, stack, latency);
    return;
  }
  ...
  if (event == EV_MOD_NMOESI_WRITE_REQUEST_REPLY) {
    ...
    stack->msg = net_try_send_ev(net, src_node, dst_node, stack->reply_size,
                                 EV_MOD_NMOESI_WRITE_REQUEST_FINISH, stack,
                                 event, stack);
    return;
  }
...
void mod_handler_nmoesi_peer(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_PEER_SEND) {
    ...
    stack->msg = net_try_send_ev(
        src->low_net, src->low_net_node, peer->low_net_node,
  ...
  if (event == EV_MOD_NMOESI_PEER_REPLY) {
    ...
    stack->msg =
        net_try_send_ev(peer->low_net, peer->low_net_node, src->low_net_node, 8,
  ...
  if (event == EV_MOD_NMOESI_INVALIDATE) {
    ...
    for (z = 0; z < dir->zsize; z++) {
      ...
      for (i = 0; i < dir->num_nodes; i++) {
        ...
        if (!dir_entry_is_sharer(dir, stack->set, stack->way, z, i)) continue;

        node = list_get(mod->high_net->node_list, i);
        sharer = node->user_data;
        if (sharer == stack->except_mod) continue;
        ...
        if (dir_entry_tag % sharer->block_size) continue;
...
void mod_handler_nmoesi_message(int event, void *data) {
  ...
  if (event == EV_MOD_NMOESI_MESSAGE) {
    ...
    /* Send message */
    stack->msg =
        net_try_send_ev(net, src_node, dst_node, 8,
