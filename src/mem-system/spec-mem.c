...
static struct spec_mem_page_t *spec_mem_page_get(struct spec_mem_t *spec_mem,
                                                 unsigned int addr) {
  ...
  while (page && page->addr != addr) page = page->next;

  /* Return it (could be NULL) */
  return page;
}

static struct spec_mem_page_t *spec_mem_page_create(struct spec_mem_t *spec_mem,
                                                    unsigned int addr) {
...
static void spec_mem_access_aligned(struct spec_mem_t *spec_mem,
  ...
  if (!page) {
    ...
    if (access == mem_access_read) {
      ...
      if (spec_mem->page_count >= SPEC_MEM_MAX_PAGE_COUNT) return;
...
static void spec_mem_access(struct spec_mem_t *spec_mem, unsigned int addr,
...
void spec_mem_read(struct spec_mem_t *spec_mem, unsigned int addr, int size,
                   void *buf) {
  spec_mem_access(spec_mem, addr, size, buf, mem_access_read);
}

void spec_mem_write(struct spec_mem_t *spec_mem, unsigned int addr, int size,
                    void *buf) {
