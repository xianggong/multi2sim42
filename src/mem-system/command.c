...
static void mem_system_command_expect(struct list_t *token_list,
                                      char *command_line) {
  if (!list_count(token_list))
    fatal("%s: unexpected end of line.\n\t> %s", __FUNCTION__, command_line);
}

static void mem_system_command_end(struct list_t *token_list,
                                   char *command_line) {
  if (list_count(token_list))
    fatal("%s: %s: end of line expected.\n\t> %s", __FUNCTION__,
          str_token_list_first(token_list), command_line);
}
...
int mem_system_command_get_high_low(struct list_t *token_list,
                                    char *command_line) {
  char *direction;
  int high_net;
...
int mem_system_command_get_in_out(struct list_t *token_list,
                                  char *command_line) {
  char *direction;
  int incoming;
...
static void mem_system_command_get_set_way(struct list_t *token_list,
                                           char *command_line,
                                           struct mod_t *mod, int *set_ptr,
                                           int *way_ptr) {
  int set;
  int way;

  /* Check valid module */
  if (!mod) fatal("%s: invalid module.\n\t> %s", __FUNCTION__, command_line);
  ...
  if (!IN_RANGE(set, 0, mod->cache->num_sets - 1))
    fatal("%s: %d: invalid set.\n\t> %s", __FUNCTION__, set, command_line);
...
static int mem_system_command_get_state(struct list_t *token_list,
  ...
  /* Get state */
  mem_system_command_expect(token_list, command_line);
  state = str_map_string_case(&cache_block_state_map,
                              str_token_list_first(token_list));
...
void mem_system_command_handler(int event, void *data) {
  ...
  if (!strcasecmp(command, "SetBlock")) {
    ...
    if (set != set_check)
      fatal("%s: %s: tag 0x%x belongs to set %d.\n\t> %s", __FUNCTION__,
            mod->name, tag, set_check, command_line);
    if (tag != tag_check)
  ...
  /* Command 'SetOwner' */
    ...
    mod = mem_system_command_get_mod(token_list, command_line);
    mem_system_command_get_set_way(token_list, command_line, mod, &set, &way);
    sub_block = mem_system_command_get_sub_block(token_list, command_line, mod,
                                                 set, way);
  ...
  /* Command 'SetSharers' */
    ...
    while (list_count(token_list)) {
      ...
      /* Set sharer */
      dir_entry_set_sharer(mod->dir, set, way, sub_block,
                           sharer->low_net_node->index);
    }
  }
...
void mem_system_end_command_handler(int event, void *data) {
  ...
  /* Get command */
  mem_system_command_get_string(token_list, command_line, command,
                                sizeof command);
  ...
  *msg_detail_str = '\0';
  ...
  if (!strcasecmp(command, "CheckBlock")) {
    ...
    str_printf(&msg_str, &msg_size,
               ...
               mod->name, set, way,
               str_map_value(&cache_block_state_map, state), tag);
  ...
  /* Command 'CheckSharers' */
    ...
    LINKED_LIST_FOR_EACH(sharers_list) {
      ...
      if (sharer) str_printf(&msg_str, &msg_size, "%s ", sharer->name);
    }
    str_printf(&msg_str, &msg_size, "}");
  ...
  /* Command 'CheckLink' */
    ...
    if (buffer->kind == net_buffer_link) {
      ...
      fprintf(stderr,
              "The connection is a BUS. BUS check is not yet implemented \n");
    }
