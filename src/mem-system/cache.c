...
struct str_map_t cache_policy_map = {3,
                                     {{"LRU", cache_policy_lru},
                                      {"FIFO", cache_policy_fifo},
                                      {"Random", cache_policy_random}}};

struct str_map_t cache_block_state_map = {6,
...
enum cache_waylist_enum { cache_waylist_head, cache_waylist_tail };

static void cache_update_waylist(struct cache_set_t *set,
                                 struct cache_block_t *blk,
                                 enum cache_waylist_enum where) {
  if (!blk->way_prev && !blk->way_next) {
    ...
    assert(set->way_head == blk && set->way_tail != blk);
    if (where == cache_waylist_head) return;
    set->way_head = blk->way_next;
    blk->way_next->way_prev = NULL;

  } else if (!blk->way_next) {
    assert(set->way_head != blk && set->way_tail == blk);
    if (where == cache_waylist_tail) return;
...
struct cache_t *cache_create(char *name, unsigned int num_sets,
  ...
  for (set = 0; set < num_sets; set++) {
    ...
    for (way = 0; way < assoc; way++) {
      ...
      block->way_next =
          way < assoc - 1 ? &cache->sets[set].blocks[way + 1] : NULL;
    }
  }
...
void cache_free(struct cache_t *cache) {
  ...
  for (set = 0; set < cache->num_sets; set++) free(cache->sets[set].blocks);
  free(cache->sets);
  free(cache->name);
  if (cache->prefetcher) prefetcher_free(cache->prefetcher);
  free(cache);
}

/* Return {set, tag, offset} for a given address */
void cache_decode_address(struct cache_t *cache, unsigned int addr,
                          int *set_ptr, int *tag_ptr,
...
int cache_find_block(struct cache_t *cache, unsigned int addr, int *set_ptr,
                     int *way_ptr, int *state_ptr) {
  int set, tag, way;
...
void cache_get_block(struct cache_t *cache, int set, int way, int *tag_ptr,
                     int *state_ptr) {
  assert(set >= 0 && set < cache->num_sets);
  assert(way >= 0 && way < cache->assoc);
...
void cache_access_block(struct cache_t *cache, int set, int way) {
   ...
   * It will also be moved if it is its first access for FIFO policy, i.e., if
   * the
...
int cache_replace_block(struct cache_t *cache, int set) {
  ...
  /* Try to find an invalid block. Do this in the LRU order, to avoid picking
   * the
