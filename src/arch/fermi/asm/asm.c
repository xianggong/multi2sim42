...
/* Table containing information of all instructions. Indexed by instruction
 * opcode */
...
static struct str_map_t frm_inst_cmp_map = {14,
                                            {{".LT", 1},
                                             {".EQ", 2},
                                             {".LE", 3},
                                             {".GT", 4},
...
static struct str_map_t frm_inst_sat_map = {2, {{"", 0}, {".SAT", 1}}};

static struct str_map_t frm_inst_logic_map = {
    3, {{".AND", 0}, {".OR", 1}, {".XOR", 2}}};

static struct str_map_t frm_inst_op_map = {8,
                                           {{".COS", 0},
                                            {".SIN", 1},
                                            {".EX2", 2},
                                            {".LG2", 3},
...
static struct str_map_t frm_inst_op67_map = {
    4, {{".AND", 0}, {".OR", 1}, {".XOR", 2}, {".PASS_B", 3}}};

static struct str_map_t frm_inst_dtype_map = {
    4, {{"", 0}, {".F16", 1}, {".F32", 2}, {".F64", 3}}};
...
static struct str_map_t frm_inst_stype_n_map = {
    4, {{"8", 0}, {"16", 1}, {"32", 2}, {"64", 3}}};

static struct str_map_t frm_inst_type_map = {7,
                                             {{".U8", 0},
                                              {".S8", 1},
                                              {".U16", 2},
                                              {".S16", 3},
...
static struct str_map_t frm_inst_NOP_op_map = {9,
                                               {
                                                {"", 0},
                                                {".FMA64", 1},
                                                {".FMA32", 2},
...
static struct str_map_t
    frm_inst_ccop_map =  // more needs to be added (look over fermi isa NOP)
    {4, {{".F", 0}, {".LT", 1}, {".EQ", 2}, {".T", 15}}};
...
static int inst_is_token(char *fmt_str, char *token_str, int *len_ptr) {
  ...
  *len_ptr = len;
  return !strncmp(fmt_str, token_str, len) && !isalnum(fmt_str[len]) &&
         fmt_str[len] != '_';
}
...
void frm_inst_dump(char *str, int size, void *buf, int inst_index) {
  ...
  if (!inst.info)
    fatal("%s: instruction not supported (offset=0x%x)", __FUNCTION__,
          inst.addr);
  ...
  while (*fmt_str && size) {
    ...
    if (inst_is_token(fmt_str, "pred", &len)) {
      ...
      if (src1 != 63) {
        ...
        if (offs != 0) str_printf(&str, &size, "+%#llx", offs);
      }
      ...
      if (inst.dword.general0.src2_mod == 0) {
        if (inst.dword.mod0_A.neg_src2) str_printf(&str, &size, "-");
        str_printf(&str, &size, "R%d", inst.dword.general0.src2 & 0x3f);
      } else if (inst.dword.general0.src2_mod == 1) {
        bank_id = inst.dword.general0.src2 >> 16;
        offset_in_bank = inst.dword.general0.src2 & 0xffff;
    ...
    /* This is a special case for src2 and src3. For FFMA,
       ...
       it prints src3 first when src2_mod > 2, however prints src2 first when
       src2_mod < 2 */
      ...
      if (inst.dword.general0.src2_mod < 2) {
        if (inst.dword.general0.src2_mod == 0)
          ...
          if (bank_id == 0 && offset_in_bank == 0)
            ...
            str_printf(&str, &size, "c [%#llx] [%#llx]", bank_id,
                       offset_in_bank);
        }
      }
      ...
      if (!inst.dword.tgt.noinc) str_printf(&str, &size, ".noinc");
    }

    else if (inst_is_token(fmt_str, "tgt_u", &len)) {
      if (inst.dword.tgt.u) str_printf(&str, &size, ".U");
    }

    else if (inst_is_token(fmt_str, "tgt_lmt", &len)) {
      if (inst.dword.tgt.noinc) str_printf(&str, &size, ".LMT");
    }

    else if (inst_is_token(fmt_str, "imm32", &len)) {
      long long int imm32;
      /* FIXME
       * Careful - We need to check whether each instruction encodes the
       * immediate
      ...
      unsigned long long int offs;
      offs = inst.dword.offs.offset;
      if (offs) str_printf(&str, &size, "+%#llx", offs);
    }

    else if (inst_is_token(fmt_str, "gen1_cmp", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_cmp_map, inst.dword.general1.cmp));
    }

    else if (inst_is_token(fmt_str, "gen1_logic", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_logic_map, inst.dword.general1.logic));
    }

    else if (inst_is_token(fmt_str, "gen1_logicftz", &len)) {
      str_printf(&str, &size, ".FTZ%s",
                 str_map_value(&frm_inst_logic_map, inst.dword.general1.logic));
    }
    ...
    /* 2nd level token such as mod0, mod1, P, Q*/
      ...
      if (inst.dword.mod0_A.satftz) str_printf(&str, &size, ".FTZ");
    }

    else if (inst_is_token(fmt_str, "mod0_A_s", &len)) {
      if (inst.dword.mod0_A.s) str_printf(&str, &size, ".S");
    }
      ...
      if (!inst.dword.mod0_A.satftz) str_printf(&str, &size, ".U32");
    }

    else if (inst_is_token(fmt_str, "mod0_A_w", &len)) {
      if (inst.dword.mod0_A.neg_src1) str_printf(&str, &size, ".W");
    }
      ...
      if (!inst.dword.mod0_B.type) str_printf(&str, &size, ".U32");
    }

    else if (inst_is_token(fmt_str, "mod0_B_brev", &len)) {
      if (inst.dword.mod0_B.cop) str_printf(&str, &size, ".brev");
    }

    else if (inst_is_token(fmt_str, "mod0_B_cop", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_cop_map, inst.dword.mod0_B.cop));
    }

    else if (inst_is_token(fmt_str, "mod0_B_type", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_type_map, inst.dword.mod0_B.type));
    }

    else if (inst_is_token(fmt_str, "mod0_C_s", &len)) {
      if (inst.dword.mod0_C.s) str_printf(&str, &size, ".S");
    }
      ...
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_ccop_map, inst.dword.mod0_C.shamt));
    }

    else if (inst_is_token(fmt_str, "mod0_D_ftzfmz", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_ftzfmz_map, inst.dword.mod0_D.ftzfmz));
    }

    else if (inst_is_token(fmt_str, "mod0_D_sat", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_sat_map, inst.dword.mod0_D.sat));
    }

    else if (inst_is_token(fmt_str, "mod0_D_op", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_op_map, inst.dword.mod0_D.ftzfmz));
    }

    else if (inst_is_token(fmt_str, "mod0_D_op67", &len)) {
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_op67_map, inst.dword.mod0_D.ftzfmz));
    }

    else if (inst_is_token(fmt_str, "gen0_mod1_B_rnd", &len)) {
      str_printf(
          &str, &size, "%s",
          str_map_value(&frm_inst_rnd_map, inst.dword.general0_mod1_B.rnd));
    }

    else if (inst_is_token(fmt_str, "gen0_mod1_D_cmp", &len)) {
      str_printf(
          &str, &size, "%s",
          str_map_value(&frm_inst_cmp_map, inst.dword.general0_mod1_D.cmp));
    }
      ...
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_stype_n_map, dtype_n));
    }
      ...
      str_printf(&str, &size, "%s",
                 str_map_value(&frm_inst_stype_n_map, stype_n));
    }
      ...
      if (sat) str_printf(&str, &size, ".SAT");
    }
      ...
      fatal("%s: unknown token: %s", __FUNCTION__, fmt_str);
    }
...
void frm_disasm(char *path) {
  ...
  for (i = 0; i < list_count(elf_file->section_list); ++i) {
    ...
    if (!strncmp(section->name, ".text.", 6)) {
      ...
      printf("\t\tFunction : %s\n", section->name + 6);
      for (inst_index = 0; inst_index < section->buffer.size / 8;
           ++inst_index) {
        frm_inst_hex_dump(stdout, (unsigned char *)(section->buffer.ptr),
                          inst_index);
        frm_inst_dump(inst_str, sizeof inst_str, section->buffer.ptr,
                      inst_index);
        printf("%s;\n", inst_str);
      }
      printf("\t\t....................................................\n\n\n");
...
void frm_disasm_text_section_buffer(struct elf_enc_buffer_t *buffer) {
  ...
  for (inst_index = 0; inst_index < buffer->size / 8; ++inst_index) {
    frm_inst_hex_dump(stdout, (unsigned char *)(buffer->ptr), inst_index);
    frm_inst_dump(inst_str, sizeof inst_str, buffer->ptr, inst_index);
    printf("%s;\n", inst_str);
  }
}
