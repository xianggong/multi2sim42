...
/* Global variables referring to the instruction that is currently being
 * emulated.
...
#define FRM_GPR_FLOAT_ELEM(_gpr, _elem) \
  (*(float *)&frm_isa_thread->gpr[(_gpr)].elem[(_elem)])
...
#define FRM_ISA_ARG_NOT_SUPPORTED(p)                                   \
  fatal("%s: %s: not supported for '" #p "' = 0x%x\n%s", __FUNCTION__, \
        frm_isa_inst->info->name, (p), err_frm_isa_note);
#define FRM_ISA_ARG_NOT_SUPPORTED_NEQ(p, v)                                 \
  {                                                                         \
    if ((p) != (v))                                                         \
      fatal("%s: %s: not supported for '" #p "' != 0x%x\n%s", __FUNCTION__, \
            frm_isa_inst->info->name, (v), err_frm_isa_note);               \
  }
#define FRM_ISA_ARG_NOT_SUPPORTED_RANGE(p, min, max)                       \
  {                                                                        \
    if ((p) < (min) || (p) > (max))                                        \
      fatal("%s: %s: not supported for '" #p "' out of range [%d:%d]\n%s", \
            __FUNCTION__, frm_isa_inst->info->name, (min), (max),          \
            err_frm_opencl_param_note);                                    \
  }
