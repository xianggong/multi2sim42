...
void frm_grid_free(struct frm_grid_t *grid) {
  ...
  if (grid->free_notify_func) grid->free_notify_func(grid->free_notify_data);
...
void frm_grid_dump(struct frm_grid_t *grid, FILE *f) {
  ...
  //	{
  //		branch_digest = tid < grid->grid_size ?
  //grid->threads[tid]->branch_digest : 0;
  //		if (tid == grid->thread_count || branch_digest !=
  //last_branch_digest)
  //		{
...
void frm_grid_setup_size(struct frm_grid_t *grid, unsigned int *block_count,
  ...
  /* Thread block counts */
  for (i = 0; i < 3; i++) grid->block_count3[i] = block_count[i];
  grid->block_count =
      grid->block_count3[0] * grid->block_count3[1] * grid->block_count3[2];

  /* Thread block sizes */
  for (i = 0; i < 3; i++) grid->block_size3[i] = block_size[i];
  grid->block_size =
      grid->block_size3[0] * grid->block_size3[1] * grid->block_size3[2];

  /* Calculate grid sizes */
  for (i = 0; i < 3; i++) grid->grid_size3[i] = block_count[i] * block_size[i];
  grid->grid_size =
      grid->grid_size3[0] * grid->grid_size3[1] * grid->grid_size3[2];
