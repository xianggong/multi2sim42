  ...
  for (i = 0; i < list_count(list); i++) {
    ...
    for (j = 0; j < warp->thread_count; j++) {
      ...
      first = warp->threads[j]->branch_digest == elem->branch_digest &&
              (j == 0 ||
               warp->threads[j - 1]->branch_digest != elem->branch_digest);
      last = warp->threads[j]->branch_digest == elem->branch_digest &&
             (j == warp->thread_count - 1 ||
              warp->threads[j + 1]->branch_digest != elem->branch_digest);
      if (first)
        fprintf(f, " %d", j);
      else if (last)
...
void frm_warp_execute(struct frm_warp_t *warp) {
  ...
  ((inst->dword).word)[1] = (warp->inst_buffer)[warp->pc / warp->inst_size];
  frm_isa_debug("%s:%d: warp[%d] executes instruction 0x%0llx\n", __FUNCTION__,
                __LINE__, warp->id, inst->dword.dword);

  /* Decode instruction */
  frm_inst_decode(inst);
  if (!inst->info)
    fatal("%s: unrecognized instruction (%08x %08x)", __FUNCTION__,
          inst->dword.word[0], inst->dword.word[1]);
  ...
  switch (inst->info->fmt) {
    case FRM_FMT_MOV_MOV:
      ...
      for (thread_id = warp->threads[0]->id_in_warp;
           thread_id < (warp)->thread_count; thread_id++) {
  ...
  if (warp->finished) {
    ...
    if (list_count(thread_block->finished_warps) == thread_block->warp_count) {
      assert(list_index_of(grid->running_thread_blocks, thread_block) != -1);
      assert(list_index_of(grid->finished_thread_blocks, thread_block) == -1);
