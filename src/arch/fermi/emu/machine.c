...
char *frm_err_isa_note =
    "\tThe NVIDIA Fermi SASS instruction set is \n"
...
void frm_isa_FADD32I_impl(struct frm_thread_t *thread,
...
void frm_isa_FMUL32I_impl(struct frm_thread_t *thread,
                          struct frm_inst_t *inst) {
  unsigned int dst_id, src1_id;
  float dst, src1, imm32;
...
void frm_isa_IMAD_impl(struct frm_thread_t *thread, struct frm_inst_t *inst) {
  ...
  /* Debug */
  frm_isa_debug(
      "%s:%d: thread[%d] pred = [%x] %x dst = [0x%x] 0x%08x "
      "src1 = [0x%x] 0x%08x src2 = [0x%x] 0x%08x "
      "src3 = [0x%x] 0x%08x\n",
      __FUNCTION__, __LINE__, thread->id, pred_id, pred, dst_id, dst, src1_id,
      src1, src2_id, src2, src3_id, src3);
}
...
void frm_isa_IADD32I_impl(struct frm_thread_t *thread,
                          struct frm_inst_t *inst) {
  unsigned int dst_id, src1_id;
...
void frm_isa_ISETP_impl(struct frm_thread_t *thread, struct frm_inst_t *inst) {
  ...
  if (pred == 1) {
    ...
    switch (compare_op) {
      ...
      case 6:
        ...
        break;
      default:
        fatal("%s: unsupported .cmp operation 0x%x", __FUNCTION__, compare_op);
    }
    ...
    switch (logic_op) {
      ...
      case 1:
        ...
        break;
      case 2:
        p = (compare_result && !r) || (!compare_result && r);
        q = (compare_result && r) || (!compare_result && !r);
        break;
      default:
        fatal("%s: unsupported .logic operation 0x%x", __FUNCTION__, logic_op);
    }
  ...
  /* Debug */
  frm_isa_debug(
      "%s:%d: thread[%d] pred = [%d] %d p = [%d] %d, "
      "q = [%d] %d, src1 = [0x%x] 0x%08x, "
      "src2 = [0x%x]0x%08x, r = [%d] %d\n",
      __FUNCTION__, __LINE__, thread->id, pred_id, pred, p_id, p, q_id, q,
      src1_id, src1, src2_id, src2, r_id, r);
}
...
void frm_isa_MOV_impl(struct frm_thread_t *thread, struct frm_inst_t *inst) {
  ...
  /* Debug */
  frm_isa_debug(
      "%s:%d: thread[%d] pred = [%x] %d dst = [0x%x] 0x%08x "
      "src = [0x%x] 0x%08x\n",
      __FUNCTION__, __LINE__, thread->id, pred_id, pred, dst_id, dst, src_id,
      src);
}
...
void frm_isa_LDS_LDU_impl(struct frm_thread_t *thread,
                          struct frm_inst_t *inst) {
  __NOT_IMPL__
}
...
void frm_isa_LONGJMP_impl(struct frm_thread_t *thread,
                          struct frm_inst_t *inst) {
  __NOT_IMPL__
}
...
void frm_isa_PLONGJMP_impl(struct frm_thread_t *thread,
                           struct frm_inst_t *inst) {
  __NOT_IMPL__
}
...
void frm_isa_EXIT_impl(struct frm_thread_t *thread, struct frm_inst_t *inst) {
  if (inst->dword.tgt.pred == 7) thread->warp->finished = 1;
}
