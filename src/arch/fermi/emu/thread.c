...
void frm_thread_free(struct frm_thread_t *thread) {
  ...
  while (list_count(thread->lds_oqa)) free(list_dequeue(thread->lds_oqa));
  while (list_count(thread->lds_oqb)) free(list_dequeue(thread->lds_oqb));
...
int frm_thread_get_active(struct frm_thread_t *thread) {
  ...
  return bit_map_get(warp->active_stack,
                     warp->stack_top * warp->thread_count + thread->id_in_warp,
                     1);
}
...
void frm_thread_set_active(struct frm_thread_t *thread, int active) {
  ...
  assert(thread->id_in_warp >= 0 && thread->id_in_warp < warp->thread_count);
  bit_map_set(warp->active_stack,
              warp->stack_top * warp->thread_count + thread->id_in_warp, 1,
              !!active);
...
void frm_thread_update_branch_digest(struct frm_thread_t *thread,
  ...
  if (!bit_map_get(warp->active_stack,
                   warp->stack_top * warp->thread_count + thread->id_in_warp,
                   1))
    return;

  /* Update mask with inst_count */
  mask = (unsigned int)inst_count *
         0x4919f71f; /* Multiply by prime number to generate sparse mask */
