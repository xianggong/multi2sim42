...
void frm_vector_mem_write(struct frm_vector_mem_unit_t *vector_mem) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    /* Access complete, remove the uop from the queue */
    uop->write_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_vector_mem_write_latency;
    ...
    if (frm_spatial_report_active) {
      if (uop->vector_mem_write) {
        frm_report_global_mem_finish(uop->sm, uop->num_global_mem_write);
      } else if (uop->vector_mem_read) {
        frm_report_global_mem_finish(uop->sm, uop->num_global_mem_read);
      } else {
...
void frm_vector_mem_mem(struct frm_vector_mem_unit_t *vector_mem) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    for (thread_id = uop->warp->threads[0]->id_in_warp;
      ...
      mod_access(vector_mem->sm->global_memory, access_kind,
                 thread_uop->global_mem_access_addr, &uop->global_mem_witness,
                 NULL, NULL, NULL);
    ...
    if (frm_spatial_report_active) {
      if (uop->vector_mem_write) {
        ...
        frm_report_global_mem_inflight(uop->sm, uop->num_global_mem_write);
      } else if (uop->vector_mem_read) {
        uop->num_global_mem_read += uop->global_mem_witness;
        frm_report_global_mem_inflight(uop->sm, uop->num_global_mem_read);
      } else
        fatal("%s: invalid access kind", __FUNCTION__);
    }
...
void frm_vector_mem_read(struct frm_vector_mem_unit_t *vector_mem) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->read_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_vector_mem_read_latency;
...
void frm_vector_mem_decode(struct frm_vector_mem_unit_t *vector_mem) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->decode_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_vector_mem_decode_latency;
