...
struct frm_sm_t *frm_sm_create() {
  ...
  sm->lds_module = mod_create(buf, mod_kind_local_memory, frm_gpu_lds_num_ports,
                              frm_gpu_lds_block_size, frm_gpu_lds_latency);

  /* Hardware structures */
  sm->num_warp_inst_queues = 2;
  sm->warp_inst_queues =
      xcalloc(sm->num_warp_inst_queues, sizeof(struct list_t *));
  sm->fetch_buffers =
      xcalloc(sm->num_warp_inst_queues, sizeof(struct list_t *));
  sm->num_simd_units = 2;
  sm->simd_units = xcalloc(sm->num_simd_units, sizeof(struct frm_simd_t *));
  ...
  for (i = 0; i < sm->num_simd_units; i++) {
    sm->simd_units[i] = xcalloc(1, sizeof(struct frm_simd_t));
    sm->simd_units[i]->id_in_sm = i;
    sm->simd_units[i]->sm = sm;
    sm->simd_units[i]->warp_inst_queue = sm->warp_inst_queues[i];
    ...
    sm->simd_units[i]->sm = sm;
    sm->simd_units[i]->wkg_util = xcalloc(1, sizeof(struct frm_util_t));
    sm->simd_units[i]->wvf_util = xcalloc(1, sizeof(struct frm_util_t));
    sm->simd_units[i]->rdy_util = xcalloc(1, sizeof(struct frm_util_t));
    sm->simd_units[i]->occ_util = xcalloc(1, sizeof(struct frm_util_t));
    sm->simd_units[i]->wki_util = xcalloc(1, sizeof(struct frm_util_t));
    sm->simd_units[i]->act_util = xcalloc(1, sizeof(struct frm_util_t));
    sm->simd_units[i]->tot_util = xcalloc(1, sizeof(struct frm_util_t));
  }

  sm->thread_blocks = xcalloc(frm_gpu_max_thread_blocks_per_sm, sizeof(void *));
...
void frm_sm_map_thread_block(struct frm_sm_t *sm,
  ...
  for (warp_id = 0; warp_id < thread_block->warp_count; warp_id++) {
    ...
    warp->id_in_sm =
        thread_block->id_in_sm * thread_block->warp_count + warp_id;
    ...
    warp->warp_inst_queue_entry = warp->warp_inst_queue->entries[entry_index];
  ...
  /* Trace */
  frm_trace(
      "frm.map_tb sm=%d tb=%d t_first=%d t_count=%d w_first=%d "
      "w_count=%d\n",
      sm->id, thread_block->id, thread_block->threads[0]->id,
      thread_block->thread_count, thread_block->warps[0]->id,
      thread_block->warp_count);

  /* Stats */
  sm->mapped_thread_blocks++;
  if (frm_spatial_report_active) frm_report_mapped_thread_block(sm);
}
...
void frm_sm_unmap_thread_block(struct frm_sm_t *sm,
   ...
   * 'sm_busy' list */
  if (!sm->thread_block_count && list_index_of(frm_gpu->sm_busy_list, sm) != -1)
...
void frm_sm_fetch(struct frm_sm_t *sm, int wiq_id) {
  ...
  if (instructions_processed == frm_gpu_fe_fetch_width) return;

  /* WIQ entry not ready */
  if (!warp->warp_inst_queue_entry->ready) return;
  ...
  if (warp->warp_inst_queue_entry->wait_for_mem) {
    if (!warp->warp_inst_queue_entry->lgkm_cnt &&
      ...
      warp->warp_inst_queue_entry->wait_for_mem = 0;
    } else {
  ...
  if (list_count(sm->fetch_buffers[wiq_id]) == frm_gpu_fe_fetch_buffer_size)
    return;
  ...
  if (frm_tracing()) {
    ...
    frm_trace(
        "si.new_inst id=%lld cu=%d ib=%d wg=%d "
        "wf=%d uop_id=%lld stg=\"f\" asm=\"%s\"\n",
        uop->id_in_sm, sm->id, uop->warp_inst_queue_id, uop->thread_block->id,
        warp->id, uop->id_in_warp, inst_str_trimmed);
  }
  ...
  for (thread_id = uop->warp->threads[0]->id_in_warp;
    ...
    /* Global memory */
    thread_uop->global_mem_access_addr = thread->global_mem_access_addr;
    thread_uop->global_mem_access_size = thread->global_mem_access_size;

    /* LDS */
    thread_uop->lds_access_count = thread->lds_access_count;
    for (j = 0; j < thread->lds_access_count; j++) {
      thread_uop->lds_access_kind[j] = thread->lds_access_type[j];
      thread_uop->lds_access_addr[j] = thread->lds_access_addr[j];
      thread_uop->lds_access_size[j] = thread->lds_access_size[j];
    }
  }
...
void frm_sm_issue_oldest(struct frm_sm_t *sm, int active_fb) {
  struct frm_uop_t *uop;
  ...
  for (issued_insts = 0; issued_insts < frm_gpu_fe_max_inst_issued_per_type;
       issued_insts++) {
    for (i = 0; i < list_entries; i++) {
      uop = list_get(sm->fetch_buffers[active_fb], list_index);
      assert(uop);
      ...
      if (oldest_uop &&
          list_count(sm->branch_unit.issue_buffer) <
              frm_gpu_branch_unit_issue_buffer_size) {
        oldest_uop->issue_ready =
            asTiming(frm_gpu)->cycle + frm_gpu_fe_issue_latency;
        list_remove(sm->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(sm->branch_unit.issue_buffer, oldest_uop);

        /* Trace */
        frm_trace(
            "si.inst id=%lld cu=%d wf=%d "
            "uop_id=%lld stg=\"i\"\n",
  ...
  for (issued_insts = 0; issued_insts < frm_gpu_fe_max_inst_issued_per_type;
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
          list_count(sm->simd_units[active_fb]->issue_buffer) <
              frm_gpu_simd_issue_buffer_size) {
        oldest_uop->issue_ready =
            asTiming(frm_gpu)->cycle + frm_gpu_fe_issue_latency;
        list_remove(sm->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(sm->simd_units[active_fb]->issue_buffer, oldest_uop);
  ...
  for (issued_insts = 0; issued_insts < frm_gpu_fe_max_inst_issued_per_type;
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
          list_count(sm->vector_mem_unit.issue_buffer) <
              frm_gpu_vector_mem_issue_buffer_size) {
        oldest_uop->issue_ready =
            asTiming(frm_gpu)->cycle + frm_gpu_fe_issue_latency;
        list_remove(sm->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(sm->vector_mem_unit.issue_buffer, oldest_uop);
  ...
  for (issued_insts = 0; issued_insts < frm_gpu_fe_max_inst_issued_per_type;
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
          list_count(sm->lds_unit.issue_buffer) <
              frm_gpu_lds_issue_buffer_size) {
        oldest_uop->issue_ready =
            asTiming(frm_gpu)->cycle + frm_gpu_fe_issue_latency;
        list_remove(sm->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(sm->lds_unit.issue_buffer, oldest_uop);
...
void frm_sm_update_fetch_visualization(struct frm_sm_t *sm, int non_active_fb) {
  struct frm_uop_t *uop;
  int list_entries;
  int i;
...
/* Decode the instruction type */
void frm_sm_issue_first(struct frm_sm_t *sm, int active_fb) {
  struct frm_uop_t *uop;
  int list_index = 0;
  int list_entries;
  int i;
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    switch (uop->inst.info->fmt) {
      ...
      case FRM_FMT_CTRL_EXIT: {
        ...
        assert(simd_insts_issued <= frm_gpu_fe_max_inst_issued_per_type);
        ...
        if (list_count(sm->simd_units[active_fb]->issue_buffer) ==
            frm_gpu_simd_issue_buffer_size) {
          frm_trace(
              "si.inst id=%lld cu=%d wf=%d "
              "uop_id=%lld stg=\"s\"\n",
        ...
        uop->issue_ready = asTiming(frm_gpu)->cycle + frm_gpu_fe_issue_latency;
        list_remove(sm->fetch_buffers[active_fb], uop);
        list_enqueue(sm->simd_units[active_fb]->issue_buffer, uop);
      ...
      case FRM_FMT_LDST_ST: {
         ...
         * issued */
        assert(mem_insts_issued <= frm_gpu_fe_max_inst_issued_per_type);
        if (mem_insts_issued == frm_gpu_fe_max_inst_issued_per_type) {
          frm_trace(
              "si.inst id=%lld cu=%d wf=%d "
              "uop_id=%lld stg=\"s\"\n",
              uop->id_in_sm, sm->id, uop->warp->id, uop->id_in_warp);
        ...
        if (list_count(sm->vector_mem_unit.issue_buffer) ==
            frm_gpu_vector_mem_issue_buffer_size) {
          frm_trace(
              "si.inst id=%lld cu=%d wf=%d "
              "uop_id=%lld stg=\"s\"\n",
        ...
        uop->issue_ready = asTiming(frm_gpu)->cycle + frm_gpu_fe_issue_latency;
        list_remove(sm->fetch_buffers[active_fb], uop);
        list_enqueue(sm->vector_mem_unit.issue_buffer, uop);
...
void frm_sm_run(struct frm_sm_t *sm) {
  ...
  /* SIMD */
  for (i = 0; i < sm->num_simd_units; i++) frm_simd_run(sm->simd_units[i]);
  ...
  /* Fetch */
  for (i = 0; i < sm->num_warp_inst_queues; i++) frm_sm_fetch(sm, i);

  /* Stats */
  sm->cycle++;

  if (frm_spatial_report_active) frm_sm_interval_update(sm);
}
