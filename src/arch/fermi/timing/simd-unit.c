...
void frm_simd_complete(struct frm_simd_t *simd) {
  ...
  for (i = 0; i < num_insts; i++) {
    ...
    if (uop->warp_last_inst) {
      ...
      //		uop->thread_block->warp_count)
      { frm_sm_unmap_thread_block(simd->sm, uop->thread_block); }
    }
  ...
  for (i = 0; i < num_insts; i++) {
    ...
    if (instructions_processed > frm_gpu_simd_width) {
      frm_trace(
          ...
          "stg=\"s\"\n",
          uop->id_in_sm, simd->sm->id, uop->warp->id, uop->id_in_warp);
      list_index++;
      continue;
    }

    /* Sanity check exec buffer */
    assert(list_count(simd->exec_buffer) <= frm_gpu_simd_exec_buffer_size);

    /* Stall if SIMD unit is full */
    if (list_count(simd->exec_buffer) == frm_gpu_simd_exec_buffer_size) {
      frm_trace(
          "si.inst id=%lld cu=%d wf=%d uop_id=%lld "
          "stg=\"s\"\n",
          uop->id_in_sm, simd->sm->id, uop->warp->id, uop->id_in_warp);
...
void frm_simd_decode(struct frm_simd_t *simd) {
  ...
  for (i = 0; i < num_insts; i++) {
    ...
    if (instructions_processed > frm_gpu_simd_width) {
      frm_trace(
          "frm.inst id=%lld sm=%d w=%d uop_id=%lld "
          "stg=\"s\"\n",
          uop->id_in_sm, simd->sm->id, uop->warp->id, uop->id_in_warp);
      list_index++;
      continue;
    ...
    /* Sanity check the decode buffer */
    assert(list_count(simd->decode_buffer) <= frm_gpu_simd_decode_buffer_size);

    /* Stall if the decode buffer is full. */
    if (list_count(simd->decode_buffer) == frm_gpu_simd_decode_buffer_size) {
      frm_trace(
          "frm.inst id=%lld sm=%d w=%d uop_id=%lld "
          "stg=\"s\"\n",
          uop->id_in_sm, simd->sm->id, uop->warp->id, uop->id_in_warp);
