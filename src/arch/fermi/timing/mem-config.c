...
void FrmGpuMemConfigCheck(Timing *self, struct config_t *config) {
  ...
  FRM_GPU_FOREACH_SM(sm_id) {
    ...
    if (!sm->global_memory)
      fatal(
          "%s: Fermi SM[%d] has no entry to memory.\n"
...
void FrmGpuMemConfigParseEntry(Timing *self, struct config_t *config,
  ...
  if (sm_id < 0)
    fatal("%s: section [%s]: invalid or missing value for 'SM'", file_name,
          section);

  /* Check SM boundaries */
  if (sm_id >= frm_gpu_num_sms) {
    warning(
        "%s: section [%s] ignored, referring to Fermi SM[%d].\n"
  ...
  if (sm->global_memory)
    fatal(
        "%s: section [%s]: entry from SM[%d] already assigned.\n"
  ...
  linked_list_add(arch_fermi->mem_entry_mod_list, sm->global_memory);
