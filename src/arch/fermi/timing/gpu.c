...
char *frm_gpu_device_extensions =
...
unsigned long long frm_gpu_global_mem_cache_size = 1ull << 23;
    /* 8MB */ /* FIXME */
unsigned long long frm_gpu_global_mem_size =
    1ull << 32; /* 4GB of global memory reported */
unsigned long long frm_gpu_max_mem_alloc_size = (1ull << 32) / 4; /* FIXME */

unsigned int frm_gpu_local_mem_type = 1; /* CL_LOCAL */

unsigned long long frm_gpu_max_constant_buffer_size = 1ull << 16;
    /* 64KB */                              /* The minimum value acceptable */
...
unsigned int frm_gpu_execution_capabilities = 1;
    /* bit field, CL_EXEC_KERNEL only */ /* FIXME */

unsigned int frm_gpu_queue_properties =
    2; /* bit field, profiling enabled, no out of order execution */
...
static void frm_gpu_map_grid(struct frm_grid_t *grid) {
  ...
  frm_gpu->thread_blocks_per_sm = frm_calc_get_thread_blocks_per_sm(
      grid->block_size, grid->num_gpr_used, grid->local_mem_top);

  /* Thread block cannot be assigned to SM */
  if (!frm_gpu->thread_blocks_per_sm) {
    fatal(
        "Thread blocks cannot be assigned to SMs.\n"
  ...
  frm_gpu->warps_per_sm =
      frm_gpu->thread_blocks_per_sm * grid->thread_blocks[0]->warp_count;
...
void frm_gpu_read_config(void) {
  ...
  if (*frm_gpu_config_file_name) config_load(gpu_config);

  /* Device */
  section = "Device";

  frm_gpu_frequency =
      config_read_int(gpu_config, section, "Frequency", frm_gpu_frequency);
  if (!IN_RANGE(frm_gpu_frequency, 1, ESIM_MAX_FREQUENCY))
    fatal("%s: invalid value for 'Frequency'.\n%s", frm_gpu_config_file_name,
          err_note);

  frm_gpu_num_sms =
      config_read_int(gpu_config, section, "NumSMs", frm_gpu_num_sms);
  ...
  frm_gpu_max_thread_blocks_per_sm =
      config_read_int(gpu_config, section, "MaxThreadBlocksPerSM",
                      frm_gpu_max_thread_blocks_per_sm);
  ...
  frm_gpu_max_warps_per_sm = config_read_int(
      gpu_config, section, "MaxWarpsPerSM", frm_gpu_max_warps_per_sm);
  if (frm_gpu_max_warps_per_sm < 1)
    fatal("%s: invalid value for 'MaxWarpsPerSM'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_num_registers_per_sm = config_read_int(
      gpu_config, section, "NumRegistersPerSM", frm_gpu_num_registers_per_sm);
  ...
  frm_gpu_fe_fetch_width = config_read_int(gpu_config, section, "FetchWidth",
                                           frm_gpu_fe_fetch_width);
  ...
  frm_gpu_fe_fetch_buffer_size = config_read_int(
      gpu_config, section, "FetchBufferSize", frm_gpu_fe_fetch_buffer_size);
  ...
  frm_gpu_fe_issue_width = config_read_int(gpu_config, section, "IssueWidth",
                                           frm_gpu_fe_issue_width);
  ...
  frm_gpu_simd_exec_latency = config_read_int(
      gpu_config, section, "ReadExecWriteLatency", frm_gpu_simd_exec_latency);
  if (frm_gpu_simd_exec_latency < 0)
    fatal("%s: invalid value for 'ReadExecWriteLatency'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_simd_exec_buffer_size =
      config_read_int(gpu_config, section, "ReadExecWriteBufferSize",
                      frm_gpu_simd_exec_buffer_size);
  ...
  frm_gpu_branch_unit_width =
      config_read_int(gpu_config, section, "Width", frm_gpu_branch_unit_width);
  if (frm_gpu_branch_unit_width < 1)
    fatal("%s: invalid value for 'Width'.\n%s", frm_gpu_config_file_name,
          err_note);

  frm_gpu_branch_unit_issue_buffer_size =
      config_read_int(gpu_config, section, "IssueBufferSize",
                      frm_gpu_branch_unit_issue_buffer_size);
  ...
  frm_gpu_branch_unit_decode_latency = config_read_int(
      gpu_config, section, "DecodeLatency", frm_gpu_branch_unit_decode_latency);
  if (frm_gpu_branch_unit_decode_latency < 0)
    fatal("%s: invalid value for 'DecodeLatency'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_branch_unit_decode_buffer_size =
      config_read_int(gpu_config, section, "DecodeBufferSize",
                      frm_gpu_branch_unit_decode_buffer_size);
  ...
  frm_gpu_branch_unit_read_latency = config_read_int(
      gpu_config, section, "ReadLatency", frm_gpu_branch_unit_read_latency);
  if (frm_gpu_branch_unit_read_latency < 0)
    fatal("%s: invalid value for 'ReadLatency'.\n%s", frm_gpu_config_file_name,
          err_note);

  frm_gpu_branch_unit_read_buffer_size =
      config_read_int(gpu_config, section, "ReadBufferSize",
                      frm_gpu_branch_unit_read_buffer_size);
  ...
  frm_gpu_branch_unit_exec_latency = config_read_int(
      gpu_config, section, "ExecLatency", frm_gpu_branch_unit_exec_latency);
  if (frm_gpu_branch_unit_exec_latency < 0)
    fatal("%s: invalid value for 'ExecLatency'.\n%s", frm_gpu_config_file_name,
          err_note);

  frm_gpu_branch_unit_exec_buffer_size =
      config_read_int(gpu_config, section, "ExecBufferSize",
                      frm_gpu_branch_unit_exec_buffer_size);
  ...
  frm_gpu_branch_unit_write_latency = config_read_int(
      gpu_config, section, "WriteLatency", frm_gpu_branch_unit_write_latency);
  if (frm_gpu_branch_unit_write_latency < 0)
    fatal("%s: invalid value for 'WriteLatency'.\n%s", frm_gpu_config_file_name,
          err_note);

  frm_gpu_branch_unit_write_buffer_size =
      config_read_int(gpu_config, section, "WriteBufferSize",
                      frm_gpu_branch_unit_write_buffer_size);
  ...
  frm_gpu_lds_width =
      config_read_int(gpu_config, section, "Width", frm_gpu_lds_width);
  if (frm_gpu_lds_width < 1)
    fatal("%s: invalid value for 'Width'.\n%s", frm_gpu_config_file_name,
          err_note);

  frm_gpu_lds_issue_buffer_size = config_read_int(
      gpu_config, section, "IssueBufferSize", frm_gpu_lds_issue_buffer_size);
  if (frm_gpu_lds_issue_buffer_size < 1)
    fatal("%s: invalid value for 'IssueBufferSize'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_lds_decode_latency = config_read_int(
      gpu_config, section, "DecodeLatency", frm_gpu_lds_decode_latency);
  if (frm_gpu_lds_decode_latency < 0)
    fatal("%s: invalid value for 'DecodeLatency'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_lds_decode_buffer_size = config_read_int(
      gpu_config, section, "DecodeBufferSize", frm_gpu_lds_decode_buffer_size);
  if (frm_gpu_lds_decode_buffer_size < 1)
    fatal("%s: invalid value for 'DecodeBufferSize'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_lds_read_latency = config_read_int(gpu_config, section, "ReadLatency",
                                             frm_gpu_lds_read_latency);
  ...
  frm_gpu_lds_read_buffer_size = config_read_int(
      gpu_config, section, "ReadBufferSize", frm_gpu_lds_read_buffer_size);
  if (frm_gpu_lds_read_buffer_size < 1)
    fatal("%s: invalid value for 'ReadBufferSize'.\n%s",
          frm_gpu_config_file_name, err_note);

  frm_gpu_lds_max_inflight_mem_accesses =
      config_read_int(gpu_config, section, "MaxInflightMem",
                      frm_gpu_lds_max_inflight_mem_accesses);
  ...
  frm_gpu_lds_write_buffer_size = config_read_int(
      gpu_config, section, "WriteBufferSize", frm_gpu_lds_write_buffer_size);
  ...
  frm_gpu_vector_mem_width =
      config_read_int(gpu_config, section, "Width", frm_gpu_vector_mem_width);
  ...
  frm_gpu_vector_mem_decode_latency = config_read_int(
      gpu_config, section, "DecodeLatency", frm_gpu_vector_mem_decode_latency);
  ...
  frm_gpu_vector_mem_read_latency = config_read_int(
      gpu_config, section, "ReadLatency", frm_gpu_vector_mem_read_latency);
  ...
  frm_gpu_vector_mem_write_latency = config_read_int(
      gpu_config, section, "WriteLatency", frm_gpu_vector_mem_write_latency);
  ...
  frm_gpu_shared_mem_size =
      config_read_int(gpu_config, section, "Size", frm_gpu_shared_mem_size);
  frm_gpu_lds_block_size =
      config_read_int(gpu_config, section, "BlockSize", frm_gpu_lds_block_size);
  frm_gpu_lds_latency =
      config_read_int(gpu_config, section, "Latency", frm_gpu_lds_latency);
  frm_gpu_lds_num_ports =
      config_read_int(gpu_config, section, "Ports", frm_gpu_lds_num_ports);

  if ((frm_gpu_lds_block_size & (frm_gpu_lds_block_size - 1)) ||
      frm_gpu_lds_block_size < 4)
    fatal(
        "%s: %s->BlockSize must be a power of two and at "
        "least 4.\n%s",
  ...
  if (frm_gpu_shared_mem_size < frm_gpu_lds_block_size)
    fatal(
        "%s: %s->Size cannot be smaller than %s->BlockSize * "
        "%s->Banks.\n%s",
        frm_gpu_config_file_name, section, section, section, err_note);
...
void frm_gpu_init(void) {
  ...
  if (frm_gpu_report_file_name[0] &&
    ...
    fatal("%s: cannot open GPU pipeline report file", frm_gpu_report_file_name);
...
void frm_gpu_done() {
  ...
  /* Spatial report */
  if (frm_spatial_report_active) frm_sm_spatial_report_done();
...
void frm_gpu_dump_report(void) {
  ...
  inst_per_cycle =
      asTiming(frm_gpu)->cycle
          ? (double)(asEmu(frm_emu)->instructions / asTiming(frm_gpu)->cycle)
          : 0.0;
  fprintf(f, "[ Device ]\n\n");
  ...
  FRM_GPU_FOREACH_SM(sm_id) {
    ...
    inst_per_cycle = sm->cycle ? (double)(sm->inst_count / sm->cycle) : 0.0;
...
int FrmGpuRun(Timing *self) {
  ...
  fatal("%s: Fermi timing simulation not supported yet", __FUNCTION__);

  /* For efficiency when no Fermi emulation is selected,
   * exit here if the list of existing grids is empty. */
  if (!list_count(frm_emu->grids)) return FALSE;
  ...
  while ((grid = list_head(frm_emu->pending_grids))) {
    ...
    frm_trace("frm.new_grid id=%d tb_first=%d tb_count=%d\n", grid->id,
              grid->thread_blocks[0]->id, grid->thread_block_count);
  ...
  if (!list_count(gpu->sm_busy_list)) {
    ...
    if (frm_emu_max_functions &&
        list_count(frm_emu->grids) >= frm_emu_max_functions) {
