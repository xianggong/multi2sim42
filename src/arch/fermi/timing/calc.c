...
int frm_calc_get_thread_blocks_per_sm(int threads_per_thread_block,
  ...
  warps_per_thread_block =
      (threads_per_thread_block + frm_emu_warp_size - 1) / frm_emu_warp_size;
   ...
   * thread. */
  registers_per_thread_block = registers_per_thread * threads_per_thread_block;
  max_thread_blocks_limitted_by_num_registers =
      registers_per_thread_block
  ...
  max_thread_blocks_per_sm =
      MIN(max_thread_blocks_per_sm, max_thread_blocks_limitted_by_max_warps);
  max_thread_blocks_per_sm = MIN(max_thread_blocks_per_sm,
                                 max_thread_blocks_limitted_by_num_registers);
  max_thread_blocks_per_sm =
      MIN(max_thread_blocks_per_sm, max_thread_blocks_limitted_by_shared_mem);
...
static void frm_calc_plot_threads_per_thread_block(void) {
  ...
  for (threads_per_thread_block = frm_emu_warp_size;
       threads_per_thread_block <
           (frm_gpu_max_warps_per_sm * frm_emu_warp_size);
    ...
    warps_per_thread_block =
        (threads_per_thread_block + frm_emu_warp_size - 1) / frm_emu_warp_size;
    warps_per_warp_pool = thread_blocks_per_warp_pool * warps_per_thread_block;
  ...
  /* Current data point */
  threads_per_thread_block =
      ROUND_UP(frm_gpu->grid->block_size, frm_emu_warp_size);
  // thread_blocks_per_warp_pool =
  //	frm_calc_get_thread_blocks_per_warp_pool(
  ...
  fprintf(script_file, "plot '%s' w linespoints lt 3 lw 5 pt 84 ps 2, ",
          data_file_name);
  fprintf(script_file,
          "'-' w linespoints lt 1 lw 4 pt 82 ps 2\n%d "
          "%d\ne\n",
          threads_per_thread_block, warps_per_warp_pool);
  fclose(script_file);
...
static void frm_calc_plot_registers_per_thread(void) {
  ...
  for (registers_per_thread = 1; registers_per_thread <= 128;
    ...
    /* Dump line to data file */
    fprintf(data_file, "%d %d\n", registers_per_thread, warps_per_warp_pool);
  }
  fclose(data_file);
...
static void frm_calc_plot_local_mem_per_thread_block(void) {
  ...
  fprintf(script_file, "set nokey\n");
  fprintf(script_file,
          "set xlabel 'Local memory used per work-group "
          "(KB)'\n");
  fprintf(script_file, "set ylabel 'Wavefronts per SIMD'\n");
  ...
  fprintf(script_file, "set grid ytics\n");
  fprintf(script_file,
          "plot '%s' u ($1/1024):2 w linespoints lt 3 "
          "lw 5 pt 84 ps 2, ",
          data_file_name);
  fprintf(script_file,
          "'-' u ($1/1024):2 w linespoints lt 1 lw 4 pt 82 "
          "ps 2\n%d %d\ne\n",
          local_mem_per_thread_block, warps_per_warp_pool);
  fclose(script_file);
...
void frm_calc_plot(void) {
  ...
  if (!*frm_gpu_calc_file_name) return;

  /* Find 'gnuplot' */
  ret = system("which gnuplot >/dev/null 2>&1");
  if (ret) {
    fatal(
        "GPU occupancy calculation plots could not be "
        "generated.\n"
