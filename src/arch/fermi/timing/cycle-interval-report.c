...
void frm_sm_spatial_report_dump(struct frm_sm_t *sm) {
  ...
  fprintf(f, "CU,%d,MemAcc,%lld,MappedWGs,%lld,Cycles,%lld\n", sm->id,
          ...
          sm->interval_mapped_thread_blocks, asTiming(frm_gpu)->cycle);
}

void frm_report_global_mem_inflight(struct frm_sm_t *sm,
...
void frm_report_global_mem_finish(struct frm_sm_t *sm,
