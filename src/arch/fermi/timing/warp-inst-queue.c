...
struct frm_warp_inst_queue_t *frm_warp_inst_queue_create() {
  ...
  warp_inst_queue->entries = xcalloc(
      frm_gpu_max_warps_per_sm, sizeof(struct frm_warp_inst_queue_entry_t *));

  for (i = 0; i < frm_gpu_max_warps_per_sm; i++) {
    warp_inst_queue->entries[i] =
        xcalloc(1, sizeof(struct frm_warp_inst_queue_entry_t));
...
void frm_warp_inst_queue_unmap_warp(
    struct frm_warp_inst_queue_t *warp_inst_queue, struct frm_warp_t *warp) {
  //	int wf_id_in_ib;
  //	int i;
  //
  //	/* Reset mapped warps */
