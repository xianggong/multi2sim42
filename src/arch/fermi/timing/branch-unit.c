...
void frm_branch_unit_complete(struct frm_branch_unit_t *branch_unit) {
  ...
  assert(list_entries <=
         frm_gpu_branch_unit_write_latency * frm_gpu_branch_unit_width);
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    if (instructions_processed > frm_gpu_branch_unit_width) {
      frm_trace(
          ...
          "stg=\"s\"\n",
          uop->id_in_sm, branch_unit->sm->id, uop->warp->id, uop->id_in_warp);
      list_index++;
      continue;
    }
    ...
    uop->write_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_branch_unit_write_latency;
...
void frm_branch_unit_execute(struct frm_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    /* Branch */
    uop->execute_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_branch_unit_exec_latency;
...
void frm_branch_unit_read(struct frm_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->read_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_branch_unit_read_latency;
...
void frm_branch_unit_decode(struct frm_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->decode_ready =
        asTiming(frm_gpu)->cycle + frm_gpu_branch_unit_decode_latency;
