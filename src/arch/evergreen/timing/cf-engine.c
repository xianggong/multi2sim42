...
static void evg_cf_engine_fetch(struct evg_compute_unit_t *compute_unit) {
  ...
  uop->last =
      DOUBLE_LINKED_LIST_MEMBER(wavefront->work_group, finished, wavefront);
  ...
  uop->inst_mem_ready =
      asTiming(evg_gpu)->cycle + evg_gpu_cf_engine_inst_mem_latency;
  ...
  if (evg_spatial_report_active) evg_cf_report_new_inst(compute_unit);

  if (uop->global_mem_write) compute_unit->cf_engine.global_mem_write_count++;
  ...
  if (evg_tracing()) {
    ...
    evg_trace(
        "evg.new_inst id=%lld cu=%d wg=%d wf=%d cat=\"cf\" stg=\"cf-fe\" "
        "asm=\"%s\"\n",
...
static void evg_cf_engine_decode(struct evg_compute_unit_t *compute_unit) {
  ...
  for (;;) {
    ...
    if (uop && uop->inst_mem_ready &&
        !compute_unit->cf_engine.inst_buffer[index])
      break;
    ...
    if (index == compute_unit->cf_engine.decode_index) return;
  }
  ...
  compute_unit->cf_engine.decode_index =
      (index + 1) % evg_gpu->wavefronts_per_compute_unit;
...
static void evg_cf_engine_execute(struct evg_compute_unit_t *compute_unit) {
  ...
  /* Complete queue must be empty. If it is not, it means that a write access is
   * trying to allocate
   * a cache port, so compute unit needs to stall. */
  if (linked_list_count(compute_unit->cf_engine.complete_queue)) return;
  ...
  for (;;) {
    ...
    index = (index + 1) % evg_gpu->wavefronts_per_compute_unit;
    if (index == compute_unit->cf_engine.execute_index) return;
  }
  ...
  compute_unit->cf_engine.execute_index =
      (index + 1) % evg_gpu->wavefronts_per_compute_unit;
...
static void evg_cf_engine_complete(struct evg_compute_unit_t *compute_unit) {
  ...
  while (linked_list_count(complete_queue)) {
     ...
     */
    if (uop->global_mem_write && uop->global_mem_witness) break;
    ...
    /* Debug */
    if (debug_status(evg_stack_debug_category)) evg_uop_debug_active_mask(uop);
