...
struct evg_compute_unit_t *evg_compute_unit_create() {
  ...
  compute_unit->local_memory =
      mod_create(buf, mod_kind_local_memory, evg_gpu_local_mem_num_ports,
                 evg_gpu_local_mem_block_size, evg_gpu_local_mem_latency);

  /* Initialize CF Engine */
  compute_unit->cf_engine.complete_queue = linked_list_create();
  compute_unit->cf_engine.fetch_buffer =
      xcalloc(evg_gpu_max_wavefronts_per_compute_unit, sizeof(void *));
  compute_unit->cf_engine.inst_buffer =
      xcalloc(evg_gpu_max_wavefronts_per_compute_unit, sizeof(void *));
  ...
  compute_unit->work_groups =
      xcalloc(evg_gpu_max_work_groups_per_compute_unit, sizeof(void *));
  return compute_unit;
}
...
void evg_compute_unit_free(struct evg_compute_unit_t *compute_unit) {
  ...
  /* CF Engine - free uops in fetch buffer, instruction buffer, and complete
   * queue */
  ...
  /* ALU Engine - free uops in fetch queue, instruction buffer, execution
   * buffer,
...
void evg_compute_unit_map_work_group(struct evg_compute_unit_t *compute_unit,
  ...
  EVG_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id) {
    ...
    wavefront->id_in_compute_unit =
        work_group->id_in_compute_unit * ndrange->wavefronts_per_work_group +
  ...
  /* Trace */
  evg_trace(
      "evg.map_wg cu=%d wg=%d wi_first=%d wi_count=%d wf_first=%d "
      "wf_count=%d\n",
...
void evg_compute_unit_unmap_work_group(struct evg_compute_unit_t *compute_unit,
                                       struct evg_work_group_t *work_group) {
  struct evg_ndrange_t *ndrange = work_group->ndrange;
  struct evg_wavefront_t *wavefront;
  ...
  if (!compute_unit->work_group_count &&
      DOUBLE_LINKED_LIST_MEMBER(evg_gpu, busy, compute_unit))
...
/* Advance one cycle in the compute unit by running every stage from last to
 * first */
void evg_compute_unit_run(struct evg_compute_unit_t *compute_unit) {
  ...
  /* Stats */
  compute_unit->cycle++;
  if (evg_spatial_report_active) evg_cu_interval_update(compute_unit);
}
