...
static void evg_alu_engine_fetch(struct evg_compute_unit_t *compute_unit) {
  ...
  struct linked_list_t *finished_queue =
      compute_unit->alu_engine.finished_queue;
  ...
  if (compute_unit->alu_engine.fetch_queue_length >=
  ...
  uop->subwavefront_count =
      (wavefront->work_item_count + evg_gpu_num_stream_cores - 1) /
  ...
  if (debug_status(evg_stack_debug_category)) evg_uop_save_active_mask(uop);
  ...
  if (evg_periodic_report_active) evg_periodic_report_new_inst(uop);
  if (evg_spatial_report_active) evg_alu_report_new_inst(compute_unit);
  ...
  if (uop->local_mem_read || uop->local_mem_write) {
    EVG_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id) {
      ...
      for (i = 0; i < work_item->local_mem_access_count; i++) {
        work_item_uop->local_mem_access_kind[i] =
            work_item->local_mem_access_type[i];
        work_item_uop->local_mem_access_addr[i] =
            work_item->local_mem_access_addr[i];
        work_item_uop->local_mem_access_size[i] =
            work_item->local_mem_access_size[i];
      }
    }
  }

  /* Array 'producers' contains those uops in execution that produce each
   * possible
   * output dependence. Find the youngest producer for this uop. */
  producer = NULL;
  for (i = 0; i < uop->idep_count; i++) {
    idep = uop->idep[i];
    if (!compute_unit->alu_engine.producers[idep]) continue;
    if (!producer ||
        producer->id < compute_unit->alu_engine.producers[idep]->id)
      producer = compute_unit->alu_engine.producers[idep];
  }

  /* If there was a producer, enqueue uop in its dependence list. Otherwise, uop
   * is ready. */
  if (producer) {
  ...
  uop->inst_mem_ready =
      asTiming(evg_gpu)->cycle + evg_gpu_alu_engine_inst_mem_latency;
  ...
  if (evg_tracing()) {
    ...
    evg_trace(
        "evg.new_inst id=%lld cu=%d wg=%d wf=%d cat=\"alu\" stg=\"alu-fe\" "
        "%s\n",
        uop->id_in_compute_unit, compute_unit->id, uop->work_group->id,
        wavefront->id, str_inst);
  }
}
...
static void evg_alu_engine_decode(struct evg_compute_unit_t *compute_unit) {
  ...
  if (uop->inst_mem_ready > asTiming(evg_gpu)->cycle) return;

  /* If instruction buffer is occupied, done */
  if (compute_unit->alu_engine.inst_buffer) return;
...
static void evg_alu_engine_read(struct evg_compute_unit_t *compute_unit) {
  ...
  if (!uop || !uop->ready) return;

  /* If there is no space in the execution buffer, done */
  if (compute_unit->alu_engine.exec_buffer) return;
...
static void evg_alu_engine_execute(struct evg_compute_unit_t *compute_unit) {
  ...
  if (!uop || uop->local_mem_witness) return;

  /* If instruction writes to local memory, check that module is available. */
  if (uop->local_mem_write && !mod_can_access(local_memory, 0)) return;
  ...
  heap_insert(compute_unit->alu_engine.event_queue,
              asTiming(evg_gpu)->cycle + evg_gpu_alu_engine_pe_latency, uop);
...
static void evg_alu_engine_write(struct evg_compute_unit_t *compute_unit) {
  ...
  for (;;) {
    ...
    if (!uop || cycle > asTiming(evg_gpu)->cycle) break;
    ...
    if (uop->local_mem_write) {
      EVG_FOREACH_WORK_ITEM_IN_WAVEFRONT(uop->wavefront, work_item_id) {
        ...
        work_item_uop = &uop->work_item_uop[work_item->id_in_wavefront];
        for (i = 0; i < work_item_uop->local_mem_access_count; i++) {
          if (work_item_uop->local_mem_access_kind[i] != mod_access_store)
            continue;
          mod_access(compute_unit->local_memory, mod_access_store,
                     work_item_uop->local_mem_access_addr[i], NULL, NULL, NULL,
                     NULL);
        }
      }
    }
    ...
    if (uop->write_subwavefront_count == uop->subwavefront_count) {
      ...
      if (wavefront->clause_kind != EVG_CLAUSE_ALU &&
          !wavefront->alu_engine_in_flight) {
        /* Extract CF uop from finished queue. Since instruction execution may
         * vary in latency, finished CF uop may not be the one at the head of
         * the queue. */
        cf_uop = uop->cf_uop;
