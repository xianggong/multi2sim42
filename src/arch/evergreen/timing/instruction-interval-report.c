...
static struct str_map_t evg_periodic_report_scope_map = {
    ...
    {{"FirstWavefrontFirstWorkgroup",
      evg_periodic_report_scope_first_wavefront_first_workgroup},
     {"FirstWavefrontAllWorkgroups",
      evg_periodic_report_scope_first_wavefront_all_workgroups},
     {"AllWavefrontsFirstWorkgroup",
      evg_periodic_report_scope_all_wavefronts_first_workgroup},
     {"AllWavefrontsAllWorkgroups",
      evg_periodic_report_scope_all_wavefronts_all_workgroups}}};

static char *evg_periodic_report_scope_str = "FirstWavefrontFirstWorkgroup";

static char *evg_periodic_report_intro =
    "The fields contained in each record are listed next, in the same order "
    "of\n"
    "occurrence:\n"
    "\n"
    "<vliw_bundles>\n"
    "\tTotal number of instructions executed so far, considering a ALU "
    "instruction\n"
    "\tforming a VLIW bundle as one instruction. This value is a multiple of "
    "the\n"
    ...
    "\n"
    "<vliw_slots>\n"
    "\tNumber of instructions executed in the interval, considering each slot "
    "in an\n"
    "\tALU VLIW bundle as a single instruction. This value should be equal or "
    "greater\n"
    "\tthan the interval specified in the periodic report configuration.\n"
    "\n"
    "<local_mem_accesses>\n"
    "\tNumber of local memory accesses performed in the interval, adding up "
    "all\n"
    ...
    "\n"
    "<global_mem_writes>\n"
    "\tNumber of global memory writes performed in the interval, adding up "
    "all\n"
    "\taccesses performed by all work-items in the wavefront.\n"
    "\n";
...
void evg_periodic_report_done(void) { str_free(evg_periodic_report_file_name); }
...
void evg_periodic_report_config_read(struct config_t *config) {
  ...
  /* Interval */
  evg_periodic_report_interval = config_read_int(config, section, "Interval",
                                                 evg_periodic_report_interval);
  ...
  /* Wavefront set */
  evg_periodic_report_scope_str = config_read_string(
      config, section, "Scope", evg_periodic_report_scope_str);
  ...
  if (!evg_periodic_report_scope)
    fatal("%s: [%s]: invalid value for 'Scope'", evg_gpu_config_file_name,
          section);
}
...
void evg_periodic_report_wavefront_init(struct evg_wavefront_t *wavefront) {
  ...
  snprintf(file_name, sizeof file_name, "%s-wg%d-wf%d",
           evg_periodic_report_file_name, work_group->id,
...
void evg_periodic_report_wavefront_done(struct evg_wavefront_t *wavefront) {
  ...
  assert(evg_periodic_report_active);
  if (!wavefront->periodic_report_file) return;
...
void evg_periodic_report_dump_entry(struct evg_wavefront_t *wavefront) {
  ...
  if (wavefront->periodic_report_vliw_bundle_count <=
      evg_periodic_report_interval) {
  ...
  fprintf(f, "%5lld ",
          asTiming(evg_gpu)->cycle - wavefront->periodic_report_cycle);
...
/* Update periodic interval report statistics. This function is called every
 * time
 * a new instruction is fetched in the CF, ALU, or TEX engines. It should be
 * only
 * called if the periodic report has been activated (evg_periodic_report_active
 * = 1).
 */
void evg_periodic_report_new_inst(struct evg_uop_t *uop) {
  ...
  if (uop->local_mem_read || uop->local_mem_write) {
    EVG_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id) {
      ...
      wavefront->periodic_report_local_mem_accesses +=
