...
static struct evg_wavefront_t *evg_schedule_round_robin(
  ...
  if (!linked_list_get(wavefront_pool)) linked_list_head(wavefront_pool);
  ...
  for (;;) {
    ...
    wavefront = linked_list_get(wavefront_pool);
    if (wavefront == temp_wavefront) return NULL;
  }
 ...
 * If no valid candidate is found in the wavefront pool, the function returns
 * NULL. */
struct evg_wavefront_t *evg_schedule(struct evg_compute_unit_t *compute_unit) {
  ...
  if (!linked_list_count(compute_unit->wavefront_pool)) return NULL;
