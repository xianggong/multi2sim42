...
void EvgGpuMemConfigDefault(Timing *self, struct config_t *config) {
  ...
  EVG_GPU_FOREACH_COMPUTE_UNIT(compute_unit_id) {
    ...
    config_write_string(config, section, "LowModules",
                        "evg-l2-0 evg-l2-1 evg-l2-2 evg-l2-3");
  ...
  for (l2_id = 0; l2_id < 4; l2_id++) {
    ...
    config_write_string(config, section, "LowModules",
                        "evg-gm-0 evg-gm-1 evg-gm-2 evg-gm-3");
...
void EvgGpuMemConfigParseEntry(Timing *self, struct config_t *config,
  ...
  if (compute_unit_id >= evg_gpu_num_compute_units) {
    warning(
        "%s: section [%s] ignored, referring to Evergreen compute unit %d.\n"
        "\tThis section refers to a compute unit that does not currently "
        "exist.\n"
  ...
  if (compute_unit->global_memory)
    fatal(
        ...
        "has already\n"
        "\tassigned an entry for this particular compute unit. Please review "
        "your\n"
...
void EvgGpuMemConfigCheck(Timing *self, struct config_t *config) {
  ...
  EVG_GPU_FOREACH_COMPUTE_UNIT(compute_unit_id) {
    ...
    if (!compute_unit->global_memory)
      fatal(
          "%s: Evergreen compute unit %d has no entry to memory.\n"
