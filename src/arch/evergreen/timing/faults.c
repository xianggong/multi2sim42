...
enum evg_fault_type_enum { evg_fault_ams, evg_fault_reg, evg_fault_mem };
...
static int evg_stack_faults_is_odep(struct evg_uop_t *uop,
  ...
  if (wavefront != uop->wavefront) return 0;
  for (i = 0; i < EVG_UOP_MAX_ODEP; i++)
    if (uop->odep[i] == EVG_UOP_DEP_REG(lo_reg)) return 1;
  return 0;
}
...
static int evg_stack_faults_is_idep(struct evg_uop_t *uop,
  ...
  for (i = 0; i < EVG_UOP_MAX_IDEP; i++)
    if (uop->idep[i] == EVG_UOP_DEP_REG(lo_reg)) return 1;
  return 0;
}
...
void evg_faults_init(void) {
  ...
  if (!*evg_faults_file_name) return;

  f = fopen(evg_faults_file_name, "rt");
  if (!f) fatal("%s: cannot open file", evg_faults_file_name);
  ...
  while (!feof(f)) {
    ...
    if (!line_ptr) goto wrong_format;
    fault->cycle = atoll(line_ptr);
    if (fault->cycle < 1)
      fatal("%s: line %d: lowest possible cycle is 1", evg_faults_file_name,
            line_num);
    if (fault->cycle < last_cycle)
      fatal("%s: line %d: cycles must be ordered", evg_faults_file_name,
            line_num);
    ...
    if (fault->compute_unit_id >= evg_gpu_num_compute_units ||
        fault->compute_unit_id < 0)
      fatal("%s: line %d: invalid compute unit ID", evg_faults_file_name,
            line_num);
    ...
    switch (fault->type) {
      case evg_fault_ams:
        ...
        if (fault->stack_id >= evg_gpu_max_wavefronts_per_compute_unit)
          fatal("%s: line %d: invalid stack ID", evg_faults_file_name,
                line_num);
        ...
        if (fault->active_mask_id >= EVG_MAX_STACK_SIZE)
          fatal("%s: line %d: invalid active mask ID", evg_faults_file_name,
                line_num);
        ...
        if (fault->bit >= evg_emu_wavefront_size)
          fatal("%s: line %d: invalid bit index", evg_faults_file_name,
                line_num);

        /* No more tokens */
        if (strtok(NULL, delim))
          fatal("%s: line %d: too many arguments", evg_faults_file_name,
                line_num);
      ...
      case evg_fault_mem:
        ...
        if (fault->byte >= evg_gpu_local_mem_size || fault->byte < 0)
          fatal("%s: line %d: invalid byte position", evg_faults_file_name,
                line_num);
        ...
        if (fault->bit > 7 || fault->bit < 0)
          fatal("%s: line %d: invalid bit position", evg_faults_file_name,
                line_num);
  ...
  wrong_format:
    fatal("%s: line %d: not enough arguments", evg_faults_file_name, line_num);
  }
...
void evg_faults_insert(void) {
  ...
  for (;;) {
    ...
    if (!fault || fault->cycle > asTiming(evg_gpu)->cycle) break;
    ...
    switch (fault->type) {
      case evg_fault_ams: {
        ...
        /* Initial debug */
        evg_faults_debug(
            "fault clk=%lld cu=%d type=\"ams\" stack=%d am=%d bit=%d ",
            asTiming(evg_gpu)->cycle, fault->compute_unit_id, fault->stack_id,
        ...
        /* Get work-group and wavefront. If wavefront ID exceeds current number,
         * dismiss */
        work_group_id =
            fault->stack_id / evg_gpu->ndrange->wavefronts_per_work_group;
        wavefront_id =
            fault->stack_id % evg_gpu->ndrange->wavefronts_per_work_group;
        ...
        evg_faults_debug("effect=\"error\" wg=%d wf=%d wi=%d", work_group->id,
                         wavefront->id, work_item->id);

        /* Inject fault */
        value = bit_map_get(
            wavefront->active_stack,
            fault->active_mask_id * wavefront->work_item_count + fault->bit, 1);
        bit_map_set(
            wavefront->active_stack,
            fault->active_mask_id * wavefront->work_item_count + fault->bit, 1,
            !value);
      ...
      case evg_fault_reg: {
        ...
        evg_faults_debug("fault clk=%lld cu=%d type=\"reg\" reg=%d bit=%d ",
                         asTiming(evg_gpu)->cycle, fault->compute_unit_id,
                         fault->reg_id, fault->bit);
        assert(fault->cycle == asTiming(evg_gpu)->cycle);
        compute_unit = evg_gpu->compute_units[fault->compute_unit_id];
        ...
        /* Get work-group */
        num_registers_per_work_group =
            kernel->bin_file->enc_dict_entry_evergreen->num_gpr_used *
            kernel->local_size;
        work_group_id_in_compute_unit =
            fault->reg_id / num_registers_per_work_group;
        if (work_group_id_in_compute_unit >=
        ...
        work_item_id_in_compute_unit =
            fault->reg_id /
            kernel->bin_file->enc_dict_entry_evergreen->num_gpr_used;
        work_item_id_in_work_group =
            work_item_id_in_compute_unit % kernel->local_size;
        ...
        evg_faults_debug("wg=%d wf=%d wi=%d lo_reg=%d ", work_group->id,
                         work_item->wavefront->id, work_item->id, lo_reg);
      ...
      case evg_fault_mem: {
        ...
        /* Get work-group */
        work_group_id_in_compute_unit =
            fault->byte / evg_gpu->ndrange->local_mem_top;
        if (work_group_id_in_compute_unit >=
            evg_gpu_max_work_groups_per_compute_unit) {
          evg_faults_debug("effect=\"mem_idle\"");
          goto end_loop;
        ...
        /* Inject fault */
        evg_faults_debug("effect=\"error\" wg=%d ", work_group->id);
