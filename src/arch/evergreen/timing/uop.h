...
struct evg_work_item_uop_t {
  ...
  enum mod_access_kind_t
      local_mem_access_kind[EVG_MAX_LOCAL_MEM_ACCESSES_PER_INST];
 ...
 * This is the structure passed from stage to stage in the compute unit
 * pipeline. */
struct evg_uop_t {
  /* Fields */
  long long id;
  ...
  unsigned int
      no_clause_trigger : 1; /* Instruction does not trigger secondary clause */
