...
char *evg_gpu_config_help =
    ...
    "the parameters of the GPU model for a detailed (architectural) GPU "
    "configuration.\n"
    "This file is passed to Multi2Sim with the '--evg-config <file>' option, "
    "and\n"
    ...
    "      Number of compute units in the GPU. A compute unit runs one or "
    "more\n"
    "      work-groups at a time.\n"
    "  NumStreamCores = <num> (Default = 16)\n"
    "      Number of stream cores in the ALU engine of a compute unit. Each "
    "work-item\n"
    "      is mapped to a stream core when a VLIW bundle is executed. Stream "
    "cores are\n"
    "      time-multiplexed to cover all work-items in a wavefront.\n"
    "  NumRegisters = <num> (Default = 16K)\n"
    "      Number of registers in a compute unit. These registers are shared "
    "among all\n"
    "      work-items running in a compute unit. This is one of the factors "
    "limiting the\n"
    ...
    "      Minimum amount of registers allocated as a chunk for each wavefront "
    "or\n"
    "      work-group, depending on the granularity. These parameters have an "
    "impact\n"
    "      in the allocation of work-groups to compute units.\n"
    "  WavefrontSize = <size> (Default = 64)\n"
    "      Number of work-items in a wavefront, executing AMD Evergreen "
    "instructions in\n"
    "      a SIMD fashion.\n"
    "  MaxWorkGroupsPerComputeUnit = <num> (Default = 8)\n"
    "  MaxWavefrontsPerComputeUnit = <num> (Default = 32)\n"
    "      Maximum number of work-groups and wavefronts allocated at a time in "
    "a compute\n"
    "      unit. These are some of the factors limiting the number of "
    "work-groups mapped\n"
    "      to a compute unit.\n"
    ...
    "\n"
    "Section '[ LocalMemory ]': defines the parameters of the local memory "
    "associated to\n"
    "each compute unit.\n"
    "\n"
    "  Size = <bytes> (Default = 32 KB)\n"
    "      Local memory capacity per compute unit. This value must be equal or "
    "larger\n"
    "      than BlockSize * Banks. This is one of the factors limiting the "
    "number of\n"
    "      work-groups mapped to a compute unit.\n"
    "  AllocSize = <bytes> (Default = 1 KB)\n"
    "      Minimum amount of local memory allocated at a time for each "
    "work-group.\n"
    "      This parameter impact on the allocation of work-groups to compute "
    "units.\n"
    "  BlockSize = <bytes> (Default = 256)\n"
    "      Access block size, used for access coalescing purposes among "
    "work-items.\n"
    "  Latency = <num_cycles> (Default = 2)\n"
    "      Hit latency in number of cycles.\n"
    "  Ports = <num> (Default = 4)\n"
    "\n"
    "Section '[ CFEngine ]': parameters for the CF Engine of the Compute "
    "Units.\n"
    "\n"
    "  InstructionMemoryLatency = <cycles> (Default = 2)\n"
    "      Latency of an access to the instruction memory in number of "
    "cycles.\n"
    "\n"
    "Section '[ ALUEngine ]': parameters for the ALU Engine of the Compute "
    "Units.\n"
    "\n"
    "  InstructionMemoryLatency = <cycles> (Default = 2)\n"
    "      Latency of an access to the instruction memory in number of "
    ...
    "      Latency of each processing element (x, y, z, w, t) of a Stream "
    "Core\n"
    "      in number of cycles. This is the time between an instruction is "
    "issued\n"
    "      to a Stream Core and the result of the operation is available.\n"
    "\n"
    "Section '[ TEXEngine ]': parameters for the TEX Engine of the Compute "
    "Units.\n"
    "\n"
    "  InstructionMemoryLatency = <cycles> (Default = 2)\n"
    "      Latency of an access to the instruction memory in number of "
    ...
    "      Size of the load queue in number of uops. This size is equal to "
    "the\n"
...
static void evg_config_dump(FILE *f) {
  ...
  fprintf(f, "RegisterAllocGranularity = %s\n",
          str_map_value(&evg_gpu_register_alloc_granularity_map,
...
static void evg_gpu_map_ndrange(struct evg_ndrange_t *ndrange) {
  ...
  evg_gpu->work_groups_per_compute_unit =
      evg_calc_get_work_groups_per_compute_unit(
          ndrange->kernel->local_size,
          ndrange->kernel->bin_file->enc_dict_entry_evergreen->num_gpr_used,
          ndrange->local_mem_top);
  if (!evg_gpu->work_groups_per_compute_unit)
    fatal(
        "work-group resources cannot be allocated to a compute unit.\n"
        "\tA compute unit in the GPU has a limit in number of wavefronts, "
        "number\n"
  ...
  /* Derived from this, calculate limit of wavefronts and work-items per compute
   * unit. */
  evg_gpu->wavefronts_per_compute_unit = evg_gpu->work_groups_per_compute_unit *
                                         ndrange->wavefronts_per_work_group;
  evg_gpu->work_items_per_compute_unit =
      evg_gpu->wavefronts_per_compute_unit * evg_emu_wavefront_size;
  assert(evg_gpu->work_groups_per_compute_unit <=
         evg_gpu_max_work_groups_per_compute_unit);
  assert(evg_gpu->wavefronts_per_compute_unit <=
         evg_gpu_max_wavefronts_per_compute_unit);
...
void evg_gpu_read_config(void) {
  ...
  /* Frequency */
  evg_gpu_frequency =
      config_read_int(gpu_config, section, "Frequency", evg_gpu_frequency);
  if (!IN_RANGE(evg_gpu_frequency, 1, ESIM_MAX_FREQUENCY))
    fatal("%s: invalid value for 'Frequency'.\n%s", evg_gpu_config_file_name,
          err_note);

  evg_gpu_num_compute_units = config_read_int(
      gpu_config, section, "NumComputeUnits", evg_gpu_num_compute_units);
  evg_gpu_num_stream_cores = config_read_int(
      gpu_config, section, "NumStreamCores", evg_gpu_num_stream_cores);
  evg_gpu_num_registers = config_read_int(gpu_config, section, "NumRegisters",
                                          evg_gpu_num_registers);
  evg_gpu_register_alloc_size = config_read_int(
      gpu_config, section, "RegisterAllocSize", evg_gpu_register_alloc_size);
  gpu_register_alloc_granularity_str = config_read_string(
      gpu_config, section, "RegisterAllocGranularity", "WorkGroup");
  evg_emu_wavefront_size = config_read_int(gpu_config, section, "WavefrontSize",
                                           evg_emu_wavefront_size);
  evg_gpu_max_work_groups_per_compute_unit =
      config_read_int(gpu_config, section, "MaxWorkGroupsPerComputeUnit",
                      evg_gpu_max_work_groups_per_compute_unit);
  evg_gpu_max_wavefronts_per_compute_unit =
      config_read_int(gpu_config, section, "MaxWavefrontsPerComputeUnit",
                      evg_gpu_max_wavefronts_per_compute_unit);
  gpu_sched_policy_str =
      config_read_string(gpu_config, section, "SchedulingPolicy", "RoundRobin");
  ...
  evg_gpu_register_alloc_granularity =
      str_map_string_case(&evg_gpu_register_alloc_granularity_map,
  ...
  evg_gpu_sched_policy =
      str_map_string_case(&evg_gpu_sched_policy_map, gpu_sched_policy_str);
  ...
  section = "LocalMemory";
  evg_gpu_local_mem_size =
      config_read_int(gpu_config, section, "Size", evg_gpu_local_mem_size);
  evg_gpu_local_mem_alloc_size = config_read_int(
      gpu_config, section, "AllocSize", evg_gpu_local_mem_alloc_size);
  evg_gpu_local_mem_block_size = config_read_int(
      gpu_config, section, "BlockSize", evg_gpu_local_mem_block_size);
  evg_gpu_local_mem_latency = config_read_int(gpu_config, section, "Latency",
                                              evg_gpu_local_mem_latency);
  evg_gpu_local_mem_num_ports = config_read_int(gpu_config, section, "Ports",
                                                evg_gpu_local_mem_num_ports);
  if ((evg_gpu_local_mem_size & (evg_gpu_local_mem_size - 1)) ||
      evg_gpu_local_mem_size < 4)
  ...
  if (evg_gpu_local_mem_size % evg_gpu_local_mem_alloc_size)
    fatal("%s: %s->Size must be a multiple of %s->AllocSize.\n%s",
          evg_gpu_config_file_name, section, section, err_note);
  if ((evg_gpu_local_mem_block_size & (evg_gpu_local_mem_block_size - 1)) ||
      evg_gpu_local_mem_block_size < 4)
  ...
  if (evg_gpu_local_mem_size < evg_gpu_local_mem_block_size)
    fatal("%s: %s->Size cannot be smaller than %s->BlockSize * %s->Banks.\n%s",
          evg_gpu_config_file_name, section, section, section, err_note);

  /* CF Engine */
  section = "CFEngine";
  evg_gpu_cf_engine_inst_mem_latency =
      config_read_int(gpu_config, section, "InstructionMemoryLatency",
                      evg_gpu_cf_engine_inst_mem_latency);
  ...
  evg_gpu_alu_engine_inst_mem_latency =
      config_read_int(gpu_config, section, "InstructionMemoryLatency",
                      evg_gpu_alu_engine_inst_mem_latency);
  evg_gpu_alu_engine_fetch_queue_size =
      config_read_int(gpu_config, section, "FetchQueueSize",
                      evg_gpu_alu_engine_fetch_queue_size);
  evg_gpu_alu_engine_pe_latency =
      config_read_int(gpu_config, section, "ProcessingElementLatency",
                      evg_gpu_alu_engine_pe_latency);
  ...
  if (evg_gpu_alu_engine_fetch_queue_size < 56)
    fatal(
        "%s: the minimum value for %s->FetchQueueSize is 56.\n"
        "This is the maximum size of one VLIW bundle, including 5 ALU "
        "instructions\n"
  ...
  evg_gpu_tex_engine_fetch_queue_size =
      config_read_int(gpu_config, section, "FetchQueueSize",
                      evg_gpu_tex_engine_fetch_queue_size);
  evg_gpu_tex_engine_load_queue_size = config_read_int(
      gpu_config, section, "LoadQueueSize", evg_gpu_tex_engine_load_queue_size);
  if (evg_gpu_tex_engine_inst_mem_latency < 1)
    fatal("%s: invalid value for %s.InstructionMemoryLatency.\n%s",
          evg_gpu_config_file_name, section, err_note);
  if (evg_gpu_tex_engine_fetch_queue_size < 16)
    fatal(
        "%s: the minimum value for %s.FetchQueueSize is 16.\n"
        "This size corresponds to the 4 words comprising a TEX Evergreen "
        "instruction.\n%s",
        evg_gpu_config_file_name, section, err_note);
...
void evg_gpu_init(void) {
  ...
  if (evg_gpu_report_file_name[0] &&
      !file_can_open_for_write(evg_gpu_report_file_name))
    fatal("%s: cannot open GPU pipeline report file", evg_gpu_report_file_name);
...
void evg_gpu_done(void) {
  ...
  /* Spatial report */
  if (evg_spatial_report_active) evg_cu_spatial_report_done();
...
void evg_gpu_dump_report(void) {
  ...
  inst_per_cycle =
      asTiming(evg_gpu)->cycle
          ? (double)asEmu(evg_emu)->instructions / asTiming(evg_gpu)->cycle
          : 0.0;
  fprintf(f, "[ Device ]\n\n");
  ...
  EVG_GPU_FOREACH_COMPUTE_UNIT(compute_unit_id) {
    ...
    cf_inst_per_cycle =
        compute_unit->cycle
            ? (double)compute_unit->cf_engine.inst_count / compute_unit->cycle
            : 0.0;
    alu_inst_per_cycle = compute_unit->alu_engine.cycle
                             ? (double)compute_unit->alu_engine.inst_count /
                                   compute_unit->alu_engine.cycle
                             : 0.0;
    tex_inst_per_cycle = compute_unit->tex_engine.cycle
                             ? (double)compute_unit->tex_engine.inst_count /
...
int EvgGpuRun(Timing *self) {
  ...
  if (!evg_emu->ndrange_list_count) return FALSE;
  ...
  while ((ndrange = evg_emu->pending_ndrange_list_head)) {
    ...
    if (gpu->ndrange)
      fatal(
          "%s: Evergreen GPU timing simulation not supported for multiple "
          "ND-Ranges",
          __FUNCTION__);
    ...
    evg_trace(
        ...
        "wg_first=%d "
        "wg_count=%d\n",
        ndrange->id, ndrange->work_group_id_first, ndrange->work_group_count);
