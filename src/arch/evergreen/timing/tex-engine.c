...
int evg_gpu_tex_engine_load_queue_size =
    8; /* Maximum number of in-flight global memory reads */
...
static void evg_tex_engine_fetch(struct evg_compute_unit_t *compute_unit) {
  ...
  uop->inst_mem_ready =
      asTiming(evg_gpu)->cycle + evg_gpu_tex_engine_inst_mem_latency;
  ...
  if (evg_spatial_report_active) evg_tex_report_new_inst(compute_unit);
  ...
  if (evg_tracing()) {
    ...
    evg_trace(
        "evg.new_inst id=%lld cu=%d wg=%d wf=%d cat=\"tex\" stg=\"tex-fe\" "
        "asm=\"%s\"\n",
...
static void evg_tex_engine_decode(struct evg_compute_unit_t *compute_unit) {
  ...
  if (compute_unit->tex_engine.inst_buffer) return;
...
static void evg_tex_engine_read(struct evg_compute_unit_t *compute_unit) {
  ...
  if (linked_list_count(compute_unit->tex_engine.load_queue) >=
  ...
  if (uop->global_mem_read) {
    ...
    if (evg_spatial_report_active)
      evg_tex_report_global_mem_inflight(compute_unit,
...
static void evg_tex_engine_write(struct evg_compute_unit_t *compute_unit) {
  ...
  if (uop->global_mem_witness) return;
