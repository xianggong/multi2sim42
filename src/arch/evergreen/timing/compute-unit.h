...
struct evg_compute_unit_t {
  ...
  int id;
  long long
      gpu_uop_id_counter; /* Counter to assign 'id_in_compute_unit' to uops */
  ...
  struct {
    /* Buffers */
    struct evg_uop_t **
        fetch_buffer; /* Array of uops (MaxWavefrontsPerComputeUnit elements) */
    struct evg_uop_t **
        inst_buffer; /* Array of uops (MaxWavefrontsPerComputeUnit elements) */
  ...
  struct {
    /* CF uop queues.
     * The CF engine enqueues a CF uop that triggers an ALU clause into
     'pending_queue'.
     * When the secondary clause finishes fetching, the CF uop is moved to
     * 'finished_queue'. When secondary clause exists pipeline, it is extracted
     from
     " 'finished_queue'. */
