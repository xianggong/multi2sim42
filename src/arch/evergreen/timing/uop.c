...
static void evg_uop_add_src_idep(struct evg_uop_t *uop, struct evg_inst_t *inst,
                                 int src_idx) {
...
void evg_uop_init() {
  /* GPU uop repository.
   * The size assigned for each 'evg_uop_t' is equals to the baseline structure
   * size plus the
   * size of a 'evg_work_item_uop_t' element for each work-item in the
   * wavefront. */
  gpu_uop_repos = repos_create(
      sizeof(struct evg_uop_t) +
          sizeof(struct evg_work_item_uop_t) * evg_emu_wavefront_size,
      "gpu_uop_repos");
}

void evg_uop_done() { repos_free(gpu_uop_repos); }
...
struct evg_uop_t *evg_uop_create_from_alu_group(
...
void evg_uop_dump_dep_list(char *buf, int size, int *dep_list, int dep_count) {
  static struct str_map_t gpu_uop_dep_map = {4,
...
void evg_uop_debug_active_mask(struct evg_uop_t *uop) {
  ...
  if (uop->active_mask_pop) {
    evg_stack_debug(
        "stack clk=%lld cu=%d stack=%d wf=%d a=\"pop\" cnt=%d top=%d mask=\"",
        asTiming(evg_gpu)->cycle, uop->compute_unit->id,
        wavefront->id_in_compute_unit, wavefront->id, uop->active_mask_pop,
  ...
  if (uop->active_mask_push) {
    evg_stack_debug(
        "stack clk=%lld cu=%d stack=%d wf=%d a=\"push\" cnt=%d top=%d mask=\"",
        asTiming(evg_gpu)->cycle, uop->compute_unit->id,
        wavefront->id_in_compute_unit, wavefront->id, uop->active_mask_push,
        uop->active_mask_stack_top);
    gpu_uop_dump_active_mask(uop, f);
    evg_stack_debug("\"\n");
  }
