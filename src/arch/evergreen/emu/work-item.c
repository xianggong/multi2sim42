...
void evg_work_item_free(struct evg_work_item_t *work_item) {
  ...
  while (list_count(work_item->lds_oqa)) free(list_dequeue(work_item->lds_oqa));
  while (list_count(work_item->lds_oqb)) free(list_dequeue(work_item->lds_oqb));
...
int evg_work_item_get_active(struct evg_work_item_t *work_item) {
  ...
  assert(work_item->id_in_wavefront >= 0 &&
         work_item->id_in_wavefront < wavefront->work_item_count);
  return bit_map_get(wavefront->active_stack,
                     wavefront->stack_top * wavefront->work_item_count +
                         work_item->id_in_wavefront,
                     1);
}
...
void evg_work_item_update_branch_digest(struct evg_work_item_t *work_item,
  ...
  if (!bit_map_get(wavefront->active_stack,
                   wavefront->stack_top * wavefront->work_item_count +
                       work_item->id_in_wavefront,
                   1))
    return;
