...
void EvgEmuDestroy(EvgEmu *self) {
  /* Free ND-Ranges */
  while (self->ndrange_list_count) evg_ndrange_free(self->ndrange_list_head);
...
int EvgEmuRun(Emu *self) {
  ...
  if (!emu->ndrange_list_count) return FALSE;
  ...
  for (ndrange = emu->running_ndrange_list_head; ndrange;
...
void evg_emu_done() {
  /* GPU report */
  if (evg_emu_report_file) fclose(evg_emu_report_file);
...
void evg_emu_disasm(char *path) {
  ...
  for (i = 0; i < list_count(elf_file->symbol_table); i++) {
    ...
    if (str_prefix(symbol->name, "__OpenCL_") &&
      ...
      amd_bin = evg_bin_file_create(section->buffer.ptr + symbol->value,
                                    symbol->size, kernel_name);

      /* Get kernel name */
      printf("**\n** Disassembly for '__kernel %s'\n**\n\n", kernel_name);
      evg_disasm_buffer(&amd_bin->enc_dict_entry_evergreen->sec_text_buffer,
                        stdout);
      printf("\n\n\n");
...
void evg_emu_opengl_disasm(char *path, int opengl_shader_index) {
  ...
  if (opengl_shader_index > list_count(amd_opengl_bin->shader_list) ||
    ...
    fatal("Shader index out of range! Please choose <index> from 1 ~ %d",
          list_count(amd_opengl_bin->shader_list));
  }

  /* Disaseemble */
  amd_opengl_shader =
      list_get(amd_opengl_bin->shader_list, opengl_shader_index - 1);
