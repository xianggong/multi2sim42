...
static void *search_elf_magic(void *ptr_buffer) {
  ...
  if (!ptr_buffer) fatal("Invalid buffer!");

  while (ptr_buffer) {
    if (!memcmp(ptr_buffer, ELF_magic, sizeof(ELF_magic))) return ptr_buffer;
    ptr_buffer++;
  }
  return NULL;
}

/* Find all ELF magic locations in a buffer & store the location offset in a
 * list */
...
static void elf_file_list_free(struct list_t *elf_file_list) {
  while (list_count(elf_file_list)) free(list_remove_at(elf_file_list, 0));
  list_free(elf_file_list);
}

/* Given a buffer and buffer size, find out whether the buffer content is an
 * external ELF or not.
...
/* Given a buffer and buffer size, create list that contains offsets of external
 * ELF magic bytes in the buffer */
static struct list_t *external_elf_file_list_create(void *ptr_buffer,
                                                    size_t buf_size) {
...
static struct elf_file_t *external_elf_file_create(
    void *ptr_buffer, int buf_size, struct list_t *elf_offset_list,
    int elf_file_index, char *name) {
  ...
  external_elf_file =
      elf_file_create_from_buffer(external_elf_base, external_elf_size, name);
...
static struct elf_file_t *internal_elf_file_create(
    struct elf_file_t *external_elf_file) {
  ...
  for (i = 0; i < list_count(external_elf_file->section_list); i++) {
    ...
    if (!strcmp(external_section->name, "binary")) {
      elf_buffer_seek(&external_elf_file->buffer,
                      external_section->header->sh_offset);
      section_buf = xcalloc(1, external_section->header->sh_size);
      memcpy(section_buf,
             external_elf_file->buffer.ptr + external_elf_file->buffer.pos,
             external_section->header->sh_size);

      internal_elf_offset = (int)(search_elf_magic(section_buf) - section_buf);
      internal_elf_base = section_buf + internal_elf_offset;
      internal_elf_size =
          external_section->header->sh_size - internal_elf_offset;

      internal_elf_file = elf_file_create_from_buffer(
          internal_elf_base, internal_elf_size, external_section->name);
...
/* Set 'isa_buffer' element for a shader object, the shader object must have the
 * 'shader_kind' element set first */
static int amd_opengl_shader_set_isa_buffer(
  ...
  if (!&opengl_shader->isa_buffer) fatal("%s: shader error", __FUNCTION__);

  /* Initialize ISA buffer */
  if (opengl_shader->internal_elf_file) {
    for (i = 0; i < list_count(opengl_shader->internal_elf_file->section_list);
         i++) {
      internal_section =
          list_get(opengl_shader->internal_elf_file->section_list, i);

      /* ISAs in '.text' section */
      if (!strcmp(internal_section->name, ".text")) {
        isa_offset = get_isa_offset(opengl_shader);
        elf_buffer_seek(&opengl_shader->internal_elf_file->buffer,
                        internal_section->header->sh_offset);

        opengl_shader->isa_buffer.ptr =
            opengl_shader->internal_elf_file->buffer.ptr +
            opengl_shader->internal_elf_file->buffer.pos + isa_offset;
        opengl_shader->isa_buffer.size =
            opengl_shader->internal_elf_file->buffer.size - isa_offset;
...
static int amd_opengl_shader_set_shader_kind(
    struct evg_opengl_shader_t *opengl_shader) {
  struct elf_file_t *internal_elf_file;
  struct elf_file_t *external_elf_file;
  ...
  if (internal_elf_file->header->e_flags == 0x1 &&
      external_elf_file->header->e_flags == 0x1) {
    opengl_shader->shader_kind = EVG_OPENGL_SHADER_FRAGMENT;
    return 1;
  } else if (internal_elf_file->header->e_flags == 0x2 &&
             external_elf_file->header->e_flags == 0x2) {
    opengl_shader->shader_kind = EVG_OPENGL_SHADER_VERTEX;
    return 1;
  } else if (internal_elf_file->header->e_flags == 0x3 &&
             external_elf_file->header->e_flags == 0x3) {
    opengl_shader->shader_kind = EVG_OPENGL_SHADER_GEOMETRY;
    return 1;
  } else if (internal_elf_file->header->e_flags == 0x5 &&
             external_elf_file->header->e_flags == 0x5) {
    opengl_shader->shader_kind = EVG_OPENGL_SHADER_CONTROL;
    return 1;
  } else if (internal_elf_file->header->e_flags == 0x6 &&
             external_elf_file->header->e_flags == 0x6) {
...
static struct evg_opengl_shader_t *amd_opengl_shader_create_from_buffer(
    void *ptr, int size, struct list_t *elf_offset_list, int elf_file_index,
    char *name) {
  struct evg_opengl_shader_t *opengl_shader;
  ...
  opengl_shader->external_elf_file = external_elf_file_create(
      ptr, size, elf_offset_list, elf_file_index, name);
  opengl_shader->internal_elf_file =
      internal_elf_file_create(opengl_shader->external_elf_file);
...
/* Create evg_opengl_bin_file_t object based on a file buffer, the corresponding
 * buffer size and the name of the file */
struct evg_opengl_bin_file_t *evg_opengl_bin_file_create(void *ptr, int size,
  ...
  /* Find external/internal ELF files associated with a shader and insert into
   * shader list */
  for (i = 0; i < list_count(elf_file_list); ++i) {
    /* Initialize shader */
    opengl_shader =
        amd_opengl_shader_create_from_buffer(ptr, size, elf_file_list, i, name);
