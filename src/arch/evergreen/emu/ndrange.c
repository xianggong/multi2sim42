...
void evg_ndrange_free(struct evg_ndrange_t *ndrange) {
  ...
  if (ndrange->inst_histogram) free(ndrange->inst_histogram);
...
void evg_ndrange_dump(struct evg_ndrange_t *ndrange, FILE *f) {
  ...
  for (work_item_id = 1; work_item_id <= ndrange->work_item_count;
       work_item_id++) {
    branch_digest = work_item_id < ndrange->work_item_count
                        ? ndrange->work_items[work_item_id]->branch_digest
                        : 0;
    if (work_item_id == ndrange->work_item_count ||
        branch_digest != last_branch_digest) {
...
int evg_ndrange_get_status(struct evg_ndrange_t *ndrange,
                           enum evg_ndrange_status_t status) {
  return (ndrange->status & status) > 0;
}

void evg_ndrange_set_status(struct evg_ndrange_t *ndrange,
                            enum evg_ndrange_status_t status) {
...
void evg_ndrange_clear_status(struct evg_ndrange_t *ndrange,
                              enum evg_ndrange_status_t status) {
...
void evg_ndrange_setup_work_items(struct evg_ndrange_t *ndrange) {
  ...
  ndrange->wavefronts_per_work_group =
      (kernel->local_size + evg_emu_wavefront_size - 1) /
      evg_emu_wavefront_size;
  ndrange->wavefront_count =
      ndrange->wavefronts_per_work_group * ndrange->work_group_count;
    ...
    for (gidy = 0; gidy < kernel->group_count3[1]; gidy++) {
      for (gidx = 0; gidx < kernel->group_count3[0]; gidx++) {
        ...
        work_group->wavefront_id_first =
            gid * ndrange->wavefronts_per_work_group;
        work_group->wavefront_id_last = work_group->wavefront_id_first +
                                        ndrange->wavefronts_per_work_group - 1;
        work_group->wavefront_count = ndrange->wavefronts_per_work_group;
        work_group->wavefronts =
            &ndrange->wavefronts[work_group->wavefront_id_first];
          ...
          for (lidy = 0; lidy < kernel->local_size3[1]; lidy++) {
            for (lidx = 0; lidx < kernel->local_size3[0]; lidx++) {
              ...
              work_item->id_in_wavefront =
                  work_item->id_in_work_group % evg_emu_wavefront_size;
              ...
              bit_map_set(wavefront->active_stack, work_item->id_in_wavefront,
                          1, 1);
  ...
  for (wid = 0; wid < ndrange->wavefront_count; wid++) {
    ...
    wavefront->cf_buf_start =
        kernel->bin_file->enc_dict_entry_evergreen->sec_text_buffer.ptr;
  ...
  evg_isa_debug("wavefronts_per_work_group = %d\n",
                ndrange->wavefronts_per_work_group);
  evg_isa_debug(
      " tid tid2 tid1 tid0   gid gid2 gid1 gid0   "
  ...
  for (tid = 0; tid < ndrange->work_item_count; tid++) {
    ...
    evg_isa_debug("%4d %4d %4d %4d  ", work_item->id_in_work_group,
...
void evg_ndrange_setup_args(struct evg_ndrange_t *ndrange) {
  ...
  for (i = 0; i < list_count(kernel->arg_list); i++) {
    ...
    if (!arg->set)
      fatal(
          "kernel '%s': argument '%s' has not been assigned with "
          "'clKernelSetArg'.",
      ...
      case EVG_OPENCL_KERNEL_ARG_KIND_IMAGE:
        ...
        switch (arg->mem_scope) {
          case EVG_OPENCL_MEM_SCOPE_GLOBAL: {
            ...
            evg_opencl_debug(
                "    arg %d: opencl_mem id 0x%x loaded into CB1[%d],"
                " device_ptr=0x%x\n",
                i, arg->value, cb_index, mem->device_ptr);
          ...
          default:
            fatal("%s: argument in memory scope %d not supported", __FUNCTION__,
                  arg->mem_scope);
        }
        break;
      ...
      case EVG_OPENCL_KERNEL_ARG_KIND_POINTER: {
        switch (arg->mem_scope) {
          ...
          case EVG_OPENCL_MEM_SCOPE_LOCAL: {
            ...
            evg_opencl_debug(
                "    arg %d: %d bytes reserved in local memory at 0x%x\n", i,
