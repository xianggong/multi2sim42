...
static int evg_wavefront_divergence_compare(const void *elem1,
                                            const void *elem2) {
...
static void evg_wavefront_divergence_dump(struct evg_wavefront_t *wavefront,
                                          FILE *f) {
  ...
  for (i = 0; i < list_count(list); i++) {
    ...
    for (j = 0; j < wavefront->work_item_count; j++) {
      int first, last;
      first =
          wavefront->work_items[j]->branch_digest == elem->branch_digest &&
          (j == 0 ||
           wavefront->work_items[j - 1]->branch_digest != elem->branch_digest);
      last =
          wavefront->work_items[j]->branch_digest == elem->branch_digest &&
          (j == wavefront->work_item_count - 1 ||
           wavefront->work_items[j + 1]->branch_digest != elem->branch_digest);
      if (first)
        fprintf(f, " %d", j);
      else if (last)
  ...
  for (i = 0; i < list_count(list); i++) free(list_get(list, i));
  list_free(list);
  hash_table_free(ht);
}
...
struct evg_wavefront_t *evg_wavefront_create() {
  ...
  wavefront->active_stack =
      bit_map_create(EVG_MAX_STACK_SIZE * evg_emu_wavefront_size);
...
void evg_wavefront_dump(struct evg_wavefront_t *wavefront, FILE *f) {
  ...
  for (i = 0; i < 5; i++) fprintf(f, " %lld", wavefront->alu_group_size[i]);
  fprintf(f, "\n");
...
void evg_wavefront_stack_push(struct evg_wavefront_t *wavefront) {
  ...
  bit_map_copy(wavefront->active_stack,
               wavefront->stack_top * wavefront->work_item_count,
               wavefront->active_stack,
               (wavefront->stack_top - 1) * wavefront->work_item_count,
               wavefront->work_item_count);
...
void evg_wavefront_execute(struct evg_wavefront_t *wavefront) {
  ...
  switch (wavefront->clause_kind) {
    case EVG_CLAUSE_CF: {
      ...
      if (debug_status(evg_isa_debug_category)) {
        evg_isa_debug("\n\n");
        evg_inst_dump(inst, inst_num, 0, debug_file(evg_isa_debug_category));
      }
      ...
      if (inst->info->flags & EVG_INST_FLAG_ACT_MASK) {
        EVG_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id) {
          ...
          evg_work_item_update_branch_digest(
              work_item, wavefront->cf_inst_count, inst_num);
        }
      }
    ...
    case EVG_CLAUSE_ALU: {
      ...
      wavefront->clause_buf = evg_inst_decode_alu_group(
          wavefront->clause_buf, wavefront->alu_group_count, alu_group);
