...
struct evg_work_group_t {
  ...
  struct evg_work_item_t **
      work_items; /* Pointer to first work_item in 'kernel->work_items' */
  struct evg_wavefront_t **
      wavefronts; /* Pointer to first wavefront in 'kernel->wavefronts' */
  ...
  int compute_unit_finished_count; /* like 'finished_list_count', but when WF
                                      reaches Complete stage */
...
#define EVG_FOR_EACH_WORK_GROUP_IN_NDRANGE(NDRANGE, WORK_GROUP_ID) \
  for ((WORK_GROUP_ID) = (NDRANGE)->work_group_id_first;           \
       (WORK_GROUP_ID) <= (NDRANGE)->work_group_id_last; (WORK_GROUP_ID)++)
...
int evg_work_group_get_status(struct evg_work_group_t *work_group,
                              enum evg_work_group_status_t status);
void evg_work_group_set_status(struct evg_work_group_t *work_group,
                               enum evg_work_group_status_t status);
void evg_work_group_clear_status(struct evg_work_group_t *work_group,
                                 enum evg_work_group_status_t status);
