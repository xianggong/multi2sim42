...
struct evg_work_item_t {
   ...
   * an instruction updates the active mask of a wavefront, this digest is
   * updated
   * for active work_items by XORing a random number common for the wavefront.
   * At the end, work_items with different 'branch_digest' numbers can be
   * considered
  ...
  int local_mem_access_count; /* Number of local memory access performed by last
                                 instruction */
  ...
  int local_mem_access_type[EVG_MAX_LOCAL_MEM_ACCESSES_PER_INST]; /* 0-none,
                                                                     1-read,
                                                                     2-write */
};
