...
void evg_isa_const_mem_write(int bank, int vector, int elem, void *pvalue) {
  ...
  if (!bank && vector < 9) evg_emu->const_mem_cb0_init[vector * 4 + elem] = 1;
...
void gpu_isa_dest_value_dump(struct evg_inst_t *inst, void *value_ptr,
                             FILE *f) {
...
unsigned int evg_isa_read_gpr(struct evg_work_item_t *work_item, int gpr,
                              int rel, int chan, int im) {
  /* Check arguments */
  ...
  if (rel) fatal("%s: not supported for 'rel' != 0", __FUNCTION__);
  if (im) fatal("%s: not supported for 'im' != 0", __FUNCTION__);
...
float evg_isa_read_gpr_float(struct evg_work_item_t *work_item, int gpr,
                             int rel, int chan, int im) {
  union evg_reg_t reg;
...
void evg_isa_write_gpr(struct evg_work_item_t *work_item, int gpr, int rel,
                       int chan, unsigned int value) {
  /* Check arguments */
  if (!IN_RANGE(chan, 0, 4))
    fatal("%s: invalid value for 'chan'", __FUNCTION__);
  if (!IN_RANGE(gpr, 0, 127))
...
void evg_isa_write_gpr_float(struct evg_work_item_t *work_item, int gpr,
                             int rel, int chan, float value) {
  union evg_reg_t reg;
...
static unsigned int evg_isa_read_op_src_common(
    struct evg_work_item_t *work_item, struct evg_inst_t *inst, int src_idx,
    int *neg_ptr, int *abs_ptr) {
  ...
  if (IN_RANGE(sel, 128, 159)) {
    ...
    evg_isa_const_mem_read(kcache_bank, kcache_addr * 16 + sel - 128, chan,
                           &value);
  ...
  if (IN_RANGE(sel, 160, 191)) {
    ...
    EVG_ISA_ARG_NOT_SUPPORTED_NEQ(kcache_mode, 1);
    EVG_ISA_ARG_NOT_SUPPORTED_RANGE(chan, 0, 3);
    evg_isa_const_mem_read(kcache_bank, kcache_addr * 16 + sel - 160, chan,
                           &value);
    return value;
  }
  ...
  if (sel == 219 || sel == 221) {
    ...
    if (!pvalue) fatal("%s: LDS queue A is empty", __FUNCTION__);
    value = *pvalue;
    if (sel == 219)
  ...
  if (sel == 220 || sel == 222) {
    ...
    if (!pvalue) fatal("%s: LDS queue B is empty", __FUNCTION__);
    value = *pvalue;
    if (sel == 220)
  ...
  fatal("%s: src_idx=%d, not implemented for sel=%d", __FUNCTION__, src_idx,
        sel);
  return 0;
}
...
unsigned int evg_isa_read_op_src_int(struct evg_work_item_t *work_item,
  ...
  /* Get value */
  value = evg_isa_read_op_src_common(work_item, inst, src_idx, &neg, &abs);
...
float evg_isa_read_op_src_float(struct evg_work_item_t *work_item,
  ...
  /* Get value */
  reg.as_uint =
      evg_isa_read_op_src_common(work_item, inst, src_idx, &neg, &abs);

  /* Absolute value and negation */
  if (abs && reg.as_float < 0.0) reg.as_float = -reg.as_float;
  if (neg) reg.as_float = -reg.as_float;
...
struct evg_inst_t *evg_isa_get_alu_inst(struct evg_alu_group_t *alu_group,
  ...
  /* Not a VLIW bundle */
  if (!alu_group) panic("%s: not a VLIW bundle", __FUNCTION__);

  /* Loop for slot 'alu' */
  for (i = 0; i < alu_group->inst_count; i++)
    if (alu_group->inst[i].alu == alu) return &alu_group->inst[i];
...
void evg_isa_enqueue_write_lds(struct evg_work_item_t *work_item,
                               struct evg_inst_t *inst, unsigned int addr,
                               unsigned int value, int value_size) {
  struct evg_isa_write_task_t *wt;

  /* Inactive pixel not enqueued */
  if (!evg_work_item_get_pred(work_item)) return;
...
void evg_isa_enqueue_push_before(struct evg_work_item_t *work_item,
  ...
  if (wavefront->cf_inst.info->inst != EVG_INST_ALU_PUSH_BEFORE) return;
  ...
  for (linked_list_head(task_list); !linked_list_is_end(task_list);) {
    ...
    switch (wt->kind) {
      ...
      case EVG_ISA_WRITE_TASK_WRITE_LDS: {
        ...
        evg_isa_debug("  i%d:LDS[0x%x]<=(%u,%gf) (%d bytes)", work_item->id,
                      wt->lds_addr, lds_value.as_uint, lds_value.as_float,
                      (int)wt->lds_value_size);
        break;
      }
  ...
  for (linked_list_head(task_list); !linked_list_is_end(task_list);) {
    ...
    switch (wt->kind) {
      case EVG_ISA_WRITE_TASK_PUSH_BEFORE: {
        if (!wavefront->push_before_done) evg_wavefront_stack_push(wavefront);
      ...
      case EVG_ISA_WRITE_TASK_SET_PRED: {
        ...
        assert(inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2);
        if (update_pred) evg_work_item_set_pred(work_item, wt->cond);
        if (update_exec_mask) evg_work_item_set_active(work_item, wt->cond);
