...
enum evg_inst_flag_enum {
  ...
  EVG_INST_FLAG_TRANS_ONLY =
      0x0001, /* Only executable in transcendental unit */
  ...
  EVG_INST_FLAG_ACT_MASK =
      0x0040,                 /* Inst affects the active mask (control flow) */
...
#define DEFINST(_name, _fmt_str, _fmt0, _fmt1, _fmt2, _category, _cf_inst, \
                _flags)                                                    \
  EVG_INST_##_name,
#include "asm.dat"
#undef DEFINST
...
struct evg_inst_t {
  ...
  /* ALU instructions */
  enum evg_alu_enum
      alu; /*  identifier of assigned ALU (ALU_[X,Y,Z,W] or ALU_TRANS) */
