...
static struct evg_inst_info_t *
    evg_inst_info_cf_long[EVG_INST_INFO_CF_LONG_SIZE]; /* for 8-bit cf_inst */
static struct evg_inst_info_t *
    evg_inst_info_cf_short[EVG_INST_INFO_CF_SHORT_SIZE]; /* for 4-bit cf_inst */
static struct evg_inst_info_t *
    evg_inst_info_alu_long[EVG_INST_INFO_ALU_LONG_SIZE]; /* for ALU_OP2 */
static struct evg_inst_info_t *
    evg_inst_info_alu_short[EVG_INST_INFO_ALU_SHORT_SIZE]; /* for ALU_OP3 */
static struct evg_inst_info_t *
    evg_inst_info_tex[EVG_INST_INFO_TEX_SIZE]; /* For tex instructions */
...
#define DEFINST(_name, _fmt_str, _fmt0, _fmt1, _fmt2, _category, _opcode, \
  ...
  info->opcode = _opcode;                                                 \
  info->flags = _flags;                                                   \
  info->size = (EVG_FMT_##_fmt0 ? 1 : 0) + (EVG_FMT_##_fmt1 ? 1 : 0) +    \
               (EVG_FMT_##_fmt2 ? 1 : 0);
#include "asm.dat"
#undef DEFINST
  ...
  for (i = 1; i < EVG_INST_COUNT; i++) {
    ...
    if (info->fmt[0] == EVG_FMT_TEX_WORD0 ||
        info->fmt[0] == EVG_FMT_VTX_WORD0 ||
        info->fmt[0] == EVG_FMT_MEM_RD_WORD0) {
 ...
 * The ELF buffer representing an entire kernel '.text' section can be used
 * here. */
  ...
  while (cf_buf) {
    ...
    if (cf_inst.info->fmt[0] == EVG_FMT_CF_ALU_WORD0) {
      ...
      while (alu_buf < alu_buf_end) {
        alu_buf =
            evg_inst_decode_alu_group(alu_buf, sec_inst_count, &alu_group);
    ...
    if (cf_inst.info->flags & EVG_INST_FLAG_INC_LOOP_IDX) loop_idx++;
  }
}
...
void *evg_inst_decode_alu_group(void *buf, int group_id,
                                struct evg_alu_group_t *group) {
  ...
  do {
     ...
     * There is at least one literal slot if there is any destination element
     * CHAN_
     * There are two literal slots if there is any destination element CHAN_Z or
     * CHAN_W. */
    ...
    /* Initially, set ALU as indicated by the destination operand. For both OP2
     * and OP3 formats, field
     * 'dest_chan' is located at the same bit position. */
    dest_chan = inst->words[1].alu_word1_op2.dst_chan;
    alu = inst->info->flags & EVG_INST_FLAG_TRANS_ONLY ? EVG_ALU_TRANS
                                                       : dest_chan;
    if (alu_busy[alu]) alu = EVG_ALU_TRANS;
    if (alu_busy[alu])
      fatal("group_id=%d, inst_id=%d: cannot allocate ALU", group_id,
            group->inst_count);
    alu_busy[alu] = 1;
    inst->alu = alu;
...
void *evg_inst_decode_tc(void *buf, struct evg_inst_t *inst) {
  ...
  inst->info = IN_RANGE(tex_inst, 0, EVG_INST_INFO_TEX_SIZE - 1)
                   ? evg_inst_info_tex[tex_inst]
                   : NULL;
  if (!inst->info) fatal("unknown TEX instruction; tex_inst=%d", tex_inst);
...
struct str_map_t evg_alu_map = {5,
                                {{"x", EVG_ALU_X},
                                 {"y", EVG_ALU_Y},
                                 {"z", EVG_ALU_Z},
                                 {"w", EVG_ALU_W},
...
struct str_map_t evg_bank_swizzle_map = {6,
                                         {{"", 0},
                                          {"VEC_021", 1},
                                          {"VEC_120", 2},
                                          {"VEC_102", 3},
...
struct str_map_t evg_cf_cond_map = {
    4, {{"ACTIVE", 0}, {"FALSE", 1}, {"BOOL", 2}, {"NOT_BOOL", 3}}};

struct str_map_t evg_src_sel_map = {31,
                                    {
...
struct str_map_t evg_dst_sel_map = {
    7, {{"x", 0}, {"y", 1}, {"z", 2}, {"w", 3}, {"0", 4}, {"1", 5}, {"_", 7}}};

struct str_map_t export_type_map = {
    4, {{"PIX", 0}, {"POS", 1}, {"PARAM", 2}, {"IND_ACK", 3}}};

static int evg_inst_is_token(char *fmt_str, char *token_str,
                             int *token_str_len) {
...
static void evg_inst_dump_kcache_buf(int kcache_idx, int kcache_bank,
...
static void evg_inst_dump_gpr_buf(int gpr, int rel, int chan, int im,
  ...
  if (rel) {
    if (rel && IN_RANGE(im, 0, 3))
      str_printf(buf_ptr, size_ptr, "%s[A0.%s]", gpr_str,
                 str_map_value(&evg_alu_map, EVG_ALU_X + im));
    else if (im == 4)
  ...
  if (chan >= 0)
    str_printf(buf_ptr, size_ptr, ".%s",
               str_map_value(&evg_alu_map, EVG_ALU_X + chan));
}

static void evg_inst_dump_op_dest_buf(struct evg_inst_t *inst, char **buf_ptr,
                                      int *size_ptr) {
  int gpr, rel, chan, index_mode;

  /* Fields 'dst_gpr', 'dst_rel', and 'dst_chan' are at the same bit positions
   * in both
  ...
  if (inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2 &&
...
/* Copy an instruction. No special handling of fields is needed here, just copy
 * the whole
 * structure. */
...
/* Copy an ALU group. Fields 'alu_group' in comprising 'alu_inst' elements need
 * to be updated
 * to point to the destination ALU group. */
void evg_alu_group_copy(struct evg_alu_group_t *dest,
                        struct evg_alu_group_t *src) {
  int i;
  memcpy(dest, src, sizeof(struct evg_alu_group_t));
  for (i = 0; i < src->inst_count; i++) dest->inst[i].alu_group = dest;
}

/* Get parameters for a source register in an ALU instruction */
void evg_inst_get_op_src(struct evg_inst_t *inst, int src_idx, int *sel,
                         int *rel, int *chan, int *neg, int *abs) {
  /* Valid formats */
  ...
  switch (src_idx) {
    case 0:
      ...
      *chan = inst->words[0].alu_word0.src0_chan;
       ...
       */
      *neg = inst->info->fmt[0] == EVG_FMT_ALU_WORD0
                 ? inst->words[0].alu_word0.src0_neg
                 : 0;
       ...
       */
      *abs = inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2
                 ? inst->words[1].alu_word1_op2.src0_abs
                 : 0;
      break;
    ...
    case 1:
      ...
      *chan = inst->words[0].alu_word0.src1_chan;
       ...
       */
      *neg = inst->info->fmt[0] == EVG_FMT_ALU_WORD0
                 ? inst->words[0].alu_word0.src1_neg
                 : 0;
       ...
       */
      *abs = inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2
                 ? inst->words[1].alu_word1_op2.src1_abs
                 : 0;
      break;
    ...
    case 2:
      ...
      *chan = inst->words[1].alu_word1_op3.src2_chan;
       ...
       */
      *neg = inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP3
                 ? inst->words[1].alu_word1_op3.src2_neg
                 : 0;
...
void amd_inst_dump_op_src_buf(struct evg_inst_t *inst, int src_idx,
                              char **buf_ptr, int *size_ptr) {
  ...
  if (neg) str_printf(buf_ptr, size_ptr, "-");
  if (abs) str_printf(buf_ptr, size_ptr, "|");
  ...
  if (sel == 253) {
    ...
    str_printf(buf_ptr, size_ptr, "(0x%08x, %.9ef).%s",
               inst->alu_group->literal[chan].as_uint,
               inst->alu_group->literal[chan].as_float,
               str_map_value(&evg_alu_map, EVG_ALU_X + chan));
    goto end;
  }
...
/* Dump an instruction. Use -1 for 'count', 'loop_idx', or 'alu', if the
 * corresponding field
 * is not relevant in the instruction dump. */
void evg_inst_slot_dump_buf(struct evg_inst_t *inst, int count, int loop_idx,
                            int slot, char *buf, int size) {
  ...
  while (*fmt_str) {
    ...
    /* ALU modifiers */
      ...
      str_printf(buf_ptr, size_ptr, "%s",
                 str_map_value(&evg_bank_swizzle_map,
                               inst->words[1].alu_word1_op2.bank_swizzle));
      ...
      if (inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2 &&
          inst->words[1].alu_word1_op2.update_exec_mask)
        str_printf(buf_ptr, size_ptr, " UPDATE_EXEC_MASK");

      /* EVG_ALU_WORD1_OP2 - 'update_pred' field */
      if (inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2 &&
          inst->words[1].alu_word1_op2.update_pred)
      ...
      switch (inst->words[1].alu_word1_op2.omod) {
        case 0:
          ...
          break;
        case 1:
          str_printf(buf_ptr, size_ptr, "*2");
          break;
        case 2:
          str_printf(buf_ptr, size_ptr, "*4");
          break;
        case 3:
          str_printf(buf_ptr, size_ptr, "/2");
          break;
      ...
      assert(inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP2 ||
             inst->info->fmt[1] == EVG_FMT_ALU_WORD1_OP3);
      if (inst->words[1].alu_word1_op2.clamp ||
          inst->words[1].alu_word1_op3.clamp)
      ...
      if (pop_count) str_printf(buf_ptr, size_ptr, "POP_CNT(%d)", pop_count);
      ...
      assert(inst->info->fmt[1] == EVG_FMT_CF_WORD1 ||
             inst->info->fmt[1] == EVG_FMT_CF_ALU_WORD1);
      whole_quad_mode = inst->words[1].cf_word1.whole_quad_mode;
      if (whole_quad_mode) str_printf(buf_ptr, size_ptr, "WHOLE_QUAD");
      ...
      assert(inst->info->fmt[1] == EVG_FMT_CF_WORD1 ||
             ...
             inst->info->fmt[1] == EVG_FMT_CF_ALLOC_EXPORT_WORD1_SWIZ);
      valid_pixel_mode = inst->words[1].cf_word1.valid_pixel_mode;
      if (valid_pixel_mode) str_printf(buf_ptr, size_ptr, "VPM");
      ...
      str_printf(buf_ptr, size_ptr, "%d",
                 inst->words[1].cf_alu_word1.count + 1);
      ...
      if (mark) str_printf(buf_ptr, size_ptr, "MARK");
      ...
      evg_inst_dump_kcache_buf(0, inst->words[0].cf_alu_word0.kcache_bank0,
                               inst->words[0].cf_alu_word0.kcache_mode0,
                               inst->words[1].cf_alu_word1.kcache_addr0,
                               buf_ptr, size_ptr);
      evg_inst_dump_kcache_buf(1, inst->words[0].cf_alu_word0.kcache_bank1,
      ...
      str_printf(buf_ptr, size_ptr, "%s",
                 str_map_value(&export_type_map,
                               inst->words[0].cf_alloc_export_word0.type));
      ...
      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0);
      assert(inst->info->fmt[1] == EVG_FMT_CF_ALLOC_EXPORT_WORD1_SWIZ ||
             inst->info->fmt[1] == EVG_FMT_CF_ALLOC_EXPORT_WORD1_BUF);
      ...
      if (inst->info->fmt[1] == EVG_FMT_CF_ALLOC_EXPORT_WORD1_SWIZ) {
        ...
        if (count != 4) {
          str_printf(buf_ptr, size_ptr, "R%d.%c%c%c%c", rw_gpr,
                     src_sel_chars[0], src_sel_chars[1], src_sel_chars[2],
                     src_sel_chars[3]);
        } else {
      ...
      str_printf(
          buf_ptr, size_ptr, "%s",
          str_map_value(&evg_rat_inst_map,
                        inst->words[0].cf_alloc_export_word0_rat.rat_inst));
      ...
      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0_RAT);
      str_printf(buf_ptr, size_ptr, "%d",
                 inst->words[0].cf_alloc_export_word0_rat.rat_id);
    } else if (evg_inst_is_token(fmt_str, "rat_index_mode", &len)) {
      int rim;
      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0_RAT);
      rim = inst->words[0].cf_alloc_export_word0_rat.rat_index_mode;
      if (rim) str_printf(buf_ptr, size_ptr, "+idx%d", rim - 1);
    } else if (evg_inst_is_token(fmt_str, "rat_index_gpr", &len)) {
      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0_RAT);
      evg_inst_dump_gpr_buf(inst->words[0].cf_alloc_export_word0_rat.index_gpr,
      ...
      if (comp_mask != 0xf)
        str_printf(buf_ptr, size_ptr, ".%s%s%s%s", comp_mask & 1 ? "x" : "_",
                   comp_mask & 2 ? "y" : "_", comp_mask & 4 ? "z" : "_",
                   comp_mask & 8 ? "w" : "_");
    } else if (evg_inst_is_token(fmt_str, "rat_rw_gpr", &len)) {
      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0_RAT);
      evg_inst_dump_gpr_buf(inst->words[0].cf_alloc_export_word0_rat.rw_gpr, 0,
                            -1, 0, buf_ptr, size_ptr);
    } else if (evg_inst_is_token(fmt_str, "array_size", &len)) {
      int elem_size;
      int array_size;

      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0 ||
             inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0_RAT);
      assert(inst->info->fmt[1] == EVG_FMT_CF_ALLOC_EXPORT_WORD1_BUF);
      ...
      if (elem_size) str_printf(buf_ptr, size_ptr, ",%d", elem_size + 1);
      str_printf(buf_ptr, size_ptr, ")");
      ...
      assert(inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0 ||
             inst->info->fmt[0] == EVG_FMT_CF_ALLOC_EXPORT_WORD0_RAT);
      elem_size = inst->words[0].cf_alloc_export_word0.elem_size;
      if (elem_size) str_printf(buf_ptr, size_ptr, "ELEM_SIZE(%d)", elem_size);
      ...
      if (fetch_type)
        str_printf(buf_ptr, size_ptr, "%sFETCH_TYPE(%s)",
                   evg_inst_token_prefix(loop_idx, &nl),
      ...
      if (fetch_whole_quad) str_printf(buf_ptr, size_ptr, "WHOLE_QUAD");
      ...
      if (!use_const_fields && srf_mode)
        str_printf(buf_ptr, size_ptr, "%sSRF_MODE(%s)",
                   evg_inst_token_prefix(loop_idx, &nl),
      ...
      if (offset) str_printf(buf_ptr, size_ptr, "OFFSET(%d)", offset);
      ...
      if (!use_const_fields && endian_swap)
        str_printf(buf_ptr, size_ptr, "%sENDIAN_SWAP(%s)",
                   evg_inst_token_prefix(loop_idx, &nl),
      ...
      if (cbns)
        str_printf(buf_ptr, size_ptr, "%sCONST_BUF_NO_STRIDE",
                   evg_inst_token_prefix(loop_idx, &nl));
      ...
      str_printf(buf_ptr, size_ptr, "%s",
                 str_map_value(&evg_fmt_lds_op_map,
      ...
      /* if count == 4, only print R/T instead of R.xyzw */
      if (count != 4) {
        str_printf(buf_ptr, size_ptr, "R%d.%c%c%c%c", src_gpr, src_sel_chars[0],
                   src_sel_chars[1], src_sel_chars[2], src_sel_chars[3]);
      } else {
      ...
      /* if count == 4, only print R/T instead of R.xyzw */
      if (count != 4) {
        str_printf(buf_ptr, size_ptr, "R%d.%c%c%c%c", dst_gpr, dst_sel_chars[0],
                   dst_sel_chars[1], dst_sel_chars[2], dst_sel_chars[3]);
      } else {
      ...
      assert(inst->info->fmt[0] == EVG_FMT_MEM_RD_WORD0 ||
             inst->info->fmt[0] == EVG_FMT_MEM_GDS_WORD0);
...
void evg_inst_dump_buf(struct evg_inst_t *inst, int count, int loop_idx,
                       char *buf, int size) {
  evg_inst_slot_dump_buf(inst, count, loop_idx, -1, buf, size);
}
...
void evg_inst_slot_dump(struct evg_inst_t *inst, int count, int loop_idx,
                        int slot, FILE *f) {
  char buf[MAX_STRING_SIZE];
...
void evg_inst_dump(struct evg_inst_t *inst, int count, int loop_idx, FILE *f) {
  evg_inst_slot_dump(inst, count, loop_idx, -1, f);
}

void evg_inst_dump_debug(struct evg_inst_t *inst, int count, int loop_idx,
                         FILE *f) {
  ...
  if (count >= 0) fprintf(f, "cnt=%d ", count);
  if (loop_idx >= 0) fprintf(f, "l=%d ", loop_idx);
...
void evg_alu_group_dump_buf(struct evg_alu_group_t *alu_group, char *buf,
                            int size) {
  struct evg_inst_t *inst;
...
void evg_alu_group_dump_debug(struct evg_alu_group_t *alu_group, int count,
                              int loop_idx, FILE *f) {
  struct evg_inst_t *inst;
  ...
  for (i = 0; i < alu_group->inst_count; i++) {
    ...
    fprintf(f, "%sinst.%s=\"%s\"", spc, str_map_value(&evg_alu_map, inst->alu),
            no_spc_buf);
    spc = " ";
  }
}
