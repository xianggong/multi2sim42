...
static struct str_map_t evg_fmt_rel_map = {2,
                                           {{"Absolute", 0}, {"Relative", 1}}};

static struct str_map_t evg_fmt_chan_map = {
    4, {{"CHAN_X", 0}, {"CHAN_Y", 1}, {"CHAN_Z", 2}, {"CHAN_W", 3}}};

static struct str_map_t evg_fmt_sel_map = {7,
                                           {{"SEL_X", 0},
                                            {"SEL_Y", 1},
                                            {"SEL_Z", 2},
                                            {"SEL_W", 3},
...
static struct str_map_t evg_fmt_cf_index_mode_map = {4,
                                                     {{"CF_INDEX_NONE", 0},
                                                      {"CF_INDEX_0", 1},
                                                      {"CF_INDEX_1", 2},
                                                      {"CF_INVALID", 3}}};
...
struct str_map_t evg_inst_category_map = {7,
                                          {{"CF", EVG_INST_CAT_CF},
...
struct str_map_t evg_fmt_cf_word0_jts_map = {6,
...
void evg_fmt_cf_word0_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  jump_table_sel = %d (%s)\n", fmt->jump_table_sel,
          str_map_value(&evg_fmt_cf_word0_jts_map, fmt->jump_table_sel));
}
...
struct str_map_t evg_fmt_cf_gws_word0_gws_opcode_map = {4,
                                                        {{"GWS_SEMA_V", 0},
                                                         {"GWS_SEMA_P", 1},
                                                         {"GWS_BARRIER", 2},
                                                         {"GWS_INIT", 3}}};
...
void evg_fmt_cf_gws_word0_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  val_index_mode = %d (%s)\n", fmt->val_index_mode,
          str_map_value(&evg_fmt_cf_gws_word0_val_index_mode_map,
                        fmt->val_index_mode));
  fprintf(f, "  rsrc_index_mode = %d (%s)\n", fmt->rsrc_index_mode,
          str_map_value(&evg_fmt_cf_index_mode_map, fmt->rsrc_index_mode));
  fprintf(f, "  gws_opcode = %d (%s)\n", fmt->gws_opcode,
          str_map_value(&evg_fmt_cf_gws_word0_gws_opcode_map, fmt->gws_opcode));
}
...
struct str_map_t evg_fmt_cf_word1_cond_map = {4,
                                              {{"CF_COND_ACTIVE", 0},
                                               {"CF_COND_FALSE", 1},
                                               {"CF_COND_BOOL", 2},
                                               {"CF_COND_NOT_BOOL", 3}}};
...
void evg_fmt_cf_alu_word0_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  kcache_mode0 = %d (%s)\n", fmt->kcache_mode0,
          str_map_value(&evg_fmt_kcache_mode_map, fmt->kcache_mode0));
}
...
struct str_map_t evg_fmt_cf_alu_inst_map = {8,
                                            {{"CF_INST_ALU", 8},
...
void evg_fmt_cf_alu_word1_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  kcache_mode1 = %d (%s)\n", fmt->kcache_mode1,
          str_map_value(&evg_fmt_kcache_mode_map, fmt->kcache_mode1));
  fprintf(f, "  kcache_addr0 = 0x%x\n", fmt->kcache_addr0);
  fprintf(f, "  kcache_addr1 = 0x%x\n", fmt->kcache_addr1);
  fprintf(f, "  count=%d\n", fmt->count);
  fprintf(f, "  alt_const=%d\n", fmt->alt_const);
  fprintf(f, "  cf_inst=%d (%s)\n", fmt->cf_inst,
          str_map_value(&evg_fmt_cf_alu_inst_map, fmt->cf_inst));
  fprintf(f, "  whole_quad_mode=%d\n", fmt->whole_quad_mode);
  fprintf(f, "  barrier=%d\n", fmt->barrier);
}
...
void evg_fmt_cf_alu_word0_ext_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  kcache_mode2 = %d (%s)\n", fmt->kcache_mode2,
          str_map_value(&evg_fmt_kcache_mode_map, fmt->kcache_mode2));
}
...
void evg_fmt_cf_alu_word1_ext_dump(void *buf, FILE *f) {
  struct evg_fmt_cf_alu_word1_ext_t *fmt =
      (struct evg_fmt_cf_alu_word1_ext_t *)buf;

  fprintf(f, "CF_ALU_WORD1_EXT\n");
  fprintf(f, "  kcache_mode3 = %d (%s)\n", fmt->kcache_mode3,
          str_map_value(&evg_fmt_kcache_mode_map, fmt->kcache_mode3));
...
void evg_fmt_cf_alloc_export_word0_dump(void *buf, FILE *f) {
  struct evg_fmt_cf_alloc_export_word0_t *fmt =
      (struct evg_fmt_cf_alloc_export_word0_t *)buf;

  fprintf(f, "CF_ALLOC_EXPORT_WORD0\n");
  fprintf(f, "  array_base = %d\n", fmt->array_base);
  fprintf(f, "  type = %d (%s)\n", fmt->type,
          str_map_value(&evg_fmt_cf_alloc_export_type_map, fmt->type));
  fprintf(f, "  rw_gpr = %d\n", fmt->rw_gpr);
  fprintf(f, "  rw_rel = %d (%s)\n", fmt->rr,
          str_map_value(&evg_fmt_rel_map, fmt->rr));
...
void evg_fmt_cf_alloc_export_word0_rat_dump(void *buf, FILE *f) {
  struct evg_fmt_cf_alloc_export_word0_rat_t *fmt =
      (struct evg_fmt_cf_alloc_export_word0_rat_t *)buf;

  fprintf(f, "CF_ALLOC_EXPORT_WORD0_RAT\n");
  fprintf(f, "  rat_id = %d\n", fmt->rat_id);
  fprintf(f, "  rat_inst = %d (%s)\n", fmt->rat_inst,
          str_map_value(&evg_fmt_cf_alloc_export_rat_inst_map, fmt->rat_inst));
  fprintf(f, "  rat_index_mode = %d (%s)\n", fmt->rat_index_mode,
          str_map_value(&evg_fmt_cf_index_mode_map, fmt->rat_index_mode));
  fprintf(f, "  type = %d (%s)\n", fmt->type,
          str_map_value(&evg_fmt_cf_alloc_export_type_map, fmt->type));
  fprintf(f, "  rw_gpr = %d\n", fmt->rw_gpr);
  fprintf(f, "  rw_rel = %d (%s)\n", fmt->rr,
...
void evg_fmt_cf_alloc_export_word1_buf_dump(void *buf, FILE *f) {
  struct evg_fmt_cf_alloc_export_word1_buf_t *fmt =
      (struct evg_fmt_cf_alloc_export_word1_buf_t *)buf;
...
void evg_fmt_cf_alloc_export_word1_swiz_dump(void *buf, FILE *f) {
  struct evg_fmt_cf_alloc_export_word1_swiz_t *fmt =
      (struct evg_fmt_cf_alloc_export_word1_swiz_t *)buf;

  fprintf(f, "CF_ALLOC_EXPORT_WORD1_SWIZ\n");
  fprintf(f, "  sel_x = %d (%s)\n", fmt->sel_x,
          str_map_value(&evg_fmt_sel_map, fmt->sel_x));
  fprintf(f, "  sel_y = %d (%s)\n", fmt->sel_y,
          str_map_value(&evg_fmt_sel_map, fmt->sel_y));
  fprintf(f, "  sel_z = %d (%s)\n", fmt->sel_z,
          str_map_value(&evg_fmt_sel_map, fmt->sel_z));
  fprintf(f, "  sel_w = %d (%s)\n", fmt->sel_w,
          str_map_value(&evg_fmt_sel_map, fmt->sel_w));
  fprintf(f, "  burst_count = %d\n", fmt->burst_count);
  fprintf(f, "  valid_pixel_mode = %d\n", fmt->valid_pixel_mode);
  fprintf(f, "  end_of_program = %d\n", fmt->end_of_program);
  fprintf(f, "  cf_inst = %d (%s)\n", fmt->cf_inst,
...
struct str_map_t evg_fmt_alu_word0_pred_sel_map = {
    3, {{"PRED_SEL_OFF", 0}, {"PRED_SEL_ZERO", 2}, {"PRED_SEL_ONE", 3}}};

static void evg_fmt_alu_src_sel_dump_buf(uint32_t src_sel, char **pbuf,
                                         int *psize) {
  if (IN_RANGE(src_sel, 0, 127))
    ...
    str_printf(pbuf, psize, "%s",
               str_map_value(&evg_fmt_alu_src_sel_map, src_sel));
}
...
void evg_fmt_alu_word0_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  src0_rel = %d (%s)\n", fmt->src0_rel,
          str_map_value(&evg_fmt_rel_map, fmt->src0_rel));
  fprintf(f, "  src1_rel = %d (%s)\n", fmt->src1_rel,
          str_map_value(&evg_fmt_rel_map, fmt->src1_rel));
  fprintf(f, "  src0_chan = %d (%s)\n", fmt->src0_chan,
          str_map_value(&evg_fmt_chan_map, fmt->src0_chan));
  fprintf(f, "  src1_chan = %d (%s)\n", fmt->src1_chan,
          str_map_value(&evg_fmt_chan_map, fmt->src1_chan));
  fprintf(f, "  src0_neg = %d\n", fmt->src0_neg);
  fprintf(f, "  src1_neg = %d\n", fmt->src1_neg);
  fprintf(f, "  index_mode = %d (%s)\n", fmt->index_mode,
          str_map_value(&evg_fmt_alu_word0_index_mode_map, fmt->index_mode));
  fprintf(f, "  pred_sel = %d (%s)\n", fmt->pred_sel,
          str_map_value(&evg_fmt_alu_word0_pred_sel_map, fmt->pred_sel));
  fprintf(f, "  last = %d\n", fmt->last);
}
...
struct str_map_t evg_fmt_alu_word1_op2_omod_map = {4,
                                                   {{"ALU_OMOD_OFF", 0},
                                                    {"ALU_OMOD_M2", 1},
                                                    {"ALU_OMOD_M4", 2},
                                                    {"ALU_OMOD_D2", 3}}};
...
void evg_fmt_alu_word1_op2_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  omod = %d (%s)\n", fmt->omod,
          str_map_value(&evg_fmt_alu_word1_op2_omod_map, fmt->omod));
  fprintf(f, "  alu_inst = %d (%s)\n", fmt->alu_inst,
          str_map_value(&evg_fmt_alu_word1_op2_alu_inst_map, fmt->alu_inst));
  fprintf(f, "  bank_swizzle = %d (%s)\n", fmt->bank_swizzle,
          str_map_value(&evg_fmt_alu_bank_swizzle_map, fmt->bank_swizzle));
  fprintf(f, "  dst_gpr = %d\n", fmt->dst_gpr);
  fprintf(f, "  dst_rel = %d (%s)\n", fmt->dst_rel,
          str_map_value(&evg_fmt_rel_map, fmt->dst_rel));
  fprintf(f, "  dst_chan = %d (%s)\n", fmt->dst_chan,
          str_map_value(&evg_fmt_chan_map, fmt->dst_chan));
  fprintf(f, "  clamp = %d\n", fmt->clamp);
}
...
void evg_fmt_alu_word1_op3_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  src2_rel = %d (%s)\n", fmt->src2_rel,
          str_map_value(&evg_fmt_rel_map, fmt->src2_rel));
  fprintf(f, "  src2_chan = %d (%s)\n", fmt->src2_chan,
          str_map_value(&evg_fmt_chan_map, fmt->src2_chan));
  fprintf(f, "  src2_neg = %d\n", fmt->src2_neg);
  fprintf(f, "  alu_inst = %d (%s)\n", fmt->alu_inst,
          str_map_value(&evg_fmt_alu_word1_op3_alu_inst_map, fmt->alu_inst));
  fprintf(f, "  bank_swizzle = %d (%s)\n", fmt->bank_swizzle,
          str_map_value(&evg_fmt_alu_bank_swizzle_map, fmt->bank_swizzle));
  fprintf(f, "  dst_gpr = %d\n", fmt->dst_gpr);
  fprintf(f, "  dst_rel = %d (%s)\n", fmt->dst_rel,
...
void evg_fmt_alu_word0_lds_idx_op_dump(void *buf, FILE *f) {
  struct evg_fmt_alu_word0_lds_idx_op_t *fmt =
      (struct evg_fmt_alu_word0_lds_idx_op_t *)buf;
  char src0_sel_str[MAX_STRING_SIZE];
  char src1_sel_str[MAX_STRING_SIZE];
  char *str;
  int size;
...
struct str_map_t evg_fmt_lds_op_map = {46,
                                       {{"ADD", 0},
                                        {"SUB", 1},
                                        {"RSUB", 2},
                                        {"INC", 3},
...
void evg_fmt_alu_word1_lds_idx_op_dump(void *buf, FILE *f) {
  struct evg_fmt_alu_word1_lds_idx_op_t *fmt =
      (struct evg_fmt_alu_word1_lds_idx_op_t *)buf;
  char src2_sel_str[MAX_STRING_SIZE];
  char *str;
  int size;
  ...
  fprintf(f, "  lds_op = %d (DS_INST_%s)\n", fmt->lds_op,
          str_map_value(&evg_fmt_lds_op_map, fmt->lds_op));
...
struct str_map_t evg_fmt_vc_inst_map = {3,
                                        {{"VC_INST_FETCH", 0},
                                         {"VC_INST_SEMANTIC", 1},
...
struct str_map_t evg_fmt_vtx_src_sel_map = {
    3, {{"SEL_X", 0}, {"SEL_Y", 1}, {"SEL_Z", 2}, {"SEL_W", 3}}};
...
void evg_fmt_vtx_word0_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  vc_inst = %d (%s)\n", fmt->vc_inst,
          str_map_value(&evg_fmt_vc_inst_map, fmt->vc_inst));
  fprintf(f, "  fetch_type = %d (%s)\n", fmt->fetch_type,
          str_map_value(&evg_fmt_vtx_fetch_type_map, fmt->fetch_type));
  fprintf(f, "  fetch_whole_quad = %d\n", fmt->fetch_whole_quad);
  fprintf(f, "  buffer_id = %d\n", fmt->buffer_id);
  fprintf(f, "  src_gpr = %d\n", fmt->src_gpr);
  fprintf(f, "  src_rel = %d\n", fmt->src_rel);
  fprintf(f, "  src_sel_x = %d (%s)\n", fmt->src_sel_x,
          str_map_value(&evg_fmt_vtx_src_sel_map, fmt->src_sel_x));
...
struct str_map_t evg_fmt_vtx_data_format_map = {64,
                                                {{"UNKNOWN", 0},
                                                 {"8", 1},
                                                 {"4_4", 2},
                                                 {"3_3_2", 3},
...
void evg_fmt_vtx_word1_gpr_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  dst_sel_x = %d (%s)\n", fmt->dst_sel_x,
          str_map_value(&evg_fmt_sel_map, fmt->dst_sel_x));
  fprintf(f, "  dst_sel_y = %d (%s)\n", fmt->dst_sel_y,
          str_map_value(&evg_fmt_sel_map, fmt->dst_sel_y));
  fprintf(f, "  dst_sel_z = %d (%s)\n", fmt->dst_sel_z,
          str_map_value(&evg_fmt_sel_map, fmt->dst_sel_z));
  fprintf(f, "  dst_sel_w = %d (%s)\n", fmt->dst_sel_w,
          str_map_value(&evg_fmt_sel_map, fmt->dst_sel_w));
  fprintf(f, "  use_const_fields = %d\n", fmt->use_const_fields);
  fprintf(f, "  data_format = %d (%s)\n", fmt->data_format,
          str_map_value(&evg_fmt_vtx_data_format_map, fmt->data_format));
  fprintf(f, "  num_format_all = %d (%s)\n", fmt->num_format_all,
          str_map_value(&evg_fmt_vtx_num_format_map, fmt->num_format_all));
  fprintf(f, "  format_comp_all = %d (%s)\n", fmt->format_comp_all,
          str_map_value(&evg_fmt_vtx_format_comp_map, fmt->format_comp_all));
  fprintf(f, "  srf_mode_all = %d (%s)\n", fmt->srf_mode_all,
          str_map_value(&evg_fmt_vtx_srf_mode_map, fmt->srf_mode_all));
}
...
void evg_fmt_vtx_word2_dump(void *buf, FILE *f) {
  ...
  fprintf(f, "  endian_swap = %d (%s)\n", fmt->endian_swap,
          str_map_value(&evg_fmt_vtx_endian_swap_map, fmt->endian_swap));
...
evg_fmt_dump_func_t evg_fmt_dump_func_list[EVG_FMT_COUNT] = {
    ...
    evg_fmt_cf_word0_dump, evg_fmt_cf_gws_word0_dump, evg_fmt_cf_word1_dump,

    evg_fmt_cf_alu_word0_dump, evg_fmt_cf_alu_word1_dump,

    evg_fmt_cf_alu_word0_ext_dump, evg_fmt_cf_alu_word1_ext_dump,

    evg_fmt_cf_alloc_export_word0_dump, evg_fmt_cf_alloc_export_word0_rat_dump,
    evg_fmt_cf_alloc_export_word1_buf_dump,
    evg_fmt_cf_alloc_export_word1_swiz_dump,

    evg_fmt_alu_word0_dump, evg_fmt_alu_word1_op2_dump,
    evg_fmt_alu_word1_op3_dump,

    evg_fmt_alu_word0_lds_idx_op_dump, evg_fmt_alu_word1_lds_idx_op_dump,
    evg_fmt_invalid_dump, /* EVG_FMT_ALU_WORD1_LDS_DIRECT_LITERAL_LO */
    evg_fmt_invalid_dump, /* EVG_FMT_ALU_WORD1_LDS_DIRECT_LITERAL_HI */

    evg_fmt_vtx_word0_dump, evg_fmt_vtx_word1_gpr_dump,
...
void evg_inst_word_dump(void *buf, enum evg_fmt_enum fmt, FILE *f) {
  ...
  /* Hexadecimal dump */
  fprintf(f, "Hex dump: ");
  for (i = 0; i < 4; i++) fprintf(f, "%02x ", ((unsigned char *)buf)[i]);
  fprintf(f, "\n");
