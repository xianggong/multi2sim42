...
struct si_work_group_t *si_work_group_create(unsigned int work_group_id,
  ...
  unsigned int work_items_per_group = ndrange->local_size3[0] *
                                      ndrange->local_size3[1] *
                                      ndrange->local_size3[2];
  assert(work_items_per_group > 0);

  /* Number of wavefronts in work-group */
  unsigned int wavefronts_per_group =
      (work_items_per_group + (si_emu_wavefront_size - 1)) /
  ...
  work_group->work_items =
      xcalloc(si_emu_wavefront_size * wavefronts_per_group, sizeof(void *));
  ...
  SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id) {
    work_group->wavefronts[wavefront_id] = si_wavefront_create(
        work_group->id * wavefronts_per_group + wavefront_id, work_group);
    ...
    SI_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id) {
      ...
      work_group->work_items[wavefront_offset + work_item_id]->work_group =
          work_group;
    }
  }
  ...
  work_group->id_3d[1] =
      (work_group_id / ndrange->group_count3[0]) % ndrange->group_count3[1];
  work_group->id_3d[2] =
      work_group_id / (ndrange->group_count3[0] * ndrange->group_count3[1]);
  work_group->id = work_group_id;
  ...
  for (lidz = 0; lidz < ndrange->local_size3[2]; lidz++) {
    for (lidy = 0; lidy < ndrange->local_size3[1]; lidy++) {
      for (lidx = 0; lidx < ndrange->local_size3[0]; lidx++) {
        ...
        /* Global IDs */
        work_item->id_3d[0] = work_item_gidx_start + lidx;
        work_item->id_3d[1] = work_item_gidy_start + lidy;
        work_item->id_3d[2] = work_item_gidz_start + lidz;
        work_item->id = tid;
        ...
        if (work_item->id_in_wavefront < 32) {
          ...
          wavefront->sreg[SI_EXEC + 1].as_uint |= 1
                                                  << work_item->id_in_wavefront;
        } else {
          fatal("%s: invalid work-item id (%d)", __FUNCTION__,
                work_item->id_in_wavefront);
        }
      }
    }
  ...
  SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id) {
    ...
    SI_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id) {
      ...
      /* V0 */
      work_item->vreg[0].as_int = work_item->id_in_work_group_3d[0];
      /* V1 */
      work_item->vreg[1].as_int = work_item->id_in_work_group_3d[1];
      /* V2 */
      work_item->vreg[2].as_int = work_item->id_in_work_group_3d[2];
    }
    ...
    for (i = 0; i < user_element_count; i++) {
      if (user_elements[i].dataClass == IMM_CONST_BUFFER) {
        ...
        si_wavefront_init_sreg_with_cb(wavefront, user_elements[i].startUserReg,
                                       user_elements[i].userRegCount,
                                       user_elements[i].apiSlot);
      } else if (user_elements[i].dataClass == IMM_UAV) {
        /* Store UAV pointer in sregs */
        si_wavefront_init_sreg_with_uav(
            wavefront, user_elements[i].startUserReg,
            user_elements[i].userRegCount, user_elements[i].apiSlot);
      } else if (user_elements[i].dataClass == PTR_CONST_BUFFER_TABLE) {
        ...
        si_wavefront_init_sreg_with_fetch_shader(wavefront,
                                                 user_elements[i].startUserReg,
                                                 user_elements[i].userRegCount);
      } else if (user_elements[i].dataClass == IMM_SAMPLER) {
        /* Store sampler in sregs */
        assert(0);
      } else if (user_elements[i].dataClass == PTR_RESOURCE_TABLE) {
        /* Store resource table in sregs */
        assert(0);
      } else if (user_elements[i].dataClass == PTR_INTERNAL_GLOBAL_TABLE) {
        fatal(
            "%s: PTR_INTERNAL_GLOBAL_TABLE not "
            "supported",
            __FUNCTION__);
      } else {
        fatal(
            "%s: Unimplemented User Element: "
            "dataClass:%d",
            __FUNCTION__, user_elements[i].dataClass);
      }
    }
  }
