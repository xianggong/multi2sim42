...
void si_ndrange_free(struct si_ndrange_t *ndrange) {
  ...
  if (ndrange->inst_buffer) free(ndrange->inst_buffer);
...
void si_ndrange_setup_size(struct si_ndrange_t *ndrange,
                           unsigned int *global_size, unsigned int *local_size,
                           int work_dim) {
  int i;
  ...
  ndrange->global_size = ndrange->global_size3[0] * ndrange->global_size3[1] *
  ...
  for (i = 0; i < work_dim; i++) {
    ...
    if (ndrange->local_size3[i] < 1) {
      fatal("%s: local work size must be greater than 0", __FUNCTION__);
    }
  }
  ndrange->local_size = ndrange->local_size3[0] * ndrange->local_size3[1] *
  ...
  for (i = 0; i < 3; i++) {
    ndrange->group_count3[i] =
        ndrange->global_size3[i] / ndrange->local_size3[i];
  }
  ndrange->group_count = ndrange->group_count3[0] * ndrange->group_count3[1] *
...
void si_ndrange_setup_inst_mem(struct si_ndrange_t *ndrange, void *buf,
  ...
  if (!size || pc >= size) panic("%s: invalid value for size/pc", __FUNCTION__);
...
void si_ndrange_setup_fs_mem(struct si_ndrange_t *ndrange, void *buf, int size,
                             unsigned int pc) {
  void *buffer;
...
void si_ndrange_insert_buffer_into_uav_table(struct si_ndrange_t *ndrange,
  ...
  mem_write(si_emu->global_mem,
            ndrange->uav_table + uav * SI_EMU_UAV_TABLE_ENTRY_SIZE,
...
void si_ndrange_insert_buffer_into_vertex_buffer_table(
    struct si_ndrange_t *ndrange, struct si_buffer_desc_t *buf_desc,
  ...
  mem_write(si_emu->global_mem,
            ndrange->vertex_buffer_table +
                vertex_buffer * SI_EMU_VERTEX_BUFFER_TABLE_ENTRY_SIZE,
            sizeof(*buf_desc), buf_desc);
...
void si_ndrange_insert_buffer_into_const_buf_table(
    struct si_ndrange_t *ndrange, struct si_buffer_desc_t *buf_desc,
    unsigned int const_buf_num) {
...
void si_ndrange_insert_image_into_uav_table(struct si_ndrange_t *ndrange,
                                            struct si_image_desc_t *image_desc,
                                            unsigned int uav) {
  assert(uav < SI_EMU_MAX_NUM_UAVS);
...
void si_ndrange_const_buf_write(struct si_ndrange_t *ndrange, int const_buf_num,
...
void si_ndrange_const_buf_read(struct si_ndrange_t *ndrange, int const_buf_num,
                               int offset, void *pvalue, unsigned int size) {
  unsigned int addr;
