...
struct si_wavefront_t *si_wavefront_create(int wavefront_id,
  ...
  SI_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id) {
    ...
    wavefront->work_items[work_item_id]->id_in_wavefront = work_item_id;
...
void si_wavefront_sreg_init(struct si_wavefront_t *wavefront) {
  ...
  for (int i = 128; i < 193; i++) sreg[i].as_int = i - 128;
  for (int i = 193; i < 209; i++) sreg[i].as_int = -(i - 192);
...
void si_wavefront_execute(struct si_wavefront_t *wavefront) {
  ...
  wavefront->inst_size =
      si_inst_decode(ndrange->inst_buffer + wavefront->pc, &wavefront->inst, 0);
  ...
  switch (inst->info->fmt) {
    ...
    case SI_FMT_SOP1: {
      ...
      if (debug_status(si_isa_debug_category)) {
        si_inst_dump(inst, wavefront->inst_size, wavefront->pc,
                     ndrange->inst_buffer + wavefront->pc, inst_dump,
                     MAX_INST_STR_SIZE);
        si_isa_debug("\n%s", inst_dump);
      }
    ...
    case SI_FMT_DS: {
      ...
      if ((inst->info->opcode >= 13 && inst->info->opcode < 16) ||
        ...
        // fatal("%s: unimplemented LDS opcode: %d", __FUNCTION__,
        //      inst->info->opcode);
      }
    ...
    default: {
      fatal("%s: instruction type not implemented (%d)", __FUNCTION__,
            inst->info->fmt);
      break;
    }
  }
  ...
  if (!wavefront->finished) {
    ...
    if (work_group->wavefronts_completed_emu == work_group->wavefront_count) {
...
void si_wavefront_init_sreg_with_cb(struct si_wavefront_t *wavefront,
   ...
   * in global memory) */
  mem_read(si_emu->global_mem, buf_desc_addr, sizeof(buf_desc), &buf_desc);
...
void si_wavefront_init_sreg_with_vertex_buffer_table(
    struct si_wavefront_t *wavefront, int first_reg, int num_regs) {
  struct si_ndrange_t *ndrange = wavefront->work_group->ndrange;
  struct si_mem_ptr_t mem_ptr;
