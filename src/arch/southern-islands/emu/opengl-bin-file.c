...
static int si_opengl_shader_binary_get_isa_offset(
...
static void si_opengl_shader_binary_set_type(
    struct si_opengl_shader_binary_t *shdr) {
...
static void si_opengl_shader_binary_set_isa(
    struct si_opengl_shader_binary_t *shdr) {
...
static struct si_opengl_bin_spi_shader_pgm_rsrc2_vs_t *
si_opengl_bin_spi_shader_pgm_rsrc2_vs_create() {
  struct si_opengl_bin_spi_shader_pgm_rsrc2_vs_t *pgm_rsrc2;

  /* Allocate */
  pgm_rsrc2 =
      xcalloc(1, sizeof(struct si_opengl_bin_spi_shader_pgm_rsrc2_vs_t));

  /* Return */
  return pgm_rsrc2;
}

static void si_opengl_bin_spi_shader_pgm_rsrc2_vs_free(
    struct si_opengl_bin_spi_shader_pgm_rsrc2_vs_t *pgm_rsrc2) {
  free(pgm_rsrc2);
}

static struct si_opengl_bin_enc_dict_entry_t *
si_opengl_bin_enc_dict_entry_create() {
...
static void si_opengl_bin_enc_dict_entry_free(
    struct si_opengl_bin_enc_dict_entry_t *enc_dict) {
  int i;
...
static void si_opengl_enc_dict_set_userElements(
    struct si_opengl_shader_binary_t *shdr,
    struct si_opengl_bin_enc_dict_entry_t *enc_dict) {
...
static void si_opengl_enc_dict_set_semanticMappings(
    struct si_opengl_shader_binary_t *shdr,
    struct si_opengl_bin_enc_dict_entry_t *enc_dict) {
  /* FIXME: should get this info from binary! */
  switch (shdr->shader_kind) {
...
static void si_opengl_enc_dict_set_inputs(
    struct si_opengl_shader_binary_t *shdr,
    struct si_opengl_bin_enc_dict_entry_t *enc_dict) {
  /* FIXME: should get this info from binary! */
  int input_count = 2;
...
static void si_opengl_shader_binary_set_enc_dict(
    struct si_opengl_shader_binary_t *shdr) {
  struct si_opengl_bin_enc_dict_entry_t *enc_dict;
...
static struct list_t *si_opengl_shaders_list_create(struct elf_file_t *binary) {
  ...
  LIST_FOR_EACH(binary->symbol_table, i) {
    ...
    if (str_suffix(symbol->name, "ElfBinary_0_")) {
      ...
      shader = si_opengl_shader_binary_create(
          section->buffer.ptr + symbol->value, symbol->size, symbol->name);
      list_add(lst, shader);
    }
  }
...
struct si_opengl_program_binary_t *si_opengl_program_binary_create(
    void *buffer_ptr, int size, char *name) {
  ...
  if (!program_bin->binary)
    fatal(
        "Shader binary is not ELF formatted, please upgrade your AMD driver!");
...
void si_opengl_program_binary_free(
    struct si_opengl_program_binary_t *program_bin) {
  /* Free shader binary */
...
struct si_opengl_shader_binary_t *si_opengl_shader_binary_create(void *buffer,
                                                                 int size,
                                                                 char *name) {
  struct si_opengl_shader_binary_t *shdr;
...
void si_opengl_bin_enc_user_element_free(
    struct si_bin_enc_user_element_t *user_elem) {
  free(user_elem);
}
