...
struct si_opengl_bin_enc_dict_entry_t {
  ...
  struct si_opengl_bin_enc_semantic_mapping_t
      semanticsMapping[MAX_SEMANTICS_MAPPINGS];

  struct list_t *
      input_list; /* - Equivalent to arg_list in OpenCL, elements of type
                     si_input_t */
...
struct si_opengl_program_binary_t {
  ...
  /* List of shaders in shader binary, elements with type
   * si_opengl_shader_binary_t */
