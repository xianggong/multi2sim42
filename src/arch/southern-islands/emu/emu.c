...
int SIEmuRun(Emu *self) {
  ...
  if (!list_count(emu->running_work_groups)) return FALSE;
  ...
  while (!work_group->finished_emu) {
    SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id) {
      ...
      if (wavefront->finished || wavefront->at_barrier) continue;
...
void si_emu_init(void) {
  ...
  if (*si_emu_report_file_name) {
    ...
    if (!si_emu_report_file)
      fatal(
          "%s: cannot open report for Southern Islands "
          "emulator",
...
void si_emu_done(void) {
  /* GPU report */
  if (si_emu_report_file) fclose(si_emu_report_file);
...
void si_emu_opengl_disasm(char *path, int opengl_shader_index) {
  ...
  if (!file_buffer) fatal("%s:Invalid file!", path);

  /* Analyze the file and initialize structure */
  si_program_bin =
      si_opengl_program_binary_create(file_buffer, file_size, path);
  ...
  if (opengl_shader_index > list_count(si_program_bin->shaders) ||
    ...
    fatal(
        "Shader index out of range! Please choose <index> "
        "from 1 ~ %d",
        list_count(si_program_bin->shaders));
  }

  /* Disassemble */
  si_shader = list_get(si_program_bin->shaders, opengl_shader_index - 1);
