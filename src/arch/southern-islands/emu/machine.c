...
void si_isa_S_BUFFER_LOAD_DWORD_impl(struct si_work_item_t *work_item,
  ...
  m_offset =
      (INST.imm) ? (INST.offset * 4) : si_isa_read_sreg(work_item, INST.offset);
  ...
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("wf%d: S%u<=(%u)(%u,%gf)", work_item->wavefront->id, INST.sdst,
                 addr, value.as_uint, value.as_float);
  }
...
void si_isa_S_BUFFER_LOAD_DWORDX2_impl(struct si_work_item_t *work_item,
  ...
  if (debug_status(si_isa_debug_category)) {
    ...
    for (i = 0; i < 2; i++) {
      si_isa_debug("S%u<=(%u)(%u,%gf) ", INST.sdst + i, addr + i * 4,
...
void si_isa_S_LOAD_DWORDX4_impl(struct si_work_item_t *work_item,
  ...
  /* FIXME Set value based on type */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("S[%u,%u] <= (addr %u): ", INST.sdst, INST.sdst + 3, m_addr);
    for (i = 0; i < 4; i++) {
      si_isa_debug("S%u<=(%u,%gf) ", INST.sdst + i, value[i].as_uint,
                   value[i].as_float);
    }
  }
...
void si_isa_S_ADD_U32_impl(struct si_work_item_t *work_item,
  ...
  carry.as_uint =
      ((unsigned long long)s0.as_uint + (unsigned long long)s1.as_uint) >> 32;
...
/* D.u = S0.u ^ S1.u. scc = 1 if result is non-zero. */
#define INST SI_INST_SOP2
void si_isa_S_XOR_B32_impl(struct si_work_item_t *work_item,
                           struct si_inst_t *inst) {
  union si_reg_t s0;
  union si_reg_t s1;
  union si_reg_t result;
 ...
 */
#define INST SI_INST_SOP2
void si_isa_S_BFM_B32_impl(struct si_work_item_t *work_item,
                           struct si_inst_t *inst) {
  union si_reg_t s0;
  union si_reg_t s1;
  union si_reg_t result;
  ...
  /* Load operands from registers. */
  s0.as_uint = si_isa_read_sreg(work_item, INST.ssrc0) & 0x1F;
  s1.as_uint = si_isa_read_sreg(work_item, INST.ssrc1) & 0x1F;

  /* Calculate the result. */
  result.as_int = ((1 << s0.as_int) - 1) << s1.as_uint;
...
void si_isa_S_BFE_I32_impl(struct si_work_item_t *work_item,
  ...
  /* s1 (offset) should be [4:0] of ssrc1 and s2 (width) should be [22:16] of
   * ssrc1*/
  ...
  if (s2.as_uint == 0) {
    ...
    result.as_int =
        (s0.as_int << (32 - s1.as_uint - s2.as_uint)) >> (32 - s2.as_uint);
  } else {
...
void si_isa_S_ADDK_I32_impl(struct si_work_item_t *work_item,
  ...
  ovf.as_uint = (dst.as_int >> 31 != se_simm16 >> 31)
                    ? 0
 ...
 * non-zero. */
#define INST SI_INST_SOP1
void si_isa_S_OR_SAVEEXEC_B64_impl(struct si_work_item_t *work_item,
                                   struct si_inst_t *inst) {
  /* Assert no literal constant with a 64 bit instruction. */
  assert(!(INST.ssrc0 == 0xFF));
...
void si_isa_S_CMP_EQ_I32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("wf%d: scc<=(%u) (%u ==? %u)", work_item->wavefront->id,
                 result.as_uint, s0.as_int, s1.as_int);
  }
}
#undef INST
...
void si_isa_S_CMP_GT_I32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("scc<=(%u) (%u >? %u) ", result.as_uint, s0.as_uint,
                 s1.as_uint);
  }
}
#undef INST
...
void si_isa_S_CBRANCH_SCC1_impl(struct si_work_item_t *work_item,
  ...
  if (scc.as_uint) {
    ...
    /* Print isa debug information. */
    if (debug_status(si_isa_debug_category)) {
      si_isa_debug("wf%d: SCC=%u (taken)", work_item->wavefront->id,
                   scc.as_uint);
    }
  } else {
    /* Print isa debug information. */
    if (debug_status(si_isa_debug_category)) {
      si_isa_debug("wf%d: SCC=%u (not taken)", work_item->wavefront->id,
                   scc.as_uint);
    }
  }
}
  ...
  if (execz.as_uint) {
    ...
    if (debug_status(si_isa_debug_category)) {
      si_isa_debug("wf%d: EXEC=0x%x, EXECZ=%u (taken)",
                   work_item->wavefront->id, exec.as_uint, execz.as_uint);
    }
  } else {
    /* Print isa debug information. */
    if (debug_status(si_isa_debug_category)) {
      si_isa_debug(
          "wf%d: EXEC=0x%x, EXECZ=%u "
          "(not taken)",
          work_item->wavefront->id, exec.as_uint, execz.as_uint);
    }
  }
...
void si_isa_S_BARRIER_impl(struct si_work_item_t *work_item,
  ...
  si_isa_debug(
      "Group %d wavefront %d reached barrier "
      "(%d reached, %d left)\n",
      work_group->id, wavefront->id, work_group->wavefronts_at_barrier,
      work_group->wavefront_count - work_group->wavefronts_at_barrier);
...
void si_isa_V_MOV_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(0x%x) ", work_item->id, INST.vdst, value.as_uint);
  }
}
#undef INST
...
void si_isa_V_CVT_F64_I32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V[%u:+1]<=(%lgf) ", work_item->id, INST.vdst,
                 value.as_double);
  }
}
#undef INST
...
void si_isa_V_CVT_U32_F32_impl(struct si_work_item_t *work_item,
  ...
  /* -inf, NaN, 0, -0 --> 0 */
  if ((isinf(fvalue) && fvalue < 0.0f) || isnan(fvalue) || fvalue == 0.0f ||
      fvalue == -0.0f)
    value.as_uint = 0;
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u) ", work_item->id, INST.vdst, value.as_uint);
  }
}
#undef INST
...
void si_isa_V_FLOOR_F32_impl(struct si_work_item_t *work_item,
  ...
  floor_s0.as_float = floor(s0.as_float);

  /* Write the results. */
  si_isa_write_vreg(work_item, INST.vdst, floor_s0.as_uint);
...
/* D.f = 1.0 / sqrt(S0.f). */
#define INST SI_INST_VOP1
void si_isa_V_RSQ_F32_impl(struct si_work_item_t *work_item,
                           struct si_inst_t *inst) {
  union si_reg_t s0;
  union si_reg_t rsq;
  ...
  /* Write the results. */
  si_isa_write_vreg(work_item, INST.vdst, rsq.as_uint);

  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%gf) ", work_item->id, INST.vdst, rsq.as_float);
  }
}
#undef INST
...
void si_isa_V_SIN_F32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%gf) (sin %gf) ", work_item->id, INST.vdst,
                 result.as_float, s0.as_float);
  }
}
#undef INST
...
void si_isa_V_COS_F32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%gf) (cos %gf) ", work_item->id, INST.vdst,
                 result.as_float, s0.as_float);
  }
}
#undef INST
...
void si_isa_V_MOVRELD_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V[%u+%u]<=(0x%x) ", work_item->id, INST.vdst, m0.as_uint,
                 s0.as_uint);
  }
}
#undef INST
...
void si_isa_V_MOVRELS_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V[%u]<=(0x%x) ", work_item->id, INST.vdst, s0.as_uint);
  }
}
#undef INST
...
void si_isa_V_CNDMASK_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u) ", work_item->id, INST.vdst, result.as_uint);
  }
}
#undef INST
...
void si_isa_V_MUL_LEGACY_F32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%gf) (%gf * %gf) ", work_item->id, INST.vdst,
                 product.as_float, s0.as_float, s1.as_float);
  }
}
#undef INST
...
void si_isa_V_MIN_I32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%d)", work_item->id, INST.vdst, min.as_int);
  }
}
#undef INST
...
void si_isa_V_MIN_U32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u)", work_item->id, INST.vdst, min.as_uint);
  }
}
#undef INST
...
void si_isa_V_MAX_U32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u)", work_item->id, INST.vdst, max.as_uint);
  }
}
#undef INST
...
void si_isa_V_LSHRREV_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u) (%u >> %u) ", work_item->id, INST.vdst,
                 result.as_uint, s1.as_uint, s0.as_uint);
  }
}
#undef INST
...
void si_isa_V_ASHRREV_I32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%d, %u) ", work_item->id, INST.vdst, result.as_int,
                 result.as_uint);
  }
}
#undef INST
...
void si_isa_V_LSHLREV_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(0x%x) (%u << %u) ", work_item->id, INST.vdst,
                 result.as_uint, s1.as_uint, s0.as_uint);
  }
}
#undef INST
...
void si_isa_V_MADMK_F32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%f) (%f * %f + %f)", work_item->id, INST.vdst,
                 dst.as_float, s0.as_float, K.as_float, s1.as_float);
  }
}
#undef INST
...
void si_isa_V_ADD_I32_impl(struct si_work_item_t *work_item,
  ...
  sum.as_int = s0.as_int + s1.as_int;
  carry.as_uint = !!(((long long)s0.as_int + (long long)s1.as_int) >> 32);
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%d) (%d + %d) ", work_item->id, INST.vdst,
                 sum.as_int, s0.as_int, s1.as_int);
...
void si_isa_V_SUB_I32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%d) ", work_item->id, INST.vdst, dif.as_int);
    si_isa_debug("vcc<=(%u) ", carry.as_uint);
  }
}
#undef INST
...
/* vcc = D.u = (!isNaN(S0) || !isNaN(S1)). */
#define INST SI_INST_VOPC
void si_isa_V_CMP_U_F32_impl(struct si_work_item_t *work_item,
                             struct si_inst_t *inst) {
  union si_reg_t s0;
  union si_reg_t s1;
  union si_reg_t result;
  ...
  /* Compare the operands. */
  result.as_uint = (!isnan(s0.as_float) || !isnan(s1.as_float));
...
void si_isa_V_CNDMASK_B32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  if (INST.neg & 1) s0.as_float = -s0.as_float;
  if (INST.neg & 2) s1.as_float = -s1.as_float;
  assert(!(INST.neg & 4));
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u, %gf) (s1=%u, s0=%u)", work_item->id, INST.vdst,
                 result.as_uint, result.as_float, s1.as_uint, s0.as_uint);
  }
}
#undef INST
...
void si_isa_V_ADD_F32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  if (INST.abs & 1) s0.as_float = fabsf(s0.as_float);
  if (INST.abs & 2) s1.as_float = fabsf(s1.as_float);
  assert(!(INST.abs & 4));
...
void si_isa_V_MAX_F32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  /* Calculate the result. */
  result.as_float = (s0.as_float > s1.as_float) ? s0.as_float : s1.as_float;
...
/* D.f = S0.f * S1.f + S2.f. */
#define INST SI_INST_VOP3a
void si_isa_V_MADMK_F32_VOP3a_impl(struct si_work_item_t *work_item,
                                   struct si_inst_t *inst) {
  NOT_IMPL();
}
#undef INST
...
void si_isa_V_MAD_F32_impl(struct si_work_item_t *work_item,
  ...
  if (INST.abs & 1) s0.as_float = fabsf(s0.as_float);
  if (INST.abs & 2) s1.as_float = fabsf(s1.as_float);
  if (INST.abs & 4) s2.as_float = fabsf(s2.as_float);

  /* Apply negation modifiers. */
  if (INST.neg & 1) s0.as_float = -s0.as_float;
  if (INST.neg & 2) s1.as_float = -s1.as_float;
  if (INST.neg & 4) s2.as_float = -s2.as_float;
...
void si_isa_V_BFE_U32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: value:s0:%u, offset:s1:%u, width:s2:%u ", work_item->id,
                 s0.as_uint, s1.as_uint, s2.as_uint);
...
void si_isa_V_BFI_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Calculate the result. */
  result.as_uint = (s0.as_uint & s1.as_uint) | (~s0.as_uint & s2.as_uint);
...
void si_isa_V_FMA_F32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%gf) (%gf*%gf + %gf) ", work_item->id, INST.vdst,
                 result.as_float, s0.as_float, s1.as_float, s2.as_float);
  }
}
#undef INST
...
void si_isa_V_ALIGNBIT_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(0x%x, %u) ({0x%x,0x%x} >> %u) ", work_item->id,
                 INST.vdst, result.as_uint, result.as_uint, src.as_reg[1],
                 src.as_reg[0], src2.as_uint);
  }
}
#undef INST

/*
 *D.d = Special case divide fixup and flags(s0.d = Quotient, s1.d = Denominator,
 *s2.d = Numerator).
 */
#define INST SI_INST_VOP3a
...
/*
 *D.d = Special case divide FMA with scale and flags(s0.d = Quotient, s1.d =
 *Denominator,
 *s2.d = Numerator).
 */
#define INST SI_INST_VOP3a
...
void si_isa_V_MUL_HI_U32_impl(struct si_work_item_t *work_item,
  ...
  result.as_uint = (unsigned int)(((unsigned long long)s0.as_uint *
...
void si_isa_V_CMP_LT_F32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: S[%u:+1]<=(%u) ", work_item->id_in_wavefront, INST.vdst,
                 result.as_uint);
  }
}
#undef INST
...
/* D.u = D.u = (!isNaN(S0) || !isNaN(S1)). */
#define INST SI_INST_VOP3a
void si_isa_V_CMP_U_F32_VOP3a_impl(struct si_work_item_t *work_item,
                                   struct si_inst_t *inst) {
  union si_reg_t s0;
  union si_reg_t s1;
  union si_reg_t result;
...
void si_isa_V_CMP_LT_I32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  /* Apply absolute value modifiers. */
  if (INST.abs & 1) s0.as_int = abs(s0.as_int);
  if (INST.abs & 2) s1.as_int = abs(s1.as_int);
  assert(!(INST.abs & 4));

  /* Apply negation modifiers. */
  if (INST.neg & 1) s0.as_int = -s0.as_int;
  if (INST.neg & 2) s1.as_int = -s1.as_int;
  assert(!(INST.neg & 4));
...
void si_isa_V_CMP_GT_I32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: S[%u:%u]<=(%u) ", work_item->id_in_wavefront, INST.vdst,
                 INST.vdst + 1, result.as_uint);
  }
}
#undef INST
...
void si_isa_V_CMPX_EQ_I32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: S[%u:%u],EXEC<=(%u) ", work_item->id_in_wavefront,
                 INST.vdst, INST.vdst + 1, result.as_uint);
  }
}
#undef INST
...
void si_isa_V_CMP_LG_U32_VOP3a_impl(struct si_work_item_t *work_item,
  ...
  /* Calculate result. */
  result.as_uint = ((s0.as_uint < s1.as_uint) || (s0.as_uint > s1.as_uint));
...
void si_isa_V_MED3_I32_impl(struct si_work_item_t *work_item,
  ...
  if (s0.as_int >= s1.as_int && s0.as_int >= s2.as_int) {
    /* max(s1, s2) */
    median.as_int = (s1.as_int >= s2.as_int) ? s1.as_int : s2.as_int;
  }
  /* max3(s0, s1, s2) == s1 */
  else if (s1.as_int >= s0.as_int && s1.as_int >= s2.as_int) {
    /* max(s0, s2) */
    median.as_int = (s0.as_int >= s2.as_int) ? s0.as_int : s2.as_int;
  }
  /* max3(s0, s1, s2) == s2 */
  else if (s2.as_int >= s0.as_int && s2.as_int >= s1.as_int) {
    /* max(s0, s1) */
    median.as_int = (s0.as_int >= s1.as_int) ? s0.as_int : s1.as_int;
  } else {
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V[%u]<=(%d) ", work_item->id_in_wavefront, INST.vdst,
                 median.as_int);
  }
}
#undef INST
...
void si_isa_V_LSHR_B64_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: S[%u]<=(0x%x) ", work_item->id_in_wavefront, INST.vdst,
                 result_lo.as_uint);
    si_isa_debug("S[%u]<=(0x%x) ", INST.vdst + 1, result_hi.as_uint);
  }
}
#undef INST
...
void si_isa_V_ADD_F64_impl(struct si_work_item_t *work_item,
  ...
  /* s0 == +-denormal, +-0 */
    ...
    if (fpclassify(s1.as_double) == FP_SUBNORMAL ||
      ...
      /* s0 && s1 == -denormal, -0 */
      if (!!signbit(s0.as_double) && !!signbit(s1.as_double)) /* value <-- -0 */
        value.as_double = -0;
      else
        /* value <-- +0 */
        value.as_double = +0;
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: S[%u:+1]<=(%lgf) ", work_item->id_in_wavefront,
...
void si_isa_V_MUL_F64_impl(struct si_work_item_t *work_item,
  ...
  /* s0 == +denormal, +0 */
    ...
    /* s1 == +-infinity */
    if (isinf(s1.as_double)) /* value <-- NaN64 */
      value.as_double = NAN;
    /* s1 > 0 */
  ...
  /* s0 == +infinity */
  else if (fpclassify(s0.as_double) == FP_INFINITE && !signbit(s0.as_double)) {
    /* s1 == +-denormal, +-0 */
    if (fpclassify(s1.as_double) == FP_SUBNORMAL ||
        fpclassify(s1.as_double) == FP_ZERO)
      /* value <-- NaN64 */
...
void si_isa_V_ADDC_U32_VOP3b_impl(struct si_work_item_t *work_item,
  ...
  carry_out.as_uint =
      !!(((unsigned long long)s0.as_uint + (unsigned long long)s1.as_uint +
          (unsigned long long)carry_in.as_uint) >>
         32);
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u) ", work_item->id, INST.vdst, sum.as_uint);
...
/*
 *D.d = Special case divide preop and flags(s0.d = Quotient, s1.d = Denominator,
 *s2.d = Numerator)
...
void si_isa_DS_INC_U32_impl(struct si_work_item_t *work_item,
  ...
  if (INST.gds) {
    ...
    mem_read(work_item->work_group->lds_module, addr.as_uint, 4, &data.as_uint);
  }

  data.as_uint = data.as_uint >= data0.as_uint ? 0 : data.as_uint + 1;
  ...
  if (debug_status(si_isa_debug_category) && INST.gds) {
    si_isa_debug("t%d: GDS[%u]<=(%u,%f) ", work_item->id, addr.as_uint,
                 data.as_uint, data.as_float);
  } else {
    si_isa_debug("t%d: LDS[%u]<=(%u,%f) ", work_item->id, addr.as_uint,
                 data.as_uint, data.as_float);
  }
}
#undef INST
...
void si_isa_DS_WRITE2_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category) && INST.gds) {
    si_isa_debug("t%d: GDS[%u]<=(%u,%f) ", work_item->id, addr0.as_uint,
                 data0.as_uint, data0.as_float);
    si_isa_debug("GDS[%u]<=(%u,%f) ", addr1.as_uint, data0.as_uint,
                 data0.as_float);
  } else {
    si_isa_debug("t%d: LDS[%u]<=(%u,%f) ", work_item->id, addr0.as_uint,
                 data0.as_uint, data0.as_float);
...
void si_isa_DS_WRITE_B32_impl(struct si_work_item_t *work_item,
  ...
  if (addr.as_uint > MIN(work_item->work_group->ndrange->local_mem_top,
                         si_isa_read_sreg(work_item, SI_M0))) {
    fatal("%s: invalid address: %x\n", __FUNCTION__, addr.as_uint);
  }
...
void si_isa_DS_WRITE_B8_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category) && INST.gds) {
    si_isa_debug("t%d: GDS[%u]<=(0x%x) ", work_item->id, addr.as_uint,
                 data0.as_ubyte[0]);
  } else {
    si_isa_debug("t%d: LDS[%u]<=(0x%x) ", work_item->id, addr.as_uint,
                 data0.as_ubyte[0]);
  }
}
#undef INST
...
void si_isa_DS_READ_B32_impl(struct si_work_item_t *work_item,
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(0x%x)(0x%x,%f) ", work_item->id, INST.vdst,
                 addr.as_uint, data.as_uint, data.as_float);
  }
}
#undef INST
...
void si_isa_DS_READ2_B32_impl(struct si_work_item_t *work_item,
  ...
  if (INST.gds) {
    ...
    mem_read(work_item->work_group->lds_module, addr.as_uint + INST.offset0 * 4,
             4, &data0.as_uint);
    mem_read(work_item->work_group->lds_module, addr.as_uint + INST.offset1 * 4,
             4, &data1.as_uint);
  }
  ...
  /* Print isa debug information. */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(0x%x)(0x%x) ", work_item->id, INST.vdst,
                 addr.as_uint + INST.offset0 * 4, data0.as_uint);
...
void si_isa_BUFFER_LOAD_SBYTE_impl(struct si_work_item_t *work_item,
  ...
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: V%u<=(%u)(%d) ", work_item->id, INST.vdata, addr,
                 value.as_int);
  }
}
#undef INST
...
void si_isa_TBUFFER_LOAD_FORMAT_XY_impl(struct si_work_item_t *work_item,
  ...
  for (i = 0; i < 2; i++) {
    ...
    /* TODO Print value based on type */
    if (debug_status(si_isa_debug_category)) {
      si_isa_debug("t%d: V%u<=(%u)(%u,%gf) ", work_item->id, INST.vdata + i,
                   addr + 4 * i, value.as_uint, value.as_float);
    }
  }
...
void si_isa_TBUFFER_STORE_FORMAT_X_impl(struct si_work_item_t *work_item,
  ...
  /* TODO Print value based on type */
  if (debug_status(si_isa_debug_category)) {
    si_isa_debug("t%d: (%u)<=V%u(%u,%gf) ", work_item->id, addr, INST.vdata,
                 value.as_uint, value.as_float);
  }
}
#undef INST
...
void si_isa_TBUFFER_STORE_FORMAT_XY_impl(struct si_work_item_t *work_item,
  ...
  for (unsigned int i = 0; i < 2; i++) {
    ...
    /* TODO Print value based on type */
    if (debug_status(si_isa_debug_category)) {
      si_isa_debug("t%d: (%u)<=V%u(%u,%gf) ", work_item->id, addr,
