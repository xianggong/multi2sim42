...
void si_isa_enqueue_write_lds(unsigned int addr, unsigned int value,
                              size_t value_size);
...
#define SI_ISA_ARG_NOT_SUPPORTED(p)                                    \
  fatal("%s: %s: not supported for '" #p "' = 0x%x\n%s", __FUNCTION__, \
        si_isa_inst->info->name, (p), err_si_isa_note);
#define SI_ISA_ARG_NOT_SUPPORTED_NEQ(p, v)                                  \
  {                                                                         \
    if ((p) != (v))                                                         \
      fatal("%s: %s: not supported for '" #p "' != 0x%x\n%s", __FUNCTION__, \
            si_isa_inst->info->name, (v), err_si_isa_note);                 \
  }
#define SI_ISA_ARG_NOT_SUPPORTED_RANGE(p, min, max)                        \
  {                                                                        \
    if ((p) < (min) || (p) > (max))                                        \
      fatal("%s: %s: not supported for '" #p "' out of range [%d:%d]\n%s", \
            __FUNCTION__, si_isa_inst->info->name, (min), (max),           \
            err_si_opencl_param_note);                                     \
  }
...
typedef void (*si_isa_inst_func_t)(struct si_work_item_t *work_item,
                                   struct si_inst_t *inst);
