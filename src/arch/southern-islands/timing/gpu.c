...
/* FIXME */
unsigned int si_gpu_min_data_type_align_size =
    16; /* size of long16 in bytes
                                                             deprecated in
           OpenCL 1.2 */
...
unsigned long long si_gpu_global_mem_cache_size = 1ull << 23;
    /* 8MB */ /* FIXME */
unsigned long long si_gpu_global_mem_size =
...
unsigned long long si_gpu_max_constant_buffer_size = 1ull << 16;
...
unsigned int si_gpu_execution_capabilities = 1;
    /* bit field, CL_EXEC_KERNEL only */ /* FIXME */

unsigned int si_gpu_queue_properties =
...
void si_gpu_map_ndrange(struct si_ndrange_t *ndrange) {
  ...
  si_gpu->work_groups_per_wavefront_pool =
      si_calc_get_work_groups_per_wavefront_pool(
          ndrange->local_size, ndrange->num_vgpr_used, ndrange->local_mem_top);

  if (!si_gpu->work_groups_per_wavefront_pool) {
    fatal(
        "work-group resources cannot be allocated to a compute "
  ...
  si_gpu->work_groups_per_compute_unit =
      si_gpu->work_groups_per_wavefront_pool * si_gpu_num_wavefront_pools;
...
void si_gpu_read_config(void) {
  ...
  si_gpu_frequency =
      config_read_int(gpu_config, section, "Frequency", si_gpu_frequency);
  ...
  si_gpu_max_work_groups_per_wavefront_pool =
      config_read_int(gpu_config, section, "MaxWorkGroupsPerWavefrontPool",
                      si_gpu_max_work_groups_per_wavefront_pool);
  if (si_gpu_max_work_groups_per_wavefront_pool < 1)
    fatal(
        "%s: invalid value for 'MaxWorkGroupsPerWavefrontPool'"
        ".\n%s",
        si_gpu_config_file_name, err_note);

  si_gpu_max_wavefronts_per_wavefront_pool =
      config_read_int(gpu_config, section, "MaxWavefrontsPerWavefrontPool",
                      si_gpu_max_wavefronts_per_wavefront_pool);
  if (si_gpu_max_wavefronts_per_wavefront_pool < 1)
    fatal(
        "%s: invalid value for 'MaxWavefrontsPerWavefrontPool'"
        ".\n%s",
        si_gpu_config_file_name, err_note);
  ...
  si_gpu_fe_fetch_latency = config_read_int(gpu_config, section, "FetchLatency",
                                            si_gpu_fe_fetch_latency);
  ...
  si_gpu_fe_fetch_width =
      config_read_int(gpu_config, section, "FetchWidth", si_gpu_fe_fetch_width);
  ...
  si_gpu_fe_issue_latency = config_read_int(gpu_config, section, "IssueLatency",
                                            si_gpu_fe_issue_latency);
  ...
  si_gpu_fe_issue_width =
      config_read_int(gpu_config, section, "IssueWidth", si_gpu_fe_issue_width);
  if (si_gpu_fe_issue_width < 1)
    fatal("%s: invalid value for 'IssueWidth'.\n%s", si_gpu_config_file_name,
          err_note);

  si_gpu_fe_max_inst_issued_per_type =
      config_read_int(gpu_config, section, "MaxInstIssuedPerType",
                      si_gpu_fe_max_inst_issued_per_type);
  ...
  si_gpu_simd_width =
      config_read_int(gpu_config, section, "Width", si_gpu_simd_width);
  ...
  si_gpu_simd_decode_buffer_size = config_read_int(
      gpu_config, section, "DecodeBufferSize", si_gpu_simd_decode_buffer_size);
  ...
  si_gpu_scalar_unit_width =
      config_read_int(gpu_config, section, "Width", si_gpu_scalar_unit_width);
  ...
  si_gpu_scalar_unit_decode_latency = config_read_int(
      gpu_config, section, "DecodeLatency", si_gpu_scalar_unit_decode_latency);
  ...
  si_gpu_branch_unit_width =
      config_read_int(gpu_config, section, "Width", si_gpu_branch_unit_width);
  ...
  si_gpu_branch_unit_decode_latency = config_read_int(
      gpu_config, section, "DecodeLatency", si_gpu_branch_unit_decode_latency);
  ...
  si_gpu_lds_width =
      config_read_int(gpu_config, section, "Width", si_gpu_lds_width);
  ...
  si_gpu_lds_read_latency = config_read_int(gpu_config, section, "ReadLatency",
                                            si_gpu_lds_read_latency);
  ...
  si_gpu_vector_mem_width =
      config_read_int(gpu_config, section, "Width", si_gpu_vector_mem_width);
  ...
  si_gpu_vector_mem_decode_latency = config_read_int(
      gpu_config, section, "DecodeLatency", si_gpu_vector_mem_decode_latency);
  ...
  si_gpu_lds_size =
      config_read_int(gpu_config, section, "Size", si_gpu_lds_size);
  si_gpu_lds_alloc_size =
      config_read_int(gpu_config, section, "AllocSize", si_gpu_lds_alloc_size);
  si_gpu_lds_block_size =
      config_read_int(gpu_config, section, "BlockSize", si_gpu_lds_block_size);
  si_gpu_lds_latency =
      config_read_int(gpu_config, section, "Latency", si_gpu_lds_latency);
  si_gpu_lds_num_ports =
      config_read_int(gpu_config, section, "Ports", si_gpu_lds_num_ports);
...
void si_gpu_init(void) {
  ...
  if (si_gpu_report_file_name[0] &&
    ...
    fatal("%s: cannot open GPU pipeline report file", si_gpu_report_file_name);
...
void si_gpu_done(void) {
  ...
  /* Spatial report */
  if (si_spatial_report_active) si_cu_spatial_report_done();
...
void si_gpu_dump_report(void) {
  ...
  inst_per_cycle =
      asTiming(si_gpu)->cycle
          ? (double)(asEmu(si_emu)->instructions / asTiming(si_gpu)->cycle)
          : 0.0;
  fprintf(f, "[ Device ]\n\n");
...
int SIGpuRun(Timing *self) {
  ...
  while (list_count(gpu->available_compute_units) &&
    ...
    si_compute_unit_map_work_group(list_dequeue(gpu->available_compute_units),
                                   work_group);
  }
  ...
  if (si_emu_max_inst && asEmu(si_emu)->instructions >= si_emu_max_inst) {
  ...
  if (!list_count(si_emu->waiting_work_groups)) opencl_si_request_work();
