...
struct si_compute_unit_t *si_compute_unit_create() {
  ...
  compute_unit->lds_module =
      mod_create(buf, mod_kind_local_memory, si_gpu_lds_num_ports,
                 si_gpu_lds_block_size, si_gpu_lds_latency);
  ...
  compute_unit->fetch_buffers =
      xcalloc(compute_unit->num_wavefront_pools, sizeof(struct list_t *));
  compute_unit->simd_units =
      xcalloc(compute_unit->num_wavefront_pools, sizeof(struct si_simd_t *));
  ...
  for (i = 0; i < compute_unit->num_wavefront_pools; i++) {
    ...
    compute_unit->simd_units[i] = xcalloc(1, sizeof(struct si_simd_t));
    ...
    compute_unit->simd_units[i]->compute_unit = compute_unit;
    compute_unit->simd_units[i]->wkg_util =
        xcalloc(1, sizeof(struct si_util_t));
    compute_unit->simd_units[i]->wvf_util =
        xcalloc(1, sizeof(struct si_util_t));
    compute_unit->simd_units[i]->rdy_util =
        xcalloc(1, sizeof(struct si_util_t));
    compute_unit->simd_units[i]->occ_util =
        xcalloc(1, sizeof(struct si_util_t));
    compute_unit->simd_units[i]->wki_util =
        xcalloc(1, sizeof(struct si_util_t));
    compute_unit->simd_units[i]->act_util =
        xcalloc(1, sizeof(struct si_util_t));
    compute_unit->simd_units[i]->tot_util =
        xcalloc(1, sizeof(struct si_util_t));
  }

  compute_unit->work_groups = xcalloc(
      si_gpu_max_work_groups_per_wavefront_pool * si_gpu_num_wavefront_pools,
      sizeof(void *));
...
void si_compute_unit_map_work_group(struct si_compute_unit_t *compute_unit,
  ...
  SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id) {
    ...
    wavefront->id_in_compute_unit =
        work_group->id_in_compute_unit * work_group->wavefront_count +
        wavefront_id;
  }
  ...
  if (si_spatial_report_active) si_report_mapped_work_group(compute_unit);
}
...
void si_compute_unit_unmap_work_group(struct si_compute_unit_t *compute_unit,
  ...
  assert(list_index_of(si_emu->running_work_groups, (void *)work_group_id) >=
         0);
  ...
  if (si_spatial_report_active) si_report_unmapped_work_group(compute_unit);
...
void si_compute_unit_fetch(struct si_compute_unit_t *compute_unit,
  ...
  for (i = 0; i < si_gpu_max_wavefronts_per_wavefront_pool; i++) {
    wavefront = compute_unit->wavefront_pools[active_fb]->entries[i]->wavefront;
    ...
    if (instructions_processed == si_gpu_fe_fetch_width) continue;

    /* Wavefront isn't ready (previous instruction is still
     * in flight) */
    if (!wavefront->wavefront_pool_entry->ready) continue;
    ...
    if (wavefront->finished) continue;
    ...
    if (si_tracing()) {
      si_inst_dump(&wavefront->inst, wavefront->inst_size, wavefront->pc,
                   wavefront->work_group->ndrange->inst_buffer + wavefront->pc,
      ...
      si_trace(
          ...
          uop->id_in_compute_unit, compute_unit->id, uop->wavefront_pool_id,
          uop->work_group->id, wavefront->id, uop->id_in_wavefront,
          inst_str_trimmed);
    }
    ...
    SI_FOREACH_WORK_ITEM_IN_WAVEFRONT(uop->wavefront, work_item_id) {
      ...
      /* LDS */
      work_item_uop->lds_access_count = work_item->lds_access_count;
      for (j = 0; j < work_item->lds_access_count; j++) {
        work_item_uop->lds_access_kind[j] = work_item->lds_access_type[j];
        work_item_uop->lds_access_addr[j] = work_item->lds_access_addr[j];
        work_item_uop->lds_access_size[j] = work_item->lds_access_size[j];
      }
    }
...
void si_compute_unit_issue_oldest(struct si_compute_unit_t *compute_unit,
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
        ...
        oldest_uop->issue_ready =
            asTiming(si_gpu)->cycle + si_gpu_fe_issue_latency;
        list_remove(compute_unit->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(compute_unit->branch_unit.issue_buffer, oldest_uop);
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
        ...
        oldest_uop->issue_ready =
            asTiming(si_gpu)->cycle + si_gpu_fe_issue_latency;
        list_remove(compute_unit->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(compute_unit->scalar_unit.issue_buffer, oldest_uop);
  ...
  for (issued_insts = 0; issued_insts < si_gpu_fe_max_inst_issued_per_type;
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
          list_count(compute_unit->simd_units[active_fb]->issue_buffer) <
              si_gpu_simd_issue_buffer_size) {
        oldest_uop->issue_ready =
            asTiming(si_gpu)->cycle + si_gpu_fe_issue_latency;
        list_remove(compute_unit->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(compute_unit->simd_units[active_fb]->issue_buffer,
                     oldest_uop);
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
        ...
        oldest_uop->issue_ready =
            asTiming(si_gpu)->cycle + si_gpu_fe_issue_latency;
        list_remove(compute_unit->fetch_buffers[active_fb], oldest_uop);
        list_enqueue(compute_unit->vector_mem_unit.issue_buffer, oldest_uop);
  ...
  for (issued_insts = 0; issued_insts < si_gpu_fe_max_inst_issued_per_type;
    ...
    for (i = 0; i < list_entries; i++) {
      ...
      if (oldest_uop &&
          list_count(compute_unit->lds_unit.issue_buffer) <
    ...
    switch (uop->inst.info->fmt) {
      ...
      case SI_FMT_SOPP: {
        ...
        if (uop->inst.micro_inst.sopp.op > 1 &&
           ...
           * already issued */
          assert(branch_insts_issued <= si_gpu_fe_max_inst_issued_per_type);
          ...
          uop->issue_ready = asTiming(si_gpu)->cycle + si_gpu_fe_issue_latency;
      ...
      case SI_FMT_VOP3b: {
        ...
        if (list_count(compute_unit->simd_units[active_fb]->issue_buffer) ==
            si_gpu_simd_issue_buffer_size) {
          si_trace(
              "si.inst id=%lld cu=%d wf=%d "
        ...
        uop->issue_ready = asTiming(si_gpu)->cycle + si_gpu_fe_issue_latency;
        list_remove(compute_unit->fetch_buffers[active_fb], uop);
        list_enqueue(compute_unit->simd_units[active_fb]->issue_buffer, uop);
...
void si_compute_unit_run(struct si_compute_unit_t *compute_unit) {
  ...
  for (i = 0; i < num_simd_units; i++) si_simd_run(compute_unit->simd_units[i]);
  ...
  /* Fetch */
  for (i = 0; i < num_simd_units; i++) si_compute_unit_fetch(compute_unit, i);

  /* Stats */
  compute_unit->cycle++;

  if (si_spatial_report_active) si_cu_interval_update(compute_unit);
}
