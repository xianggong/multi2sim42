  ...
  for (i = 0; i < list_entries; i++) {
    ...
    if (instructions_processed > si_gpu_lds_width) {
      si_trace(
          ...
          "stg=\"s_lds_wr_wth\"\n",
          uop->id_in_compute_unit, lds->compute_unit->id, uop->wavefront->id,
          uop->id_in_wavefront);
      list_index++;
      continue;
    }
    ...
    /* Sanity check the write buffer */
    assert(list_count(lds->write_buffer) <= si_gpu_lds_write_buffer_size);

    /* Stop if the write buffer is full */
    if (list_count(lds->write_buffer) >= si_gpu_lds_write_buffer_size) {
      si_trace(
          "si.inst id=%lld cu=%d wf=%d uop_id=%lld "
          "stg=\"s_lds_wr_buf\"\n",
          uop->id_in_compute_unit, lds->compute_unit->id, uop->wavefront->id,
...
void si_lds_mem(struct si_lds_t *lds) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    assert(list_count(lds->mem_buffer) <= si_gpu_lds_max_inflight_mem_accesses);
    ...
    SI_FOREACH_WORK_ITEM_IN_WAVEFRONT(uop->wavefront, work_item_id) {
      ...
      for (j = 0; j < work_item_uop->lds_access_count; j++) {
        if (work_item->lds_access_type[j] == 1) {
          ...
          fatal(
              "%s: invalid lds access "
              "type (%d)",
              __FUNCTION__, work_item->lds_access_type[j]);
        }

        mod_access(lds->compute_unit->lds_module, access_type,
                   work_item_uop->lds_access_addr[j], &uop->lds_witness, NULL,
                   NULL, NULL);
        uop->lds_witness--;
      }
    }
...
void si_lds_decode(struct si_lds_t *lds) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    /* Sanity check the decode buffer */
    assert(list_count(lds->decode_buffer) <= si_gpu_lds_decode_buffer_size);

    /* Stall if the decode buffer is full. */
    if (list_count(lds->decode_buffer) == si_gpu_lds_decode_buffer_size) {
      si_trace(
          "si.inst id=%lld cu=%d wf=%d uop_id=%lld "
          "stg=\"s_lds_dec_buf\"\n",
          uop->id_in_compute_unit, lds->compute_unit->id, uop->wavefront->id,
    ...
    if (si_spatial_report_active) si_lds_report_new_inst(lds->compute_unit);
