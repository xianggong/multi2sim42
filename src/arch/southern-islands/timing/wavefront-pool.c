...
struct si_wavefront_pool_t *si_wavefront_pool_create() {
  ...
  for (i = 0; i < si_gpu_max_wavefronts_per_wavefront_pool; i++) {
    wavefront_pool->entries[i] =
        xcalloc(1, sizeof(struct si_wavefront_pool_entry_t));
...
void si_wavefront_pool_unmap_wavefronts(
  ...
  assert(wavefront_pool->wavefront_count >= work_group->wavefront_count);

  for (i = 0; i < work_group->wavefront_count; i++) {
    wavefront = work_group->wavefronts[i];
    wf_id_in_wfp = wavefront->wavefront_pool_entry->id_in_wavefront_pool;
    ...
    si_wavefront_pool_entry_clear(wavefront_pool->entries[wf_id_in_wfp]);
  }
