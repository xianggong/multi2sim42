...
void SIGpuMemConfigDefault(Timing *self, struct config_t *config) {
  ...
  for (i = 0; i < (si_gpu_num_compute_units + 3) / 4; i++) {
    ...
    config_write_string(config, section, "Geometry", "si-geo-scalar-l1");
  ...
  for (i = 0; i < si_gpu_num_compute_units; i++) {
    ...
    config_write_string(config, section, "Type", "Cache");
    config_write_string(config, section, "Geometry", "si-geo-vector-l1");
    config_write_string(config, section, "LowNetwork", "si-net-l1-l2");
    config_write_string(config, section, "LowModules",
                        "si-l2-0 si-l2-1 si-l2-2 si-l2-3 si-l2-4 si-l2-5");
  }
...
void SIGpuMemConfigParseEntry(Timing *self, struct config_t *config,
  ...
  separate_present = config_var_exists(config, section, "DataModule") &&
                     config_var_exists(config, section, "ConstantDataModule");
  ...
  if (compute_unit_id < 0) {
    fatal(
        "%s: section [%s]: invalid or missing value for "
        "'ComputeUnit'",
        file_name, section);
  }
  ...
  if (separate_present) {
    vector_module_name =
        config_read_string(config, section, "DataModule", NULL);
    scalar_module_name =
        config_read_string(config, section, "ConstantDataModule", NULL);
  } else {
