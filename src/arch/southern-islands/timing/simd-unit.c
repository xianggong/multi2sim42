  ...
  for (i = 0; i < list_entries; i++) {
    ...
    if (instructions_processed > si_gpu_simd_width) {
      si_trace(
          ...
          "stg=\"s_simd_exe_wth\"\n",
          uop->id_in_compute_unit, simd->compute_unit->id, uop->wavefront->id,
          uop->id_in_wavefront);
      list_index++;
      continue;
    }
    ...
    assert(list_count(simd->exec_buffer) <= si_gpu_simd_exec_buffer_size);

    /* Stall if SIMD unit is full */
    if (list_count(simd->exec_buffer) == si_gpu_simd_exec_buffer_size) {
      si_trace(
          "si.inst id=%lld cu=%d wf=%d uop_id=%lld "
          "stg=\"s_simd_exe_buf\"\n",
          uop->id_in_compute_unit, simd->compute_unit->id, uop->wavefront->id,
     ...
     * all subwavefronts */
    uop->execute_ready = asTiming(si_gpu)->cycle + si_gpu_simd_exec_latency;
...
void si_simd_decode(struct si_simd_t *simd) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    /* Sanity check the decode buffer */
    assert(list_count(simd->decode_buffer) <= si_gpu_simd_decode_buffer_size);

    /* Stall if the decode buffer is full. */
    if (list_count(simd->decode_buffer) == si_gpu_simd_decode_buffer_size) {
      si_trace(
          "si.inst id=%lld cu=%d wf=%d uop_id=%lld "
          "stg=\"s_simd_dec_buf\"\n",
          uop->id_in_compute_unit, simd->compute_unit->id, uop->wavefront->id,
    ...
    if (si_spatial_report_active) si_alu_report_new_inst(simd->compute_unit);
