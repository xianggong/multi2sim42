...
void si_scalar_unit_complete(struct si_scalar_unit_t *scalar_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    if (uop->barrier_wait_inst) {
      ...
      barrier_complete = 1;
      SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(uop->work_group, wavefront_id) {
      ...
      if (barrier_complete) {
        SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(uop->work_group, wavefront_id) {
          assert(uop->work_group->wavefronts[wavefront_id]
                     ->wavefront_pool_entry->wait_for_barrier);
          uop->work_group->wavefronts[wavefront_id]
...
void si_scalar_unit_write(struct si_scalar_unit_t *scalar_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    if (uop->scalar_mem_read) {
      ...
      uop->write_ready =
          asTiming(si_gpu)->cycle + si_gpu_scalar_unit_write_latency;
...
void si_scalar_unit_execute(struct si_scalar_unit_t *scalar_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    if (uop->scalar_mem_read) {
      ...
      mod_access(scalar_unit->compute_unit->scalar_cache, mod_access_load,
                 uop->global_mem_access_addr, &uop->global_mem_witness, NULL,
                 NULL, NULL);
    ...
    {
      uop->execute_ready =
          asTiming(si_gpu)->cycle + si_gpu_scalar_unit_exec_latency;
...
void si_scalar_unit_read(struct si_scalar_unit_t *scalar_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->read_ready = asTiming(si_gpu)->cycle + si_gpu_scalar_unit_read_latency;
...
void si_scalar_unit_decode(struct si_scalar_unit_t *scalar_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->decode_ready =
        asTiming(si_gpu)->cycle + si_gpu_scalar_unit_decode_latency;
