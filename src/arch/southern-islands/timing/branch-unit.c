...
void si_branch_unit_complete(struct si_branch_unit_t *branch_unit) {
  ...
  assert(list_entries <=
         si_gpu_branch_unit_write_latency * si_gpu_branch_unit_width);
...
void si_branch_unit_write(struct si_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->write_ready =
        asTiming(si_gpu)->cycle + si_gpu_branch_unit_write_latency;
...
void si_branch_unit_execute(struct si_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    /* Branch */
    uop->execute_ready =
        asTiming(si_gpu)->cycle + si_gpu_branch_unit_exec_latency;
...
void si_branch_unit_read(struct si_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->read_ready = asTiming(si_gpu)->cycle + si_gpu_branch_unit_read_latency;
...
void si_branch_unit_decode(struct si_branch_unit_t *branch_unit) {
  ...
  for (i = 0; i < list_entries; i++) {
    ...
    uop->decode_ready =
        asTiming(si_gpu)->cycle + si_gpu_branch_unit_decode_latency;
