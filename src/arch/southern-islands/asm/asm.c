...
static struct si_inst_info_t *
    si_inst_info_sopp[SI_INST_INFO_SOPP_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_sopc[SI_INST_INFO_SOPC_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_sop1[SI_INST_INFO_SOP1_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_sopk[SI_INST_INFO_SOPK_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_sop2[SI_INST_INFO_SOP2_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_smrd[SI_INST_INFO_SMRD_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_vop3[SI_INST_INFO_VOP3_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_vopc[SI_INST_INFO_VOPC_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_vop1[SI_INST_INFO_VOP1_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_vop2[SI_INST_INFO_VOP2_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_vintrp[SI_INST_INFO_VINTRP_MAX_VALUE + 1];
static struct si_inst_info_t *si_inst_info_ds[SI_INST_INFO_DS_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_mtbuf[SI_INST_INFO_MTBUF_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_mubuf[SI_INST_INFO_MUBUF_MAX_VALUE + 1];
static struct si_inst_info_t *
    si_inst_info_mimg[SI_INST_INFO_MIMG_MAX_VALUE + 1];
static struct si_inst_info_t *si_inst_info_exp[SI_INST_INFO_EXP_MAX_VALUE + 1];

struct str_map_t si_inst_fmt_map = {18,
                                    {{"<invalid>", SI_FMT_NONE},
                                     {"sop2", SI_FMT_SOP2},
                                     {"sopk", SI_FMT_SOPK},
                                     {"sop1", SI_FMT_SOP1},
...
struct str_map_t si_inst_sdst_map = {24,
                                     {{"reserved", 0},
                                      {"reserved", 1},
                                      {"vcc_lo", 2},
                                      {"vcc_hi", 3},
...
struct str_map_t si_inst_ssrc_map = {16,
                                     {{"0.5", 0},
                                      {"-0.5", 1},
                                      {"1.0", 2},
                                      {"-1.0", 3},
...
struct str_map_t si_inst_OP16_map = {16,
                                     {
                                      {"f", 0},
                                      {"lt", 1},
                                      {"eq", 2},
...
struct str_map_t si_inst_OP8_map = {8,
                                    {
                                     {"f", 0},
                                     {"lt", 1},
                                     {"eq", 2},
...
struct str_map_t si_inst_special_reg_map = {4,
...
int si_inst_decode(void *buf, struct si_inst_t *inst, unsigned int offset) {
  ...
  if (inst->micro_inst.sopp.enc == 0x17F) {
    ...
    if (!si_inst_info_exp[0]) fatal("Unimplemented Instruction: EXP\n");
...
void si_disasm_buffer(struct elf_buffer_t *buffer, FILE *f) {
  ...
  int label_addr[buffer->size /
                 4]; /* A list of created labels sorted by rel_addr. */
  ...
  while (inst_buf) {
    ...
    si_inst_dump(&inst, inst_size, rel_addr, inst_buf, line, line_size);
    fprintf(f, " %s", line);
...
void operand_dump_series(char *str, int operand, int operand_end) {
  ...
  if (operand <= 103) {
    ...
    if (operand >= 112 && operand <= 123) {
      ...
      str_printf(&pstr, &str_size, "ttmp[%d:%d]", operand - 112,
                 operand_end - 112);
    } else {
...
void operand_dump_scalar(char *str, int operand) { operand_dump(str, operand); }
...
void line_dump(char *inst_str, unsigned int rel_addr, void *buf, char *line,
               int line_size, int inst_size) {
  ...
  if (inst_size == 4) {
    ...
    str_printf(&dat_str, &dat_str_size, "// %08X: %08X %08X", rel_addr,
...
void si_inst_VOP3_64_SRC_dump(struct si_inst_t *inst, unsigned int src, int neg,
                              char *operand_str, char **inst_str,
                              int str_size) {
...
void si_inst_MADDR_dump(struct si_inst_t *inst, char *operand_str,
  ...
  /* offen */
  if (inst->micro_inst.mtbuf.offen) str_printf(inst_str, &str_size, " offen");

  /* index */
  if (inst->micro_inst.mtbuf.idxen) str_printf(inst_str, &str_size, " idxen");
...
void si_inst_DUG_dump(struct si_inst_t *inst, char *operand_str,
  ...
  /* UNORM */
  if (inst->micro_inst.mimg.unorm) str_printf(inst_str, &str_size, " unorm");

  /* GLC */
  if (inst->micro_inst.mimg.glc) str_printf(inst_str, &str_size, " glc");
}
...
void si_inst_dump(struct si_inst_t *inst, unsigned int inst_size,
  ...
  while (*fmt_str) {
    ...
    if (is_token(fmt_str, "WAIT_CNT", &token_len)) {
      ...
      si_inst_SSRC_dump(inst, inst->micro_inst.sop2.ssrc0, operand_str,
                        &inst_str, str_size);
    } else if (is_token(fmt_str, "64_SSRC0", &token_len)) {
      si_inst_64_SSRC_dump(inst, inst->micro_inst.sop2.ssrc0, operand_str,
                           &inst_str, str_size);
    } else if (is_token(fmt_str, "SSRC1", &token_len)) {
      si_inst_SSRC_dump(inst, inst->micro_inst.sop2.ssrc1, operand_str,
                        &inst_str, str_size);
    } else if (is_token(fmt_str, "64_SSRC1", &token_len)) {
      si_inst_64_SSRC_dump(inst, inst->micro_inst.sop2.ssrc1, operand_str,
                           &inst_str, str_size);
      ...
      operand_dump_series_scalar(operand_str, inst->micro_inst.sop2.sdst,
      ...
      operand_dump_series(operand_str, inst->micro_inst.vopc.src0,
      ...
      operand_dump_series_vector(operand_str, inst->micro_inst.vopc.vsrc1,
      ...
      operand_dump_series_vector(operand_str, inst->micro_inst.vop1.vdst,
      ...
      operand_dump_series_scalar(operand_str, inst->micro_inst.vop3a.vdst,
      ...
      operand_dump_series_vector(operand_str, inst->micro_inst.vop3a.vdst,
                                 inst->micro_inst.vop3a.vdst + 1);
      str_printf(&inst_str, &str_size, "%s", operand_str);
    } else if (is_token(fmt_str, "VOP3_64_SDST", &token_len)) {
      operand_dump_series_scalar(operand_str, inst->micro_inst.vop3b.sdst,
                                 inst->micro_inst.vop3b.sdst + 1);
      str_printf(&inst_str, &str_size, "%s", operand_str);
    } else if (is_token(fmt_str, "VOP3_SRC0", &token_len)) {
      si_inst_VOP3_SRC_dump(inst, inst->micro_inst.vop3a.src0, 1, operand_str,
                            &inst_str, str_size);
    } else if (is_token(fmt_str, "VOP3_64_SRC0", &token_len)) {
      si_inst_VOP3_64_SRC_dump(inst, inst->micro_inst.vop3a.src0, 1,
                               operand_str, &inst_str, str_size);
    } else if (is_token(fmt_str, "VOP3_SRC1", &token_len)) {
      si_inst_VOP3_SRC_dump(inst, inst->micro_inst.vop3a.src1, 2, operand_str,
                            &inst_str, str_size);
    } else if (is_token(fmt_str, "VOP3_64_SRC1", &token_len)) {
      si_inst_VOP3_64_SRC_dump(inst, inst->micro_inst.vop3a.src1, 2,
                               operand_str, &inst_str, str_size);
    } else if (is_token(fmt_str, "VOP3_SRC2", &token_len)) {
      si_inst_VOP3_SRC_dump(inst, inst->micro_inst.vop3a.src2, 4, operand_str,
                            &inst_str, str_size);
    } else if (is_token(fmt_str, "VOP3_64_SRC2", &token_len)) {
      si_inst_VOP3_64_SRC_dump(inst, inst->micro_inst.vop3a.src2, 4,
                               operand_str, &inst_str, str_size);
    } else if (is_token(fmt_str, "VOP3_OP16", &token_len)) {
      str_printf(
          &inst_str, &str_size, "%s",
          str_map_value(&si_inst_OP16_map, (inst->micro_inst.vop3a.op & 15)));
    } else if (is_token(fmt_str, "VOP3_OP8", &token_len)) {
      str_printf(
          &inst_str, &str_size, "%s",
          str_map_value(&si_inst_OP8_map, (inst->micro_inst.vop3a.op & 15)));
      ...
      /* S_MEMTIME */
        ...
        fatal(
            "S_MEMTIME instruction not currently"
            "supported");
      }
      /* S_DCACHE_INV */
      else if (op == 31) {
        fatal(
            "S_DCACHE_INV instruction not"
            "currently supported");
      } else {
      ...
      /* S_DCACHE_INV */
        ...
        fatal(
            "S_DCACHE_INV instruction not currently"
            " supported");
      } else {
        fatal("Invalid smrd opcode");
      }
      ...
      operand_dump_vector(operand_str, inst->micro_inst.ds.data1);
      str_printf(&inst_str, &str_size, "%s", operand_str);
      ...
      operand_dump_series_vector(operand_str, inst->micro_inst.ds.vdst,
      ...
      if (inst->micro_inst.mubuf.glc) str_printf(&inst_str, &str_size, "glc");
    } else if (is_token(fmt_str, "VADDR", &token_len)) {
      if (inst->micro_inst.mtbuf.offen && inst->micro_inst.mtbuf.idxen) {
        operand_dump_series_vector(operand_str, inst->micro_inst.mtbuf.vaddr,
      ...
      operand_dump_series_scalar(operand_str, inst->micro_inst.mtbuf.srsrc << 2,
      ...
      operand_dump_series_vector(operand_str, inst->micro_inst.mimg.vdata,
                                 inst->micro_inst.mimg.vdata + 3);
      str_printf(&inst_str, &str_size, "%s", operand_str);
    } else if (is_token(fmt_str, "MIMG_VADDR", &token_len)) {
      operand_dump_series_vector(operand_str, inst->micro_inst.mimg.vaddr,
                                 inst->micro_inst.mimg.vaddr + 3);
      str_printf(&inst_str, &str_size, "%s", operand_str);
    } else if (is_token(fmt_str, "MIMG_SERIES_SRSRC", &token_len)) {
      assert((inst->micro_inst.mimg.srsrc << 2) % 4 == 0);
      operand_dump_series_scalar(operand_str, inst->micro_inst.mimg.srsrc << 2,
      ...
      operand_dump_series_scalar(operand_str, inst->micro_inst.mimg.ssamp << 2,
...
void si_disasm(char *path) {
  ...
  for (i = 0; i < list_count(elf_file->symbol_table); i++) {
    ...
    if (str_prefix(symbol->name, "__OpenCL_") &&
      ...
      /* Decode internal ELF */
      str_substr(kernel_name, sizeof(kernel_name), symbol->name, 9,
                 strlen(symbol->name) - 16);
      amd_bin = si_bin_file_create(section->buffer.ptr + symbol->value,
                                   symbol->size, kernel_name);

      /* Get kernel name */
      printf("**\n** Disassembly for '__kernel %s'\n**\n\n", kernel_name);
      si_disasm_buffer(
          &amd_bin->enc_dict_entry_southern_islands->sec_text_buffer, stdout);
      printf("\n\n\n");
