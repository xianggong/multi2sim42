...
struct str_map_t si_arg_dimension_map = {2, {{"2D", 2}, {"3D", 3}}};

struct str_map_t si_arg_access_type_map = {3,
                                           {{"RO", si_arg_read_only},
                                            {"WO", si_arg_write_only},
                                            {"RW", si_arg_read_write}}};

struct str_map_t si_arg_data_type_map = {16,
                                         {{"i1", si_arg_i1},
                                          {"i8", si_arg_i8},
                                          {"i16", si_arg_i16},
                                          {"i32", si_arg_i32},
...
struct str_map_t si_arg_scope_map = {10,
                                     {{"g", si_arg_global},
...
struct str_map_t si_arg_reflection_map = {14,
                                          {{"char", si_arg_i8},
                                           {"short", si_arg_i16},
                                           {"int", si_arg_i32},
                                           {"long", si_arg_i64},
...
struct si_arg_t *si_arg_create(enum si_arg_type_t type, char *name) {
  struct si_arg_t *arg;
...
void si_arg_free(struct si_arg_t *arg) {
  ...
  switch (arg->type) {
    case si_arg_value:
      ...
      if (arg->value.value_ptr) free(arg->value.value_ptr);
      break;
...
int si_arg_get_data_size(enum si_arg_data_type_t data_type) {
  switch (data_type) {
    ...
    default:
      ...
      panic("%s: invalid data type (%d)", __FUNCTION__, data_type);
      return 0;
  }
}
