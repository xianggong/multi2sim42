...
struct si_input_t *si_input_create() {
  ...
  /* NOTE:  the data type input expected is configured by runtime call
...
void si_input_free(struct si_input_t *input) { free(input); }

void si_input_set_usage_index(struct si_input_t *input,
                              unsigned int usage_index) {
  if (input) input->usage_index = usage_index;
}
