...
void mips_asm_init() {
  ...
  mips_asm_table[MIPS_OPCODE_SPECIAL].next_table = mips_asm_table_special;
  ...
  mips_asm_table[MIPS_OPCODE_COP1].next_table = mips_asm_table_cop1;
  ...
  mips_asm_table_cop1[MIPS_OPCODE_COP1_S].next_table = mips_asm_table_cop1_s;
  ...
  mips_asm_table_cop1[MIPS_OPCODE_COP1_D].next_table = mips_asm_table_cop1_d;
  ...
  mips_asm_table[MIPS_OPCODE_SPECIAL2].next_table = mips_asm_table_special2;
  mips_asm_table[MIPS_OPCODE_SPECIAL2].next_table_low = 0;
  mips_asm_table[MIPS_OPCODE_SPECIAL2].next_table_high = 5;

  mips_asm_table[MIPS_OPCODE_SPECIAL3].next_table = mips_asm_table_special3;
  ...
  if (fmt_str) {
    while (*fmt_str) {
      ...
      if (mips_token_comp(fmt_str, "sll", &token_len)) {
        if ((rd | rt | sa) == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "nop");
          break;
        }
      }
      /* ADDU RD, RS, ZERO => MOVE RD, RS */
        ...
        if (rt == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "move\t");
          mips_inst_dump_RD(inst_str_ptr, &inst_str_size, &inst);
          str_printf(inst_str_ptr, &inst_str_size, ",");
          mips_inst_dump_RS(inst_str_ptr, &inst_str_size, &inst);
          break;
        }
      }
      /* BGEZAL ZERO, OFFSET => BAL OFFSET */
      else if (mips_token_comp(fmt_str, "bgezal", &token_len)) {
        if (rs == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "bal\t");
          mips_inst_dump_OFFSETBR(inst_str_ptr, &inst_str_size, &inst,
      ...
      /* BEQ RS, ZERO, OFFSET => BEQZ RS, OFFSET */
        ...
        if ((rs | rt) == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "b\t");
          mips_inst_dump_OFFSETBR(inst_str_ptr, &inst_str_size, &inst,
                                  inst_addr, print_symbol_address);
          break;
        } else if (rt == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "beqz\t");
          mips_inst_dump_RS(inst_str_ptr, &inst_str_size, &inst);
          str_printf(inst_str_ptr, &inst_str_size, ",");
          mips_inst_dump_OFFSETBR(inst_str_ptr, &inst_str_size, &inst,
                                  inst_addr, print_symbol_address);
      ...
      else if (mips_token_comp(fmt_str, "addiu", &token_len)) {
        if (rs == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "li\t");
          mips_inst_dump_RT(inst_str_ptr, &inst_str_size, &inst);
          str_printf(inst_str_ptr, &inst_str_size, ",");
          mips_inst_dump_IMM(inst_str_ptr, &inst_str_size, &inst);
          break;
        }
      }
      /* ORI RT, ZERO, IMM => LI RT, IMM */
        ...
        if (rs == 0) {
          ...
          mips_inst_dump_RT(inst_str_ptr, &inst_str_size, &inst);
          str_printf(inst_str_ptr, &inst_str_size, ",");
          mips_inst_dump_IMMHEX(inst_str_ptr, &inst_str_size, &inst);
          break;
        }
      }
      /* BNE RS, ZERO, OFFSET => BNEZ RS, OFFSET */
        ...
        if (rt == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "bnez\t");
          mips_inst_dump_RS(inst_str_ptr, &inst_str_size, &inst);
          str_printf(inst_str_ptr, &inst_str_size, ",");
          mips_inst_dump_OFFSETBR(inst_str_ptr, &inst_str_size, &inst,
                                  inst_addr, print_symbol_address);
      ...
      /* SUBU RD, ZERO, RT => NEGU RD, RT */
        ...
        if (rs == 0) {
          str_printf(inst_str_ptr, &inst_str_size, "negu\t");
          mips_inst_dump_RD(inst_str_ptr, &inst_str_size, &inst);
          str_printf(inst_str_ptr, &inst_str_size, ",");
          mips_inst_dump_RT(inst_str_ptr, &inst_str_size, &inst);
          break;
      ...
      if (i == 0) str_printf(inst_str_ptr, &inst_str_size, "\t");
      i = 1;
      ++fmt_str;
      if (mips_token_comp(fmt_str, "sa", &token_len))
        mips_inst_dump_SA(inst_str_ptr, &inst_str_size, &inst);
        ...
        mips_inst_dump_RS(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "target", &token_len)) {
        mips_inst_dump_TARGET(inst_str_ptr, &inst_str_size, &inst,
                              print_symbol_address);
      } else if (mips_token_comp(fmt_str, "offset", &token_len))
        mips_inst_dump_OFFSET(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "offsetbr", &token_len)) {
        mips_inst_dump_OFFSETBR(inst_str_ptr, &inst_str_size, &inst, inst_addr,
                                print_symbol_address);
      } else if (mips_token_comp(fmt_str, "Imm", &token_len))
        mips_inst_dump_IMM(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "Immhex", &token_len))
        mips_inst_dump_IMMHEX(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "base", &token_len))
        mips_inst_dump_BASE(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "sel", &token_len))
        mips_inst_dump_SEL(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "cc", &token_len))
        mips_inst_dump_CC(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "pos", &token_len))
        mips_inst_dump_POS(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "size", &token_len))
        mips_inst_dump_SIZE(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "fs", &token_len))
        mips_inst_dump_FS(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "ft", &token_len))
        mips_inst_dump_FT(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "fd", &token_len))
        mips_inst_dump_FD(inst_str_ptr, &inst_str_size, &inst);
      else if (mips_token_comp(fmt_str, "code", &token_len))
        mips_inst_dump_CODE(inst_str_ptr, &inst_str_size, &inst);
      else
...
void mips_inst_dump_RD(char **inst_str_ptr, int *inst_str_size,
  ...
  if (mips_token_comp(inst->info->name, "RDHWR", &token_len))
    ...
    if (rd == (int)31) {
      ...
      str_printf(inst_str_ptr, inst_str_size, ",");
      str_printf(inst_str_ptr, inst_str_size, "%s", mips_reg_name[rd]);
    }
  } else
    str_printf(inst_str_ptr, inst_str_size, "%s", mips_reg_name[rd]);
}
...
void mips_emu_disasm(char *path) {
  ...
  for (i = 0; i < list_count(elf_file->section_list); i++) {
    ...
    for (inst_ptr = section->buffer.ptr;
         inst_ptr < section->buffer.ptr + section->buffer.size; inst_ptr += 4) {
      /* Symbol */
      while (symbol &&
             symbol->value < (section->header->sh_addr + section->buffer.pos)) {
        curr_sym++;
        symbol = list_get(elf_file->symbol_table, curr_sym);
      }
      if (symbol &&
          symbol->value == (section->header->sh_addr + section->buffer.pos))
        printf("\n\n%08x <%s>:", section->header->sh_addr + section->buffer.pos,
               symbol->name);
...
void mips_inst_debug_dump(struct mips_inst_t *inst, FILE *f) {
  ...
  mips_inst_dump(f, inst_str, MAX_STRING_SIZE, inst_ptr, inst->addr, inst->addr,
                 &print_symbol_address);
}
