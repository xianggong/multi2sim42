...
static void mips_ctx_loader_add_args_vector(struct mips_ctx_t *ctx, int argc,
                                            char **argv) {
  char *arg;
  int i;
...
static void mips_ctx_loader_load_sections(struct mips_ctx_t *ctx,
...
void mips_ctx_loader_load_exe(struct mips_ctx_t *ctx, char *exe) {
  ...
  /* Alternative stdin */
  mips_ctx_loader_get_full_path(ctx, ctx->stdin_file, stdin_file_full_path,
                                MAX_STRING_SIZE);
  ...
  mips_ctx_loader_get_full_path(ctx, ctx->stdout_file, stdout_file_full_path,
                                MAX_STRING_SIZE);
...
void mips_ctx_execute(struct mips_ctx_t *ctx) {
  ...
  if (!buffer_ptr) fatal("mem_get_buffer error\n");
  /*{
...
void mips_ctx_finish_group(struct mips_ctx_t *ctx, int status) {
  ...
  if (mips_ctx_get_status(ctx, mips_ctx_finished | mips_ctx_zombie)) return;
  ...
  DOUBLE_LINKED_LIST_FOR_EACH(mips_emu, context, aux) {
    ...
    */
    ...
    if (aux == ctx)
      mips_ctx_set_status(aux,
                          aux->parent ? mips_ctx_zombie : mips_ctx_finished);
    else
      mips_ctx_set_status(aux, mips_ctx_finished);
 ...
 * to 'mips_ctx_finished'. If it has, its status is set to 'mips_ctx_zombie',
 * waiting for
 * a call to 'waitpid'.
...
static void mips_ctx_update_status(struct mips_ctx_t *ctx,
                                   enum mips_ctx_status_t status) {
  ...
  if (status_diff & ~mips_ctx_spec_mode) mips_emu->context_reschedule = 1;
  ...
  if (!(ctx->status & mips_ctx_suspended) &&
      !(ctx->status & mips_ctx_finished) && !(ctx->status & mips_ctx_zombie) &&
  ...
  if (debug_status(mips_ctx_debug_category) &&
      (status_diff & ~mips_ctx_spec_mode)) {
    char sstatus[200];
...
void mips_ctx_set_status(struct mips_ctx_t *ctx,
                         enum mips_ctx_status_t status) {
  mips_ctx_update_status(ctx, ctx->status | status);
}

void mips_ctx_clear_status(struct mips_ctx_t *ctx,
                           enum mips_ctx_status_t status) {
  mips_ctx_update_status(ctx, ctx->status & ~status);
}
...
void mips_ctx_loader_get_full_path(struct mips_ctx_t *ctx, char *file_name,
