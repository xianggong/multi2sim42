...
int mips_sys_debug_category;

static char *err_mips_sys_note =
...
/* System call names */
static char *mips_sys_call_name[] = {
...
/* System call codes */
enum {
#define DEFSYSCALL(name, code) mips_sys_code_##name = code,
#include "syscall.dat"
#undef DEFSYSCALL
  mips_sys_code_count
};
...
static int (*mips_sys_call_func[mips_sys_code_count + 1])(
...
/* Statistics */
static int mips_sys_call_freq[mips_sys_code_count + 1];
...
static struct str_map_t mips_sys_error_code_map = {34,
                                                   {{"EPERM", 1},
                                                    {"ENOENT", 2},
                                                    {"ESRCH", 3},
                                                    {"EINTR", 4},
...
void mips_sys_call(struct mips_ctx_t *ctx) {
  ...
  mips_sys_debug("'%s' (code %d, inst %lld, pid %d)\n",
                 mips_sys_call_name[code], code, asEmu(mips_emu)->instructions,
                 ctx->pid);
  mips_isa_call_debug("system call '%s' (code %d, inst %lld, pid %d)\n",
                      mips_sys_call_name[code], code,
                      asEmu(mips_emu)->instructions, ctx->pid);
  ...
  if (code != mips_sys_code_sigreturn &&
      !mips_ctx_get_status(ctx, mips_ctx_suspended))
    regs->regs_R[2] = err;

  /* Debug */
  mips_sys_debug("  ret=(%d, 0x%x)", err, err);
  if (err < 0 && err >= -SIM_ERRNO_MAX)
    mips_sys_debug(", errno=%s)",
                   str_map_value(&mips_sys_error_code_map, -err));
  mips_sys_debug("\n");
}
...
static int mips_sys_close_impl(struct mips_ctx_t *ctx) {
  ...
  mips_sys_debug("  guest_fd=%d\n", guest_fd);
  host_fd = mips_file_desc_table_get_host_fd(ctx->file_desc_table, guest_fd);
  mips_sys_debug("  host_fd=%d\n", host_fd);

  /* Get file descriptor table entry. */
  fd = mips_file_desc_table_entry_get(ctx->file_desc_table, guest_fd);
  if (!fd) return -EBADF;
   ...
   * virtual file. */
  if (fd->kind == mips_file_desc_virtual)
    mips_sys_debug("    host file '%s': temporary file deleted\n", fd->path);
  mips_file_desc_table_entry_free(ctx->file_desc_table, fd->guest_fd);
...
static int mips_sys_read_impl(struct mips_ctx_t *ctx) {
  ...
  /* Arguments */
  guest_fd = regs->regs_R[4];
  buf_ptr = regs->regs_R[5];
  count = regs->regs_R[6];
  mips_sys_debug("  guest_fd=%d, buf_ptr=0x%x, count=0x%x\n", guest_fd, buf_ptr,
                 count);
  ...
  if (fds.revents || (fd->flags & O_NONBLOCK)) {
    ...
    if (err > 0) {
      ...
      mips_sys_debug_buffer("  buf", buf, err);
    }
...
static int mips_sys_write_impl(struct mips_ctx_t *ctx) {
  ...
  mips_sys_debug_buffer("  buf", buf, count);
...
static struct str_map_t mips_sys_open_flags_map = {16,
                                                   {{"O_RDONLY", 00000000},
                                                    {"O_WRONLY", 00000001},
                                                    {"O_RDWR", 00000002},
                                                    {"O_CREAT", 00000100},
...
static int mips_sys_open_impl(struct mips_ctx_t *ctx) {
  ...
  if (!strncmp(full_path, "/proc/", 6)) {
    ...
    if (!strcmp(full_path, "/proc/self/maps")) {
      ...
      desc = mips_file_desc_table_entry_new(ctx->file_desc_table,
                                            mips_file_desc_virtual, host_fd,
                                            temp_path, flags);
  ...
  desc = mips_file_desc_table_entry_new(
      ctx->file_desc_table, mips_file_desc_regular, host_fd, full_path, flags);
...
static int mips_sys_gettimeofday_impl(struct mips_ctx_t *ctx) {
  ...
  /* Arguments */
  tv_ptr = regs->regs_R[4];
  tz_ptr = regs->regs_R[5];
  mips_sys_debug("  tv_ptr=0x%x, tz_ptr=0x%x\n", tv_ptr, tz_ptr);
...
static int mips_sys_mmap(struct mips_ctx_t *ctx, unsigned int addr,
...
static int mips_sys_uname_impl(struct mips_ctx_t *ctx) {
  ...
  /* Arguments */
  utsname_ptr = regs->regs_R[4];
  mips_sys_debug("  putsname=0x%x\n", utsname_ptr);
  mips_sys_debug("  sysname='%s', nodename='%s'\n", sim_utsname.sysname,
                 sim_utsname.nodename);
  mips_sys_debug("  release='%s', version='%s'\n", sim_utsname.release,
                 sim_utsname.version);
  mips_sys_debug("  machine='%s', domainname='%s'\n", sim_utsname.machine,
...
//
//	/* Debug */
//	arm_sys_debug("  addr=0x%x, len=%u, prot=0x%x, flags=0x%x, guest_fd=%d,
//offset=0x%x\n",
//		addr, len, prot, flags, guest_fd, offset);
...
//	return arm_sys_mmap(ctx, addr, len, prot, flags, guest_fd, offset <<
//MEM_PAGE_SHIFT);
//}
...
static void mips_sys_stat_host_to_guest(struct sim_stat64_t *guest,
                                        struct stat *host) {
  ...
  mips_sys_debug("  stat64 structure:\n");
  mips_sys_debug("    dev=%lld, ino=%lld, mode=%d, nlink=%d\n", guest->dev,
                 guest->ino, guest->mode, guest->nlink);
  mips_sys_debug("    uid=%d, gid=%d, rdev=%lld\n", guest->uid, guest->gid,
                 guest->rdev);
  mips_sys_debug("    size=%lld, blksize=%d, blocks=%lld\n", guest->size,
...
static int mips_sys_getuid_impl(struct mips_ctx_t *ctx) { return getuid(); }

/*
 * System call 'getgid' (code 200)
 */

static int mips_sys_getgid_impl(struct mips_ctx_t *ctx) { return getgid(); }

/*
 * System call 'geteuid' (code 201)
 */

static int mips_sys_geteuid_impl(struct mips_ctx_t *ctx) { return geteuid(); }

/*
 * System call 'getegid' (code 202)
 */

static int mips_sys_getegid_impl(struct mips_ctx_t *ctx) { return getegid(); }
...
static int mips_sys_set_thread_area_impl(struct mips_ctx_t *ctx) {
  ...
  /*		mem_read(mem, uinfo_ptr, sizeof uinfo, &uinfo);
                  mips_sys_debug("  entry_number=0x%x, base_addr=0x%x,
     limit=0x%x\n",
                          uinfo.entry_number, uinfo.base_addr, uinfo.limit);
                  mips_sys_debug("  seg_32bit=0x%x, contents=0x%x,
     read_exec_only=0x%x\n",
                          uinfo.seg_32bit, uinfo.contents,
     uinfo.read_exec_only);
                  mips_sys_debug("  limit_in_pages=0x%x, seg_not_present=0x%x,
     useable=0x%x\n",
                          uinfo.limit_in_pages, uinfo.seg_not_present,
     uinfo.useable);
                  if (!uinfo.seg_32bit)
                          fatal("syscall set_thread_area: only 32-bit segments
     supported");
  */
...
static int mips_sys_writev_impl(struct mips_ctx_t *ctx) {
  ...
  /* Arguments */
  guest_fd = regs->regs_R[4];
  iovec_ptr = regs->regs_R[5];
  vlen = regs->regs_R[6];
  mips_sys_debug("  guest_fd=%d, iovec_ptr = 0x%x, vlen=0x%x\n", guest_fd,
                 iovec_ptr, vlen);
  ...
  for (v = 0; v < vlen; v++) {
    ...
    len = writev(host_fd, buf, iov_len);
    if (len == -1) {
      free(buf);
      return -errno;
    }
...
static int mips_sys_getpid_impl(struct mips_ctx_t *ctx) { return ctx->pid; }
...
SYS_NOT_IMPL(mmap2)
SYS_NOT_IMPL(break)
SYS_NOT_IMPL(unused18)
SYS_NOT_IMPL(unused28)
SYS_NOT_IMPL(stty)
SYS_NOT_IMPL(gtty)
SYS_NOT_IMPL(ftime)
SYS_NOT_IMPL(prof)
SYS_NOT_IMPL(umount2)
SYS_NOT_IMPL(lock)
SYS_NOT_IMPL(mpx)
SYS_NOT_IMPL(ulimit)
SYS_NOT_IMPL(unused59)
SYS_NOT_IMPL(reserved82)
SYS_NOT_IMPL(unused84)
SYS_NOT_IMPL(profil)
SYS_NOT_IMPL(unused109)
SYS_NOT_IMPL(idle)
SYS_NOT_IMPL(create_module)
SYS_NOT_IMPL(get_kernel_syms)
SYS_NOT_IMPL(afs_syscall)
SYS_NOT_IMPL(_llseek)
SYS_NOT_IMPL(_newselect)
SYS_NOT_IMPL(cacheflush)
SYS_NOT_IMPL(cachectl)
SYS_NOT_IMPL(sysmips)
SYS_NOT_IMPL(unused150)
SYS_NOT_IMPL(_sysctl)
SYS_NOT_IMPL(_mlock)
SYS_NOT_IMPL(accept)
SYS_NOT_IMPL(bind)
SYS_NOT_IMPL(connect)
SYS_NOT_IMPL(getpeername)
SYS_NOT_IMPL(getsockname)
SYS_NOT_IMPL(getsockopt)
SYS_NOT_IMPL(listen)
SYS_NOT_IMPL(recv)
SYS_NOT_IMPL(recvfrom)
SYS_NOT_IMPL(recvmsg)
SYS_NOT_IMPL(send)
SYS_NOT_IMPL(sendmsg)
SYS_NOT_IMPL(sendto)
SYS_NOT_IMPL(setsockopt)
SYS_NOT_IMPL(shutdown)
SYS_NOT_IMPL(socket)
SYS_NOT_IMPL(socketpair)
SYS_NOT_IMPL(query_module)
SYS_NOT_IMPL(getpmsg)
SYS_NOT_IMPL(putpmsg)
SYS_NOT_IMPL(reserved221)
SYS_NOT_IMPL(restart_syscall)
SYS_NOT_IMPL(vserver)
SYS_NOT_IMPL(timerfd_create)
SYS_NOT_IMPL(timerfd_gettime)
SYS_NOT_IMPL(timerfd_settime)
SYS_NOT_IMPL(signalfd4)
SYS_NOT_IMPL(eventfd2)
SYS_NOT_IMPL(epoll_create1)
SYS_NOT_IMPL(dup3)
SYS_NOT_IMPL(pipe2)
SYS_NOT_IMPL(inotify_init1)
SYS_NOT_IMPL(preadv)
SYS_NOT_IMPL(pwritev)
SYS_NOT_IMPL(rt_tgsigqueueinfo)
SYS_NOT_IMPL(perf_event_open)
SYS_NOT_IMPL(accept4)
SYS_NOT_IMPL(sys_setaltroot)
