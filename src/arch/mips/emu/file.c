...
void mips_file_desc_table_free(struct mips_file_desc_table_t *table) {
  ...
  for (i = 0; i < list_count(table->mips_file_desc_list); i++) {
    ...
    if (desc) mips_file_desc_free(desc);
  }
...
struct mips_file_desc_table_t *mips_file_desc_table_link(
    struct mips_file_desc_table_t *fdt) {
  fdt->num_links++;
  return fdt;
}
...
void mips_file_desc_table_entry_dump(struct mips_file_desc_table_t *table,
  ...
  /* Invalid entry */
  if (index < 0 || index >= list_count(table->mips_file_desc_list)) return;
...
struct mips_file_desc_t *mips_file_desc_table_entry_new(
  ...
  return mips_file_desc_table_entry_new_guest_fd(table, kind, -1, host_fd, path,
                                                 flags);
}
