...
void MIPSEmuDestroy(MIPSEmu *self) {
  ...
  for (ctx = self->context_list_head; ctx; ctx = ctx->context_list_next)
    if (!mips_ctx_get_status(ctx, mips_ctx_finished)) mips_ctx_finish(ctx, 0);

  /* Free contexts */
  while (self->context_list_head) mips_ctx_free(self->context_list_head);
}
...
int MIPSEmuRun(Emu *self) {
  ...
  while (emu->finished_list_head) mips_ctx_free(emu->finished_list_head);
