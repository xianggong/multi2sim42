...
struct str_map_t arch_sim_kind_map = {2,
...
void arch_dump(struct arch_t *arch, FILE *f) {
  ...
  if (arch->sim_kind == arch_sim_kind_functional) return;
...
struct arch_t *arch_register(
  ...
  /* Check no duplicates */
  arch = arch_get(name);
  if (arch) panic("%s: duplicated architecture", __FUNCTION__);
...
void arch_init(void) {
  ...
  for (i = 0; i < arch_list_count; i++) {
    ...
    if (arch->sim_kind == arch_sim_kind_detailed) {
      ...
      if (arch->timing_read_config_func) arch->timing_read_config_func();
    }

    /* Initialize emulator */
    if (arch->emu_init_func) arch->emu_init_func();

    /* Initialize timing simulator */
    if (arch->sim_kind == arch_sim_kind_detailed) {
      /* Register frequency domain */
      if (arch->timing_init_func) arch->timing_init_func();
    }
  }
}
...
void arch_done(void) {
  ...
  for (i = 0; i < arch_list_count; i++) {
    ...
    if (arch->sim_kind == arch_sim_kind_detailed) {
      if (arch->timing_done_func) arch->timing_done_func();
    }
    if (arch->emu_done_func) arch->emu_done_func();
...
void arch_for_each(arch_callback_func_t callback_func, void *user_data) {
  ...
  for (i = 0; i < arch_list_count; i++) callback_func(arch_list[i], user_data);
}
...
struct arch_t *arch_get(char *name) {
  ...
  /* Search architecture */
  for (i = 0; i < arch_list_count; i++)
    if (!strcasecmp(arch_list[i]->name, name)) return arch_list[i];
...
int arch_get_sim_kind_detailed_count(void) {
  ...
  count = 0;
  for (i = 0; i < arch_list_count; i++)
    if (arch_list[i]->sim_kind == arch_sim_kind_detailed) count++;
  return count;
}
