...
static int runtime_file_matches_lib(char *file_name, char *lib_name) {
  ...
  if (!str_prefix(file_name, "lib")) return 0;
  file_name += 3;

  /* Then, we need to find the library name */
  if (!str_prefix(file_name, lib_name)) return 0;
  file_name += strlen(lib_name);

  /* Then, '.so' */
  if (!str_prefix(file_name, ".so")) return 0;
  file_name += 3;
  ...
  while (*file_name) {
    ...
    /* At least one digit */
    if (!isdigit(*file_name)) return 0;
    file_name++;

    /* More digits */
    while (isdigit(*file_name)) file_name++;
  }
...
static int get_build_tree_root(char *buf, int size) {
  ...
  *file_name = '\0';
  ...
  /* Discard suffix '/bin' */
  file_name = rindex(exe_name, '/');
  if (!file_name || strcmp(file_name, "/bin")) return 0;
  *file_name = '\0';
...
void runtime_init(void) { runtime_list = list_create(); }
...
void runtime_done(void) {
  ...
  LIST_FOR_EACH(runtime_list, index) {
    ...
    if (runtime->open_attempt)
      warning(
          "failed attempts detected to access runtime library '%s'.\n"
          "\tMulti2Sim has detected that the dynamic linker of your guest "
          "program\n"
          "\thas attempted to access a runtime and not succeeded. Please make "
          "sure\n"
          "\tthat file 'lib%s.so' is present in any of these three locations:\n"
          "\t  - Current working directory.\n"
          "\t  - Multi2Sim build tree at multi2sim/lib/.libs\n"
          "\t  - Default library path, as listed in LD_LIBRARY_PATH "
          "environment\n"
          "\t    variable.\n"
          "\tA safer and portable alternative is linking your guest program "
          "statically\n"
...
int runtime_redirect(char *path, char *redirect_path, int redirect_path_size) {
  ...
  LIST_FOR_EACH(runtime_list, index) {
    ...
    matches_redirect_lib =
        runtime_file_matches_lib(file_name, runtime->redirect_lib_name);
    if (!matches_lib && !matches_redirect_lib) continue;
    ...
    if (!access(buf, R_OK)) {
      ...
      runtime->open_attempt = 0;
      warning(
          "path '%s' has been redirected to\n"
          "\tpath '%s' while loading runtime '%s'.\n%s",
          path, redirect_path, runtime->lib_name, runtime_err_redirect_current);
    ...
    if (get_build_tree_root(build_tree_root, sizeof build_tree_root)) {
      snprintf(buf, sizeof buf, "%s/lib/.libs/lib%s.so", build_tree_root,
               runtime->redirect_lib_name);
      if (!access(buf, R_OK)) {
        snprintf(redirect_path, redirect_path_size, "%s", buf);
        runtime->open_attempt = 0;
        warning(
            "path '%s' has been redirected to\n"
            "\tpath '%s' while loading runtime '%s'.\n%s",
            path, redirect_path, runtime->lib_name, runtime_err_redirect_root);
        return 1;
      }
    }
...
struct runtime_t *runtime_get_from_syscall_code(int syscall_code) {
  ...
  LIST_FOR_EACH(runtime_list, index) {
    runtime = list_get(runtime_list, index);
    if (runtime->syscall_code == syscall_code) return runtime;
  }
