...
void X86ThreadInitRegFile(X86Thread *self) {
  ...
  self->reg_file = reg_file = x86_reg_file_create(x86_reg_file_int_local_size,
                                                  x86_reg_file_fp_local_size,
                                                  x86_reg_file_xmm_local_size);
...
void X86ThreadDumpRegFile(X86Thread *self, FILE *f) {
  ...
  for (i = 0; i < x86_reg_file_int_local_size; i++) {
    fprintf(f, "  %3d%c[%d-%d]", i,
            self->reg_file->int_phreg[i].busy ? ' ' : '*',
            self->reg_file->int_phreg[i].busy,
            self->reg_file->int_phreg[i].pending);
    if (i % 5 == 4 && i != x86_reg_file_int_local_size - 1) fprintf(f, "\n");
  }

  fprintf(f, "\nInteger Register Aliasing Table:\n");
  for (i = x86_dep_int_first; i <= x86_dep_int_last; i++) {
    fprintf(f, "  %2d->%-3d", i,
            self->reg_file->int_rat[i - x86_dep_int_first]);
    if ((i - x86_dep_int_first) % 8 == 7) fprintf(f, "\n");
  }
  ...
  for (i = 0; i < x86_reg_file_fp_local_size; i++) {
    fprintf(
        f, "  %3d%c[%d-%d]", i, self->reg_file->fp_phreg[i].busy ? ' ' : '*',
        self->reg_file->fp_phreg[i].busy, self->reg_file->fp_phreg[i].pending);
    if (i % 5 == 4 && i != x86_reg_file_fp_local_size - 1) fprintf(f, "\n");
  }
  ...
  for (i = x86_dep_fp_first; i <= x86_dep_fp_last; i++) {
    ...
    if ((i - x86_dep_fp_first) % 8 == 7) fprintf(f, "\n");
  }

  fprintf(f, "\n");
  fprintf(
      f, "fp_free_phreg_count  %d  # Number of free floating-point registers\n",
  ...
  for (i = 0; i < x86_reg_file_xmm_local_size; i++) {
    ...
    if (i % 5 == 4 && i != x86_reg_file_xmm_local_size - 1) fprintf(f, "\n");
  }

  fprintf(f, "\nXMM Register Aliasing Table:\n");
  for (i = x86_dep_xmm_first; i <= x86_dep_xmm_last; i++) {
    fprintf(f, "  %2d->%-3d", i,
            self->reg_file->xmm_rat[i - x86_dep_xmm_first]);
    if ((i - x86_dep_xmm_first) % 8 == 7) fprintf(f, "\n");
  }
...
int X86ThreadCanRenameUop(X86Thread *self, struct x86_uop_t *uop) {
  ...
  if (x86_reg_file_kind == x86_reg_file_kind_private) {
    if (self->reg_file_int_count + uop->ph_int_odep_count >
        x86_reg_file_int_local_size)
      return 0;
    if (self->reg_file_fp_count + uop->ph_fp_odep_count >
        x86_reg_file_fp_local_size)
      return 0;
    if (self->reg_file_xmm_count + uop->ph_xmm_odep_count >
        x86_reg_file_xmm_local_size)
      return 0;
  } else {
    if (core->reg_file_int_count + uop->ph_int_odep_count >
        x86_reg_file_int_local_size)
      return 0;
    if (core->reg_file_fp_count + uop->ph_fp_odep_count >
        x86_reg_file_fp_local_size)
      return 0;
    if (core->reg_file_xmm_count + uop->ph_xmm_odep_count >
        x86_reg_file_xmm_local_size)
      return 0;
  }
...
void X86ThreadRenameUop(X86Thread *self, struct x86_uop_t *uop) {
  ...
  for (dep = 0; dep < X86_UINST_MAX_IDEPS; dep++) {
    ...
    if (X86_DEP_IS_INT_REG(loreg)) {
      ...
      streg = (loreg - x86_dep_fp_first + reg_file->fp_top_of_stack) % 8 +
              x86_dep_fp_first;
  ...
  for (dep = 0; dep < X86_UINST_MAX_ODEPS; dep++) {
    ...
    if (X86_DEP_IS_FLAG(loreg)) {
      ...
      if (flag_phreg < 0) flag_phreg = phreg;
  ...
  /* Rename flags */
  if (flag_count > 0) {
    if (flag_phreg < 0) flag_phreg = X86ThreadRequestIntReg(self);
    for (dep = 0; dep < X86_UINST_MAX_ODEPS; dep++) {
      loreg = uop->uinst->odep[dep];
      if (!X86_DEP_IS_FLAG(loreg)) continue;
...
int X86ThreadIsUopReady(X86Thread *self, struct x86_uop_t *uop) {
  ...
  for (dep = 0; dep < X86_UINST_MAX_IDEPS; dep++) {
    ...
    if (X86_DEP_IS_INT_REG(loreg) && reg_file->int_phreg[phreg].pending)
      return 0;
    if (X86_DEP_IS_FP_REG(loreg) && reg_file->fp_phreg[phreg].pending) return 0;
 ...
 * obtained by 'X86ThreadIsUopReady'. The 'uop->ready' field is redundant and
 * should always
...
void X86ThreadIsUopListReady(X86Thread *self, struct linked_list_t *uop_list) {
  ...
  LINKED_LIST_FOR_EACH(uop_list) {
    ...
    if (uop->ready || !X86ThreadIsUopReady(self, uop)) continue;
    uop->ready = 1;
  }
}
...
enum x86_reg_file_kind_t x86_reg_file_kind =
    x86_reg_file_kind_private;  /* Sharing policy for register file */
int x86_reg_file_int_size = 80; /* Per-thread integer register file size */
int x86_reg_file_fp_size =
    40; /* Per-thread floating-point register file size */
...
void X86ReadRegFileConfig(struct config_t *config) {
  ...
  x86_reg_file_kind =
      config_read_enum(config, section, "RfKind", x86_reg_file_kind_private,
                       x86_reg_file_kind_map, 2);
