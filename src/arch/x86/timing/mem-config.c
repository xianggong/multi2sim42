...
void X86CpuMemConfigDefault(Timing *self, struct config_t *config) {
  ...
  for (i = 0; i < x86_cpu_num_cores; i++) {
    ...
    for (j = 0; j < x86_cpu_num_threads; j++) {
      snprintf(section, sizeof section, "Entry x86-core-%d-thread-%d", i, j);
...
void X86CpuMemConfigParseEntry(Timing *self, struct config_t *config,
                               char *section) {
  ...
  if (!(unified_present ^ data_inst_present))
    fatal(
        ...
        "\tAn x86 entry to the memory hierarchy needs to specify either a "
        "unified\n"
        "\tentry for data and instructions (variable 'Module'), or two "
        "separate\n"
        "\tentries for data and instructions (variables 'DataModule' and "
        "'InstModule'),\n"
        "\tbut not both.\n",
        file_name, section);
  ...
  if (core_index < 0)
    fatal("%s: section [%s]: invalid or missing value for 'Core'", file_name,
          section);

  /* Read thread */
  thread_index = config_read_int(config, section, "Thread", -1);
  if (thread_index < 0)
    fatal("%s: section [%s]: invalid or missing value for 'Thread'", file_name,
          section);

  /* Check bounds */
  if (core_index >= x86_cpu_num_cores || thread_index >= x86_cpu_num_threads) {
    warning(
        "%s: section [%s] ignored, referring to x86 Core %d, Thread %d.\n"
        "\tThis section refers to a core or thread that does not currently "
        "exists.\n"
        "\tPlease review your x86 configuration file if this behavior is not "
        "desired.\n",
  ...
  if (thread->data_mod || thread->inst_mod) {
    ...
    fatal(
        "%s: section [%s]: entry from Core %d, Thread %d already assigned.\n"
        "\tA different [Entry <name>] section in the memory configuration file "
        "has already\n"
        "\tassigned an entry for this particular core and thread. Please "
        "review your\n"
...
void X86CpuMemConfigCheck(Timing *self, struct config_t *config) {
  ...
  for (i = 0; i < x86_cpu_num_cores; i++) {
    ...
    for (j = 0; j < x86_cpu_num_threads; j++) {
      ...
      if (!thread->data_mod || !thread->inst_mod)
        fatal(
            "%s: x86 Core %d, Thread %d lacks a data/instruction entry to "
            "memory.\n"
