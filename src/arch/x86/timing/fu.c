...
void X86CoreFreeFunctionalUnits(X86Core *self) { free(self->fu); }
...
void X86CoreDumpFunctionalUnitsReport(X86Core *self, FILE *f) {
  ...
  for (i = 1; i < x86_fu_count; i++) {
    ...
    fprintf(
        f, "fu.%s.WaitingTime = %.4g\n", x86_fu_name[i],
        fu->accesses[i] ? (double)fu->waiting_time[i] / fu->accesses[i] : 0.0);
  }
...
int X86CoreReserveFunctionalUnit(X86Core *self, struct x86_uop_t *uop) {
  ...
  if (!fu_class) return 1;

  /* First time uop tries to reserve f.u. */
  if (!uop->issue_try_when) uop->issue_try_when = asTiming(cpu)->cycle;
  ...
  for (i = 0; i < x86_fu_res_pool[fu_class].count; i++) {
    if (fu->cycle_when_free[fu_class][i] <= asTiming(cpu)->cycle) {
      ...
      fu->cycle_when_free[fu_class][i] =
          asTiming(cpu)->cycle + x86_fu_res_pool[fu_class].issuelat;
...
char *x86_fu_name[x86_fu_count] = {
    ...
    "IntAdd",      "IntMult",      "IntDiv",

    "EffAddr",     "Logic",

    "FloatSimple", "FloatAdd",     "FloatComp",
    "FloatMult",   "FloatDiv",     "FloatComplex",

    "XMMIntAdd",   "XMMIntMult",   "XMMIntDiv",

    "XMMLogic",

    "XMMFloatAdd", "XMMFloatComp", "XMMFloatMult",
    "XMMFloatDiv", "XMMFloatConv", "XMMFloatComplex",
};
