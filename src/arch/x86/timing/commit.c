...
int X86ThreadCanCommit(X86Thread *self) {
  ...
  if (!X86ThreadCanDequeueFromROB(self)) return 0;
  ...
  if (uop->uinst->opcode == x86_uinst_store) {
    if (!uop->ready && X86ThreadIsUopReady(self, uop)) uop->ready = 1;
    return uop->ready;
  }
...
void X86ThreadCommit(X86Thread *self, int quant) {
  ...
  while (quant && X86ThreadCanCommit(self)) {
    ...
    /* Trace cache */
    if (x86_trace_cache_present) X86ThreadRecordUopInTraceCache(self, uop);
    ...
    ctx->inst_count++;
    if (uop->trace_cache) self->trace_cache->num_committed_uinst++;
    if (!uop->mop_index) cpu->num_committed_inst++;
    if (uop->flags & X86_UINST_CTRL) {
...
void X86CpuCommit(X86Cpu *self) {
  ...
  self->stage = "commit";
  for (i = 0; i < x86_cpu_num_cores; i++) X86CoreCommit(self->cores[i]);
}
