...
void x86_uop_free_if_not_queued(struct x86_uop_t *uop) {
  ...
  if (uop->in_fetch_queue || uop->in_uop_queue || uop->in_iq || uop->in_lq ||
      uop->in_sq || uop->in_preq || uop->in_rob || uop->in_event_queue ||
 ...
 * the magic number. */
int x86_uop_exists(struct x86_uop_t *uop) { return uop->magic == UOP_MAGIC; }
