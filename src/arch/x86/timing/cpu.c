...
char *x86_config_help =
    ...
    "the parameters of the CPU model used for a detailed (architectural) "
    "simulation.\n"
    "This configuration file is passed to Multi2Sim with option '--x86-config "
    "<file>,\n"
    "which must be accompanied by option '--x86-sim detailed'.\n"
    "\n"
    "The following is a list of the sections allowed in the CPU configuration "
    "file,\n"
    ...
    "      Number of hardware threads per core. The total number of computing "
    "nodes\n"
    ...
    "      If ContextSwitch is true, maximum number of cycles that a context "
    "can occupy\n"
    "      a CPU hardware thread before it is replaced by other pending "
    "context.\n"
    "  ThreadQuantum = <cycles> (Default = 1k)\n"
    "      For multithreaded processors (Threads > 1) configured as "
    "coarse-grain multi-\n"
    "      threading (FetchKind = SwitchOnEvent), number of cycles in which "
    "instructions\n"
    "      are fetched from the same thread before switching.\n"
    "  ThreadSwitchPenalty = <cycles> (Default = 0)\n"
    "      For coarse-grain multithreaded processors (FetchKind = "
    "SwitchOnEvent), number\n"
    "      of cycles that the fetch stage stalls after a thread switch.\n"
    "  RecoverKind = {Writeback|Commit} (Default = Writeback)\n"
    "      On branch misprediction, stage in the execution of the mispredicted "
    "branch\n"
    ...
    "      If specified as false, the cpu will ignore any prefetch "
    "hints/instructions.\n"
    "  PrefetchHistorySize = <size> (Default = 10)\n"
    "      Number of past prefetches to keep track of, so as to avoid "
    "redundant prefetches\n"
    "      from being issued from the cpu to the cache module.\n"
    "  InstructionCachePerfect = {t|f} (Default = False)\n"
    "      Set these options to true to simulate a perfect data/instruction "
    "caches,\n"
    "      respectively, where every access results in a hit. If set to false, "
    "the\n"
    "      parameters of the caches are given in the memory configuration "
    "file\n"
    "\n"
    "Section '[ Pipeline ]':\n"
    "\n"
    "  FetchKind = {Shared|TimeSlice|SwitchOnEvent} (Default = TimeSlice)\n"
    "      Policy for fetching instruction from different threads. A shared "
    "fetch stage\n"
    "      fetches instructions from different threads in the same cycle; a "
    "time-slice\n"
    "      fetch switches between threads in a round-robin fashion; option "
    "SwitchOnEvent\n"
    "      switches thread fetch on long-latency operations or thread quantum "
    "expiration.\n"
    ...
    "      Policy for dispatching instructions from different threads. If "
    "shared,\n"
    "      instructions from different threads are dispatched in the same "
    "cycle. Otherwise,\n"
    "      instruction dispatching is done in a round-robin fashion at a cycle "
    "granularity.\n"
    ...
    "      Policy for issuing instructions from different threads. If shared, "
    "instructions\n"
    "      from different threads are issued in the same cycle; otherwise, "
    "instruction issue\n"
    ...
    "      Policy for committing instructions from different threads. If "
    "shared,\n"
    "      instructions from different threads are committed in the same "
    "cycle; otherwise,\n"
    ...
    "      Calculate structures occupancy statistics. Since this computation "
    "requires\n"
    "      additional overhead, the option needs to be enabled explicitly. "
    "These statistics\n"
    ...
    "      Number of floating-point physical registers (if private, "
    "per-thread).\n"
    ...
    "      If true, a trace cache is included in the model. If false, the rest "
    "of the\n"
    "      options in this section are ignored.\n"
    "  Sets = <num_sets> (Default = 64)\n"
    "      Number of sets in the trace cache.\n"
    "  Assoc = <num_ways> (Default = 4)\n"
    "      Associativity of the trace cache. The product Sets * Assoc is the "
    "total\n"
    ...
    "  where <func_unit> refers to a functional unit type, and <field> refers "
    "to a\n"
    ...
    "      IssueLat    Latency since an instruction was issued until the "
    "functional\n"
    "                  unit is available for the next use. For pipelined "
    "operators,\n"
    "                  IssueLat is smaller than OpLat.\n"
    "\n"
    "Section '[ BranchPredictor ]':\n"
    "\n"
    "  Kind = {Perfect|Taken|NotTaken|Bimodal|TwoLevel|Combined} (Default = "
    "TwoLevel)\n"
  ...
  {                                                                        \
    ...
    if (x86_cpu_occupancy_stats)                                           \
      fprintf(f, #NAME ".Occupancy = %.2f\n",                              \
              asTiming(self)->cycle                                        \
                  ? (double)core->ITEM##_occupancy / asTiming(self)->cycle \
                  : 0.0);                                                  \
...
#define DUMP_THREAD_STRUCT_STATS(NAME, ITEM)                                 \
  {                                                                          \
    ...
    if (x86_cpu_occupancy_stats)                                             \
      fprintf(f, #NAME ".Occupancy = %.2f\n",                                \
              asTiming(self)->cycle                                          \
                  ? (double)thread->ITEM##_occupancy / asTiming(self)->cycle \
                  : 0.0);                                                    \
    fprintf(f, #NAME ".Full = %lld\n", thread->ITEM##_full);                 \
    fprintf(f, #NAME ".Reads = %lld\n", thread->ITEM##_reads);               \
    fprintf(f, #NAME ".Writes = %lld\n", thread->ITEM##_writes);             \
...
void X86CpuRunStages(X86Cpu *self) {
  ...
  if (x86_cpu_occupancy_stats) X86CpuUpdateOccupancyStats(self);
}
...
#define UPDATE_THREAD_OCCUPANCY_STATS(ITEM)                               \
  {                                                                       \
    ...
    if (thread->ITEM##_count == x86_##ITEM##_size) thread->ITEM##_full++; \
  }
