...
void X86CoreWriteback(X86Core *self) {
  ...
  for (;;) {
    ...
    if (uop->flags & X86_UINST_MEM) uop->when = asTiming(cpu)->cycle;
    if (uop->when > asTiming(cpu)->cycle) break;
     ...
     * freed, which interferes with the temporary extraction from the
     * event_queue. */
    if (recover) X86ThreadRecover(thread);
  }
}
...
void X86CpuWriteback(X86Cpu *self) {
  ...
  for (i = 0; i < x86_cpu_num_cores; i++) X86CoreWriteback(self->cores[i]);
}
