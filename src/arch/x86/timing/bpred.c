...
void X86ThreadFreeBranchPred(X86Thread *self) { x86_bpred_free(self->bpred); }
...
int X86ThreadLookupBranchPred(X86Thread *self, struct x86_uop_t *uop) {
  ...
  /* Taken predictor */
  if (x86_bpred_kind == x86_bpred_kind_taken) uop->pred = 1;

  /* Not-taken predictor */
  if (x86_bpred_kind == x86_bpred_kind_nottaken) uop->pred = 0;

  /* Bimodal predictor */
  if (x86_bpred_kind == x86_bpred_kind_bimod ||
      x86_bpred_kind == x86_bpred_kind_comb) {
  ...
  if (x86_bpred_kind == x86_bpred_kind_twolevel ||
    ...
    uop->twolevel_pred =
        bpred->twolevel_pht[uop->twolevel_pht_row * x86_bpred_twolevel_l2size +
                            uop->twolevel_pht_col] > 1;
...
/* Return multiple predictions for an address. This can only be done for
 * two-level
 * adaptive predictors, since they use global history. The prediction of the
 * primary branch is stored in the least significant bit (bit 0), whereas the
 * prediction
 * of the last branch is stored in bit 'count-1'. */
int X86ThreadLookupBranchPredMultiple(X86Thread *self, unsigned int eip,
                                      int count) {
  struct x86_bpred_t *bpred = self->bpred;
   ...
   * uop for a later call to X86ThreadUpdateBranchPred, and makes the first
   * prediction
  ...
  pred = temp_pred =
      bpred->twolevel_pht[bhr * x86_bpred_twolevel_l2size + pht_col] > 1;
...
void X86ThreadUpdateBranchPred(X86Thread *self, struct x86_uop_t *uop) {
  ...
  /* Stats */
  bpred->accesses++;
  if (uop->neip == uop->pred_neip) bpred->hits++;
  ...
  if (x86_bpred_kind == x86_bpred_kind_perfect) return;
  if (uop->flags & X86_UINST_UNCOND) return;
   ...
   * predictions differ. */
  if (x86_bpred_kind == x86_bpred_kind_comb &&
      uop->bimod_pred != uop->twolevel_pred) {
    pctr = &bpred->choice[uop->choice_index];
    *pctr = uop->bimod_pred == taken ? MAX(*pctr - 1, 0) : MIN(*pctr + 1, 3);
  }
}

/* Lookup BTB. If it contains the uop address, return target. The BTB also
 * contains
 * information about the type of branch, i.e., jump, call, ret, or conditional.
 * If
...
unsigned int X86ThreadLookupBTB(X86Thread *self, struct x86_uop_t *uop) {
  ...
  if (x86_bpred_kind == x86_bpred_kind_perfect) return uop->neip;

  /* Internal branch (string operations) always predicted to jump to itself */
  if (uop->uinst->opcode == x86_uinst_ibranch) return uop->eip;
  ...
  for (way = 0; way < x86_bpred_btb_assoc; way++) {
    ...
    if (entry->source != uop->eip) continue;
    target = entry->target;
    hit = 1;
    break;
  }
  ...
  if (hit && uop->uinst->opcode == x86_uinst_ret && !uop->specmode) {
    bpred->ras_index =
        (bpred->ras_index + x86_bpred_ras_size - 1) % x86_bpred_ras_size;
...
void X86ThreadUpdateBTB(X86Thread *self, struct x86_uop_t *uop) {
  ...
  if (found) {
    for (way = 0; way < x86_bpred_btb_assoc; way++) {
      entry = X86_BPRED_BTB_ENTRY(set, way);
      if (entry->counter > found->counter) entry->counter--;
    }
...
unsigned int X86ThreadGetNextBranch(X86Thread *self, unsigned int eip,
  ...
  while (eip < limit) {
    ...
    for (way = 0; way < x86_bpred_btb_assoc; way++) {
      entry = X86_BPRED_BTB_ENTRY(set, way);
      if (entry->source == eip) return eip;
    }
    eip++;
  }
  return 0;
...
struct x86_bpred_t *x86_bpred_create(char *name) {
  ...
  if (x86_bpred_kind == x86_bpred_kind_bimod ||
    ...
    for (i = 0; i < x86_bpred_bimod_size; i++) bpred->bimod[i] = 2;
  }
  ...
  if (x86_bpred_kind == x86_bpred_kind_twolevel ||
      x86_bpred_kind == x86_bpred_kind_comb) {
    bpred->twolevel_bht =
        xcalloc(x86_bpred_twolevel_l1size, sizeof(unsigned int));
    bpred->twolevel_pht = xcalloc(
        x86_bpred_twolevel_l2size * x86_bpred_twolevel_l2height, sizeof(char));
    for (i = 0; i < x86_bpred_twolevel_l2size * x86_bpred_twolevel_l2height;
         i++)
  ...
  if (x86_bpred_kind == x86_bpred_kind_comb) {
    ...
    for (i = 0; i < x86_bpred_choice_size; i++) bpred->choice[i] = 2;
  }

  /* Allocate BTB and assign LRU counters */
  bpred->btb = xcalloc(x86_bpred_btb_sets * x86_bpred_btb_assoc,
                       sizeof(struct x86_bpred_btb_entry_t));
...
void x86_bpred_free(struct x86_bpred_t *bpred) {
  ...
  /* Choice table */
  if (x86_bpred_kind == x86_bpred_kind_comb) free(bpred->choice);
...
char *x86_bpred_kind_map[] = {"Perfect", "Taken",    "NotTaken",
                              "Bimodal", "TwoLevel", "Combined"};
...
int x86_bpred_twolevel_hist_size; /* Two-level adaptive predictor: level-2
                                     history size */
...
void X86ReadBranchPredConfig(struct config_t *config) {
  ...
  x86_bpred_kind = config_read_enum(
      config, section, "Kind", x86_bpred_kind_twolevel, x86_bpred_kind_map, 6);
  ...
  x86_bpred_twolevel_l1size =
      config_read_int(config, section, "TwoLevel.L1Size", 1);
  x86_bpred_twolevel_l2size =
      config_read_int(config, section, "TwoLevel.L2Size", 1024);
  x86_bpred_twolevel_hist_size =
      config_read_int(config, section, "TwoLevel.HistorySize", 8);
