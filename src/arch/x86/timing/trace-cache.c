...
void X86ThreadInitTraceCache(X86Thread *self) {
  ...
  /* Trace cache present */
  if (!x86_trace_cache_present) return;

  /* Initialization */
  snprintf(name, sizeof name, "Core[%d].Thread[%d].TraceCache", core->id,
           self->id_in_core);
...
void X86ThreadDumpTraceCacheReport(X86Thread *self, FILE *f) {
  ...
  fprintf(f, "TraceCache.HitRatio = %.4g\n",
          trace_cache->accesses
              ? (double)trace_cache->hits / trace_cache->accesses
              : 0.0);
  ...
  fprintf(f, "TraceCache.TraceLength = %.2f\n",
          trace_cache->trace_length_count
              ? (double)trace_cache->trace_length_acc /
                    trace_cache->trace_length_count
              : 0);
  fprintf(f, "\n");
}
...
static void X86ThraceDumpTraceCacheEntry(X86Thread *self,
  ...
  for (i = 0; i < entry->mop_count; i++) {
    ...
    mop_name_length =
        mop_name_end ? mop_name_end - inst.format : strlen(inst.format);
...
void X86ThreadRecordUopInTraceCache(X86Thread *self, struct x86_uop_t *uop) {
  ...
  if (uop->mop_count > x86_trace_cache_trace_size) return;

  /* First instruction. Store trace tag. */
  if (!trace->uop_count) trace->tag = uop->eip;
...
int X86ThreadLookupTraceCache(X86Thread *self, unsigned int eip, int pred,
  ...
  for (way = 0; way < x86_trace_cache_assoc; way++) {
    entry = X86_TRACE_CACHE_ENTRY(set, way);
    if (entry->tag == eip &&
        ((pred & entry->branch_mask) == entry->branch_flags)) {
      found_entry = entry;
      break;
    }
  }
  ...
  /* Statistics */
  trace_cache->accesses++;
  if (found_entry) trace_cache->hits++;
...
void X86ReadTraceCacheConfig(struct config_t *config) {
  ...
  x86_trace_cache_trace_size =
      config_read_int(config, section, "TraceSize", 16);
  x86_trace_cache_branch_max = config_read_int(config, section, "BranchMax", 3);
  x86_trace_cache_queue_size =
      config_read_int(config, section, "QueueSize", 32);

  /* Integrity checks */
  if ((x86_trace_cache_num_sets & (x86_trace_cache_num_sets - 1)) ||
      !x86_trace_cache_num_sets)
    fatal("%s: %s: 'Sets' must be a power of 2 greater than 0", file_name,
          section);
  if ((x86_trace_cache_assoc & (x86_trace_cache_assoc - 1)) ||
      !x86_trace_cache_assoc)
    fatal("%s: %s: 'Assoc' must be a power of 2 greater than 0", file_name,
          section);
  if (!x86_trace_cache_trace_size)
    fatal("%s: %s: Invalid value for 'TraceSize'", file_name, section);
  if (!x86_trace_cache_branch_max)
    fatal("%s: %s: Invalid value for 'BranchMax'", file_name, section);
  ...
  if (x86_trace_cache_branch_max > 31)
    fatal("%s: %s: Maximum value for 'BranchMax' is 31", file_name, section);
}
