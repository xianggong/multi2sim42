 ...
 *This
 *     association is done once when the context is created, or during context
 *migration
 ...
 *context
 *     and start effectively executing it, allocating pipeline resources in the
 *node.
...
void X86ThreadEvictContextSignal(X86Thread *self, X86Context *context) {
  ...
  if (X86ThreadIsPipelineEmpty(self)) X86ThreadEvictContext(self, context);
}
...
void X86ThreadSchedule(X86Thread *self) {
  ...
  if (ctx) {
    ...
    if (!ctx->evict_signal &&
        asTiming(cpu)->cycle >= ctx->alloc_cycle + x86_cpu_context_quantum) {
      int found = 0;

      /* Find a running context mapped to the same node */
      DOUBLE_LINKED_LIST_FOR_EACH(self, mapped, tmp_ctx) {
        if (tmp_ctx != ctx && X86ContextGetState(tmp_ctx, X86ContextRunning)) {
          found = 1;
          break;
        }
      }
  ...
  if (!self->ctx) {
    ...
    DOUBLE_LINKED_LIST_FOR_EACH(self, mapped, tmp_ctx) {
      /* No affinity */
      if (!bit_map_get(tmp_ctx->affinity, node, 1)) continue;

      /* Not running */
      if (!X86ContextGetState(tmp_ctx, X86ContextRunning)) continue;

      /* Good candidate */
      if (!ctx || ctx->evict_cycle > tmp_ctx->evict_cycle) ctx = tmp_ctx;
    }

    /* Allocate context if found */
    if (ctx) X86CpuAllocateContext(cpu, ctx);
  }
}
...
void X86CpuAllocateContext(X86Cpu *self, X86Context *ctx) {
  ...
  x86_trace("x86.map_ctx ctx=%d core=%d thread=%d ppid=%d\n", ctx->pid,
            core_index, thread_index, ctx->parent ? ctx->parent->pid : 0);
}
...
void X86CpuMapContext(X86Cpu *self, X86Context *ctx) {
  ...
  for (core = 0; core < x86_cpu_num_cores; core++) {
    for (thread = 0; thread < x86_cpu_num_threads; thread++) {
      ...
      if (!bit_map_get(ctx->affinity, node, 1)) continue;

      /* Check if this thread is better */
      if (min_core < 0 ||
          self->cores[core]->threads[thread]->mapped_list_count <
              self->cores[min_core]->threads[min_thread]->mapped_list_count) {
        min_core = core;
        min_thread = thread;
      }
    }
  ...
  DOUBLE_LINKED_LIST_INSERT_TAIL(self->cores[core]->threads[thread], mapped,
                                 ctx);
...
void X86CpuUpdateMinAllocCycle(X86Cpu *self) {
  ...
  for (i = 0; i < x86_cpu_num_cores; i++) {
    for (j = 0; j < x86_cpu_num_threads; j++) {
      ...
      if (ctx && !ctx->evict_signal && ctx->alloc_cycle < self->min_alloc_cycle)
...
void X86CpuSchedule(X86Cpu *self) {
  ...
  quantum_expired =
      asTiming(self)->cycle >= self->min_alloc_cycle + x86_cpu_context_quantum;
  ...
  if (!quantum_expired && !emu->schedule_signal) return;
  ...
  if (!X86ContextGetState(ctx, X86ContextMapped)) X86CpuMapContext(self, ctx);
