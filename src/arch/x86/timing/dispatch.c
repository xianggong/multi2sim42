...
static enum x86_dispatch_stall_t X86ThreadCanDispatch(X86Thread *self) {
  ...
  if (!uop)
    return !self->ctx || !X86ContextGetState(self->ctx, X86ContextRunning)
               ? x86_dispatch_stall_ctx
               : x86_dispatch_stall_uop_queue;

  /* If iq/lq/sq/rob full, done */
  if (!X86CoreCanEnqueueInROB(core, uop)) return x86_dispatch_stall_rob;
  ...
  if (!X86ThreadCanRenameUop(self, uop)) return x86_dispatch_stall_rename;
    ...
    case x86_cpu_dispatch_kind_shared:
      ...
      do {
        self->dispatch_current =
            (self->dispatch_current + 1) % x86_cpu_num_threads;
...
void X86CpuDispatch(X86Cpu *self) {
  ...
  self->stage = "dispatch";
  for (i = 0; i < x86_cpu_num_cores; i++) X86CoreDispatch(self->cores[i]);
}
