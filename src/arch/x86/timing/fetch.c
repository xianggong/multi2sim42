...
static int X86ThreadCanFetch(X86Thread *self) {
  ...
  if (!ctx || !X86ContextGetState(ctx, X86ContextRunning)) return 0;
  ...
  if (self->fetchq_occ >= x86_fetch_queue_size) return 0;
  ...
  if (block != self->fetch_block) {
    ...
    if (!mod_can_access(self->inst_mod, phy_addr)) return 0;
  }
...
static struct x86_uop_t *X86ThreadFetchInst(X86Thread *self,
  ...
  while (list_count(x86_uinst_list)) {
    ...
    if (uop->flags & X86_UINST_MEM)
      uop->phy_addr =
          mmu_translate(self->ctx->address_space_index, uinst->address);
    ...
    if (x86_tracing()) {
      ...
      /* Speculative mode */
      if (uop->specmode) str_printf(&str_ptr, &str_size, " spec=\"t\"");
    ...
    if (!ret_uop || (uop->flags & X86_UINST_CTRL)) ret_uop = uop;

    /* Insert into fetch queue */
    list_add(self->fetch_queue, uop);
    if (fetch_trace_cache) self->trace_cache_queue_occ++;

    /* Statistics */
    cpu->num_fetched_uinst++;
    self->num_fetched_uinst++;
    if (fetch_trace_cache) self->trace_cache->num_fetched_uinst++;
  ...
  if (ret_uop && !fetch_trace_cache) self->fetchq_occ += ret_uop->mop_size;
  return ret_uop;
}
...
static int X86ThreadFetchTraceCache(X86Thread *self) {
  ...
  if (self->trace_cache_queue_occ >= x86_trace_cache_queue_size) return 0;
  ...
  mpred = eip_branch ? X86ThreadLookupBranchPredMultiple(
                           self, eip_branch, x86_trace_cache_branch_max)
                     : 0;
  ...
  for (i = 0; i < mop_count; i++) {
    ...
    if (!X86ContextGetState(self->ctx, X86ContextRunning)) break;
...
static void X86ThreadFetch(X86Thread *self) {
  ...
  if (x86_trace_cache_present && X86ThreadFetchTraceCache(self)) return;
  ...
  if (block != self->fetch_block) {
    ...
    self->fetch_access = mod_access(self->inst_mod, mod_access_load, phy_addr,
                                    NULL, NULL, NULL, NULL);
    self->btb_reads++;

    /* MMU statistics */
    if (*mmu_report_file_name) mmu_access_page(phy_addr, mmu_access_execute);
  }
  ...
  while ((self->fetch_neip & ~(self->inst_mod->block_size - 1)) == block) {
    /* If instruction caused context to suspend or finish */
    if (!X86ContextGetState(ctx, X86ContextRunning)) break;

    /* If fetch queue full, stop fetching */
    if (self->fetchq_occ >= x86_fetch_queue_size) break;
    ...
    if (!ctx->inst
             .size) /* x86_isa_inst invalid - no forward progress in loop */
      break;
...
static void X86CoreFetch(X86Core *self) {
  ...
  switch (x86_cpu_fetch_kind) {
    ...
    case x86_cpu_fetch_kind_switchonevent: {
      ...
      thread = self->threads[self->fetch_current];
      if (thread->fetch_stall_until >= asTiming(cpu)->cycle) break;
      ...
      if (must_switch) {
        ...
        for (new_index = (thread->id_in_core + 1) % x86_cpu_num_threads;
          ...
          if (!X86ThreadCanFetch(new_thread)) continue;

          /* Choose it if we need to switch */
          if (must_switch) break;
          ...
          if (!X86ThreadLongLatencyInEventQueue(new_thread)) break;
        }
        ...
        if (new_index != thread->id_in_core) {
          ...
          new_thread->fetch_stall_until =
              asTiming(cpu)->cycle + x86_cpu_thread_switch_penalty - 1;
        }
      }

      /* Fetch */
      thread = self->threads[self->fetch_current];
      if (X86ThreadCanFetch(thread)) X86ThreadFetch(thread);
      break;
    }
...
void X86CpuFetch(X86Cpu *self) {
  ...
  self->stage = "fetch";
  for (i = 0; i < x86_cpu_num_cores; i++) X86CoreFetch(self->cores[i]);
}
