...
void X86CoreInitROB(X86Core *self) {
  ...
  for (i = 0; i < x86_rob_total_size; i++) list_add(self->rob, NULL);
}
...
static void X86CoreTrimROB(X86Core *self) {
  ...
  while (self->rob_count) {
    ...
    if (uop) break;
    self->rob_head =
        self->rob_head == x86_rob_total_size - 1 ? 0 : self->rob_head + 1;
    self->rob_count--;
  }
  ...
  switch (x86_rob_kind) {
    case x86_rob_kind_private: {
      for (i = 0; i < x86_cpu_num_threads; i++) {
        thread = self->threads[i];
        fprintf(f,
                "  rob for thread %s (entries %d to %d), count=%d, size=%d\n",
        ...
        for (j = thread->rob_left_bound; j <= thread->rob_right_bound; j++) {
          ...
          fprintf(f, "   %c%c ", j == thread->rob_head ? 'H' : ' ',
                  j == thread->rob_tail ? 'T' : ' ');
          if (uop) {
    ...
    case x86_rob_kind_shared: {
      ...
      for (j = 0; j < x86_rob_total_size; j++) {
        uop = list_get(self->rob, j);
        fprintf(f, " %c%c ", j == self->rob_head ? 'H' : ' ',
                j == self->rob_tail ? 'T' : ' ');
        if (uop) {
          x86_uinst_dump(uop->uinst, f);
          fprintf(f, "\n");
...
int X86ThreadCanDequeueFromROB(X86Thread *self) {
  ...
  switch (x86_rob_kind) {
    ...
    case x86_rob_kind_shared:
      ...
      X86CoreTrimROB(core);
      if (!core->rob_count) return 0;

      uop = list_get(core->rob, core->rob_head);
      assert(x86_uop_exists(uop));
      if (uop->thread == self) return 1;
      break;
  }
  return 0;
}
...
struct x86_uop_t *X86ThreadGetROBTail(X86Thread *self) {
  ...
  switch (x86_rob_kind) {
    case x86_rob_kind_private:
      ...
      if (self->rob_count > 0) {
        idx = self->rob_tail == self->rob_left_bound ? self->rob_right_bound
                                                     : self->rob_tail - 1;
        uop = list_get(core->rob, idx);
        return uop;
      }
...
struct x86_uop_t *X86GetROBEntry(X86Thread *self, int index) {
  ...
  if (index < 0 || index >= self->rob_count) return NULL;
...
int X86CoreCanEnqueueInROB(X86Core *self, struct x86_uop_t *uop) {
  ...
  switch (x86_rob_kind) {
    case x86_rob_kind_private:
      ...
      thread = uop->thread;
      if (thread->rob_count < x86_rob_size) return 1;
      break;

    case x86_rob_kind_shared:

      X86CoreTrimROB(self);
      if (self->rob_count < x86_rob_total_size) return 1;
      break;
  }
...
void X86CoreEnqueueInROB(X86Core *self, struct x86_uop_t *uop) {
  ...
  switch (x86_rob_kind) {
    ...
    case x86_rob_kind_shared:
      ...
      self->rob_tail =
          self->rob_tail == x86_rob_total_size - 1 ? 0 : self->rob_tail + 1;
      self->rob_count++;
      thread->rob_count++;
      break;
  }
