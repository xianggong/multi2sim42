...
#define op_jcc_rel8(cc, idep1, idep2)                                  \
  void x86_isa_j##cc##_rel8_impl(X86Context *ctx) {                    \
    ...
    if (cc_##cc) regs->eip = ctx->target_eip;                          \
...
#define op_cmov_r16_rm16(cc, idep1, idep2)                         \
  void x86_isa_cmov##cc##_r16_rm16_impl(X86Context *ctx) {         \
    if (cc_##cc) X86ContextStoreR16(ctx, X86ContextLoadRm16(ctx)); \
    x86_uinst_new(ctx, x86_uinst_move, idep1, idep2, x86_dep_rm16, \
                  x86_dep_r16, 0, 0, 0);                           \
  }

#define op_cmov_r32_rm32(cc, idep1, idep2)                         \
  void x86_isa_cmov##cc##_r32_rm32_impl(X86Context *ctx) {         \
    if (cc_##cc) X86ContextStoreR32(ctx, X86ContextLoadRm32(ctx)); \
    x86_uinst_new(ctx, x86_uinst_move, idep1, idep2, x86_dep_rm32, \
                  x86_dep_r32, 0, 0, 0);                           \
  }

#define op_cc_all(ccop)                                                      \
  op_##ccop(a, x86_dep_cf, x86_dep_zps) op_##ccop(ae, x86_dep_cf, 0)         \
      op_##ccop(b, x86_dep_cf, 0) op_##ccop(be, x86_dep_cf, x86_dep_zps)     \
          op_##ccop(e, x86_dep_zps, 0) op_##ccop(g, x86_dep_zps, x86_dep_of) \
...
op_cc_all(setcc) op_cc_all(jcc_rel8) op_cc_all(jcc_rel32)
    op_cc_all(cmov_r16_rm16) op_cc_all(cmov_r32_rm32)

        void x86_isa_jecxz_rel8_impl(X86Context *ctx) {
  struct x86_regs_t *regs = ctx->regs;

  ctx->target_eip = regs->eip + ctx->inst.imm.b;
  if (!X86ContextLoadReg(ctx, x86_inst_reg_ecx)) regs->eip = ctx->target_eip;
...
void x86_isa_jcxz_rel8_impl(X86Context *ctx) {
  ...
  ctx->target_eip = regs->eip + ctx->inst.imm.b;
  if (!X86ContextLoadReg(ctx, x86_inst_reg_cx)) regs->eip = ctx->target_eip;
  x86_uinst_new(ctx, x86_uinst_branch, x86_dep_ecx, 0, 0, 0, 0, 0, 0);
}
