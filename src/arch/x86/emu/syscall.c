...
static struct str_map_t x86_sys_error_code_map = {34,
                                                  {{"EPERM", 1},
                                                   {"ENOENT", 2},
                                                   {"ESRCH", 3},
                                                   {"EINTR", 4},
...
void x86_sys_dump_stats(FILE *f) {
  ...
  fprintf(f, "%-20s %s\n", "System call", "Count");
  for (i = 0; i < 30; i++) fprintf(f, "-");
  fprintf(f, "\n");

  /* Summary */
  for (i = 1; i <= x86_sys_code_count; i++)
    if (x86_sys_call_freq[i])
      fprintf(f, "%-20s %d\n", x86_sys_call_name[i], x86_sys_call_freq[i]);
  fprintf(f, "\n");
}
...
void X86ContextSyscall(X86Context *self) {
  ...
  x86_sys_debug("system call '%s' (code %d, inst %lld, pid %d)\n",
                x86_sys_call_name[code], code, asEmu(emu)->instructions,
                self->pid);
  ...
  if (code != x86_sys_code_sigreturn &&
      !X86ContextGetState(self, X86ContextSuspended))
    regs->eax = err;
...
static struct str_map_t sys_open_flags_map = {16,
                                              {{"O_RDONLY", 00000000},
                                               {"O_WRONLY", 00000001},
                                               {"O_RDWR", 00000002},
                                               {"O_CREAT", 00000100},
...
static struct x86_file_desc_t *x86_sys_open_virtual(X86Context *ctx, char *path,
                                                    int flags, int mode) {
  ...
  desc = x86_file_desc_table_entry_new(ctx->file_desc_table, file_desc_virtual,
                                       host_fd, temp_path, flags);
...
static int x86_sys_open_impl(X86Context *ctx) {
  ...
  desc = x86_file_desc_table_entry_new(ctx->file_desc_table, file_desc_regular,
...
static struct str_map_t sys_waitpid_options_map = {8,
                                                   {{"WNOHANG", 0x00000001},
...
static int x86_sys_waitpid_impl(X86Context *ctx) {
  ...
  x86_sys_debug("  pid=%d, pstatus=0x%x, options=0x%x\n", pid, status_ptr,
                options);
  str_map_flags(&sys_waitpid_options_map, options, options_str,
                sizeof options_str);
  ...
  if (child) {
    if (status_ptr) mem_write(mem, status_ptr, 4, &child->exit_code);
...
static int x86_sys_unlink_impl(X86Context *ctx) {
  ...
  file_name_ptr = regs->ebx;
  length = mem_read_string(mem, file_name_ptr, sizeof file_name, file_name);
  if (length >= MAX_PATH_SIZE) fatal("%s: buffer too small", __FUNCTION__);
  X86ContextGetFullPath(ctx, file_name, full_path, sizeof full_path);
...
static char *err_sys_execve_note =
    "\tA system call 'execve' is trying to run a command prefixed with "
    "'/bin/sh -c'.\n"
    "\tThis is usually the result of the execution of the 'system()' function "
    "from\n"
    "\tthe guest application to run a shell command. Multi2Sim will execute "
    "this\n"
...
static int x86_sys_execve_impl(X86Context *ctx) {
  ...
  if (length >= sizeof name) fatal("%s: buffer too small", __FUNCTION__);
  ...
  for (;;) {
    ...
    if (length >= sizeof arg_str) fatal("%s: buffer too small", __FUNCTION__);
  ...
  for (i = 0;; i++) {
    ...
    if (length >= sizeof env) fatal("%s: buffer too small", __FUNCTION__);
  ...
  if (!strcmp(full_path, "/bin/sh") && list_count(arg_list) == 3 &&
    ...
    for (i = 0; i < list_count(arg_list); i++) free(list_get(arg_list, i));
    list_free(arg_list);
    return 0;
  }
...
static int x86_sys_chdir_impl(X86Context *ctx) {
  ...
  if (length >= sizeof path) fatal("%s: buffer 'path' too small", __FUNCTION__);
  ...
  if (chdir(loader->cwd))
    fatal("%s: %s: cannot cd to guest working directory", __FUNCTION__,
          loader->cwd);
...
static int x86_sys_time_impl(X86Context *ctx) {
  ...
  /* Host call */
  t = time(NULL);
  if (time_ptr) mem_write(mem, time_ptr, 4, &t);
...
static int x86_sys_chmod_impl(X86Context *ctx) {
  ...
  if (len >= sizeof file_name) fatal("%s: buffer too small", __FUNCTION__);
  X86ContextGetFullPath(ctx, file_name, full_path, sizeof full_path);
...
static int x86_sys_getpid_impl(X86Context *ctx) { return ctx->pid; }
...
static void sys_utime_guest_to_host(struct utimbuf *host,
                                    struct sim_utimbuf *guest) {
...
static int x86_sys_utime_impl(X86Context *ctx) {
  ...
  /* Read file name */
  len = mem_read_string(mem, file_name_ptr, sizeof file_name, file_name);
  if (len >= MAX_PATH_SIZE) fatal("%s: buffer too small", __FUNCTION__);
...
static int x86_sys_kill_impl(X86Context *ctx) {
  ...
  if (!temp_ctx) fatal("%s: invalid pid %d", __FUNCTION__, pid);
...
static int x86_sys_rename_impl(X86Context *ctx) {
  ...
  if (len >= sizeof old_path) fatal("%s: buffer too small", __FUNCTION__);

  /* Get new path */
  len = mem_read_string(mem, new_path_ptr, sizeof new_path, new_path);
  if (len >= sizeof new_path) fatal("%s: buffer too small", __FUNCTION__);
...
static int x86_sys_mkdir_impl(X86Context *ctx) {
  ...
  if (len >= sizeof path) fatal("%s: buffer too small", __FUNCTION__);
...
static int x86_sys_dup_impl(X86Context *ctx) {
  ...
  if (dup_host_fd == -1) return -errno;

  /* Create a new entry in the file descriptor table. */
  dup_desc =
      x86_file_desc_table_entry_new(ctx->file_desc_table, file_desc_regular,
                                    dup_host_fd, desc->path, desc->flags);
...
static int x86_sys_pipe_impl(X86Context *ctx) {
  ...
  /* Create host pipe */
  err = pipe(host_fd);
  if (err == -1) fatal("%s: cannot create pipe", __FUNCTION__);
  x86_sys_debug("  host pipe created: fd={%d, %d}\n", host_fd[0], host_fd[1]);

  /* Create guest pipe */
  read_desc = x86_file_desc_table_entry_new(
      ctx->file_desc_table, file_desc_pipe, host_fd[0], "", O_RDONLY);
  write_desc = x86_file_desc_table_entry_new(
      ctx->file_desc_table, file_desc_pipe, host_fd[1], "", O_WRONLY);
...
static struct str_map_t sys_rlimit_res_map = {16,
                                              {
...
static void sys_rusage_host_to_guest(struct sim_rusage *guest,
                                     struct rusage *host) {
...
static int x86_sys_readlink_impl(X86Context *ctx) {
  ...
  if (!strcmp(full_path, "/proc/self/exe")) {
    ...
    if (err == sizeof dest_path) fatal("%s: buffer too small", __FUNCTION__);
    if (err == -1) return -errno;
  }
...
static int x86_sys_mmap_impl(X86Context *ctx) {
  ...
  x86_sys_debug(
      "  addr=0x%x, len=%u, prot=0x%x, flags=0x%x, "
...
static void sim_statfs_host_to_guest(struct sim_statfs_t *host,
                                     struct statfs *guest) {
  ...
  memcpy(&host->fsid, &guest->f_fsid,
         MIN(sizeof host->fsid, sizeof guest->f_fsid));
...
static int x86_sys_statfs_impl(X86Context *ctx) {
  ...
  /* Read path */
  length = mem_read_string(mem, path_ptr, sizeof path, path);
  if (length == sizeof path) fatal("%s: buffer too small", __FUNCTION__);
  x86_sys_debug("  path='%s'\n", path);
...
static struct str_map_t sys_socketcall_call_map = {17,
                                                   {{"SYS_SOCKET", 1},
                                                    {"SYS_BIND", 2},
                                                    {"SYS_CONNECT", 3},
                                                    {"SYS_LISTEN", 4},
...
static struct str_map_t sys_socket_family_map = {29,
                                                 {{"PF_UNSPEC", 0},
                                                  {"PF_UNIX", 1},
                                                  {"PF_INET", 2},
                                                  {"PF_AX25", 3},
...
static struct str_map_t sys_socket_type_map = {7,
                                               {{"SOCK_STREAM", 1},
                                                {"SOCK_DGRAM", 2},
                                                {"SOCK_RAW", 3},
                                                {"SOCK_RDM", 4},
...
static int x86_sys_socketcall_impl(X86Context *ctx) {
  ...
  switch (call) {
    ...
    case 1: {
      ...
      if ((type & 0xff) != 1)
        fatal("%s: SYS_SOCKET: only type SOCK_STREAM supported", __FUNCTION__);
      ...
      desc = x86_file_desc_table_entry_new(
          ctx->file_desc_table, file_desc_socket, host_fd, "", O_RDWR);
    ...
    case 3: {
      ...
      M2S_HOST_GUEST_MATCH((void *)&addr->sa_data - (void *)&addr->sa_family,
                           2);
    ...
    default:
      ...
      fatal("%s: call '%s' not implemented", __FUNCTION__, call_str);
  }
...
static struct str_map_t sys_itimer_which_map = {
    3, {{"ITIMER_REAL", 0}, {"ITIMER_VIRTUAL", 1}, {"ITIMER_PROF", 2}}};
...
static void sim_itimerval_dump(struct sim_itimerval *sim_itimerval) {
  x86_sys_debug("    it_interval: tv_sec=%u, tv_usec=%u\n",
                sim_itimerval->it_interval.tv_sec,
                sim_itimerval->it_interval.tv_usec);
  x86_sys_debug("    it_value: tv_sec=%u, tv_usec=%u\n",
                sim_itimerval->it_value.tv_sec,
                sim_itimerval->it_value.tv_usec);
}
...
static int x86_sys_setitimer_impl(X86Context *ctx) {
  ...
  x86_sys_debug("  which=%d (%s), value_ptr=0x%x, old_value_ptr=0x%x\n", which,
                str_map_value(&sys_itimer_which_map, which), value_ptr,
                old_value_ptr);
  ...
  if (which >= 3) fatal("%s: invalid value for 'which'", __FUNCTION__);

  /* Set 'itimer_value' and 'itimer_interval' (usec) */
  ctx->itimer_value[which] =
      now + itimerval.it_value.tv_sec * 1000000 + itimerval.it_value.tv_usec;
  ctx->itimer_interval[which] =
      itimerval.it_interval.tv_sec * 1000000 + itimerval.it_interval.tv_usec;
...
static const unsigned int sys_clone_supported_flags =
    SIM_CLONE_VM | SIM_CLONE_FS | SIM_CLONE_FILES | SIM_CLONE_SIGHAND |
    SIM_CLONE_THREAD | SIM_CLONE_SYSVSEM | SIM_CLONE_SETTLS |
    SIM_CLONE_PARENT_SETTID | SIM_CLONE_CHILD_CLEARTID | SIM_CLONE_CHILD_SETTID;
...
static int x86_sys_clone_impl(X86Context *ctx) {
  ...
  x86_sys_debug(
      "  flags=0x%x, newsp=0x%x, parent_tidptr=0x%x, child_tidptr=0x%x\n",
  ...
  x86_sys_debug("  exit_signal=%d (%s)\n", exit_signal,
                x86_signal_name(exit_signal));

  /* New stack pointer defaults to current */
  if (!new_esp) new_esp = regs->esp;
  ...
  if (flags & SIM_CLONE_THREAD) {
    ...
    new_ctx->group_parent = ctx->group_parent ? ctx->group_parent : ctx;
  } else {
  ...
  if (flags & SIM_CLONE_SETTLS) {
    ...
    if (uinfo.limit_in_pages) uinfo.limit <<= 12;
...
static int x86_sys_llseek_impl(X86Context *ctx) {
  ...
  host_fd = x86_file_desc_table_get_host_fd(ctx->file_desc_table, fd);
  x86_sys_debug(
      "  fd=%d, offset_high=0x%x, offset_low=0x%x, result_ptr=0x%x, "
      "origin=0x%x\n",
  ...
  if (result_ptr) mem_write(mem, result_ptr, 8, &offset);
...
static int x86_sys_getdents_impl(X86Context *ctx) {
  ...
  if (nread == -1) fatal("%s: host call failed", __FUNCTION__);
  ...
  while (host_offs < nread) {
    ...
    x86_sys_debug("d_reclen=%u(host),%u(guest) ", dirent->d_reclen,
                  sim_dirent.d_reclen);
    ...
    if (guest_offs > count) fatal("%s: buffer too small", __FUNCTION__);
  }
...
static int sim_fd_set_read(X86Context *ctx, uint32_t addr, fd_set *fds, int n) {
  struct mem_t *mem = ctx->mem;
...
static void sim_fd_set_write(X86Context *ctx, unsigned int addr, fd_set *fds,
                             int n) {
  struct mem_t *mem = ctx->mem;
...
static int x86_sys_select_impl(X86Context *ctx) {
   ...
   * int select(int n, fd_set *inp, fd_set *outp, fd_set *exp, struct timeval
   * *tvp);
   */
  ...
  /* Read file descriptor bitmaps. If any file descriptor is invalid, return
   * EBADF. */
  ...
  if (tvp) mem_read(mem, tvp, sizeof sim_tv, &sim_tv);
  x86_sys_debug("  tv:\n");
...
static struct str_map_t sys_msync_flags_map = {
    3, {{"MS_ASYNC", 1}, {"MS_INAVLIAGE", 2}, {"MS_SYNC", 4}}};
...
static int x86_sys_mremap_impl(X86Context *ctx) {
  ...
  if (new_len == old_len) return addr;
  ...
  if (new_len > old_len &&
      mem_map_space(mem, addr + old_len, new_len - old_len) == addr + old_len) {
  ...
  if (new_addr == -1) fatal("%s: out of guest memory", __FUNCTION__);

  /* Map new region and copy old one */
  mem_map(mem, new_addr, new_len, mem_access_read | mem_access_write);
...
static int x86_sys_clock_gettime_impl(X86Context *ctx) {
  ...
  switch (clk_id) {
    ...
    case 6: /* CLOCK_MONOTONIC_COARSE */
      ...
      fatal("%s: not implemented for 'clk_id' = %d", __FUNCTION__, clk_id);
      break;

    default:
      fatal("%s: invalid value for 'clk_id' (%d)", __FUNCTION__, clk_id);
  }
...
static struct str_map_t sys_poll_event_map = {6,
                                              {{"POLLIN", 0x0001},
                                               {"POLLPRI", 0x0002},
                                               {"POLLOUT", 0x0004},
                                               {"POLLERR", 0x0008},
...
static int x86_sys_poll_impl(X86Context *ctx) {
  ...
  str_map_flags(&sys_poll_event_map, guest_fds.events, events_str,
                MAX_STRING_SIZE);
  ...
  /* If host 'poll' returned a value greater than 0, the guest call is
   * non-blocking,
  ...
  ctx->wakeup_time = 0;
  if (timeout >= 0) ctx->wakeup_time = now + (long long)timeout * 1000;
  ctx->wakeup_fd = guest_fd;
...
static int x86_sys_rt_sigaction_impl(X86Context *ctx) {
  ...
  /* Invalid signal */
  if (sig < 1 || sig > 64) fatal("%s: invalid signal (%d)", __FUNCTION__, sig);
  ...
  if (act_ptr) ctx->signal_handler_table->sigaction[sig - 1] = act;
...
static struct str_map_t sys_sigprocmask_how_map = {
    3, {{"SIG_BLOCK", 0}, {"SIG_UNBLOCK", 1}, {"SIG_SETMASK", 2}}};
...
static int x86_sys_rt_sigprocmask_impl(X86Context *ctx) {
  ...
  /* Return old set */
  if (old_set_ptr) mem_write(mem, old_set_ptr, 8, &old_set);
...
static int x86_sys_mmap2_impl(X86Context *ctx) {
  ...
  return x86_sys_mmap(ctx, addr, len, prot, flags, guest_fd,
...
static void sys_stat_host_to_guest(struct sim_stat64_t *guest,
                                   struct stat *host) {
...
static int x86_sys_stat64_impl(X86Context *ctx) {
  ...
  /* Read file name */
  length = mem_read_string(mem, file_name_ptr, sizeof file_name, file_name);
  if (length == sizeof file_name) fatal("%s: buffer too small", __FUNCTION__);
...
static int x86_sys_getuid_impl(X86Context *ctx) { return getuid(); }

/*
 * System call 'getgid' (code 200)
 */

static int x86_sys_getgid_impl(X86Context *ctx) { return getgid(); }

/*
 * System call 'geteuid' (code 201)
 */

static int x86_sys_geteuid_impl(X86Context *ctx) { return geteuid(); }

/*
 * System call 'getegid' (code 202)
 */

static int x86_sys_getegid_impl(X86Context *ctx) { return getegid(); }
...
static int x86_sys_getdents64_impl(X86Context *ctx) {
  ...
  host_fd = x86_file_desc_table_get_host_fd(ctx->file_desc_table, fd);
  x86_sys_debug("  host_fd=%d\n", host_fd);
  if (host_fd < 0) return -EBADF;

  /* Host call */
  buf = xcalloc(1, count);
  nread = syscall(SYS_getdents, host_fd, buf, count);
  if (nread < 0) fatal("%s: host call failed", __FUNCTION__);
  ...
  while (host_offs < nread) {
    ...
    guest_dirent.d_type =
        *(char *)(buf + host_offs + host_dirent->d_reclen - 1);

    x86_sys_debug("    d_ino=%lld ", guest_dirent.d_ino);
    x86_sys_debug("d_off=%lld ", guest_dirent.d_off);
    x86_sys_debug("d_reclen=%u(host),%u(guest) ", host_dirent->d_reclen,
                  guest_dirent.d_reclen);
    x86_sys_debug("d_name='%s'\n", host_dirent->d_name);
    ...
    if (guest_offs > count) fatal("getdents: host buffer too small");
  }
...
static struct str_map_t sys_fcntl_cmp_map = {15,
                                             {{"F_DUPFD", 0},
                                              {"F_GETFD", 1},
                                              {"F_SETFD", 2},
                                              {"F_GETFL", 3},
...
static struct str_map_t sys_futex_cmd_map = {13,
                                             {{"FUTEX_WAIT", 0},
                                              {"FUTEX_WAKE", 1},
                                              {"FUTEX_FD", 2},
                                              {"FUTEX_REQUEUE", 3},
...
static int x86_sys_futex_impl(X86Context *ctx) {
  ...
  /* Prototype: sys_futex(void *addr1, int op, int val1, struct timespec
   * *timeout,
   *   void *addr2, int val3); */
  ...
  val3 = regs->ebp;
  x86_sys_debug(
      "  addr1=0x%x, op=%d, val1=%d, ptimeout=0x%x, addr2=0x%x, val3=%d\n",
  ...
  x86_sys_debug("  futex=%d, cmd=%d (%s)\n", futex, cmd,
                str_map_value(&sys_futex_cmd_map, cmd));
  ...
  switch (cmd) {
    ...
    case 4: /* FUTEX_CMP_REQUEUE */
    {
      ...
      if (timeout_ptr != 0x7fffffff)
        fatal("%s: FUTEX_CMP_REQUEUE: only supported for ptimeout=INTMAX",
              __FUNCTION__);

      /* The value of val3 must be the same as the value of the futex
       * at 'addr1' (stored in 'futex') */
      if (futex != val3) return -EAGAIN;

      /* Wake up 'val1' threads from futex at 'addr1'. The number of woken up
       * threads
       * is the return value of the system call. */
      ret = X86ContextFutexWake(ctx, addr1, val1, 0xffffffff);
      x86_sys_debug("  futex at 0x%x: %d processes woken up\n", addr1, ret);

      /* The rest of the threads waiting in futex 'addr1' are requeued into
       * futex 'addr2' */
    ...
    case 5: /* FUTEX_WAKE_OP */
    {
      ...
      if (cond) ret += X86ContextFutexWake(ctx, addr2, val2, 0xffffffff);
    ...
    default:
      fatal("%s: not implemented for cmd=%d (%s).\n%s", __FUNCTION__, cmd,
            str_map_value(&sys_futex_cmd_map, cmd), err_x86_sys_note);
  }
...
static int x86_sys_sched_setaffinity_impl(X86Context *ctx) {
  ...
  for (i = 0; i < size; i++)
    for (j = 0; j < 8; j++)
      if (mask[i] & (1 << j)) x86_sys_debug(" %d", i * 8 + j);
  x86_sys_debug(" }\n");
...
static int x86_sys_sched_getaffinity_impl(X86Context *ctx) {
  ...
  for (i = 0; i < size && node < num_nodes; i++)
    for (j = 0; j < 8 && node < num_nodes; j++)
      mask[i] |= bit_map_get(target_ctx->affinity, node++, 1) << j;
...
static void sim_statfs64_host_to_guest(struct sim_statfs64_t *host,
                                       struct statfs *guest) {
...
static int x86_sys_openat_impl(X86Context *ctx) {
  ...
  /* Implemented cases:
   * dirfd = AT_FDCWD (-100), path is relative -> path is relative to current
   * directory.
   * path is absolute -> dirfd ignored
   * dirfd != AT_FDCWD, path is relative -> path is relative to 'difd' (not
   * implemented)
   */
  if (dirfd != -100 && path[0] != '/')
    fatal("%s: difd != AT_FDCWD with relative path not implemented",
          __FUNCTION__);
