...
struct str_map_t x86_signal_map = {31,
                                   {{"SIGHUP", 1},
                                    {"SIGINT", 2},
                                    {"SIGQUIT", 3},
                                    {"SIGILL", 4},
...
struct str_map_t x86_sigaction_flags_map = {9,
...
void x86_sigaction_dump(struct x86_sigaction_t *sim, FILE *f) {
  fprintf(f, "handler=0x%x, flags=0x%x, restorer=0x%x, mask=0x%llx",
          sim->handler, sim->flags, sim->restorer, sim->mask);
}
...
void X86ContextRunSignalHandler(X86Context *ctx, int sig) {
  ...
  ctx->signal_mask_table->pretcode =
      mem_map_space(ctx->mem, MEM_PAGE_SIZE, MEM_PAGE_SIZE);
  mem_map(ctx->mem, ctx->signal_mask_table->pretcode, MEM_PAGE_SIZE,
          mem_access_exec | mem_access_init);
  x86_sys_debug("  return code of signal handler allocated at 0x%x\n",
                ctx->signal_mask_table->pretcode);
  mem_access(ctx->mem, ctx->signal_mask_table->pretcode,
             sizeof(x86_signal_retcode), x86_signal_retcode, mem_access_init);
  ...
  if (!handler) fatal("%s: invalid signal handler", __FUNCTION__);
...
void X86ContextCheckSignalHandler(X86Context *ctx) {
  ...
  if (X86ContextGetState(ctx, X86ContextHandler)) return;
