...
void x86_regs_free(struct x86_regs_t *regs) { free(regs); }
...
void x86_regs_dump(struct x86_regs_t *regs, FILE *f) {
  ...
  fprintf(f, "  es=%x, cs=%x, ss=%x, ds=%x, fs=%x, gs=%x\n", regs->es, regs->cs,
          regs->ss, regs->ds, regs->fs, regs->gs);
  fprintf(f, "  eip=%08x\n", regs->eip);
  fprintf(f, "  flags=%04x (cf=%d  pf=%d  af=%d  zf=%d  sf=%d  df=%d  of=%d)\n",
          regs->eflags, (regs->eflags & (1 << x86_inst_flag_cf)) > 0,
  ...
  for (i = 0; i < 8; i++) {
    fprintf(f, "  xmm%d =", i);
    for (j = 0; j < 16; j++) fprintf(f, " %02x", regs->xmm[i][j]);
    fprintf(f, "\n");
  }
}
