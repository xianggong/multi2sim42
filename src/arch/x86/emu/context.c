...
static void X86ContextDoCreate(X86Context *self, X86Emu *emu) {
  ...
  for (i = 0; i < num_nodes; i++) bit_map_set(self->affinity, i, 1, 1);
...
void X86ContextCreateAndClone(X86Context *self, X86Context *cloned) {
  ...
  /* Signal handlers and file descriptor table */
  self->signal_handler_table =
      x86_signal_handler_table_link(cloned->signal_handler_table);
...
void X86ContextExecute(X86Context *self) {
  ...
  if (!buffer_ptr) {
     ...
     * actual instruction, and they lie on a page with no permissions, this
     * would
...
/* Force a new 'eip' value for the context. The forced value should be the same
 * as
 * the current 'eip' under normal circumstances. If it is not, speculative
 * execution
...
static void X86ContextUpdateState(X86Context *self, X86ContextState state) {
  ...
  if (status_diff & ~X86ContextSpecMode) emu->schedule_signal = 1;
...
X86Context *X86ContextGetZombie(X86Context *self, int pid) {
  ...
  for (context = emu->zombie_list_head; context;
    ...
    if (context->parent != self) continue;
    if (context->pid == pid || pid == -1) return context;
  }
  return NULL;
}
...
void X86ContextFinishGroup(X86Context *self, int state) {
  ...
  /* Get group parent */
  if (self->group_parent) self = self->group_parent;
  assert(!self->group_parent); /* Only one level */

  /* Context already finished */
  if (X86ContextGetState(self, X86ContextFinished | X86ContextZombie)) return;

  /* Finish all contexts in the group */
  DOUBLE_LINKED_LIST_FOR_EACH(emu, context, aux) {
    if (aux->group_parent != self && aux != self) continue;
 ...
 * to 'x86_ctx_finished'. If it has, its state is set to 'x86_ctx_zombie',
 * waiting for
 * a call to 'waitpid'.
  ...
  while (count) {
    ...
    for (self = emu->suspended_list_head; self;
      ...
      if (!X86ContextGetState(self, X86ContextFutex) ||
        ...
        continue;
      if (!(self->wakeup_futex_bitset & bitset)) continue;
      if (!wakeup_ctx ||
          self->wakeup_futex_sleep < wakeup_ctx->wakeup_futex_sleep)
        wakeup_ctx = self;
    }
...
void X86ContextExitRobustList(X86Context *self) {
  ...
  for (;;) {
    ...
    if (!next || next == self->robust_list_head) break;
    lock_entry = next;
  }
}
...
void *X86EmuHostThreadSuspend(void *arg) {
   ...
   * its resources. The thread termination can be observed by atomically
   * checking
  ...
  if (X86ContextGetState(self, X86ContextNanosleep)) {
    ...
    err = poll(&host_fds, 1, -1);
    if (err < 0) fatal("syscall 'write': unexpected error in host 'write'");
  }
...
void *X86ContextHostThreadTimer(void *arg) {
  ...
  /* Detach this thread - we don't want the parent to have to join it to release
   * its resources. The thread termination can be observed by thread-safely
   * checking
