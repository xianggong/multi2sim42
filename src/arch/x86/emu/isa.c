...
void x86_isa_inst_stat_reset(void) {
  int i;
  for (i = 1; i < x86_inst_opcode_count; i++) x86_inst_freq[i] = 0;
}
...
void X86ContextMemRead(X86Context *self, unsigned int addr, int size,
                       void *buf) {
...
void X86ContextMemWrite(X86Context *self, unsigned int addr, int size,
                        void *buf) {
...
void X86ContextError(X86Context *self, char *fmt, ...) {
  ...
  if (self->state & X86ContextSpecMode) return;

  /* Error */
  fprintf(stderr, "fatal: x86 context %d at 0x%08x inst %lld: ", self->pid,
          self->curr_eip, asEmu(emu)->instructions);
...
static void X86ContextDebugCallInst(X86Context *self) {
  ...
  /* Debug it */
  for (i = 0; i < self->function_level; i++) X86ContextDebugCall("| ");
...
/* Load/store the value of a register. If the register size is less than 32
 * bits,
...
void X86ContextStoreReg(X86Context *self, enum x86_inst_reg_t reg,
                        unsigned int value) {
  struct x86_regs_t *regs = self->regs;
  ...
  *reg_ptr = (*reg_ptr & ~mask) | (value & mask);
  X86ContextDebugISA("  %s <- 0x%x", str_map_value(&x86_inst_reg_map, reg),
                     value);
}
...
static unsigned int X86ContextLinearAddress(X86Context *self,
                                            unsigned int offset) {
  /* No segment override */
  if (!self->inst.segment) return offset;
...
unsigned int X86ContextEffectiveAddress(X86Context *self) {
   ...
   * generation of micro-instructions. We need to record it to avoid calling
   * this
   * function again later, since the source register used to calculate the
   * effective
...
unsigned long long X86ContextLoadM64(X86Context *self) {
  ...
  X86ContextDebugISA("  [0x%x]=0x%llx", X86ContextEffectiveAddress(self),
                     value);
  return value;
}
...
void X86ContextStoreRm8(X86Context *self, unsigned char value) {
  ...
  X86ContextDebugISA("  [0x%x] <- 0x%x", X86ContextEffectiveAddress(self),
                     value);
}
...
void X86ContextStoreFpu(X86Context *self, int index, unsigned char *value) {
  ...
  if (debug_status(x86_context_isa_debug_category))
    X86ContextDebugISA("  st(%d) <- %g", index,
                       X86ContextExtendedToDouble(value));
}
...
void X86ContextPushFpu(X86Context *self, unsigned char *value) {
  ...
  /* Debug */
  if (debug_status(x86_context_isa_debug_category))
    X86ContextDebugISA("  st(0) <- %g (pushed)",
                       X86ContextExtendedToDouble(value));
...
void X86ContextPopFpu(X86Context *self, unsigned char *value) {
  ...
  if (value) {
    ...
    if (debug_status(x86_context_isa_debug_category))
      X86ContextDebugISA("  st(0) -> %g (popped)",
                         X86ContextExtendedToDouble(value));
  }
...
float X86ContextLoadFloat(X86Context *self) {
  ...
  X86ContextMemRead(self, X86ContextEffectiveAddress(self), 4, &value);
  X86ContextDebugISA("  [0x%x]=%g", X86ContextEffectiveAddress(self),
                     (double)value);
...
void X86ContextStoreFpuCode(X86Context *self, unsigned short status) {
  ...
  regs->fpu_code = 0;
  if (GETBIT32(status, 14)) regs->fpu_code |= 0x8;
...
unsigned short X86ContextLoadFpuStatus(X86Context *self) {
  ...
  if (GETBIT32(regs->fpu_code, 3)) status |= 0x4000;
...
void X86ContextDumpXMM(X86Context *self, unsigned char *value, FILE *f) {
  ...
  for (i = 0; i < 16; i++) fprintf(f, "%02x ", xmm->as_uchar[i]);
...
void X86ContextExecuteInst(X86Context *self) {
  ...
  /* Debug */
  if (debug_status(x86_context_isa_debug_category)) {
    X86ContextDebugISA("%d %8lld %x: ", self->pid, asEmu(emu)->instructions,
                       self->curr_eip);
  ...
  if (self->inst.opcode) x86_context_inst_func[self->inst.opcode](self);
