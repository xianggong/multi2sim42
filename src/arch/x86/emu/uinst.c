...
struct str_map_t x86_uinst_dep_map = {13,
                                      {
                                       {"*RM8*", x86_dep_rm8},
                                       {"*RM16*", x86_dep_rm16},
                                       {"*RM32*", x86_dep_rm32},
 ...
 * generate a new address computation, but just read the 'x86_dep_ea'
 * dependence. */
static int x86_uinst_effaddr_emitted;

/* If dependence 'index' in 'uinst' is a memory operand, return its size in
 * bytes.
 * Otherwise, return 0.
 * Also returns a regular dependence corresponding the memory dependence data
 * type. */
static int x86_uinst_mem_dep_size(struct x86_uinst_t *uinst, int index,
  ...
  switch (dep) {
    ...
    case x86_dep_rm32:
      ...
      if (ctx->inst.modrm_mod == 3) return 0;
...
static void x86_uinst_emit_effaddr(struct x86_uinst_t *uinst, int index,
  ...
  if (!x86_uinst_mem_dep_size(uinst, index, ctx, NULL)) return;
  ...
  new_uinst->idep[0] = ctx->inst.segment
                           ? ctx->inst.segment - x86_inst_reg_es + x86_dep_es
                           : x86_dep_none;
  new_uinst->idep[1] = ctx->inst.ea_base
                           ? ctx->inst.ea_base - x86_inst_reg_eax + x86_dep_eax
                           : x86_dep_none;
  new_uinst->idep[2] = ctx->inst.ea_index
                           ? ctx->inst.ea_index - x86_inst_reg_eax + x86_dep_eax
                           : x86_dep_none;
...
static void x86_uinst_parse_dep(struct x86_uinst_t *uinst, int index,
  ...
  assert(index >= 0 && index < X86_UINST_MAX_DEPS);
  dep = uinst->dep[index];
  if (X86_DEP_IS_VALID(dep)) return;
  ...
  switch (dep) {
    ...
    case x86_dep_easeg:
      ...
      uinst->dep[index] = ctx->inst.segment
                              ? ctx->inst.segment - x86_inst_reg_es + x86_dep_es
                              : x86_dep_none;
      break;
    ...
    case x86_dep_eabas:
      ...
      uinst->dep[index] =
          ctx->inst.ea_base ? ctx->inst.ea_base - x86_inst_reg_eax + x86_dep_eax
                            : x86_dep_none;
      break;
     ...
     * Otherwise, they would have been handled by 'parse_idep'/'parse_odep'
     * functions. */
    case x86_dep_rm8:

      assert(ctx->inst.modrm_mod == 3);
      uinst->dep[index] = ctx->inst.modrm_rm < 4
                              ? x86_dep_eax + ctx->inst.modrm_rm
    ...
    case x86_dep_ir8:
      ...
      uinst->dep[index] = ctx->inst.opindex < 4
                              ? x86_dep_eax + ctx->inst.opindex
...
static int x86_uinst_add_idep(struct x86_uinst_t *uinst, enum x86_dep_t dep) {
  ...
  for (i = 0; i < X86_UINST_MAX_IDEPS; i++)
    if (!uinst->idep[i]) break;
  if (i == X86_UINST_MAX_IDEPS) return 0;
  uinst->idep[i] = dep;
  return 1;
}
...
void x86_uinst_free(struct x86_uinst_t *uinst) { free(uinst); }

void __x86_uinst_new_mem(X86Context *ctx, enum x86_uinst_opcode_t opcode,
                         unsigned int address, int size, enum x86_dep_t idep0,
                         enum x86_dep_t idep1, enum x86_dep_t idep2,
                         enum x86_dep_t odep0, enum x86_dep_t odep1,
                         enum x86_dep_t odep2, enum x86_dep_t odep3) {
  struct x86_uinst_t *uinst;
  int i;
  ...
  for (i = 0; i < X86_UINST_MAX_IDEPS; i++) x86_uinst_parse_idep(uinst, i, ctx);
...
void __x86_uinst_new(X86Context *ctx, enum x86_uinst_opcode_t opcode,
                     enum x86_dep_t idep0, enum x86_dep_t idep1,
                     enum x86_dep_t idep2, enum x86_dep_t odep0,
                     enum x86_dep_t odep1, enum x86_dep_t odep2,
                     enum x86_dep_t odep3) {
  __x86_uinst_new_mem(ctx, opcode, 0, 0, idep0, idep1, idep2, odep0, odep1,
                      odep2, odep3);
}
...
void x86_uinst_dump_buf(struct x86_uinst_t *uinst, char *buf, int size) {
  ...
  if (!dep_count) str_printf(&buf, &size, "-");
