...
struct x86_file_desc_t *x86_file_desc_create(enum x86_file_desc_kind_t kind,
  ...
  /* Path */
  if (path) desc->path = xstrdup(path);
...
void x86_file_desc_table_free(struct x86_file_desc_table_t *table) {
  ...
  for (i = 0; i < list_count(table->file_desc_list); i++) {
    ...
    if (desc) x86_file_desc_free(desc);
  }
...
struct x86_file_desc_table_t *x86_file_desc_table_link(
    struct x86_file_desc_table_t *fdt) {
  fdt->num_links++;
  return fdt;
}
...
void x86_file_desc_table_dump(struct x86_file_desc_table_t *table, FILE *f) {
  ...
  fprintf(f, "  %d table entries, %d busy\n", list_count(table->file_desc_list),
          busy);
}

int x86_file_desc_table_get_host_fd(struct x86_file_desc_table_t *table,
                                    int guest_fd) {
  struct x86_file_desc_t *desc;
...
int x86_file_desc_table_get_guest_fd(struct x86_file_desc_table_t *table,
                                     int host_fd) {
  struct x86_file_desc_t *desc;
  int i;
  ...
  for (i = 0; i < list_count(table->file_desc_list); i++) {
    desc = list_get(table->file_desc_list, i);
    if (desc->host_fd == host_fd) return i;
  }
...
void x86_file_desc_table_entry_dump(struct x86_file_desc_table_t *table,
                                    int index, FILE *f) {
  struct x86_file_desc_t *desc;

  /* Invalid entry */
  if (index < 0 || index >= list_count(table->file_desc_list)) return;
  ...
  if (desc) {
    ...
    fprintf(f, "    %2d  host_fd=%-2d  kind=%-2d  path='%s'\n", desc->guest_fd,
            desc->host_fd, desc->kind, desc->path);
  } else {
    fprintf(f, "    %2d  empty\n", index);
  }
}

struct x86_file_desc_t *x86_file_desc_table_entry_get(
    struct x86_file_desc_table_t *table, int index) {
  return list_get(table->file_desc_list, index);
}

struct x86_file_desc_t *x86_file_desc_table_entry_new(
    struct x86_file_desc_table_t *table, enum x86_file_desc_kind_t kind,
    int host_fd, char *path, int flags) {
  return x86_file_desc_table_entry_new_guest_fd(table, kind, -1, host_fd, path,
                                                flags);
}

struct x86_file_desc_t *x86_file_desc_table_entry_new_guest_fd(
    struct x86_file_desc_table_t *table, enum x86_file_desc_kind_t kind,
  ...
  for (i = 0; i < list_count(table->file_desc_list) && guest_fd < 0; i++)
    if (!list_get(table->file_desc_list, i)) guest_fd = i;
...
void x86_file_desc_table_entry_free(struct x86_file_desc_table_t *table,
                                    int index) {
  struct x86_file_desc_t *desc;
