...
void X86EmuDestroy(X86Emu *self) {
  ...
  for (ctx = self->context_list_head; ctx; ctx = ctx->context_list_next)
    if (!X86ContextGetState(ctx, X86ContextFinished)) X86ContextFinish(ctx, 0);

  /* Free contexts */
  while (self->context_list_head) delete (self->context_list_head);
}
...
void X86EmuProcessEvents(X86Emu *self) {
  ...
  for (ctx = self->suspended_list_head; ctx; ctx = next) {
    ...
    if (X86ContextGetState(ctx, X86ContextNanosleep)) {
      ...
      /* If 'X86EmuHostThreadSuspend' is still running for this context, do
       * nothing. */
      if (ctx->host_thread_suspend_active) continue;

      /* Timeout expired */
      if (ctx->wakeup_time <= now) {
        if (rmtp) mem_write(ctx->mem, rmtp, 8, &zero);
      ...
      if (pthread_create(&ctx->host_thread_suspend, NULL,
                         X86EmuHostThreadSuspend, ctx))
    ...
    if (X86ContextGetState(ctx, X86ContextSigsuspend)) {
      ...
      if (ctx->signal_mask_table->pending & ~ctx->signal_mask_table->blocked) {
        ...
        x86_sys_debug(
            "syscall 'rt_sigsuspend' - interrupted by signal (pid %d)\n",
            ctx->pid);
    ...
    if (X86ContextGetState(ctx, X86ContextPoll)) {
      ...
      if (!fd) fatal("syscall 'poll': invalid 'wakeup_fd'");
      ...
      if (err < 0) fatal("syscall 'poll': unexpected error in host 'poll'");
      ...
      if (ctx->wakeup_events & host_fds.revents & POLLOUT) {
        ...
        ctx->regs->eax = 1;
        x86_sys_debug(
            "syscall poll - continue (pid %d) - POLLOUT occurred in file\n",
            ctx->pid);
      ...
      if (ctx->wakeup_events & host_fds.revents & POLLIN) {
        ...
        ctx->regs->eax = 1;
        x86_sys_debug(
            "syscall poll - continue (pid %d) - POLLIN occurred in file\n",
            ctx->pid);
        x86_sys_debug("  retval=%d\n", ctx->regs->eax);
        X86ContextClearState(ctx, X86ContextSuspended | X86ContextPoll);
        continue;
    ...
    if (X86ContextGetState(ctx, X86ContextWrite)) {
      ...
      /* Get file descriptor */
      fd = x86_file_desc_table_entry_get(ctx->file_desc_table, ctx->wakeup_fd);
      if (!fd) fatal("syscall 'write': invalid 'wakeup_fd'");

      /* Check if data is ready in file by polling it */
      host_fds.fd = fd->host_fd;
      host_fds.events = POLLOUT;
      err = poll(&host_fds, 1, 0);
      if (err < 0) fatal("syscall 'write': unexpected error in host 'poll'");
      ...
      /* Data is not ready to be written - launch 'X86EmuHostThreadSuspend'
       * again */
      ctx->host_thread_suspend_active = 1;
      if (pthread_create(&ctx->host_thread_suspend, NULL,
                         X86EmuHostThreadSuspend, ctx))
    ...
    if (X86ContextGetState(ctx, X86ContextRead)) {
      ...
      /* Get file descriptor */
      fd = x86_file_desc_table_entry_get(ctx->file_desc_table, ctx->wakeup_fd);
      if (!fd) fatal("syscall 'read': invalid 'wakeup_fd'");

      /* Check if data is ready in file by polling it */
      host_fds.fd = fd->host_fd;
      host_fds.events = POLLIN;
      err = poll(&host_fds, 1, 0);
      if (err < 0) fatal("syscall 'read': unexpected error in host 'poll'");
      ...
      if (host_fds.revents) {
        ...
        if (count < 0) fatal("syscall 'read': unexpected error in host 'read'");
    ...
    if (X86ContextGetState(ctx, X86ContextWaitpid)) {
      ...
      if (child) {
        ...
        if (pstatus) mem_write(ctx->mem, pstatus, 4, &child->exit_code);
     ...
     * calls started using it. It is nicer, since it allows for a check of wake
     * up
     * conditions together with the system call itself, without having
     * distributed
  ...
  for (ctx = self->context_list_head; ctx; ctx = ctx->context_list_next) {
    ...
    if (ctx->host_thread_timer_active) continue;

    /* Check for any expired 'itimer': itimer_value < now
     * In this case, send corresponding signal to process.
     * Then calculate next 'itimer' occurrence: itimer_value = now +
     * itimer_interval */
    for (i = 0; i < 3; i++) {
      /* Timer inactive or not expired yet */
      if (!ctx->itimer_value[i] || ctx->itimer_value[i] > now) continue;

      /* Timer expired - send a signal.
       * The target process might be suspended, so the host thread is canceled,
       * and a new
       * call to 'X86EmuProcessEvents' is scheduled. Since
       * 'ke_process_events_mutex' is
       * already locked, the thread-unsafe version of
       * 'x86_ctx_host_thread_suspend_cancel' is used. */
    ...
    for (i = 0; i < 3; i++) {
      if (!ctx->itimer_value[i]) continue;
    ...
    if (ctx->host_thread_timer_wakeup) {
      ...
      if (pthread_create(&ctx->host_thread_timer, NULL,
                         X86ContextHostThreadTimer, ctx))
...
int X86EmuRun(Emu *self) {
  ...
  while (emu->finished_list_head) delete (emu->finished_list_head);
...
X86Context *X86EmuGetContext(X86Emu *self, int pid) {
  ...
  while (context && context->pid != pid) context = context->context_list_next;
  return context;
}

void X86EmuLoadContextsFromConfig(X86Emu *self, struct config_t *config,
                                  char *section) {
  X86Context *ctx;
  ...
  if (!*exe) fatal("%s: [%s]: invalid executable", config_file_name, section);
  ...
  if (*cwd)
    ...
    if (!loader->cwd) panic("%s: buffer too small", __FUNCTION__);
