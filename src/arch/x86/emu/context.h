...
unsigned int
    str_op_esi; /* Initial value for register 'esi' in string operation */
unsigned int
    str_op_edi;   /* Initial value for register 'edi' in string operation */
...
/* Host thread that suspends and then schedules call to
 * 'x86_emu_process_events'. */
/* The 'host_thread_suspend_active' flag is set when a 'host_thread_suspend'
 * thread
...
long long itimer_value[3]; /* Time when current occurrence of timer expires
                              (0=inactive) */
long long
    itimer_interval[3]; /* Interval (in usec) of repetition (0=inactive) */
