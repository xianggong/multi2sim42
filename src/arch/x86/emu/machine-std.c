...
#define op_stdop_al_imm8(stdop, wb, cin, uinst)                       \
  void x86_isa_##stdop##_al_imm8_impl(X86Context *ctx) {              \
    ...
    if (wb) {                                                         \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_eax, cin_dep, 0, x86_dep_eax, \
                    x86_dep_zps, x86_dep_cf, x86_dep_of);             \
    } else {                                                          \
      x86_uinst_new(ctx, uinst, x86_dep_eax, cin_dep, 0, x86_dep_zps, \
...
#define op_stdop_rm8_imm8(stdop, wb, cin, uinst)                      \
  void x86_isa_##stdop##_rm8_imm8_impl(X86Context *ctx) {             \
    ...
    if (wb) {                                                         \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_rm8, cin_dep, 0, x86_dep_rm8, \
                    x86_dep_zps, x86_dep_cf, x86_dep_of);             \
    } else {                                                          \
      x86_uinst_new(ctx, uinst, x86_dep_rm8, cin_dep, 0, x86_dep_zps, \
                    x86_dep_cf, x86_dep_of, 0);                       \
    }                                                                 \
    regs->eflags = flags;                                             \
  }
...
#define op_stdop_rm16_imm16(stdop, wb, cin, uinst)                      \
  void x86_isa_##stdop##_rm16_imm16_impl(X86Context *ctx) {             \
    ...
    if (wb) {                                                           \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_rm16, cin_dep, 0, x86_dep_rm16, \
                    x86_dep_zps, x86_dep_cf, x86_dep_of);               \
    } else {                                                            \
      x86_uinst_new(ctx, uinst, x86_dep_rm16, cin_dep, 0, x86_dep_zps,  \
                    x86_dep_cf, x86_dep_of, 0);                         \
    }                                                                   \
    regs->eflags = flags;                                               \
  }
...
#define op_stdop_rm32_imm32(stdop, wb, cin, uinst)                      \
  void x86_isa_##stdop##_rm32_imm32_impl(X86Context *ctx) {             \
    ...
    if (wb) {                                                           \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_rm32, cin_dep, 0, x86_dep_rm32, \
                    x86_dep_zps, x86_dep_cf, x86_dep_of);               \
    } else {                                                            \
      x86_uinst_new(ctx, uinst, x86_dep_rm32, cin_dep, 0, x86_dep_zps,  \
                    x86_dep_cf, x86_dep_of, 0);                         \
    }                                                                   \
    regs->eflags = flags;                                               \
  }
...
#define op_stdop_rm8_r8(stdop, wb, cin, uinst)                                 \
  void x86_isa_##stdop##_rm8_r8_impl(X86Context *ctx) {                        \
    ...
    if (wb) {                                                                  \
      X86ContextStoreRm8(ctx, rm8);                                            \
      x86_uinst_new(ctx, uinst, x86_dep_rm8, x86_dep_r8, cin_dep, x86_dep_rm8, \
                    x86_dep_zps, x86_dep_cf, x86_dep_of);                      \
    } else {                                                                   \
      x86_uinst_new(ctx, uinst, x86_dep_rm8, x86_dep_r8, cin_dep, x86_dep_zps, \
                    x86_dep_cf, x86_dep_of, 0);                                \
    }                                                                          \
    regs->eflags = flags;                                                      \
  }
...
#define op_stdop_rm16_r16(stdop, wb, cin, uinst)                        \
  void x86_isa_##stdop##_rm16_r16_impl(X86Context *ctx) {               \
    ...
    if (wb) {                                                           \
      X86ContextStoreRm16(ctx, rm16);                                   \
      x86_uinst_new(ctx, uinst, x86_dep_rm16, x86_dep_r16, cin_dep,     \
...
#define op_stdop_rm32_r32(stdop, wb, cin, uinst)                        \
  void x86_isa_##stdop##_rm32_r32_impl(X86Context *ctx) {               \
    ...
    if (wb) {                                                           \
      X86ContextStoreRm32(ctx, rm32);                                   \
      x86_uinst_new(ctx, uinst, x86_dep_rm32, x86_dep_r32, cin_dep,     \
...
#define op_stdop_r8_rm8(stdop, wb, cin, uinst)                                 \
  void x86_isa_##stdop##_r8_rm8_impl(X86Context *ctx) {                        \
    ...
    if (wb) {                                                                  \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_r8, x86_dep_rm8, cin_dep, x86_dep_r8,  \
                    x86_dep_zps, x86_dep_cf, x86_dep_of);                      \
    } else {                                                                   \
      x86_uinst_new(ctx, uinst, x86_dep_r8, x86_dep_rm8, cin_dep, x86_dep_zps, \
                    x86_dep_cf, x86_dep_of, 0);                                \
    }                                                                          \
    regs->eflags = flags;                                                      \
  }
...
#define op_stdop_r16_rm16(stdop, wb, cin, uinst)                       \
  void x86_isa_##stdop##_r16_rm16_impl(X86Context *ctx) {              \
    ...
    if (wb) {                                                          \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_r16, x86_dep_rm16, cin_dep,    \
                    x86_dep_r16, x86_dep_zps, x86_dep_cf, x86_dep_of); \
    } else {                                                           \
      x86_uinst_new(ctx, uinst, x86_dep_r16, x86_dep_rm16, cin_dep,    \
                    x86_dep_zps, x86_dep_cf, x86_dep_of, 0);           \
    }                                                                  \
...
#define op_stdop_r32_rm32(stdop, wb, cin, uinst)                       \
  void x86_isa_##stdop##_r32_rm32_impl(X86Context *ctx) {              \
    ...
    if (wb) {                                                          \
      ...
      x86_uinst_new(ctx, uinst, x86_dep_r32, x86_dep_rm32, cin_dep,    \
                    x86_dep_r32, x86_dep_zps, x86_dep_cf, x86_dep_of); \
    } else {                                                           \
      x86_uinst_new(ctx, uinst, x86_dep_r32, x86_dep_rm32, cin_dep,    \
                    x86_dep_zps, x86_dep_cf, x86_dep_of, 0);           \
    }                                                                  \
...
#define op_stdop_all(stdop, wb, cin, uinst)                                 \
  op_stdop_al_imm8(stdop, wb, cin, uinst) op_stdop_ax_imm16(                \
      stdop, wb, cin, uinst) op_stdop_eax_imm32(stdop, wb, cin, uinst)      \
      op_stdop_rm8_imm8(stdop, wb, cin, uinst) op_stdop_rm16_imm16(         \
          stdop, wb, cin, uinst) op_stdop_rm32_imm32(stdop, wb, cin, uinst) \
...
op_stdop_all(adc, 1, 1, x86_uinst_add) op_stdop_all(add, 1, 0, x86_uinst_add)
