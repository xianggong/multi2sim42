...
static struct cfg_stack_elem_t *cfg_stack_elem_create(
    struct bin_config_elem_t *elem, char *key);
...
static void save_memory_data(struct mem_t *mem) {
  ...
  for (i = 0; i < MEM_PAGE_COUNT; i++) {
    ...
    for (page = mem->pages[i]; page; page = page->next) save_memory_page(page);
  }
...
static void load_memory_range(struct mem_t *mem) {
  ...
  if (have_data) load_value("data", (void **)&data, size);

  if (addr % MEM_PAGE_SIZE || size % MEM_PAGE_SIZE)
    fatal(
        "Memory range %s (addr: 0x%x, size: %d)"
  ...
  for (offset = 0; offset < size; offset += MEM_PAGE_SIZE) {
    if (mem_page_get(mem, addr + offset))
      fatal(
          "Checkpoint element %s duplicates "
...
static void save_memory_page(struct mem_page_t *page) {
  ...
  if (page->data) save_value_no_dup("data", page->data, MEM_PAGE_SIZE);
...
static void load_fd(struct x86_file_desc_table_t *fdt) {
  ...
  new_flags = flags & ~O_CREAT & ~O_EXCL & ~O_NOCTTY & ~O_TRUNC;

  host_fd = open(path, new_flags);
  if (host_fd < 0) {
    warning("Ignoring %s: could not open %s", cfg_path(), path);
    return;
  }

  if (new_flags != flags) {
    warning("Flags for %s changed from %x to %x", cfg_path(), flags, new_flags);
  }
  ...
  x86_file_desc_table_entry_new_guest_fd(fdt, file_desc_regular, guest_fd,
                                         host_fd, (char *)path, flags);
...
static void save_fds(struct x86_file_desc_table_t *fdt) {
  ...
  for (i = 0; i < list_count(fdt->file_desc_list); i++) {
    ...
    if (fd->kind == file_desc_regular) {
      save_int32("offset", lseek(fd->host_fd, 0, SEEK_CUR));
    }
...
static void save_threads(X86Context *process_ctx) {
  ...
  for (thread_ctx = emu->context_list_head; thread_ctx;
...
static void save_regs(struct x86_regs_t *regs) {
  ...
  /* x87 */
  {
    ...
    for (rel_idx = 0; rel_idx < 8; rel_idx++) {
      ...
      ftw = SETBITVALUE32(ftw, abs_idx, regs->fpu_stack[abs_idx].valid);
    }
    save_int16("ftw", ftw);
  }
...
static int cfg_try_descend(char *key) {
  ...
  if (ckp->error_code == BIN_CONFIG_ERR_NOT_FOUND) return 0;
  check();
...
static void cfg_descend(char *key) {
  if (!cfg_try_descend(key))
    fatal("Loading checkpoint from %s: could not find %s/%s", ckp->file_name,
          cfg_path(), key);
}
...
static int cfg_next_child(void) {
  ...
  if (!child && ckp->error_code == BIN_CONFIG_ERR_NOT_FOUND) return 0;
  check();
...
static char *cfg_path(void) {
  ...
  for (i = 1; i < cfg_stack->count; i++) {
    len += snprintf(buf + len, MAX_STRING_SIZE - len, "/%s",
                    ((struct cfg_stack_elem_t *)list_get(cfg_stack, i))->key);
  }
...
static void load_value(char *key, void **value, int size) {
  ...
  if (size != ckp_size)
    fatal(
        "Checkpoint element data size mismatch (element: %s/%s, "
...
static char *load_str(char *key) {
  ...
  if (!memchr(value, 0, size))
    fatal("Null terminator not found in string (element: %s/%s)", cfg_path(),
          key);
