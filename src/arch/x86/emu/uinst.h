...
#define X86_DEP_IS_INT_REG(dep) \
  ((dep) >= x86_dep_int_first && (dep) <= x86_dep_int_last)
#define X86_DEP_IS_FP_REG(dep) \
  ((dep) >= x86_dep_fp_first && (dep) <= x86_dep_fp_last)
#define X86_DEP_IS_XMM_REG(dep) \
  ((dep) >= x86_dep_xmm_first && (dep) <= x86_dep_xmm_last)
#define X86_DEP_IS_FLAG(dep) \
  ((dep) >= x86_dep_flag_first && (dep) <= x86_dep_flag_last)
#define X86_DEP_IS_VALID(dep) \
  (X86_DEP_IS_INT_REG(dep) || X86_DEP_IS_FP_REG(dep) || X86_DEP_IS_XMM_REG(dep))
...
/* To prevent performance degradation in functional simulation, do the check
 * before the actual
 * function call. Notice that 'x86_uinst_new' calls are done for every x86
 * instruction emulation. */
#define x86_uinst_new(ctx, opcode, idep0, idep1, idep2, odep0, odep1, odep2, \
                      odep3)                                                 \
  {                                                                          \
    if (x86_uinst_active)                                                    \
      __x86_uinst_new(ctx, opcode, idep0, idep1, idep2, odep0, odep1, odep2, \
                      odep3);                                                \
  }
#define x86_uinst_new_mem(ctx, opcode, addr, size, idep0, idep1, idep2, odep0, \
                          odep1, odep2, odep3)                                 \
  {                                                                            \
    if (x86_uinst_active)                                                      \
      __x86_uinst_new_mem(ctx, opcode, addr, size, idep0, idep1, idep2, odep0, \
                          odep1, odep2, odep3);                                \
  }
...
void __x86_uinst_new_mem(X86Context *ctx, enum x86_uinst_opcode_t opcode,
                         unsigned int addr, int size, enum x86_dep_t idep0,
