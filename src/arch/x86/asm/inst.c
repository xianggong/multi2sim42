...
/* Register names */
struct str_map_t x86_inst_reg_map = {30,
...
static void x86_inst_moffs_addr_dump_buf(struct x86_inst_t *inst, char **pbuf,
                                         int *psize) {
  enum x86_inst_reg_t reg;

  reg = inst->segment ? inst->segment : x86_inst_reg_ds;
  str_printf(pbuf, psize, "%s:0x%x", str_map_value(&x86_inst_reg_map, reg),
             inst->imm.d);
}

static void x86_inst_addr_dump_buf(struct x86_inst_t *inst, char **pbuf,
                                   int *psize) {
  int putsign = 0;
  char seg[20];
  ...
  if (inst->ea_base) {
    str_printf(pbuf, psize, "%s",
               str_map_value(&x86_inst_reg_map, inst->ea_base));
    putsign = 1;
  }
  if (inst->ea_index) {
    str_printf(pbuf, psize, "%s%s", putsign ? "+" : "",
               str_map_value(&x86_inst_reg_map, inst->ea_index));
    if (inst->ea_scale > 1) str_printf(pbuf, psize, "*%d", inst->ea_scale);
    putsign = 1;
  }
  if (inst->disp > 0)
    str_printf(pbuf, psize, "%s0x%x", putsign ? "+" : "", inst->disp);
  if (inst->disp < 0) str_printf(pbuf, psize, "-0x%x", -inst->disp);
...
void x86_inst_free(struct x86_inst_t *inst) { free(inst); }
...
void x86_inst_dump_buf(struct x86_inst_t *inst, char *buf, int size) {
  ...
  while (*fmt) {
    ...
    if (asm_is_token(fmt, "r8", &length)) {
      str_printf(
          &buf, &size, "%s",
          str_map_value(&x86_inst_reg_map, inst->modrm_reg + x86_inst_reg_al));
    } else if (asm_is_token(fmt, "r16", &length)) {
      str_printf(
          &buf, &size, "%s",
          str_map_value(&x86_inst_reg_map, inst->modrm_reg + x86_inst_reg_ax));
    } else if (asm_is_token(fmt, "r32", &length)) {
      str_printf(
          &buf, &size, "%s",
          str_map_value(&x86_inst_reg_map, inst->modrm_reg + x86_inst_reg_eax));
    } else if (asm_is_token(fmt, "rm8", &length)) {
      if (inst->modrm_mod == 0x03)
        str_printf(
            &buf, &size, "%s",
            str_map_value(&x86_inst_reg_map, inst->modrm_rm + x86_inst_reg_al));
      else {
      ...
      if (inst->modrm_mod == 0x03)
        str_printf(
            &buf, &size, "%s",
            str_map_value(&x86_inst_reg_map, inst->modrm_rm + x86_inst_reg_ax));
      else {
      ...
      str_printf(&buf, &size, "%x",
                 (int8_t)inst->imm.b + inst->eip + inst->size);
    } else if (asm_is_token(fmt, "rel16", &length)) {
      str_printf(&buf, &size, "%x",
                 (int16_t)inst->imm.w + inst->eip + inst->size);
      ...
      str_printf(
          &buf, &size, "%s",
          str_map_value(&x86_inst_reg_map, inst->opindex + x86_inst_reg_al));
    } else if (asm_is_token(fmt, "ir16", &length)) {
      str_printf(
          &buf, &size, "%s",
          str_map_value(&x86_inst_reg_map, inst->opindex + x86_inst_reg_ax));
    } else if (asm_is_token(fmt, "ir32", &length)) {
      str_printf(
          &buf, &size, "%s",
          str_map_value(&x86_inst_reg_map, inst->opindex + x86_inst_reg_eax));
    } else if (asm_is_token(fmt, "sreg", &length)) {
      str_printf(&buf, &size, "%s",
                 str_map_value(&x86_inst_reg_map, inst->reg + x86_inst_reg_es));
    ...
    while (*fmt && isalnum(*fmt)) str_printf(&buf, &size, "%c", *fmt++);
    ...
    while (*fmt && !isalnum(*fmt)) {
      if (*fmt == '_') {
        if (name_printed) {
          ...
          name_printed = 1;
          for (i = 0; i < 7 - name_length; i++) str_printf(&buf, &size, " ");
        }
      } else {
...
char *x86_inst_get_name(enum x86_inst_opcode_t opcode) {
  assert(x86_asm);
  if (!IN_RANGE(opcode, 1, x86_inst_opcode_count - 1)) return "<invalid>";
...
void x86_inst_decode(struct x86_inst_t *inst, unsigned int eip, void *buf) {
  ...
  for (elem = table[index]; elem; elem = elem->next) {
    info = elem->info;
    if (info->nomatch_mask &&
        (buf32 & info->nomatch_mask) == info->nomatch_result)
      continue;
  ...
  if (inst->modrm_size) {
    ...
    /* Access ModRM table */
    modrm_table_entry =
        &x86_inst_modrm_table[(inst->modrm_mod << 3) | inst->modrm_rm];
    ...
    if (inst->sib_size) {
      ...
      inst->ea_index = inst->sib_index == 0x04
                           ? x86_inst_reg_none
