...
static void x86_asm_inst_info_insert(X86Asm *as, struct x86_inst_info_t *info) {
  struct x86_inst_info_elem_t *elem;
...
static void x86_asm_inst_info_elem_free_list(
    struct x86_inst_info_elem_t *elem) {
...
void X86AsmCreate(X86Asm *self) {
  ...
  self->inst_info_list =
      xcalloc(x86_inst_opcode_count, sizeof(struct x86_inst_info_t));
...
#undef DEFINST
  ...
  for (op = 1; op < x86_inst_opcode_count; op++) {
    ...
    /* Immediate size */
    if (info->imm & IB) info->imm_size = 1;
    if (info->imm & IW) info->imm_size = 2;
    if (info->imm & ID) info->imm_size = 4;
  }
}
...
void x86_asm_done(void) { delete (x86_asm); }
...
void x86_asm_disassemble_binary(char *path) {
  ...
  LIST_FOR_EACH(elf_file->section_list, index) {
    ...
    while (buffer->pos < buffer->size) {
      ...
      if (inst.size > 7) {
        printf("%8x:\t", eip + 7);
        for (i = 7; i < 14 && i < inst.size; i++) printf("%02x ", ptr[i]);
        printf("\n");
      }
    }
