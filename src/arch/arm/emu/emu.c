...
void ARMEmuDestroy(ARMEmu *self) {
  ...
  for (ctx = self->context_list_head; ctx; ctx = ctx->context_list_next)
    if (!arm_ctx_get_status(ctx, arm_ctx_finished)) arm_ctx_finish(ctx, 0);

  /* Free contexts */
  while (self->context_list_head) arm_ctx_free(self->context_list_head);
}
...
void ARMEmuListInsertHead(ARMEmu *self, enum arm_emu_list_kind_t list,
                          struct arm_ctx_t *ctx) {
...
void ARMEmuListInsertTail(ARMEmu *self, enum arm_emu_list_kind_t list,
                          struct arm_ctx_t *ctx) {
  assert(!ARMEmuListMember(arm_emu, list, ctx));
  switch (list) {
    case arm_emu_list_context:
      DOUBLE_LINKED_LIST_INSERT_TAIL(self, context, ctx);
      break;
    case arm_emu_list_running:
      DOUBLE_LINKED_LIST_INSERT_TAIL(self, running, ctx);
      break;
    case arm_emu_list_finished:
      DOUBLE_LINKED_LIST_INSERT_TAIL(self, finished, ctx);
      break;
    case arm_emu_list_zombie:
      DOUBLE_LINKED_LIST_INSERT_TAIL(self, zombie, ctx);
      break;
    case arm_emu_list_suspended:
      DOUBLE_LINKED_LIST_INSERT_TAIL(self, suspended, ctx);
      break;
    case arm_emu_list_alloc:
      DOUBLE_LINKED_LIST_INSERT_TAIL(self, alloc, ctx);
      break;
  }
}

void ARMEmuListRemove(ARMEmu *self, enum arm_emu_list_kind_t list,
                      struct arm_ctx_t *ctx) {
  assert(ARMEmuListMember(arm_emu, list, ctx));
  switch (list) {
    case arm_emu_list_context:
      DOUBLE_LINKED_LIST_REMOVE(self, context, ctx);
      break;
    case arm_emu_list_running:
      DOUBLE_LINKED_LIST_REMOVE(self, running, ctx);
      break;
    case arm_emu_list_finished:
      DOUBLE_LINKED_LIST_REMOVE(self, finished, ctx);
      break;
    case arm_emu_list_zombie:
      DOUBLE_LINKED_LIST_REMOVE(self, zombie, ctx);
      break;
    case arm_emu_list_suspended:
      DOUBLE_LINKED_LIST_REMOVE(self, suspended, ctx);
      break;
    case arm_emu_list_alloc:
      DOUBLE_LINKED_LIST_REMOVE(self, alloc, ctx);
      break;
  }
}

int ARMEmuListMember(ARMEmu *self, enum arm_emu_list_kind_t list,
                     struct arm_ctx_t *ctx) {
  switch (list) {
    case arm_emu_list_context:
      return DOUBLE_LINKED_LIST_MEMBER(self, context, ctx);
    case arm_emu_list_running:
      return DOUBLE_LINKED_LIST_MEMBER(self, running, ctx);
    case arm_emu_list_finished:
      return DOUBLE_LINKED_LIST_MEMBER(self, finished, ctx);
    case arm_emu_list_zombie:
      return DOUBLE_LINKED_LIST_MEMBER(self, zombie, ctx);
    case arm_emu_list_suspended:
      return DOUBLE_LINKED_LIST_MEMBER(self, suspended, ctx);
    case arm_emu_list_alloc:
      return DOUBLE_LINKED_LIST_MEMBER(self, alloc, ctx);
  }
  return 0;
}
...
int ARMEmuRun(Emu *self) {
  ...
  while (emu->finished_list_head) arm_ctx_free(emu->finished_list_head);
