...
void arm_file_desc_table_free(struct arm_file_desc_table_t *table) {
  ...
  for (i = 0; i < list_count(table->arm_file_desc_list); i++) {
    ...
    if (desc) arm_file_desc_free(desc);
  }
...
struct arm_file_desc_table_t *arm_file_desc_table_link(
    struct arm_file_desc_table_t *fdt) {
  fdt->num_links++;
  return fdt;
}
...
int arm_file_desc_table_get_host_fd(struct arm_file_desc_table_t *table,
                                    int guest_fd) {
  struct arm_file_desc_t *desc;
...
int arm_file_desc_table_get_guest_fd(struct arm_file_desc_table_t *table,
                                     int host_fd) {
  struct arm_file_desc_t *desc;
  int i;
...
void arm_file_desc_table_entry_dump(struct arm_file_desc_table_t *table,
                                    int index, FILE *f) {
  struct arm_file_desc_t *desc;

  /* Invalid entry */
  if (index < 0 || index >= list_count(table->arm_file_desc_list)) return;
...
struct arm_file_desc_t *arm_file_desc_table_entry_get(
    struct arm_file_desc_table_t *table, int index) {
  return list_get(table->arm_file_desc_list, index);
}

struct arm_file_desc_t *arm_file_desc_table_entry_new(
    struct arm_file_desc_table_t *table, enum arm_file_desc_kind_t kind,
    int host_fd, char *path, int flags) {
  return arm_file_desc_table_entry_new_guest_fd(table, kind, -1, host_fd, path,
                                                flags);
}

struct arm_file_desc_t *arm_file_desc_table_entry_new_guest_fd(
    struct arm_file_desc_table_t *table, enum arm_file_desc_kind_t kind,
  ...
  for (i = 0; i < list_count(table->arm_file_desc_list) && guest_fd < 0; i++)
    if (!list_get(table->arm_file_desc_list, i)) guest_fd = i;
...
void arm_file_desc_table_entry_free(struct arm_file_desc_table_t *table,
                                    int index) {
  struct arm_file_desc_t *desc;
