    ...
    if (inst->dword.sdtr_ins.idx_typ) /* Pre- Indexed */
    {
      if (inst->dword.sdtr_ins.up_dn) /* Increment */
      {
        switch ((shift >> 1) & 0x00000003) {
          ...
          case ARM_OPTR_ASR:
            ret_addr = rn_val + (rm_val / (1 << shift_val));
            break;
      ...
      if (inst->dword.sdtr_ins.wb) arm_isa_reg_store(ctx, rn, ret_addr);
    }
    ...
    else /* Post Indexed */
    {
      if (inst->dword.sdtr_ins.up_dn) /* Increment */
      {
        switch ((shift >> 1) & 0x00000003) {
          case ARM_OPTR_LSL:
            ret_addr = rn_val;
            rn_val = ret_addr + (rm_val << shift_val);
            break;
          ...
          case ARM_OPTR_ASR:
            ret_addr = rn_val;
            rn_val = ret_addr + (rm_val / (1 << shift_val));
            break;

          case ARM_OPTR_ROR:
            ret_addr = rn_val;
            rn_val = ret_addr + arm_rotr(rm_val, shift_val);
            break;
        }
      } else /* Decrement */
      {
...
int arm_isa_get_addr_amode3_imm(struct arm_ctx_t *ctx) {
  ...
  else /* Post Indexed */
  {
    ...
    if (ctx->inst.dword.hfwrd_imm_ins.wb) {
      fatal(
          "%s: Arm instruction not according to v7 ISA specification,"
...
int arm_isa_op2_get(struct arm_ctx_t *ctx, unsigned int op2,
                    enum arm_isa_op2_cat_t cat) {
  unsigned int imm;
  unsigned int rotate;
  int op_val;
  unsigned int shift;
...
int arm_isa_op2_carry(struct arm_ctx_t *ctx, unsigned int op2,
                      enum arm_isa_op2_cat_t cat) {
  unsigned int imm;
  unsigned int rotate;
  unsigned int shift;
...
int arm_isa_check_cond(struct arm_ctx_t *ctx) {
  ...
  switch (cond) {
    case (EQ):
      ...
      arm_isa_inst_debug("  Cond = EQ\n");
      break;

    case (NE):
      ret_val = (!(regs->cpsr.z)) ? 1 : 0;
      arm_isa_inst_debug("  Cond = NE\n");
      break;

    case (CS):
      ret_val = (regs->cpsr.C) ? 1 : 0;
      arm_isa_inst_debug("  Cond = CS\n");
      break;

    case (CC):
      ret_val = (!(regs->cpsr.C)) ? 1 : 0;
      arm_isa_inst_debug("  Cond = CC\n");
      break;

    case (MI):
      ret_val = (regs->cpsr.n) ? 1 : 0;
      arm_isa_inst_debug("  Cond = MI\n");
      break;

    case (PL):
      ret_val = (!(regs->cpsr.n)) ? 1 : 0;
      arm_isa_inst_debug("  Cond = PL\n");
      break;

    case (VS):
      ret_val = (regs->cpsr.v) ? 1 : 0;
      arm_isa_inst_debug("  Cond = VS\n");
      break;

    case (VC):
      ret_val = (!(regs->cpsr.v)) ? 1 : 0;
      arm_isa_inst_debug("  Cond = VC\n");
      break;

    case (HI):
      ret_val = (!(regs->cpsr.z) && (regs->cpsr.C)) ? 1 : 0;
      arm_isa_inst_debug("  Cond = HI\n");
      break;

    case (LS):
      ret_val = ((regs->cpsr.z) | !(regs->cpsr.C)) ? 1 : 0;
      arm_isa_inst_debug("  Cond = LS\n");
      break;

    case (GE):
      ret_val = (((regs->cpsr.n) & (regs->cpsr.v)) |
                 (!(regs->cpsr.n) & !(regs->cpsr.v)))
                    ? 1
                    : 0;
      arm_isa_inst_debug("  Cond = GE\n");
      break;
    ...
    case (LT):
      ret_val = (((regs->cpsr.n) & !(regs->cpsr.v)) |
                 (!(regs->cpsr.n) && (regs->cpsr.v)))
                    ? 1
                    : 0;
      arm_isa_inst_debug("  Cond = LT\n");
      break;

    case (GT):
      ret_val = (((regs->cpsr.n) & (regs->cpsr.v) & !(regs->cpsr.z)) |
                 (!(regs->cpsr.n) & !(regs->cpsr.v) & !(regs->cpsr.z)))
                    ? 1
                    : 0;
      arm_isa_inst_debug("  Cond = GT\n");
      break;

    case (LE):
      ret_val = (((regs->cpsr.z) | (!(regs->cpsr.n) && (regs->cpsr.v)) |
                  ((regs->cpsr.n) && !(regs->cpsr.v))))
                    ? 1
                    : 0;
      arm_isa_inst_debug("  Cond = LE\n");
      break;
...
void arm_isa_amode4s_ld(struct arm_ctx_t *ctx) {
  ...
  if (ctx->inst.dword.bdtr_ins.idx_typ) /* Pre indexed */
  {
    if (ctx->inst.dword.bdtr_ins.up_dn) {
      ...
      if (ctx->inst.dword.bdtr_ins.wb)
        arm_isa_reg_store(ctx, ctx->inst.dword.bdtr_ins.base_rn,
                          (read_val - 4));
    }
...
void arm_thumb_isa_subtract(struct arm_ctx_t *ctx, unsigned int rd,
                            unsigned int rn, int op2, unsigned int op3,
...
void arm_isa_subtract(struct arm_ctx_t *ctx, unsigned int rd, unsigned int rn,
                      int op2, unsigned int op3) {
  int rd_val;
  int rn_val;
  int operand2;
  unsigned int rd_val_safe;
...
void arm_thumb_isa_rev_subtract(struct arm_ctx_t *ctx, unsigned int rd,
                                unsigned int rn, int op2, unsigned int op3,
                                unsigned int flag_set) {
  int rd_val;
  int rn_val;
...
void arm_isa_subtract_rev(struct arm_ctx_t *ctx, unsigned int rd,
                          unsigned int rn, int op2, unsigned int op3) {
  int rd_val;
  int rn_val;
  int operand2;
...
void arm_thumb_add_isa(struct arm_ctx_t *ctx, unsigned int rd, unsigned int rn,
                       int op2, unsigned int op3, unsigned int flag_set) {
  int rd_val;
  int rn_val;
  struct arm_regs_t *regs;
  unsigned long flags;
...
void arm_isa_add(struct arm_ctx_t *ctx, unsigned int rd, unsigned int rn,
                 int op2, unsigned int op3) {
  int rd_val;
  int rn_val;
  struct arm_regs_t *regs;
...
unsigned int arm_isa_invalid_addr_str(unsigned int addr, int value,
...
unsigned int arm_isa_invalid_addr_ldr(unsigned int addr, unsigned int *value,
  ...
  if (addr == 0x5bd4dc) /* Fault subroutine return address */
  {
    if (ctx->fault_addr == addr) *(value) = ctx->fault_value;
    return 1;
  } else if (addr == 0x5bd4c8) /* Fault subroutine return address */
  {
    *(value) = 0x8fd6c;
...
void arm_isa_execute_inst(struct arm_ctx_t *ctx) {
  ...
  if (debug_status(arm_isa_inst_debug_category)) {
    arm_isa_inst_debug("%d %8lld %x: ", ctx->pid, asEmu(arm_emu)->instructions,
                       ctx->curr_ip);
    if (ctx->regs->cpsr.thumb) {
      if (ctx->inst_type == THUMB32) {
        arm_th32_inst_debug_dump(&ctx->inst_th_32,
                                 debug_file(arm_isa_inst_debug_category));
        arm_isa_inst_debug("  (%d bytes)", ctx->inst_th_32.info->size);
      } else if (ctx->inst_type == THUMB16) {
        arm_th16_inst_debug_dump(&ctx->inst_th_16,
                                 debug_file(arm_isa_inst_debug_category));
  ...
  /* Debug */
  arm_isa_inst_debug("\n");
  if (debug_status(arm_isa_call_debug_category)) arm_isa_debug_call(ctx);
}

unsigned int arm_thumb32_isa_immd12(struct arm_thumb32_inst_t *inst,
...
void arm_thumb32_isa_branch_link(struct arm_ctx_t *ctx) {
  ...
  if (cat == ARM_THUMB32_CAT_BRANCH) {
    ...
    if ((inst->addr + 2) % 4) addr = (inst->addr + 4) + addr;
...
unsigned int arm_isa_thumb_check_cond(struct arm_ctx_t *ctx,
                                      unsigned int cond) {
  struct arm_regs_t *regs;
  unsigned int ret_val;
  regs = ctx->regs;
