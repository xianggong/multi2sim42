...
static struct str_map_t elf_section_flags_map = {
    3, {{"SHF_WRITE", 1}, {"SHF_ALLOC", 2}, {"SHF_EXECINSTR", 4}}};

static struct str_map_t elf_program_header_type_map = {8,
                                                       {{"PT_NULL", 0},
                                                        {"PT_LOAD", 1},
                                                        {"PT_DYNAMIC", 2},
                                                        {"PT_INTERP", 3},
...
static void arm_ctx_loader_add_args_vector(struct arm_ctx_t *ctx, int argc,
                                           char **argv) {
  char *arg;
  int i;
...
static void arm_ctx_loader_add_environ(struct arm_ctx_t *ctx, char *env) {
  ...
  for (i = 0; environ[i]; i++) linked_list_add(ctx->env, xstrdup(environ[i]));
...
static void arm_ctx_loader_load_program_headers(struct arm_ctx_t *ctx) {
  ...
  /* Program header PT_PHDR, specifying location and size of the program header
   * table itself. */
  /* Search for program header PT_PHDR, specifying location and size of the
  ...
  for (i = 0; i < list_count(elf_file->program_header_list); i++) {
    ...
    arm_loader_debug(
        "    filesz=%d, memsz=%d, flags=%d, align=%d\n",
        program_header->header->p_filesz, program_header->header->p_memsz,
        program_header->header->p_flags, program_header->header->p_align);
...
static void arm_ctx_loader_load_sections(struct arm_ctx_t *ctx,
  ...
  for (i = 0; i < list_count(elf_file->section_list); i++) {
    ...
    str_map_flags(&elf_section_flags_map, section->header->sh_flags, flags_str,
                  sizeof(flags_str));
    arm_loader_debug(
        "  section %d: name='%s', offset=0x%x, addr=0x%x,"
        " size=%u, flags=%s\n",
        i, section->name, section->header->sh_offset, section->header->sh_addr,
        section->header->sh_size, flags_str);

    /* Process section */
    if (section->header->sh_flags & SHF_ALLOC) {
      /* Permissions */
      if (section->header->sh_flags & SHF_WRITE) perm |= mem_access_write;
      if (section->header->sh_flags & SHF_EXECINSTR) perm |= mem_access_exec;

      /* Load section */
      mem_map(mem, section->header->sh_addr, section->header->sh_size, perm);
      mem->heap_break = MAX(
          mem->heap_break, section->header->sh_addr + section->header->sh_size);
      ...
      if (section->header->sh_type == 8) {
        ...
        mem_access(mem, section->header->sh_addr, section->header->sh_size, ptr,
                   mem_access_init);
        free(ptr);
      } else {
...
static void arm_ctx_loader_load_stack(struct arm_ctx_t *ctx) {
  ...
  mem_map(mem, ctx->stack_top, ctx->stack_size,
  ...
  if (sp > LD_STACK_BASE)
    fatal("%s: initial stack overflow, increment LD_MAX_ENVIRON", __FUNCTION__);
}
...
void arm_ctx_loader_load_exe(struct arm_ctx_t *ctx, char *exe) {
  ...
  /* Alternative stdin */
  arm_ctx_loader_get_full_path(ctx, ctx->stdin_file, stdin_file_full_path,
                               MAX_STRING_SIZE);
  if (*stdin_file_full_path) {
    ...
    if (fd->host_fd < 0) fatal("%s: cannot open stdin", ctx->stdin_file);
    arm_loader_debug("%s: stdin redirected\n", stdin_file_full_path);
  }

  /* Alternative stdout/stderr */
  arm_ctx_loader_get_full_path(ctx, ctx->stdout_file, stdout_file_full_path,
                               MAX_STRING_SIZE);
  if (*stdout_file_full_path) {
    ...
    assert(fd1 && fd2);
    fd1->host_fd = fd2->host_fd = open(
        stdout_file_full_path, O_CREAT | O_APPEND | O_TRUNC | O_WRONLY, 0660);
    if (fd1->host_fd < 0)
  ...
  if (!strncmp(symbol->name, "$t", 2)) {
    ...
    fatal("%s: Wrong entry point selected", __FUNCTION__);
  } else {
    mode = ARM;
  }
...
void arm_ctx_free(struct arm_ctx_t *ctx) {
  ...
  if (ctx->num_links) ctx->num_links--;
  /*assert(!ctx->num_links);*/

  /* Free ELF file  */
  if (ctx->elf_file) elf_file_free(ctx->elf_file);
...
void arm_ctx_execute(struct arm_ctx_t *ctx) {
  ...
  arm_mode =
      arm_ctx_operate_mode_tag(ctx->thumb_symbol_list, (ctx->regs->pc - 2));
  ...
  /*if (!buffer_ptr)
  {
           Disable safe mode. If a part of the 20 read bytes does not belong to
  the
           * actual instruction, and they lie on a page with no permissions,
  this would
  ...
  */
   ...
   * increase the pc by 2 and fetch pc 4 size 4, but instruction commit will
   * make pc = pc +2 for thumb */
  ...
  if (ctx->regs->cpsr.thumb) {
    if (arm_test_thumb32(buffer_ptr)) {
      ...
      if (ctx->inst_th_32.info->name ==
          ARM_THUMB32_INST_NONE) /*&& !spec_mode)*/
        fatal("0x%x: not supported arm instruction (%02x %02x %02x %02x...)",
              (regs->pc - 4), buffer_ptr[0], buffer_ptr[1], buffer_ptr[2],
              buffer_ptr[3]);
  ...
  if (ctx->iteq_inst_num && ctx->iteq_block_flag) {
    if (arm_isa_thumb_check_cond(ctx,
                                 ctx->inst_th_16.dword.if_eq_ins.first_cond))
...
void arm_ctx_finish_group(struct arm_ctx_t *ctx, int status) {
  ...
  /* Get group parent */
  if (ctx->group_parent) ctx = ctx->group_parent;
  assert(!ctx->group_parent); /* Only one level */

  /* Context already finished */
  if (arm_ctx_get_status(ctx, arm_ctx_finished | arm_ctx_zombie)) return;

  /* Finish all contexts in the group */
  DOUBLE_LINKED_LIST_FOR_EACH(arm_emu, context, aux) {
    if (aux->group_parent != ctx && aux != ctx) continue;
 ...
 * to 'arm_ctx_finished'. If it has, its status is set to 'arm_ctx_zombie',
 * waiting for
 * a call to 'waitpid'.
 * The children of the finished context will set their 'parent' attribute to
 * NULL.
...
static void arm_ctx_update_status(struct arm_ctx_t *ctx,
                                  enum arm_ctx_status_t status) {
  ...
  if (status_diff & ~arm_ctx_spec_mode) arm_emu->context_reschedule = 1;
  ...
  if (!(ctx->status & arm_ctx_suspended) && !(ctx->status & arm_ctx_finished) &&
      !(ctx->status & arm_ctx_zombie) && !(ctx->status & arm_ctx_locked))
  ...
  if (debug_status(arm_ctx_debug_category) &&
      (status_diff & ~arm_ctx_spec_mode)) {
    char sstatus[200];
...
struct arm_ctx_t *arm_ctx_get(int pid) {
  ...
  while (ctx && ctx->pid != pid) ctx = ctx->context_list_next;
  return ctx;
}
...
void arm_ctx_load_from_command_line(int argc, char **argv) {
  ...
  if (!ctx->cwd) panic("%s: buffer too small", __FUNCTION__);
...
void arm_ctx_loader_get_full_path(struct arm_ctx_t *ctx, char *file_name,
                                  char *full_path, int size) {
  /* Remove './' prefix from 'file_name' */
  while (file_name && !strncmp(file_name, "./", 2)) file_name += 2;
...
void arm_ctx_gen_proc_self_maps(struct arm_ctx_t *ctx, char *path) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (!page) break;
      page_perm =
          page->perm & (mem_access_read | mem_access_write | mem_access_exec);
      if (page_perm != perm) break;
      end += MEM_PAGE_SIZE;
      perm = page_perm;
    }
...
void arm_ctx_thumb_symbol_list_sort(struct list_t *thumb_symbol_list,
                                    struct elf_file_t *elf_file) {
  struct elf_symbol_t *symbol;
  unsigned int i;
...
enum arm_mode_t arm_ctx_operate_mode_tag(struct list_t *thumb_symbol_list,
                                         unsigned int addr) {
  struct elf_symbol_t *symbol;
