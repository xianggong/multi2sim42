...
void arm_inst_decode(struct arm_inst_t *inst) {
  ...
  arg1 = ((inst->dword.bytes[3] & 0x0f) << 4) |
         ((inst->dword.bytes[2] & 0xf0) >> 4);
...
unsigned int arm_rotr(unsigned int value, unsigned int shift) {
  // Rotating 32 bits on a 32-bit integer is the same as rotating 0 bits; 33
  // bits -> 1 bit; etc.
...
void arm_inst_dump_OP2(char **inst_str_ptr, int *inst_str_size,
  ...
  if (!inst->dword.dpr_ins.imm) {
    ...
    if (shift & 0x00000001) {
      ...
      switch ((shift >> 1) & 0x00000003) {
        case (ARM_OPTR_LSL):
          str_printf(inst_str_ptr, inst_str_size, "r%d , LSL #%d   ;0x%x", rm,
                     ((shift >> 3) & 0x0000001f), ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_LSR):
          str_printf(inst_str_ptr, inst_str_size, "r%d , LSR #%d   ;0x%x", rm,
                     ((shift >> 3) & 0x0000001f), ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_ASR):
          str_printf(inst_str_ptr, inst_str_size, "r%d , ASR #%d   ;0x%x", rm,
                     ((shift >> 3) & 0x0000001f), ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_ROR):
          str_printf(inst_str_ptr, inst_str_size, "r%d , ROR #%d   ;0x%x", rm,
                     ((shift >> 3) & 0x0000001f), ((shift >> 3) & 0x0000001f));
          break;
      }
    }
...
void arm_amode2_disasm(char **inst_str_ptr, int *inst_str_size,
  ...
  if (inst->dword.sdtr_ins.imm == 1) {
    ...
    if (inst->dword.sdtr_ins.up_dn) {
      switch ((shift >> 1) & 0x00000003) {
        case (ARM_OPTR_LSL):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, lsl #%d]", rn, rm,
                     ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_LSR):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, lsr #%d]", rn, rm,
                     ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_ASR):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, asr #%d]", rn, rm,
                     ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_ROR):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, ror #%d]", rn, rm,
                     ((shift >> 3) & 0x0000001f));
          break;
      }
    } else {
      switch ((shift >> 1) & 0x00000003) {
        case (ARM_OPTR_LSL):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, lsl #-%d]", rn,
                     rm, ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_LSR):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, lsr #-%d]", rn,
                     rm, ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_ASR):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, asr #-%d]", rn,
                     rm, ((shift >> 3) & 0x0000001f));
          break;

        case (ARM_OPTR_ROR):
          str_printf(inst_str_ptr, inst_str_size, "[r%d, r%d, ror #-%d]", rn,
                     rm, ((shift >> 3) & 0x0000001f));
          break;
      }
    }
...
void arm_inst_dump_IDX(char **inst_str_ptr, int *inst_str_size,
  ...
  if (idx == 1) {
    if (inst->dword.sdtr_ins.off) str_printf(inst_str_ptr, inst_str_size, "!");
  } else
    str_printf(inst_str_ptr, inst_str_size, " ");
}
...
void arm_inst_dump_IMMD16(char **inst_str_ptr, int *inst_str_size,
  ...
  if (cat == ARM_CAT_DPR_REG)
    ...
    immd16 = ((inst->dword.dpr_ins.op1_reg << 12) | inst->dword.dpr_ins.op2);
  else if (cat == ARM_CAT_DPR_SAT)
    fatal("%d: movt_movw fmt not recognized", cat);
  else if (cat == ARM_CAT_PSR)
    fatal("%d: movt_movw fmt not recognized", cat);
...
void arm_inst_dump_VFP1STM(char **inst_str_ptr, int *inst_str_size,
  ...
  if (cat == ARM_CAT_VFP) vfp1 = inst->dword.vfp_mv_ins.immd8;
  /* TODO: destinations for BDTR CDTR CDO*/
  else
...
void arm_inst_dump_VFP_REGS(char **inst_str_ptr, int *inst_str_size,
  ...
  if (cat == ARM_CAT_VFP) {
    ...
    reg_start =
        ((inst->dword.vfp_mv_ins.d << 4) | (inst->dword.vfp_mv_ins.vd)) &
        (0x0000001f);
  }
  /* TODO: destinations for BDTR CDTR CDO*/
  else
...
void arm_inst_dump_FREG(char **inst_str_ptr, int *inst_str_size,
  ...
  if (cat == ARM_CAT_CPR_DTR) freg = inst->dword.cpr_dtr_ins.cpr_sr_dst;
  /* TODO: destinations for BDTR CDTR CDO*/
  else
...
void arm_inst_dump_RT(char **inst_str_ptr, int *inst_str_size,
  ...
  if (cat == ARM_CAT_VFP) rt = inst->dword.vfp_strreg_tr_ins.vfp_rt;
  /* TODO: destinations for BDTR CDTR CDO*/
  else
...
unsigned int arm_dump_word_symbol(struct elf_file_t *elf_file,
                                  unsigned int inst_addr, void *inst_ptr) {
  struct elf_symbol_t *symbol;
...
unsigned int thumb_dump_word_symbol(struct elf_file_t *elf_file,
                                    unsigned int inst_addr, void *inst_ptr) {
  struct elf_symbol_t *symbol;
  unsigned int word_flag;
  symbol = elf_symbol_get_by_address(elf_file, inst_addr, NULL);
...
void arm_thumb_symbol_list_sort(struct list_t *thumb_symbol_list,
...
int arm_dissassembly_mode_tag(struct list_t *thumb_symbol_list,
                              unsigned int addr) {
  struct elf_symbol_t *symbol;
...
void arm_thumb16_inst_hex_dump(FILE *f, void *inst_ptr,
                               unsigned int inst_addr) {
  printf("%8x:	%04x		", inst_addr, *(unsigned short *)inst_ptr);
}

void arm_thumb32_inst_hex_dump(FILE *f, void *inst_ptr,
                               unsigned int inst_addr) {
  int thumb_32;
  thumb_32 = *(unsigned int *)inst_ptr;
  printf("%8x:	%04x %04x	", inst_addr, (thumb_32 & 0x0000ffff),
...
void arm_emu_disasm(char *path) {
  ...
  for (i = 0; i < list_count(elf_file->section_list); ++i) {
    ...
    if (!strncmp(section->name, ".text", 5)) break;
  }
  ...
  for (inst_ptr = section->buffer.ptr;
       inst_ptr < section->buffer.ptr + section->buffer.size;) {
    disasm_mode = arm_dissassembly_mode_tag(
        thumb_symbol_list, (section->header->sh_addr + inst_index));

    if (disasm_mode == ARM_DISASM) {
      prev_symbol = arm_elf_function_symbol(
          elf_file, (section->header->sh_addr + inst_index), prev_symbol,
          disasm_mode);

      arm_inst_hex_dump(stdout, inst_ptr,
                        (section->header->sh_addr + inst_index));

      if (!arm_dump_word_symbol(
              elf_file, (section->header->sh_addr + inst_index), inst_ptr)) {
      ...
      if (arm_test_thumb32(inst_ptr)) {
        ...
        arm_thumb32_inst_hex_dump(stdout, inst_ptr,
                                  (section->header->sh_addr + inst_index));
        ...
        arm_thumb16_inst_hex_dump(stdout, inst_ptr,
                                  (section->header->sh_addr + inst_index));
        if (!arm_dump_word_symbol(
                elf_file, (section->header->sh_addr + inst_index), inst_ptr)) {
...
/* Pointer to 'inst' is declared volatile to avoid optimizations when calling
 * 'memset' */
...
void thumb16_disasm(void *buf, unsigned int ip,
                    volatile struct arm_thumb16_inst_t *inst) {
  unsigned int byte_index;
  inst->addr = ip;
...
void thumb32_disasm(void *buf, unsigned int ip,
                    volatile struct arm_thumb32_inst_t *inst) {
  unsigned int byte_index;
  inst->addr = ip - 2;
  for (byte_index = 0; byte_index < 4; ++byte_index)
    inst->dword.bytes[byte_index] =
        *(unsigned char *)(buf + ((byte_index + 2) % 4));
...
void arm_th16_inst_debug_dump(struct arm_thumb16_inst_t *inst, FILE *f) {
  ...
  inst_ptr = &inst->dword.bytes;
  arm_thumb16_inst_dump(f, inst_str, MAX_STRING_SIZE, inst_ptr, inst->addr,
                        inst->addr);
}
...
void arm_th32_inst_debug_dump(struct arm_thumb32_inst_t *inst, FILE *f) {
  ...
  for (byte_index = 0; byte_index < 4; ++byte_index)
    inst_ptr.dword.bytes[byte_index] =
        inst->dword.bytes[((byte_index + 2) % 4)];

  arm_thumb32_inst_dump(f, inst_str, MAX_STRING_SIZE, &inst_ptr.dword.bytes,
                        inst->addr, inst->addr);
}
