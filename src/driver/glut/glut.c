...
static char *glut_err_code =
    "\tAn invalid function code was generated by your application in a GLUT "
    "system\n"
...
static char *glut_err_one_window =
    "\tThe guest GLUT application is trying to create more than one window. "
    "Multi2Sim\n"
...
char *glut_call_name[glut_call_count + 1] = {NULL,
...
static glut_abi_func_t glut_abi_table[glut_call_count + 1] = {NULL,
...
void glut_event_free(struct glut_event_t *event) { free(event); }
...
void glut_done(void) {
  ...
  /* Kill GLUT thread */
  if (glut_thread) /* Free list of events */
  ...
  if (glut_window_properties) free(glut_window_properties);
  if (glut_window_title) free(glut_window_title);
...
static int glut_abi_test_draw(X86Context *ctx) {
  ...
  for (i = 0; i < width; i++) {
    ...
    for (j = 0; j < height; j++) glut_frame_buffer_pixel(i, j, color);
  }
