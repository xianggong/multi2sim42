...
struct cuda_function_t *cuda_function_create(
    struct cuda_module_t *module, char *function_name,
    unsigned long long int *inst_buffer, unsigned int inst_buffer_size,
  ...
  function->inst_buffer =
      (unsigned long long int *)xcalloc(1, (unsigned long int)inst_buffer_size);
...
void cuda_function_free(struct cuda_function_t *function) {
  ...
  for (i = 0; i < list_count(function->arg_list); i++)
    cuda_function_arg_free(
        (struct cuda_function_arg_t *)list_get(function->arg_list, i));
