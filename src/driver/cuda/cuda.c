...
/* Error messages */
char *cuda_err_code =
    "\tAn invalid function code was generated by your application in a CUDA "
    "system\n"
    "\tcall. Probably, this means that your application is using an "
    "incompatible\n"
    "\tversion of the Multi2Sim CUDA runtime/driver library ('libm2s-cuda'). "
    "Please\n"
    "\trecompile your application and try again.\n";

/* List of CUDA driver call names */
char *cuda_call_name[cuda_call_count + 1] = {NULL,
...
cuda_func_t cuda_func_table[cuda_call_count + 1] = {NULL,
...
void cuda_done(void) {
  ...
  LIST_FOR_EACH(module_list, i) {
    ...
    if (module) cuda_module_free(module);
  }
  ...
  LIST_FOR_EACH(function_list, i) {
    ...
    if (function) cuda_function_free(function);
  }
...
int cuda_func_cuDeviceTotalMem(X86Context *ctx) {
  ...
  mem_write(mem, total_global_mem_size, sizeof(unsigned int),
            &(frm_emu->total_global_mem_size));
...
int cuda_func_cuModuleLoad(X86Context *ctx) {
  ...
  /* Get kernel binary */
  if (regs->ecx != 0) mem_read(mem, regs->ecx, MAX_STRING_SIZE, cubin_path);
...
int cuda_func_cuMemGetInfo(X86Context *ctx) {
  ...
  mem_write(mem, total, sizeof(unsigned int),
            &(frm_emu->total_global_mem_size));
