...
static char *err_evg_opencl_evergreen_format =
    "\tYour application tried to load a pre-compiled OpenCL kernel binary "
    "which\n"
    "\tdoes not contain code in the Evergreen ISA. Please, check that the "
    "off-line\n"
    ...
    "\tstatically compiled x86 binary that is trying to load a Southern "
    "Islands\n"
...
void evg_opencl_program_build(struct evg_opencl_program_t *program) {
  /* Open ELF file and check that it corresponds to an Evergreen pre-compiled
   * kernel */
...
/* Look for a symbol name in program binary and read it from its corresponding
 * section.
 * The contents pointed to by the symbol are stored in an ELF buffer.
 * No allocation happens here, the target buffer will just point to the contents
 * of
 * an existing section. */
...
static char *err_evg_opencl_elf_symbol =
    ...
    "\tmachine with an installation of the APP SDK (using 'm2s-opencl-kc') "
    "but\n"
...
void evg_opencl_program_read_symbol(struct evg_opencl_program_t *program,
  ...
  if (!symbol)
    fatal("%s: ELF symbol '%s' not found.\n%s", __FUNCTION__, symbol_name,
          err_evg_opencl_elf_symbol);
