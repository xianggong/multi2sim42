...
void evg_opencl_repo_remove_object(struct evg_opencl_repo_t *repo,
  ...
  linked_list_find(object_list, object);
  if (object_list->error_code) fatal("%s: object does not exist", __FUNCTION__);
...
void *evg_opencl_repo_get_object(struct evg_opencl_repo_t *repo,
  ...
  if (object_id >> 16 != type)
    fatal("%s: requested OpenCL object of incorrect type", __FUNCTION__);
  ...
  LINKED_LIST_FOR_EACH(object_list) {
    ...
    if (current_object_id == object_id) return object;
  }

  /* Not found */
  fatal("%s: requested OpenCL does not exist (id=0x%x)", __FUNCTION__,
        object_id);
  return NULL;
}
...
void *evg_opencl_repo_get_object_of_type(struct evg_opencl_repo_t *repo,
  ...
  LINKED_LIST_FOR_EACH(object_list) {
    ...
    if (object_id >> 16 == type) return object;
  }
 ...
 * object type, while the 16 least significant bits represent a unique object
 * ID. */
