...
struct evg_opencl_device_t *evg_opencl_device_create(EvgEmu *emu) {
  ...
  device->id =
      evg_opencl_repo_new_object_id(emu->opencl_repo, evg_opencl_object_device);
  device->emu = emu;
...
unsigned int evg_opencl_device_get_info(struct evg_opencl_device_t *device,
  ...
  unsigned long long global_mem_size =
      1ull << 31; /* 2GB of global memory reported */
  ...
  switch (name) {
    ...
    default:
      fatal(
          "opencl_device_get_info: invalid or not implemented value for 'name' "
          "(0x%x)\n%s",
