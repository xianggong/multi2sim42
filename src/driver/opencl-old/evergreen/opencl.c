...
#define EVG_SYS_OPENCL_IMPL_VERSION            \
  ((EVG_SYS_OPENCL_IMPL_VERSION_MAJOR << 16) | \
...
#define DEF_OPENCL_FUNC(_name, _argc) \
...
char *evg_err_opencl_note =
    "\tThe OpenCL interface is implemented in library 'm2s-opencl.so' as a set "
    "of\n"
    "\tsystem calls, intercepted by Multi2Sim and emulated in 'opencl.c'.\n"
    "\tHowever, only a subset of this interface is currently implemented in "
    "the simulator.\n"
...
char *evg_err_opencl_param_note =
    "\tNote that a real OpenCL implementation would return an error code to "
    "the\n"
    "\tcalling program. However, the purpose of this OpenCL implementation is "
    "to\n"
    "\tsupport correctly written programs. Thus, a detailed implementation of "
    "OpenCL\n"
    "\terror handling is not provided, and any OpenCL error will cause the\n"
    "\tsimulation to stop.\n";

char *evg_err_opencl_compiler =
    "\tThe Multi2Sim implementation of the OpenCL interface does not support "
    "runtime\n"
    "\tcompilation of kernel sources. To run OpenCL kernels, you should first "
    "compile\n"
    "\tthem off-line using an Evergreen-compatible target device. Then, you "
    "have three\n"
    "\toptions to load them:\n"
    "\t  1) Replace 'clCreateProgramWithSource' calls by "
    "'clCreateProgramWithBinary'\n"
    "\t     in your source files, referencing the pre-compiled kernel.\n"
    "\t  2) Tell Multi2Sim to provide the application with your pre-compiled "
    "kernel\n"
    "\t     using command-line option '--evg-kernel-binary'.\n"
    "\t  3) If you are trying to run one of the OpenCL benchmarks provided in "
    "the\n"
    "\t     simulator website, option '--load' can be used as a program "
    "argument\n"
    "\t     (not a simulator argument). This option allows you to specify the "
    "path\n"
    "\t     for the pre-compiled kernel, which is provided in the downloaded "
    "package.\n";

char *evg_err_opencl_binary_note =
    "\tYou have selected a pre-compiled OpenCL kernel binary to be passed to "
    "your\n"
    "\tOpenCL application when it requests a kernel compilation. It is your\n"
    "\tresponsibility to check that the chosen binary corresponds to the "
    "kernel\n"
    "\tthat your application is expecting to load.\n";

char *evg_err_opencl_version_note =
    "\tThe version of the Multi2Sim OpenCL library ('libm2s-opencl') that your "
    "program\n"
    "\tis using is too old. You need to re-link your program with a version of "
    "the\n"
    "\tlibrary compatible for this Multi2Sim release. Please see the Multi2Sim "
    "Guide\n"
 ...
 * a context gets suspended during the execution of the OpenCL call, and later
 * the
...
int evg_opencl_clGetPlatformIDs_impl(X86Context *ctx, int *argv) {
  ...
  if (opencl_impl_version < EVG_SYS_OPENCL_IMPL_VERSION)
    fatal(
        "wrong Multi2Sim OpenCL library version (provided=%d.%d.%d, "
        "required=%d.%d.%d).\n%s",
        opencl_impl_version_major, opencl_impl_version_minor,
        opencl_impl_version_build, EVG_SYS_OPENCL_IMPL_VERSION_MAJOR,
        EVG_SYS_OPENCL_IMPL_VERSION_MINOR, EVG_SYS_OPENCL_IMPL_VERSION_BUILD,
        evg_err_opencl_version_note);
  ...
  /* Get platform id */
  evg_opencl_debug(
      "  num_entries=%d, platforms=0x%x, num_platforms=0x%x, version=0x%x\n",
      num_entries, platforms, num_platforms, opencl_impl_version);
  if (num_platforms) mem_write(mem, num_platforms, 4, &one);
...
int evg_opencl_clGetPlatformInfo_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int param_value_size_ret =
      argv[4]; /* size_t *param_value_size_ret */

  struct evg_opencl_platform_t *platform;
  unsigned int size_ret;

  evg_opencl_debug(
      "  platform=0x%x, param_name=0x%x, param_value_size=0x%x,\n"
      "  param_value=0x%x, param_value_size_ret=0x%x\n",
      platform_id, param_name, param_value_size, param_value,
      param_value_size_ret);

  platform = evg_opencl_repo_get_object(
      evg_emu->opencl_repo, evg_opencl_object_platform, platform_id);
  size_ret = evg_opencl_platform_get_info(platform, param_name, mem,
                                          param_value, param_value_size);
  if (param_value_size_ret) mem_write(mem, param_value_size_ret, 4, &size_ret);
...
int evg_opencl_clGetDeviceIDs_impl(X86Context *ctx, int *argv) {
  ...
  if (num_devices) mem_write(mem, num_devices, 4, &one);
  ...
  if (devices && num_entries > 0) {
    ...
    if (!device) panic("%s: no device", __FUNCTION__);
...
int evg_opencl_clGetDeviceInfo_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  device=0x%x, param_name=0x%x, param_value_size=%d\n"
      "  param_value=0x%x, param_value_size_ret=0x%x\n",
  ...
  size_ret = evg_opencl_device_get_info(device, param_name, mem, param_value,
                                        param_value_size);
  if (param_value_size_ret) mem_write(mem, param_value_size_ret, 4, &size_ret);
...
int evg_opencl_clCreateContext_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int properties =
      argv[0]; /* const cl_context_properties *properties */
  unsigned int num_devices = argv[1]; /* cl_uint num_devices */
  unsigned int devices = argv[2];     /* const cl_device_id *devices */
  unsigned int pfn_notify =
      argv[3]; /* void (CL_CALLBACK *pfn_notify)(const char *errinfo,
  ...
  evg_opencl_debug(
      "  properties=0x%x, num_devices=%d, devices=0x%x\n"
  ...
  /* Return success */
  if (errcode_ret) mem_write(mem, errcode_ret, 4, &zero);
...
int evg_opencl_clCreateContextFromType_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int pfn_notify = argv[2];  /* void (*pfn_notify)(const char *, const
                                         void *, size_t , void *) */
  ...
  evg_opencl_debug(
      "  properties=0x%x, device_type=0x%x, pfn_notify=0x%x,\n"
...
int evg_opencl_clReleaseContext_impl(X86Context *ctx, int *argv) {
  ...
  if (!--context->ref_count) evg_opencl_context_free(context);
...
int evg_opencl_clGetContextInfo_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  context=0x%x, param_name=0x%x, param_value_size=0x%x,\n"
      "  param_value=0x%x, param_value_size_ret=0x%x\n",
  ...
  context = evg_opencl_repo_get_object(evg_emu->opencl_repo,
                                       evg_opencl_object_context, context_id);
  size_ret = evg_opencl_context_get_info(context, param_name, mem, param_value,
                                         param_value_size);
  if (param_value_size_ret) mem_write(mem, param_value_size_ret, 4, &size_ret);
...
int evg_opencl_clCreateCommandQueue_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int properties =
      argv[2]; /* cl_command_queue_properties properties */
  ...
  evg_opencl_debug(
      "  context=0x%x, device=0x%x, properties=0x%x, errcode_ret=0x%x\n",
      context_id, device_id, properties, errcode_ret);

  /* Check that context and device are valid */
  evg_opencl_repo_get_object(evg_emu->opencl_repo, evg_opencl_object_context,
                             context_id);
  evg_opencl_repo_get_object(evg_emu->opencl_repo, evg_opencl_object_device,
                             device_id);
...
int evg_opencl_clReleaseCommandQueue_impl(X86Context *ctx, int *argv) {
  ...
  command_queue = evg_opencl_repo_get_object(
      evg_emu->opencl_repo, evg_opencl_object_command_queue, command_queue_id);

  /* Release command queue */
  assert(command_queue->ref_count > 0);
  if (!--command_queue->ref_count) evg_opencl_command_queue_free(command_queue);
...
int evg_opencl_clCreateBuffer_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  context=0x%x, flags=%s, size=%d, host_ptr=0x%x, errcode_ret=0x%x\n",
      context_id, flags_str, size, host_ptr, errcode_ret);

  /* Check flags */
  if ((flags & 0x10) && host_ptr)
    fatal(
        "%s: CL_MEM_ALLOC_HOST_PTR not compatible with CL_MEM_USE_HOST_PTR\n%s",
...
int evg_opencl_clCreateImage2D_impl(X86Context *ctx, int *argv) {
  ...
  str_map_flags(&evg_opencl_create_image_flags_map, flags, flags_str,
                sizeof(flags_str));
  evg_opencl_debug(
      "  context=0x%x, flags=%s, channel order =0x%x, "
  ...
  switch (channel_order) {
    ...
    default:
      fatal("%s: image channel order %u not supported\n%s", __FUNCTION__,
            channel_order, evg_err_opencl_param_note);
  }
  ...
  switch (channel_type) {
    ...
    default:
      fatal("%s: image channel type %u not supported\n%s", __FUNCTION__,
            channel_type, evg_err_opencl_param_note);
  }
  ...
  if (image_row_pitch == 0) {
    ...
    fatal(
        "%s: image_row_pitch must be 0 or >= image_width * size of element in "
        "bytes\n%s",
        __FUNCTION__, evg_err_opencl_param_note);
  }
  ...
  /* Return success */
  if (errcode_ret_ptr) mem_write(mem, errcode_ret_ptr, 4, &zero);
...
int evg_opencl_clCreateImage3D_impl(X86Context *ctx, int *argv) {
  ...
  str_map_flags(&evg_opencl_create_image_flags_map, flags, flags_str,
                sizeof(flags_str));
  evg_opencl_debug(
      "  context=0x%x, flags=%s, channel order =0x%x, channel_type=0x%x\n"
  ...
  if (image_slice_pitch == 0)
    ...
    fatal(
        "%s: image_slice_pitch must be 0 or >= image_row_pitch * "
        "image_height\n%s",
        __FUNCTION__, evg_err_opencl_param_note);
...
int evg_opencl_clReleaseMemObject_impl(X86Context *ctx, int *argv) {
  ...
  if (!--opencl_mem->ref_count) evg_opencl_mem_free(opencl_mem);
...
int evg_opencl_clCreateSampler_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int addressing_mode =
      argv[2];                        /* cl_addressing_mode addressing_mode */
  ...
  evg_opencl_debug(
      "  context=0x%x, normalized_coords=%d, addressing_mode=0x%x,"
...
int evg_opencl_clCreateProgramWithSource_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  context=0x%x, count=%d, strings=0x%x, lengths=0x%x, "
      "errcode_ret=0x%x\n",
  ...
  warning("%s: binary '%s' used as pre-compiled kernel.\n%s", __FUNCTION__,
          evg_emu_opencl_binary_name, evg_err_opencl_binary_note);
...
int evg_opencl_clCreateProgramWithBinary_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  context=0x%x, num_devices=%d, device_list=0x%x, lengths=0x%x\n"
      "  binaries=0x%x, binary_status=0x%x, errcode_ret=0x%x\n",
      context_id, num_devices, device_list, lengths, binaries, binary_status,
      errcode_ret);
  EVG_OPENCL_ARG_NOT_SUPPORTED_NEQ(num_devices, 1);
  ...
  /* Return success */
  if (binary_status) mem_write(mem, binary_status, 4, &zero);
  if (errcode_ret) mem_write(mem, errcode_ret, 4, &zero);
...
int evg_opencl_clReleaseProgram_impl(X86Context *ctx, int *argv) {
  ...
  if (!--program->ref_count) evg_opencl_program_free(program);
...
int evg_opencl_clBuildProgram_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int pfn_notify =
      argv[4]; /* void (CL_CALLBACK *pfn_notify)(cl_program program,
                  void *user_data) */
  ...
  options_str[0] = 0;
  if (options) mem_read_string(mem, options, MAX_STRING_SIZE, options_str);

  evg_opencl_debug(
      "  program=0x%x, num_devices=%d, device_list=0x%x, options=0x%x\n"
...
int evg_opencl_clCreateKernel_impl(X86Context *ctx, int *argv) {
  ...
  if (mem_read_string(mem, kernel_name, MAX_STRING_SIZE, kernel_name_str) ==
      MAX_STRING_SIZE)
  ...
  for (i = 0; i < 25; i++) {
    ...
    if (constant_tmp) list_set(kernel->constant_buffer_list, i, constant_tmp);
  }
...
int evg_opencl_clReleaseKernel_impl(X86Context *ctx, int *argv) {
  ...
  if (!--kernel->ref_count) evg_opencl_kernel_free(kernel);
...
int evg_opencl_clSetKernelArg_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  kernel_id=0x%x, arg_index=%d, arg_size=%d, arg_value=0x%x\n",
      kernel_id, arg_index, arg_size, arg_value);

  /* Check */
  kernel = evg_opencl_repo_get_object(evg_emu->opencl_repo,
                                      evg_opencl_object_kernel, kernel_id);
  if (arg_value) EVG_OPENCL_ARG_NOT_SUPPORTED_NEQ(arg_size, 4);
  ...
  arg->set = 1;
  arg->size = arg_size;
  if (arg_value) mem_read(mem, arg_value, 4, &arg->value);
...
int evg_opencl_clGetKernelWorkGroupInfo_impl(X86Context *ctx, int *argv) {
  ...
  unsigned int param_value_size_ret =
      argv[5]; /* size_t *param_value_size_ret */

  struct evg_opencl_kernel_t *kernel;
  unsigned int size_ret;

  evg_opencl_debug(
      "  kernel=0x%x, device=0x%x, param_name=0x%x, param_value_size=0x%x,\n"
      "  param_value=0x%x, param_value_size_ret=0x%x\n",
  ...
  evg_opencl_repo_get_object(evg_emu->opencl_repo, evg_opencl_object_device,
                             device_id);
  size_ret = evg_opencl_kernel_get_work_group_info(
      kernel, param_name, mem, param_value, param_value_size);
  if (param_value_size_ret) mem_write(mem, param_value_size_ret, 4, &size_ret);
...
int evg_opencl_clGetEventInfo_impl(X86Context *ctx, int *argv) {
  ...
  switch (param_name) {
    case 0x11d0: /* CL_EVENT_COMMAND_QUEUE */
      fatal(
          "param_name CL_EVENT_COMMAND_QUEUE not supported for clGetEventInfo");
      break;

    case 0x11d1: /* CL_EVENT_COMMAND_TYPE */
      fatal(
          "param_name CL_EVENT_COMMAND_TYPE not supported for clGetEventInfo");
      break;

    case 0x11d2: /* CL_EVENT_REFERENCE_COUNT */
      fatal(
          "param_name CL_EVENT_REFERNCE_COUNT not supported for "
          "clGetEventInfo");
      break;
...
int evg_opencl_clReleaseEvent_impl(X86Context *ctx, int *argv) {
  ...
  if (!--event->ref_count) evg_opencl_event_free(event);
...
int evg_opencl_clGetEventProfilingInfo_impl(X86Context *ctx, int *argv) {
  ...
  evg_opencl_debug(
      "  event=0x%x, param_name=0x%x, param_value_size=0x%x,\n"
      "  param_value=0x%x, param_value_size_ret=0x%x\n",
...
void evg_opencl_clEnqueueReadBuffer_wakeup(X86Context *ctx, void *data) {
  ...
  buf = xmalloc(argv.cb);
  mem_read(evg_emu->global_mem, opencl_mem->device_ptr + argv.offset, argv.cb,
           buf);
...
void evg_opencl_clEnqueueWriteBuffer_wakeup(X86Context *ctx, void *data) {
  ...
  mem_write(evg_emu->global_mem, opencl_mem->device_ptr + argv.offset, argv.cb,
            buf);
  free(buf);
  ...
  if (argv.event_ptr) {
    ...
    event->time_start = evg_opencl_event_timer();
    event->time_end =
        evg_opencl_event_timer(); /* FIXME: change for asynchronous exec */
    mem_write(ctx->mem, argv.event_ptr, 4, &event->id);
    evg_opencl_debug("    event: 0x%x\n", event->id);
  }
...
int evg_opencl_clEnqueueWriteBuffer_impl(X86Context *ctx, int *argv_ptr) {
  ...
  evg_opencl_debug(
      ...
      "  offset=0x%x, cb=0x%x, ptr=0x%x, num_events_in_wait_list=0x%x,\n"
      "  event_wait_list=0x%x, event=0x%x\n",
      argv->command_queue, argv->buffer, argv->blocking_write, argv->offset,
      argv->cb, argv->ptr, argv->num_events_in_wait_list, argv->event_wait_list,
      argv->event_ptr);
...
void evg_opencl_clEnqueueCopyBuffer_wakeup(X86Context *ctx, void *data) {
  ...
  /* Copy buffers */
  buf = xmalloc(argv.cb);
  mem_read(evg_emu->global_mem, src_mem->device_ptr + argv.src_offset, argv.cb,
           buf);
  mem_write(evg_emu->global_mem, dst_mem->device_ptr + argv.dst_offset, argv.cb,
            buf);
  free(buf);
...
int evg_opencl_clEnqueueCopyBuffer_impl(X86Context *ctx, int *argv_ptr) {
  ...
  evg_opencl_debug(
      "  command_queue=0x%x, src_buffer=0x%x, dst_buffer=0x%x,\n"
      "  src_offset=0x%x, dst_offset=0x%x, cb=%u, "
      "num_events_in_wait_list=0x%x,\n"
      "  event_wait_list=0x%x, event=0x%x\n",
...
void evg_opencl_clEnqueueReadImage_wakeup(X86Context *ctx, void *data) {
  ...
  /* Determine image geometry */
  /* NOTE size_t is 32-bits on 32-bit systems, but 64-bits on 64-bit systems.
   * Since
   * the simulator may be on a 64-bit system, we should not use size_t here, but
   * unsigned int
   * instead. */
  ...
  if (!argv.row_pitch)
    ...
    fatal(
        "%s: row_pitch must be 0 or >= image_width * size of element in "
        "bytes\n%s",
        __FUNCTION__, evg_err_opencl_param_note);
  ...
  if (read_origin[0] != 0 || read_origin[1] != 0 || read_origin[2] != 0 ||
      read_region[0] != opencl_mem->width ||
      read_region[1] != opencl_mem->height ||
...
int evg_opencl_clEnqueueReadImage_impl(X86Context *ctx, int *argv_ptr) {
  ...
  evg_opencl_debug(
      "  command_queue=0x%x, image=0x%x, blocking_read=0x%x,\n"
...
void evg_opencl_clEnqueueMapBuffer_wakeup(X86Context *ctx, void *data) {
  ...
  /* Get memory object */
  evg_opencl_repo_get_object(evg_emu->opencl_repo, evg_opencl_object_mem,
                             argv.buffer);
  ...
  /* Return success */
  if (argv.errcode_ret) mem_write(ctx->mem, argv.errcode_ret, 4, &zero);
...
int evg_opencl_clEnqueueMapBuffer_impl(X86Context *ctx, int *argv_ptr) {
  ...
  evg_opencl_debug(
      ...
      "  offset=0x%x, cb=0x%x, num_events_in_wait_list=0x%x, "
      "event_wait_list=0x%x,\n"
      "  event=0x%x, errcode_ret=0x%x\n",
      argv->command_queue, argv->buffer, argv->blocking_map, argv->map_flags,
      argv->offset, argv->cb, argv->num_events_in_wait_list,
      argv->event_wait_list, argv->event_ptr, argv->errcode_ret);
...
void evg_opencl_clEnqueueNDRangeKernel_wakeup(X86Context *ctx, void *data) {
  ...
  /* Local work sizes.
   * If no pointer provided, assign the same as global size - FIXME: can be done
   * better. */
  memcpy(kernel->local_size3, kernel->global_size3, 12);
  if (argv.local_work_size_ptr) {
    for (i = 0; i < argv.work_dim; i++) {
      mem_read(ctx->mem, argv.local_work_size_ptr + i * 4, 4,
  ...
  kernel->local_size =
      kernel->local_size3[0] * kernel->local_size3[1] * kernel->local_size3[2];
  ...
  kernel->group_count = kernel->group_count3[0] * kernel->group_count3[1] *
  ...
  task =
      evg_opencl_command_create(evg_opencl_command_queue_task_ndrange_kernel);
...
int evg_opencl_clEnqueueNDRangeKernel_impl(X86Context *ctx, int *argv_ptr) {
  ...
  evg_opencl_debug(
      ...
      "  global_work_offset=0x%x, global_work_size_ptr=0x%x, "
      "local_work_size_ptr=0x%x,\n"
      "  num_events_in_wait_list=0x%x, event_wait_list=0x%x, event=0x%x\n",
