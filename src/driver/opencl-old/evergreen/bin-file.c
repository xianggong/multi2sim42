...
#define EVG_BIN_FILE_NOT_SUPPORTED(__var)                          \
  fatal("%s: value 0x%x not supported for parameter '" #__var "'", \
        __FUNCTION__, (__var))
#define EVG_BIN_FILE_NOT_SUPPORTED_NEQ(__var, __val)                        \
  if ((__var) != (__val))                                                   \
  fatal("%s: parameter '" #__var "' was expected to be 0x%x", __FUNCTION__, \
        (__val))
...
static struct str_map_t enc_dict_machine_map = {20,
                                                {{"R600", 0},
                                                 {"RV610", 1},
                                                 {"RV630", 2},
                                                 {"RV670", 3},
...
static struct str_map_t prog_info_entry_map = {
    ...
    {{"mmSQ_PGM_START_LS", 0xa234},
     ...
     {"AMU_ABI_SET_BUFFER_FOR_UAV_RET_BUFFER0",
      0x80000010}, /* UAV 0 return buffer stride */
     ...
     {"AMU_ABI_NUM_GLOBAL_UAV",
      0x80000022}, /* Number of UAVs used with global return buffers */
     {"AMU_ABI_NUM_GLOBAL_RETURN_BUFFER",
      0x80000023}, /* Number of constants used for global return buffers */
     {"AMU_ABI_GLOBAL_RETURN_BUFFER_SIZE",
      0x80000024}, /* Size of dword global return buffer */
     ...
     {"AMU_ABI_UNCACHED_FETCH_CONST_UAV0",
      0x80000040}, /* UAV uncached fetch const */
     ...
     {"AMU_ABI_CACHED_FETCH_CONST_UAV0",
      0x80000050}, /* UAV cached fetch const */
...
static void evg_bin_file_read_note_header(
  ...
  switch (header->type) {
    ...
    case 10: /* ELF_NOTE_ATI_CONSTANT_BUFFERS */
    {
      ...
      elf_debug(
          "\tnote including number and size of constant buffers (%d entries)\n",
          constant_buffer_count);
...
static void evg_bin_file_read_notes(
    struct evg_bin_file_t *bin_file,
    struct evg_bin_enc_dict_entry_t *enc_dict_entry) {
  struct elf_buffer_t *buffer;
...
static void evg_bin_file_read_enc_dict(struct evg_bin_file_t *bin_file) {
  ...
  assert(program_header->header->p_filesz %
             sizeof(struct evg_bin_enc_dict_entry_header_t) ==
         0);
  ...
  for (i = 0; i < list_count(bin_file->enc_dict); i++) {
    ...
    for (j = 0; j < list_count(elf_file->program_header_list); j++) {
      ...
      if (program_header->header->p_offset < enc_dict_entry->header->d_offset ||
        ...
        continue;
      assert(program_header->header->p_offset +
      ...
      if (program_header->header->p_type == PT_NOTE) {
        if (enc_dict_entry->pt_note_buffer.size)
          fatal("%s: more than one PT_NOTE for encoding dictionary entry",
                __FUNCTION__);
        enc_dict_entry->pt_note_buffer.ptr =
            elf_file->buffer.ptr + program_header->header->p_offset;
      ...
      if (program_header->header->p_type == PT_LOAD) {
        if (enc_dict_entry->pt_load_buffer.size)
          fatal("%s: more than one PT_LOAD for encoding dictionary entry",
                __FUNCTION__);
        enc_dict_entry->pt_load_buffer.ptr =
            elf_file->buffer.ptr + program_header->header->p_offset;
    ...
    elf_debug("  Dict. entry %d: PT_NOTE segment: offset=0x%x, size=%d\n", i,
              (int)(enc_dict_entry->pt_note_buffer.ptr - elf_file->buffer.ptr),
              enc_dict_entry->pt_note_buffer.size);
    elf_debug("  Dict. entry %d: PT_LOAD segment: offset=0x%x, size=%d\n", i,
              (int)(enc_dict_entry->pt_load_buffer.ptr - elf_file->buffer.ptr),
              enc_dict_entry->pt_load_buffer.size);
  }
}
  ...
  for (i = 0; i < list_count(bin_file->enc_dict); i++) {
    ...
    for (j = 0; j < list_count(elf_file->section_list); j++) {
      ...
      if (!strcmp(section->name, ".text")) {
        ...
        enc_dict_entry->sec_text_buffer.ptr =
            elf_file->buffer.ptr + section->header->sh_offset;
        ...
        if (buffer->size) fatal("%s: duplicated '.data' section", __FUNCTION__);
        ...
        enc_dict_entry->sec_symtab_buffer.ptr =
            elf_file->buffer.ptr + section->header->sh_offset;
...
struct evg_bin_file_t *evg_bin_file_create(void *ptr, int size, char *name) {
  ...
  if (!bin_file->enc_dict_entry_evergreen)
    fatal(
        "%s: no encoding dictionary entry for Evergreen.\n"
        "\tThe OpenCL kernel binary that your application is trying to load "
        "does not\n"
        "\tcontain Evergreen assembly code. Please make sure that a Cypress "
        "device\n"
        "\tis selected when compiling the OpenCL kernel source. In some cases, "
        "even\n"
        "\ta proper selection of this architecture causes Evergreen assembly "
        "not to\n"
        "\tbe included if the APP SDK is not correctly installed when "
        "compiling your\n"
        "\town kernel sources.\n",
