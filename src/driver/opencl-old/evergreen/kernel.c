...
struct evg_opencl_kernel_t *evg_opencl_kernel_create() {
  ...
  kernel->uav_read_list =
      list_create_with_size(12); /* FIXME Repalce with MAX_UAVS? */
  kernel->uav_write_list =
      list_create_with_size(12); /* FIXME Repalce with MAX_UAVS? */
  kernel->constant_buffer_list =
      list_create_with_size(25); /* For constant buffers (128 to 153) */
  /* FIXME Replace with new list functionality */
  for (i = 0; i < 12; i++) {
    list_add(kernel->uav_read_list, NULL);
    list_add(kernel->uav_write_list, NULL);
  }
  for (i = 0; i < 25; i++) list_add(kernel->constant_buffer_list, NULL);
...
#define EVG_OPENCL_KERNEL_METADATA_TOKEN_COUNT(_tc)                   \
  if (token_count != (_tc))                                           \
    fatal("%s: meta data entry '%s' expects %d tokens", __FUNCTION__, \
          line_ptrs[0], (_tc));
...
static void evg_opencl_kernel_load_metadata(
    struct evg_opencl_kernel_t *kernel) {
  ...
  for (;;) {
    ...
    /* Ignored entries */
    if (!line_ptrs[0] || !strcmp(line_ptrs[0], "ARGSTART") ||
        !strcmp(line_ptrs[0], "version") || !strcmp(line_ptrs[0], "device") ||
        !strcmp(line_ptrs[0], "uniqueid") || !strcmp(line_ptrs[0], "uavid") ||
    ...
    if (!strcmp(line_ptrs[0], "pointer")) {
      ...
      /* Metadata version 3:1:104 (as specified in entry 'version') uses 12
       * items. */
     ...
     * is extracted from the argument descriptions in 'value' and 'pointer'
     * entries.
     */
     ...
     * Observed first in version 3:1:104 of metadata. Not sure what this entry
     * is for.
     */
     ...
     * Observed first in version 3:1:104 of metadata. It shows up when an
     * argument
    ...
    warning("kernel '%s': unknown meta data entry '%s'", kernel->name,
            line_ptrs[0]);
  }
}

/* Extract and analyze information from the program binary associated with
 * 'kernel_name' */
void evg_opencl_kernel_load(struct evg_opencl_kernel_t *kernel,
                            char *kernel_name) {
  ...
  program = evg_opencl_repo_get_object(
      evg_emu->opencl_repo, evg_opencl_object_program, kernel->program_id);

  /* Read 'metadata' symbol */
  snprintf(symbol_name, MAX_STRING_SIZE, "__OpenCL_%s_metadata", kernel_name);
  evg_opencl_program_read_symbol(program, symbol_name,
   ...
   * The internal ELF is contained in the buffer pointer to by the 'kernel'
   * symbol. */
  snprintf(name, sizeof(name), "clKernel<%s>.InternalELF", kernel_name);
  kernel->bin_file = evg_bin_file_create(kernel->kernel_buffer.ptr,
...
uint32_t evg_opencl_kernel_get_work_group_info(
    struct evg_opencl_kernel_t *kernel, uint32_t name, struct mem_t *mem,
  ...
  assert(info);
  if (addr && size >= size_ret) mem_write(mem, addr, size_ret, info);
  return size_ret;
}
