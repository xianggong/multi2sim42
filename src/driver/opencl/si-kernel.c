...
static char *opencl_err_si_kernel_symbol =
    ...
    "table.\n"
    "\tIf it is not found, it probably means that your application is "
    "requesting\n"
    "\texecution of a kernel function that is not present in the encoded "
    "binary.\n"
    "\tPlease, check the parameters passed to the 'clCreateKernel' function "
    "in\n"
    "\tyour application.\n"
    ...
    "\twith an incorrect or missing installation of the GPU driver. In this "
    "case,\n"
    "\tthe tool will still compile the kernel into LLVM, but the ISA section "
    "will\n"
    "\tbe missing in the kernel binary.\n";

static char *opencl_err_si_kernel_metadata =
    "\tThe kernel binary loaded by your application is a valid ELF file. In "
    "this\n"
    "\tfile, a '.rodata' section contains specific information about the "
    "OpenCL\n"
    "\tkernel. However, this information is only partially supported by "
    "Multi2Sim.\n"
    "\tTo request support for this error, please email "
...
void opencl_si_kernel_list_init(void) {
  /* Already initialized */
  if (opencl_si_kernel_list) return;
...
void opencl_si_kernel_list_done(void) {
  ...
  LIST_FOR_EACH(opencl_si_kernel_list, index) {
    ...
    if (kernel) opencl_si_kernel_free(kernel);
  }
...
static void opencl_si_kernel_expect(struct opencl_si_kernel_t *kernel,
  ...
  if (strcmp(token, head_token))
    fatal("%s: token '%s' expected, '%s' found.\n%s", __FUNCTION__, head_token,
          token, opencl_err_si_kernel_metadata);
}
...
static void opencl_si_kernel_expect_count(struct opencl_si_kernel_t *kernel,
  ...
  if (token_list->count != count)
    fatal("%s: %d tokens expected for '%s', %d found.\n%s", __FUNCTION__, count,
          head_token, token_list->count, opencl_err_si_kernel_metadata);
}

static void opencl_si_kernel_load_metadata_v3(
    struct opencl_si_kernel_t *kernel) {
  ...
  for (;;) {
    ...
    if (!strcmp(token, "value")) {
      ...
      token = str_token_list_shift(token_list);
      arg->value.data_type =
          str_map_string_err(&si_arg_data_type_map, token, &err);
      if (err)
      ...
      arg->size =
          arg->value.num_elems * si_arg_get_data_size(arg->value.data_type);

      /* Debug */
      opencl_debug(
          "\targument '%s' - value stored in "
    ...
    if (!strcmp(token, "pointer")) {
      ...
      /* Token 1 - Name */
      token = str_token_list_shift(token_list);
      arg = si_arg_create(si_arg_pointer, token);
      ...
      token = str_token_list_shift(token_list);
      arg->pointer.scope = str_map_string_err(&si_arg_scope_map, token, &err);
      if (err)
      ...
      arg->pointer.access_type =
          str_map_string_err(&si_arg_access_type_map, token, &err);
      if (err)
    ...
    if (!strcmp(token, "image")) {
      ...
      token = str_token_list_shift(token_list);
      arg->image.dimension =
          str_map_string_err(&si_arg_dimension_map, token, &err);
      if (err)
    ...
    fatal("kernel '%s': unknown metadata entry '%s'", kernel->name, token);
  }
}
...
struct opencl_si_kernel_t *opencl_si_kernel_create(
  ...
  if (!elf_file) fatal("%s: program not built", __FUNCTION__);
  ...
  if (!metadata_symbol || !header_symbol || !kernel_symbol)
    fatal(
        "%s: invalid kernel function (ELF symbol '__OpenCL_%s_xxx "
        "missing')\n%s",
  ...
  opencl_debug("\theader symbol: offset=0x%x, size=%u\n", header_symbol->value,
               kernel->header_buffer.size);
  opencl_debug("\tkernel symbol: offset=0x%x, size=%u\n", kernel_symbol->value,
               kernel->kernel_buffer.size);
  ...
  kernel->bin_file = si_bin_file_create(
      kernel->kernel_buffer.ptr, kernel->kernel_buffer.size, symbol_name);
...
void opencl_si_kernel_setup_ndrange_constant_buffers(
  ...
  si_ndrange_const_buf_write(ndrange, 0, 0, &ndrange->global_size3[0], 4);
  si_ndrange_const_buf_write(ndrange, 0, 4, &ndrange->global_size3[1], 4);
  si_ndrange_const_buf_write(ndrange, 0, 8, &ndrange->global_size3[2], 4);
  ...
  si_ndrange_const_buf_write(ndrange, 0, 16, &ndrange->local_size3[0], 4);
  si_ndrange_const_buf_write(ndrange, 0, 20, &ndrange->local_size3[1], 4);
  si_ndrange_const_buf_write(ndrange, 0, 24, &ndrange->local_size3[2], 4);
  ...
  si_ndrange_const_buf_write(ndrange, 0, 32, &ndrange->group_count3[0], 4);
  si_ndrange_const_buf_write(ndrange, 0, 36, &ndrange->group_count3[1], 4);
  si_ndrange_const_buf_write(ndrange, 0, 40, &ndrange->group_count3[2], 4);
...
void opencl_si_kernel_setup_ndrange_args(struct opencl_si_kernel_t *kernel,
  ...
  LIST_FOR_EACH(kernel->arg_list, index) {
    ...
    if (!arg->set)
      fatal("%s: kernel '%s': argument '%s' not set", __FUNCTION__,
    ...
    switch (arg->type) {
      case si_arg_value:
         ...
         * memory */
        assert(arg->size);
        si_ndrange_const_buf_write(ndrange, arg->value.constant_buffer_num,
                                   arg->value.constant_offset,
      ...
      case si_arg_pointer:
        ...
        switch (arg->pointer.scope) {
          ...
          case si_arg_hw_local:
             ...
             * space for it. */
            si_ndrange_const_buf_write(
                ndrange, arg->pointer.constant_buffer_num,
                arg->pointer.constant_offset, &ndrange->local_mem_top, 4);

            opencl_debug(
                "\targument %d: %u bytes reserved"
                " in local memory at 0x%x\n",
                index, arg->size, ndrange->local_mem_top);
          ...
          case si_arg_uav: {
            ...
            opencl_si_create_buffer_desc(arg->pointer.device_ptr, arg->size,
                                         arg->pointer.num_elems,
                                         arg->pointer.data_type, &buffer_desc);

            /* Add to UAV table */
            si_ndrange_insert_buffer_into_uav_table(ndrange, &buffer_desc,
                                                    arg->pointer.buffer_num);

            /* Write 0 to CB1 */
            si_ndrange_const_buf_write(ndrange,
                                       arg->pointer.constant_buffer_num,
                                       arg->pointer.constant_offset, &zero, 4);
          ...
          case si_arg_hw_constant: {
            ...
            si_ndrange_insert_buffer_into_const_buf_table(
                ndrange, &buffer_desc, arg->pointer.buffer_num);
          ...
          default:
            ...
            fatal("%s: not implemented memory scope", __FUNCTION__);
        }
      ...
      default:
        ...
        fatal("%s: argument type not recognized (%d)", __FUNCTION__, arg->type);
    }
  }
  ...
  for (index = 2; index < 25; index++) {
    constant_buffer = list_get(kernel->program->constant_buffer_list, index);

    if (!constant_buffer) break;

    opencl_si_create_buffer_desc(constant_buffer->device_ptr,
                                 constant_buffer->size, 4, si_arg_float,
                                 &buffer_desc);
    ...
    /* Add to Constant Buffer table */
    si_ndrange_insert_buffer_into_const_buf_table(ndrange, &buffer_desc, index);
  }
}
  ...
  switch (data_type) {
    ...
    case si_arg_u8:
      ...
      switch (num_elems) {
        ...
        default:
          fatal("%s: invalid number of i8/u8 elements (%d)", __FUNCTION__,
                num_elems);
      }
    ...
    case si_arg_u16:
      ...
      switch (num_elems) {
        ...
        default:
          fatal("%s: invalid number of i16/u16 elements (%d)", __FUNCTION__,
                num_elems);
      }
    ...
    case si_arg_u32:
      ...
      switch (num_elems) {
        ...
        default:
          fatal("%s: invalid number of i32/u32 elements (%d)", __FUNCTION__,
                num_elems);
      }
    ...
    case si_arg_float:
      ...
      switch (num_elems) {
        ...
        default:
          fatal("%s: invalid number of float elements (%d)", __FUNCTION__,
                num_elems);
      }
      elem_size = 4 * num_elems;
      break;
    ...
    default:
      fatal("%s: invalid data type for SI buffer (%d)", __FUNCTION__,
            data_type);
  }
...
void opencl_si_kernel_debug_ndrange_state(struct opencl_si_kernel_t *kernel,
  ...
  si_isa_debug("\t| Const Buffer table | [%10u:%10u] |\n",
               ...
               ndrange->const_buf_table + SI_EMU_CONST_BUF_TABLE_SIZE - 1);
  si_isa_debug("\t| Resource table     | [%10u:%10u] |\n",
               ndrange->resource_table,
               ndrange->resource_table + SI_EMU_RESOURCE_TABLE_SIZE - 1);
  ...
  unsigned int userElementCount =
      kernel->bin_file->enc_dict_entry_southern_islands->userElementCount;
  struct si_bin_enc_user_element_t *userElements =
      kernel->bin_file->enc_dict_entry_southern_islands->userElements;
    ...
    if (userElements[i].dataClass == IMM_CONST_BUFFER) {
      ...
      if (userElements[i].userRegCount > 1) {
        si_isa_debug(
            ...
            userElements[i].startUserReg + userElements[i].userRegCount - 1,
            userElements[i].apiSlot);
      } else {
        si_isa_debug(
            "\t| SREG[%2d]    |  CB%1d "
            "Descriptor         |\n",
            userElements[i].startUserReg, userElements[i].apiSlot);
      }
    }
  ...
  for (i = 0; i < SI_EMU_MAX_NUM_CONST_BUFS; i++) {
    ...
    mem_read(si_emu->global_mem,
             ndrange->const_buf_table + i * SI_EMU_CONST_BUF_TABLE_ENTRY_SIZE,
