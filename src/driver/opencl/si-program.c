...
void opencl_si_program_list_init(void) {
  /* Already initialized */
  if (opencl_si_program_list) return;
...
void opencl_si_program_list_done(void) {
  ...
  LIST_FOR_EACH(opencl_si_program_list, index) {
    ...
    if (program) opencl_si_program_free(program);
  }
...
static void opencl_si_program_initialize_constant_buffers(
  ...
  for (i = 2; i < 25; i++) {
    ...
    mem_write(si_emu->video_mem, si_emu->video_mem_top, symbol->size,
              elf_buffer.ptr);

    /* Create buffer */
    constant_buffer = opencl_si_constant_buffer_create(i, si_emu->video_mem_top,
                                                       symbol->size);
...
void opencl_si_program_free(struct opencl_si_program_t *program) {
  ...
  if (program->constant_buffer_list) {
    ...
    opencl_si_constant_buffer_free(
...
void opencl_si_program_set_binary(struct opencl_si_program_t *program,
  ...
  /* Already set */
  if (program->elf_file) fatal("%s: binary already set", __FUNCTION__);
