...
char *opencl_abi_call_name[opencl_abi_call_count + 1] = {NULL,
...
static int opencl_abi_si_kernel_create_impl(X86Context *ctx) {
  ...
  if (size == sizeof func_name) fatal("%s: buffer too small", __FUNCTION__);
...
static int opencl_abi_si_kernel_set_arg_value_impl(X86Context *ctx) {
  ...
  if (!kernel) fatal("%s: invalid kernel ID (%d)", __FUNCTION__, kernel_id);
  ...
  /* Check valid size */
  if (size != arg->size)
    fatal("%s: argument %d: size %d expected, %d found", __FUNCTION__, index,
          arg->size, size);
...
static int opencl_abi_si_ndrange_initialize_impl(X86Context *ctx) {
  ...
  if (si_gpu_fused_device) si_emu->global_mem = ctx->mem;
  ...
  ndrange->wg_id_sgpr = kernel->bin_file->enc_dict_entry_southern_islands
  ...
  elf_buffer =
      &kernel->bin_file->enc_dict_entry_southern_islands->sec_text_buffer;
  if (!elf_buffer->size) fatal("%s: cannot load kernel code", __FUNCTION__);
...
static int opencl_abi_si_ndrange_send_work_groups_can_wakeup(X86Context *ctx,
                                                             void *user_data) {
  assert(!user_data);
...
static void opencl_abi_si_ndrange_send_work_groups_wakeup(X86Context *ctx,
                                                          void *user_data) {
  assert(!user_data);
}
...
static int opencl_abi_si_ndrange_send_work_groups_impl(X86Context *ctx) {
  ...
  X86ContextSuspend(ctx, opencl_abi_si_ndrange_send_work_groups_can_wakeup,
                    NULL, opencl_abi_si_ndrange_send_work_groups_wakeup, NULL);
...
static int opencl_abi_si_ndrange_finish_impl(X86Context *ctx) {
  ...
  if (!list_count(si_emu->running_work_groups) &&
    ...
    X86ContextSuspend(ctx, opencl_abi_si_ndrange_finish_can_wakeup, NULL,
                      opencl_abi_si_ndrange_finish_wakeup, NULL);
  }
...
static int opencl_abi_si_ndrange_pass_mem_objs_impl(X86Context *ctx) {
  ...
  if (si_gpu_fused_device) {
    ...
    si_emu->ndrange->resource_table =
        (si_emu->ndrange->const_buf_table + SI_EMU_CONST_BUF_TABLE_SIZE + 16) &
        0xFFFFFFF0;

    si_emu->ndrange->uav_table =
        (si_emu->ndrange->resource_table + SI_EMU_RESOURCE_TABLE_SIZE + 16) &
        0xFFFFFFF0;

    si_emu->ndrange->cb0 = constant_buffers_ptr;
    si_emu->ndrange->cb1 = si_emu->ndrange->cb0 + SI_EMU_CONST_BUF_0_SIZE;
  }
