...
static char *opengl_err_not_impl =
    "\tThe OpenGL interface is implemented in library 'm2s-opengl.so' as a set "
    "of\n"
...
static char *opengl_version_code =
    "\tAn invalid function code was generated by your application in a OpenGL "
    "system\n"
    "\tcall. Probably, this means that your application is using an "
    "incompatible\n"
    "\tversion of the Multi2Sim OpenGL runtime library ('libm2s-opengl'). "
    "Please\n"
...
char *opengl_abi_call_name[opengl_abi_call_count + 1] = {NULL,
 ...
 *
 * Associate a binary to a Southern Islands program. Also setup associated
 *shaders.
 *
...
static int opengl_abi_si_program_set_binary_impl(X86Context *ctx) {
  ...
  /* Get program */
  program = list_get(opengl_si_program_list, program_id);
  if (!program) fatal("%s: invalid program ID (%d)", __FUNCTION__, program_id);
...
static int opengl_abi_si_shader_set_input_impl(X86Context *ctx) {
  ...
  index = args[5];
  opengl_debug(
      "\tshader_id = %d, device_ptr = 0x%d, num_elems = %d, type = %d, size = "
      "%d, index = %d\n",
  ...
  if (!input || input->usage_index != index)
    fatal(
        "Vertex attribute array at index %d is not needed by the vertex "
        "shader\n",
        index);
  else {
    input->set = 1;
 ...
 * @param int shader_id
 *
 * 	Shader ID, assigned in OpenGL runtime and bound to certain OpenGL
 *program
 *
 * @param int work_dim
 *
...
static int opengl_abi_si_ndrange_initialize_impl(X86Context *ctx) {
  ...
  if (!shader) fatal("%s: invalid shader ID (%d)", __FUNCTION__, shader_id);

  /* Create ND-Range */
  ndrange = si_ndrange_create();
  ndrange->local_mem_top = shader->mem_size_local;
  ndrange->num_sgpr_used = shader->shader_bin->shader_enc_dict->num_sgpr_used;
  ndrange->num_vgpr_used = shader->shader_bin->shader_enc_dict->num_vgpr_used;
  ndrange->wg_id_sgpr =
      shader->shader_bin->shader_enc_dict->shader_pgm_rsrc2_vs->user_sgpr;
  si_ndrange_setup_size(ndrange, global_size, local_size, work_dim);

  /* Copy user elements from shader to ND-Range */
  user_element_count = shader->shader_bin->shader_enc_dict->userElementCount;
  user_elements = shader->shader_bin->shader_enc_dict->userElements;
  ...
  if (!elf_buffer->size) fatal("%s: cannot load shader code", __FUNCTION__);
  ...
  if (si_gpu) si_gpu_map_ndrange(ndrange);
...
static int opengl_abi_si_ndrange_get_num_buffer_entries_impl(X86Context *ctx) {
  struct x86_regs_t *regs = ctx->regs;
  struct mem_t *mem = ctx->mem;
...
static int opengl_abi_si_ndrange_send_work_groups_can_wakeup(X86Context *ctx,
                                                             void *user_data) {
  assert(!user_data);
...
static void opengl_abi_si_ndrange_send_work_groups_wakeup(X86Context *ctx,
                                                          void *user_data) {
  assert(!user_data);
}
...
static int opengl_abi_si_ndrange_send_work_groups_impl(X86Context *ctx) {
  ...
  for (i = work_group_start[2]; i < work_group_start[2] + work_group_count[2];
       i++) {
    for (j = work_group_start[1]; j < work_group_start[1] + work_group_count[1];
         j++) {
      for (k = work_group_start[0];
           k < work_group_start[0] + work_group_count[0]; k++) {
        work_group_id = (i * work_group_sizes[1] * work_group_sizes[0]) +
                        (j * work_group_sizes[0]) + k;
  ...
  X86ContextSuspend(ctx, opengl_abi_si_ndrange_send_work_groups_can_wakeup,
                    NULL, opengl_abi_si_ndrange_send_work_groups_wakeup, NULL);
...
static int opengl_abi_si_ndrange_finish_impl(X86Context *ctx) {
  ...
  if (!list_count(si_emu->running_work_groups) &&
    ...
    X86ContextSuspend(ctx, opengl_abi_si_ndrange_finish_can_wakeup, NULL,
                      opengl_abi_si_ndrange_finish_wakeup, NULL);
  }
