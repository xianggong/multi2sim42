  ...
  switch (data_type) {
    ...
    case si_input_uint_2_10_10_10_rev:
      ...
      switch (num_elems) {
        ...
        default:
          fatal("%s: invalid number of elements (%d)", __FUNCTION__, num_elems);
      }
      elem_size = 4 * num_elems;
      break;
...
void opengl_si_shader_list_init(void) {
  /* Already initialized */
  if (opengl_si_shader_list) return;
...
void opengl_si_shader_list_done(void) {
  ...
  LIST_FOR_EACH(opengl_si_shader_list, index) {
    ...
    if (shdr) opengl_si_shader_free(shdr);
  }
  list_free(opengl_si_shader_list);
}

struct opengl_si_shader_t *opengl_si_shader_create(unsigned int shader_id,
...
void opengl_si_shader_init(struct opengl_si_program_t *program,
  ...
  /* FIXME: is it true that an OpenGL program binary can only contain 1 shader
   * of each kind */
  LIST_FOR_EACH(shaders_list, i) {
    shdr_bin = list_get(shaders_list, i);
    if (shdr_bin->shader_kind == shdr->shader_kind) shdr->shader_bin = shdr_bin;
  }
}
...
void opengl_si_shader_setup_ndrange_inputs(struct opengl_si_shader_t *shdr,
  ...
  LIST_FOR_EACH(shdr->shader_bin->shader_enc_dict->input_list, index) {
    ...
    if (!input->set)
      fatal("%s: shader %p: input #%d not set", __FUNCTION__, shdr,
            input->usage_index);

    /* Create descriptor for input */
    opengl_si_create_buffer_desc(input->device_ptr, input->size,
                                 input->num_elems, input->type, &buffer_desc);

    opengl_debug(
        "\tinput created, device_ptr = %d, size = %d, num_elems = %d, type = "
        "%x\n",
        input->device_ptr, input->size, input->num_elems, input->type);

    /* Add to Vertex Buffer table */
    si_ndrange_insert_buffer_into_vertex_buffer_table(ndrange, &buffer_desc,
                                                      input->usage_index);
  }
}
